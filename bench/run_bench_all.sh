#!/bin/bash
# Benchmark stryke vs perl5 vs python3 vs ruby.
#
# Extends run_bench.sh with Python 3 and Ruby columns. Same methodology:
...
printf '\n'
printf ' stryke benchmark harness (4-way)\n'
printf ' ──────────────────────────────────────────────\n'
printf '  stryke:  %s\n'   "$(stryke_version)"
printf '  perl5:   %s\n'   "$(perl5_version)"
printf '  python:  %s\n'   "$(python_version)"
...
for name in startup fib loop string hash array regex map_grep; do
    ...
    printf '  %-12s %10.1f  %10.1f  %10.1f  %10.1f  %10s  %10s  %10s\n' \
        "$name" "$rs_mean" "$p5_mean" "$py_mean" "$rb_mean" "$r_p5" "$r_py" "$r_rb"
done

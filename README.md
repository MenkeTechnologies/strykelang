...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** for **pure integer** linear bytecode: `LoadInt`, `Add` / `Sub` / `Mul`, `Negate`, `BitXor` / `BitNot`, `Shl` / `Shr` (shift amount masked like Rust `wrapping_shl` / `wrapping_shr`), `Pop`, `Dup`, optional trailing `Halt`. Uses native `i64` wrapping arithmetic and bitwise ops aligned with the VM’s integer paths. `Div` / `Mod` / `Pow`, `BitAnd` / `BitOr` (set ops), slots, calls, and control flow stay on the interpreter. Eligible chunks are detected at VM entry; compiled code is cached (bounded). Hot-loop tracing and `GetScalarSlot` from native code are not implemented yet.

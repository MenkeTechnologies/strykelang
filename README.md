...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** for linear **pure integer** bytecode: `LoadInt`, `LoadConst` when the constant pool entry is an integer (`PerlValue::as_integer`), `Add` / `Sub` / `Mul`, `Negate`, `LogNot` (via `perlrs_jit_lognot_i64`, matching `PerlValue::integer` truth), `NumEq` / `NumNe` / `NumLt` / `NumGt` / `NumLe` / `NumGe` / `Spaceship` (signed `i64` compares, matching the VM `int_cmp` integer path; mixed float/string operands stay on the VM), `BitXor` / `BitAnd` / `BitOr` / `BitNot` (integer paths; Perl set-value `&`/`|` stay on the VM), `Shl` / `Shr`, `Div` (only when the VM would take the exact-integer quotient), `Mod` (divisor must be provably non-zero), `Pow` (via `perlrs_jit_pow_i64` when `0 ≤ exp ≤ 63` on the VM integer path, including a variable `i64` base with a constant exponent), `Pop`, `Dup`, optional `Halt`. `DeclareScalarSlot` / `PreIncSlot` / `PostIncSlot` / `PreDecSlot` / `PostDecSlot` share the same slot table as `GetScalarSlot` / `SetScalarSlot` / `SetScalarSlotKeep` (undef slot prefill only when no `GetScalarSlot` runs before the slot is written in this sequence). `GetScalarPlain` / `GetArg` use dense `i64` tables when every referenced index materializes as `i64` via `as_integer` (plain reads use `scope.get_scalar`; args use the caller’s stack region at `call_frame.stack_base`); native code is either nullary or `fn(*slot, *plain, *arg) -> i64` with unused pointers null when a table is not referenced. Constant-fold simulation rejects inexact division and unsafe `%`. Calls and control flow stay on the interpreter; compiled code is cached (bounded, keyed by op sequence and relevant constant values). Hot-loop tracing is not implemented yet.

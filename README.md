...
- **10,453 standard library primaries** in `%b` (11,186 keys in `%all` including aliases and keywords) — largest bareword library of any language; clears Wolfram v14.3's high-band estimate (~7,300) by ~3,149
...
| **Crypto** | `sha1`, `sha224`, `sha256`, `sha384`, `sha512`, `md5`, `hmac`, `hmac_sha256`, `crc32`, `uuid`, `base64_encode/decode`, `base64url_encode/decode`, `hex_encode/decode` |
...
| String | `chomp`, `chop`, `length`, `substr`, `index`, `rindex`, `split`, `join`, `sprintf`, `printf`, `uc`/`lc`/`ucfirst`/`lcfirst`, `chr`, `ord`, `hex`, `oct`, `crypt`, `fc`, `pos`, `study`, `quotemeta`, `trim`, `lines`, `words`, `chars`, `digits`, `numbers`, `graphemes`, `columns`, `sentences`, `paragraphs`, `sections`, `snake_case`, `camel_case`, `kebab_case`, `template`, `shell_quote`, `shell_split` |
...
| Serialization | `to_json`, `to_csv`, `to_toml`, `to_yaml`, `to_xml`, `to_html`, `to_markdown`, `to_table`/`tbl`, `to_jsonl`/`from_jsonl`, `to_ini`/`from_ini`, `ddump`, `stringify`/`str`, `json_encode`/`json_decode` |
...
- **`@INC` / `%INC` / `require` / `use` / `import`** — `@INC` is built from `-I`, `vendor/perl`, system `perl`'s `@INC` (set `STRYKE_NO_PERL_INC` to skip), the script dir, `STRYKE_INC`, then `.`. List utilities (`sum`, `min`, `max`, `uniq`, `reduce`, `pairs`, `zip`, `mesh`, …) are stryke-native bare-name builtins implemented in Rust at `strykelang/list_builtins.rs` — no Perl module shim, no module to import. `use Module qw(a b);` honors `@EXPORT_OK`/`@EXPORT` for actual user modules. **`import Module qw(a b);` is a 1:1 alias for `use` — same parse, same AST, same semantics; spell it whichever way reads better for the file.** Built-in pragmas (`strict`, `warnings`, `utf8`, `feature`, `open`, `Env`) do not load files. **`use Module VERSION` (Perl 5's per-module pin syntax — `use Foo::Bar 2.13;`) routes resolution directly to `~/.stryke/store/<name>@<VERSION>/`** — skips lockfile and store-scan; surfaces a hard error if that exact extraction isn't on disk (run `s install`). Plain `use Module` resolves three ways: (1) project-local `lib/Foo.stk` always wins; (2) inside a project, the `stryke.lock` pin is authoritative — missing store dir errors instead of falling through to a different version; (3) outside a project, the resolver scans `~/.stryke/store/<canonical>@*/` and picks the HIGHEST semver (`2.0 > 1.99`, `0.10.0 > 0.3.0`; release beats pre-release so `1.0.0 > 1.0.0-rc1`; within pre-releases `rc10 > rc2 > rc1`). The store-scan honors the namespace bridge (`use GUI` finds `stryke-gui@*`) and is independent of `installed.toml` — multiple versions of the same package coexist in the store and the newest wins unless you pin.
...
- **zsh parameter expansion** *(world-first outside zsh)* — double-quoted strings support the full zsh `${ … }` grammar: defaults (`${x:-d}` `${x:=d}` `${x:?m}` `${x:+a}`), trim (`${x#p}` `${x##p}` `${x%p}` `${x%%p}`), substitution (`${x/p/r}` `${x//p/r}`), length (`${#x}`), flags (`${(U)x}` `${(L)x}` …), and subscripts + subscript flags (`${a[2]}` `${a[2,4]}` `${a[(i)v]}` `${a[(r)pat]}`). Delegates to zshrs's `paramsubst` engine — single source of truth, zero reimplementation. **Gated off under `--compat`**, where `${ … }` keeps pure Perl 5 semantics. For the statically-ambiguous forms (`${#x}` length-vs-count, positional defaults) call the **`zpexpand`** builtin with explicit-typed args, arrays by reference: `zpexpand('${#arr}', "arr", \@arr)` → element count.
...
stryke add /work/vendored/mylib                       # path dep (absolute)
stryke add mylib --path=../mylib                      # explicit --path override
...
stryke update [NAME]                                  # re-resolve within manifest constraints, rewrite stryke.lock
stryke upgrade [NAME]                                 # bump stryke.toml pins to latest upstream, then re-resolve
...
stryke list -g
stryke upgrade -g                 # re-pin all global packages at latest upstream (github releases / path source)
...
**Status**: path deps + workspace deps + **GitHub-release deps (`{ github = "OWNER/REPO" [, version = "..."] }`, downloads the prebuilt host-triple tarball with SHA-256 verification — this is the path FFI cdylib packages like the 14 `stryke-*` connectors take)** + **source-clone git deps (`{ git = "...", branch|tag|rev = ... }`, for pure-source dependencies hosted anywhere)** + full CLI surface (`new`/`init`/`add`/`remove`/`install`/`update`/`outdated`/`audit`/`tree`/`info`/`vendor`/`clean`/`run`/`install -g`/`upgrade -g` etc.) are wired and tested today. `s add github.com/OWNER/REPO[@TAG]` auto-writes the `github = "..."` form; `s add ./PATH` auto-writes the `path = "..."` form. Registry deps + the PubGrub semver resolver land when the registry endpoint is deployed — the CLI stubs for `search`/`publish`/`yank` already exist and return clear "registry not deployed yet" diagnostics so the surface matches the RFC end-state.

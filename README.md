...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** with two tiers for **pure integer** bytecode. **Linear JIT**: compiles straight-line (no branches) sequences in a single Cranelift basic block. **Block JIT**: compiles integer bytecode **with control flow** — loops (`Jump`), conditionals (`JumpIfTrue` / `JumpIfFalse`), and short-circuit `&&`/`||` (`JumpIfFalseKeep` / `JumpIfTrueKeep`) — via a full basic-block CFG built by BFS over jump targets, with stack values passed as Cranelift block parameters at branch points. Unreachable dead-code blocks (after unconditional jumps with no incoming edges) are emitted as traps. Eligible data ops (both tiers): `LoadInt`, `LoadFloat` (exact integers only, e.g. `3.0`), `LoadConst` (integer pool entries), `Add` / `Sub` / `Mul`, `Negate`, `LogNot` (via `perlrs_jit_lognot_i64`), `NumEq` / `NumNe` / `NumLt` / `NumGt` / `NumLe` / `NumGe` / `Spaceship`, `BitXor` / `BitAnd` / `BitOr` / `BitNot`, `Shl` / `Shr`, `Div` (exact quotient only), `Mod` (divisor provably non-zero), `Pow` (via `perlrs_jit_pow_i64`, `0 ≤ exp ≤ 63`, dynamic base with constant exponent supported), `Pop`, `Dup`, `Halt`. Slot ops: `DeclareScalarSlot` / `GetScalarSlot` / `SetScalarSlot` / `SetScalarSlotKeep` / `PreIncSlot` / `PostIncSlot` / `PreDecSlot` / `PostDecSlot` share a dense `i64` table (undef prefill when no read before write). Plain-name ops: `GetScalarPlain` / `SetScalarPlain` / `SetScalarKeepPlain` / `PreInc` / `PostInc` / `PreDec` / `PostDec` share a **mutable** dense `i64` table with VM writeback. `GetArg` uses a dense `i64` arg table. Native code is either nullary or `fn(*slot, *plain, *arg) -> i64`. Constant-fold [`Cell`] simulation validates `sdiv`/`srem`/`pow` safety. Compiled code is cached per tier (bounded to 256 entries, keyed by op sequence and relevant constant values). Not JIT’d: function calls, string/array/hash ops, `LoadUndef` (would lose `is_undef` distinction), non-integer values.

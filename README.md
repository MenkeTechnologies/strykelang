...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** for linear **pure integer** bytecode: `LoadInt`, `Add` / `Sub` / `Mul`, `Negate`, `BitXor` / `BitNot`, `Shl` / `Shr`, `Div` (only when the VM would take the exact-integer quotient), `Mod` (divisor must be provably non-zero), `Pow` (via `perlrs_jit_pow_i64` when `0 ≤ exp ≤ 63`), `Pop`, `Dup`, optional `Halt`. Constant-fold simulation rejects inexact division and unsafe `%`. `BitAnd` / `BitOr` (set ops), slots, calls, and control flow stay on the interpreter; compiled code is cached (bounded). Hot-loop tracing and `GetScalarSlot` from native code are not implemented yet.

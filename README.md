...
- **`string` at 134x slower is a real bug.** The `.=` append is almost certainly reallocating the whole buffer on every iteration instead of amortizing — see `ConcatAppend` / `ConcatAppendSlot` in the VM. Fix that and this column should collapse to within ~2x of perl5.
- **`loop` (5M integer adds) at 8x slower** is expected for a match-dispatch interpreter without a hot-loop optimizer. The main dispatch loop (`VM::run_main_dispatch_loop` in [`src/vm.rs`](src/vm.rs)) has substantial per-op overhead (JIT hot-sub probe, signal poll, line lookup, closure wrapper for exception unwinding) that perl5’s op-tree threading avoids. This is where the next real work belongs.
- **`fib(30)` at 1.75x slower** is dominated by call/return overhead: double sub-name lookup (`find_sub_entry` + `resolve_sub_by_name`), heap `CallFrame` push, profiler probes on every call. JIT-on (327 ms) is ~noise-even with JIT-off (323 ms) — the hot-sub probe pays more than the JIT saves on this workload. Fix candidates: interned `CallStaticSub` op, lighter frame, gated hot-sub probe.
...
```
  map  (50k items, per-item work):  265.5 ms
  pmap (50k items, 18 cores):       527.3 ms   →  0.50x
```

...
- **`loop` (5M integer adds) at 8x slower** — the main dispatch loop ([`VM::run_main_dispatch_loop`](src/vm.rs)) still pays match-dispatch vs perl5’s threaded ops; it now skips redundant `lines.get` when the current op shares a source line with the previous op, stops incrementing the per-sub JIT warmup counter once past the threshold, and still uses the `% 1024` signal poll / execution cap. Further wins need superinstructions or a different dispatch model.
- **`fib(30)` call overhead** — bareword calls compile to stash-qualified [`Chunk::names`](src/bytecode.rs) keys (matching [`Interpreter::qualify_sub_key`](src/interpreter.rs)), then [`Op::CallStaticSubId`](src/bytecode.rs) after [`Compiler::patch_static_sub_calls`](src/compiler.rs) so the VM jumps to the compiled entry without a linear `sub_entries` scan; closure restore uses a single [`Interpreter::subs`](src/interpreter.rs) hash lookup by that key instead of [`resolve_sub_by_name`](src/interpreter.rs)’s package fallback path.

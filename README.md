...
- **Special variables** — Not full perlvar(5); see [`SPECIAL_VARIABLES.md`](SPECIAL_VARIABLES.md). **`$"`** is the list separator for **`@array`** inside double-quoted strings; any scalar name starting with **`^`** (including **`${^NAME}`**) goes through **`get_special_var`** (unknown names read **`undef`** from a stash; many are stubs). Covered: `$_`, `$/`, **`$!`** (errno dualvar), `$1`…, `%+`, `@-`/`@+`, **`@{^CAPTURE}`**, **`$*`** (multiline → `(?s)` in **`compile_regex`**), **`$^C`** (SIGINT latch on Unix), `$]`/`$;`, **`$^O`** / **`$^T`** / **`$^V`** / **`$^E`** / **`$^H`**, **`${^WARNING_BITS}`** / **`${^GLOBAL_PHASE}`**, **`$<`**/**`$>`** (uid), **`$(`**/**`$)`** (gid lists on Unix), **`${^MATCH}`** / **`${^PREMATCH}`** / **`${^POSTMATCH}`** (same data as `$&` / `` $` `` / `$'` when driven by the regex engine), **`$+`**, format-related **`$%`**/**`$=`**/**`$-`**/**`$:`**/**`$^`** (top-of-form), **`$^A`**/**`$^F`**/**`$^L`**/**`$^M`**/**`$^N`**/**`$^X`**, **`$INC`** (iterator stub), `$^I`/`$^D`/`$^P`/`$^S`/`$^W`, `$ARGV` with `<>`, `@ARGV`/`%ENV`/`@INC`/`%INC`, **`%SIG`** (on Unix see the Perl-compat **`%SIG`** bullet above; non-Unix no delivery), **`$?`** / **`$|`** (see Perl-compat bullets above). Still missing vs Perl 5: **`$@`** as a full dualvar (perlrs keeps a string); **`English`**; full **`$^V`** as a version object; **`${^GLOBAL_PHASE}`** transitions (perlrs keeps a string, default **`RUN`**). See [`SPECIAL_VARIABLES.md`](SPECIAL_VARIABLES.md) for **`exists`/`delete`** coverage (e.g. **`$href->{k}`** is supported).
...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** for linear **pure integer** bytecode: `LoadInt`, `LoadConst` when the constant pool entry is an integer (`PerlValue::as_integer`), `Add` / `Sub` / `Mul`, `Negate`, `BitXor` / `BitAnd` / `BitOr` / `BitNot` (integer paths; Perl set-value `&`/`|` stay on the VM), `Shl` / `Shr`, `Div` (only when the VM would take the exact-integer quotient), `Mod` (divisor must be provably non-zero), `Pow` (via `perlrs_jit_pow_i64` when `0 ≤ exp ≤ 63`), `Pop`, `Dup`, optional `Halt`. `GetScalarSlot` is emitted when every referenced slot index materializes as `i64` via `PerlValue::as_integer` (otherwise the VM interprets). Constant-fold simulation rejects inexact division and unsafe `%`. Calls and control flow stay on the interpreter; compiled code is cached (bounded, keyed by op sequence and relevant constant values). Hot-loop tracing is not implemented yet.

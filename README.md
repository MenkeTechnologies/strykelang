...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** for linear **pure integer** bytecode: `LoadInt`, `LoadConst` when the constant pool entry is an integer (`PerlValue::as_integer`), `Add` / `Sub` / `Mul`, `Negate`, `LogNot` (via `perlrs_jit_lognot_i64`, matching `PerlValue::integer` truth), `NumEq` / `NumNe` / `NumLt` / `NumGt` / `NumLe` / `NumGe` / `Spaceship` (signed `i64` compares, matching the VM `int_cmp` integer path; mixed float/string operands stay on the VM), `BitXor` / `BitAnd` / `BitOr` / `BitNot` (integer paths; Perl set-value `&`/`|` stay on the VM), `Shl` / `Shr`, `Div` (only when the VM would take the exact-integer quotient), `Mod` (divisor must be provably non-zero), `Pow` (via `perlrs_jit_pow_i64` when `0 ≤ exp ≤ 63`), `Pop`, `Dup`, optional `Halt`. `GetScalarSlot` / `GetScalarPlain` use dense `i64` tables when every referenced index materializes as `i64` via `as_integer` (plain reads use `scope.get_scalar` for the name pool); native signatures are nullary, one table, or slot+plain pointers. Constant-fold simulation rejects inexact division and unsafe `%`. Calls and control flow stay on the interpreter; compiled code is cached (bounded, keyed by op sequence and relevant constant values). Hot-loop tracing is not implemented yet.

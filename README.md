...
- **JIT** // `src/jit.rs` — Cranelift **method JIT** for linear **pure integer** bytecode: `LoadInt`, `Add` / `Sub` / `Mul`, `Negate`, `BitXor` / `BitNot`, `Shl` / `Shr`, `Div` (only when the VM would take the exact-integer quotient), `Mod` (divisor must be provably non-zero), `Pow` (via `perlrs_jit_pow_i64` when `0 ≤ exp ≤ 63`), `Pop`, `Dup`, optional `Halt`. `GetScalarSlot` is emitted when every referenced slot index materializes as `i64` via `PerlValue::as_integer` (otherwise the VM interprets). Constant-fold simulation rejects inexact division and unsafe `%`. `BitAnd` / `BitOr` (set ops), calls, and control flow stay on the interpreter; compiled code is cached (bounded). Hot-loop tracing is not implemented yet.

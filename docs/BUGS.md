...
## BUG-251 — `tr///c` complement flag is silently ignored

```sh
$ s -e 'my $s = "abcde123"; print "$s\n", scalar($s =~ tr/0-9//), "\n", scalar($s =~ tr/0-9//c), "\n"'
abcde123
3
3
```

In Perl, `tr/0-9//c` complements the search list (i.e., matches
non-digits). Counts of digits vs non-digits in `"abcde123"` should
differ (3 vs 5). Stryke returns the same count for both, indicating
the `/c` flag is silently dropped from the operation.

Combined `/cd` also breaks: `tr/A-Za-z//cd` should keep alphas and
delete non-alphas (`"Hello, World!"` → `"HelloWorld"`), but stryke
returns `", !"` — the opposite of the intended semantics.

Pin: `tr_c_flag_ignored_per_bug_251`,
`tr_cd_combination_broken_per_bug_251` in `tests/suite/tr_translit_pin.rs`.

Severity: **bug** (P2; silently produces wrong output on a common
Perl idiom; affects any code using `tr/[set]//c` or `tr/[set]//cd`).


## BUG-252 — `tr///s` squeeze flag is silently ignored

```sh
$ s -e 'my $s = "aaabbbccc"; $s =~ tr/abc//s; print "$s\n"'
aaabbbccc
```

In Perl, `tr/abc//s` squeezes consecutive runs of any matched
character to a single occurrence: `"aaabbbccc"` → `"abc"`. Stryke
leaves the input unchanged.

Translit + squeeze combination also broken:
`tr/abc/xyz/s` on `"aaabbbccc"` should give `"xyz"` (translate then
squeeze); stryke returns `"xxxyyyzzz"` — squeeze step skipped.

Pin: `tr_s_flag_ignored_per_bug_252`,
`tr_translit_s_combo_broken_per_bug_252` in
`tests/suite/tr_translit_pin.rs`.

Severity: **bug** (P2; silently produces wrong output on a less
common but real Perl pattern — particularly relevant for whitespace
normalisation idioms like `tr/ \t\n/ /s`).


## BUG-253 — `splice(@arr, OFFSET, LEN, @list)` flattens `@list` to its length

```sh
$ s -e 'my @t = (1,2,5,6); my @inj = (3,4); splice(@t, 2, 0, @inj); print join(",", @t), "\n"'
1,2,2,5,6
```

In Perl, the LIST argument to `splice` flattens any arrays in the
list: `splice(@t, 2, 0, @inj)` would insert `3,4` into `@t`. Stryke
evaluates the array argument in scalar context (its length: 2)
and inserts a single integer.

Workaround: write the elements as a literal list, or splat via
`@{[@inj]}` which dereferences a temporary arrayref:

```stryke
splice(@target, 2, 0, 3, 4);            # literal list — works
splice(@target, 2, 0, @{[@injection]}); # forced flatten — works
```

Pin: `splice_inject_array_flattens_to_count_per_bug_253`,
`splice_reinsert_via_array_only_inserts_count_per_bug_253`,
`splice_inject_via_list_literal_works` in `tests/suite/splice_pin.rs`.

Severity: **bug** (P1; breaks the most common idiom for splicing one

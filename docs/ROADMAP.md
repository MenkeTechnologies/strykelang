...
1. ✅ Package registry MVP: parse stryke.toml, set up ~/.stryke/store/, local path deps + GitHub-release deps (`{ github = "OWNER/REPO" }` — host-triple tarball + SHA-256 verify, the path FFI cdylib packages take) + source-clone git deps (`{ git = "...", branch|tag|rev = ... }`).
2. ✅ CLI surface: s new, s init, s add (auto-detects github URL → release dep / local path / registry name), s remove, s install (plus update, outdated, audit, vendor, clean, run, install -g).
...
26a. ✅ **Scriptable distributed compute API** (29-verb religious vocab over controller/agent) — `congregation`/`ordain`/`muster`/`pray`/`annex`/`harvest`/`excommunicate`/`smite`/`bestow`/`enshrine`/`exhume`/`smother`/`amen`/`anoint`/`welcome`/`pilgrimage`/`bow`/`lick`/`peruse`/`chant`/`profess`/`apostatize`/`cathedral`/`cloister`/`recant`/`martyr`/`resurrect`/`divine`/`interrogate`. Layered on existing TCP+bincode plumbing. World-first single-keyword scatter-gather + state-harvest + secure-erase as language primitives. Includes `:cloistered` ACL via `AGENT_AUTH` frame, Rayon-parallel scatter, in-process cathedral name registry. Pin tests: `tests/suite/scriptable_controller_pin.rs`. Example: `examples/distributed_congregation.stk`. Design: `docs/killer-features-brainstorm.md` "Scriptable Master/Slave".
...
44. ✅ LSP server (rust-analyzer-class) — full hover/completion surface in `strykelang/lsp.rs`. Now also: `codeAction` with *Extract to variable / constant / function* refactorings, `foldingRange` (`{…}` blocks + `=pod`/`=cut` + `#`-line comment runs), `formatting` (whole-file reformat via `fmt::format_program`), `rename`, 60+ snippet completions with tab-stop placeholders.
45. ✅ DAP debugger integration — `stryke --dap [HOST:PORT]` (stdio or TCP), shared `Debugger` state machine with the TTY `-d` front-end. JetBrains plugin under `editors/intellij/` consumes it with full XDebugProcess: line + function breakpoints, step over / into / out / pause / run-to-cursor, frames, recursive variable drill-down (incl. struct / class / enum / set / sketch types), Evaluate dialog with scalar prelude injection, real-time Console output.
...
- [x] **IntelliJ plugin** — `editors/intellij/`. Full LSP client (semantic tokens, signature help, code actions with *Extract to variable / constant / function*, folding ranges, document formatting, rename, 60+ snippets), DAP debugger (line/function breakpoints, step over/in/out with depth-aware same-line guard, recursive variable drill-down incl. struct/class/enum/set/sketch types, Evaluate dialog), `#` line + `=pod`/`=cut` block comments via `Commenter`, reflection tool window with 9 tabs of `%stryke::*` (≈25k entries), 44-slot color scheme. Works in RustRover, IDEA Ultimate, GoLand, PyCharm Pro, WebStorm, RubyMine, PhpStorm, CLion, Rider, DataGrip, Aqua (paid editions only — LSP API not in Community).

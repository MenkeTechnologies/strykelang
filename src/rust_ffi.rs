...
//! 3. If `~/.cache/forge/ffi/<hash>.(dylib|so)` exists, dlopen and register.
//! 4. Otherwise: spit the body into `~/.cache/forge/ffi/<hash>.rs` wrapped in a minimal
...
/// Perl builtin `__forge_rust_compile(BASE64, LINE)` — invoked at BEGIN time by the code
...
pub fn compile_and_register(body_b64: &str, line: usize) -> PerlResult<()> {
    ...
    // Hash the body (not the wrapped crate source): same body → same dylib across forge
    ...
    // Cache dir: `~/.cache/forge/ffi/<hash>.*`.
...
fn ffi_cache_dir() -> Result<PathBuf, String> {
    ...
    let dir = base.join("forge").join("ffi");
...
const WRAPPER_SALT: &[u8] = b"forge-rust-ffi-v1";
...
fn wrap_crate_source(body: &str) -> String {
    ...
    format!(
        "// auto-generated by forge rust FFI\n\
         #![crate_type = \"cdylib\"]\n\
         #![allow(unused)]\n\

...
fn doc_for_label(label: &str) -> Option<Documentation> {
    ...
    let md: &'static str = match key {
        ...
        "compact" => "perlrs: Remove undef and empty string values from a list (streaming).",
        "reject" => "perlrs: Inverse of filter — keep items where block returns false (streaming).",
        "concat" => "perlrs: Concatenate multiple lists/iterators into one (streaming).",
        "chain" => "perlrs: Alias for `concat` — concatenate multiple lists/iterators (streaming).",
        ...
        "enumerate" => "`enumerate ITERATOR` — yields `[$index, $item]` pairs (streaming).",
        "chunk" => "`chunk N, ITERATOR` — yields N-element arrayrefs (streaming).",
        "dedup" => "`dedup ITERATOR` — drops consecutive duplicates (streaming).",
        "range" => "`range N, M` — lazy integer sequence from N to M (inclusive).",

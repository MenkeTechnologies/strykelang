...
fn doc_for_label(label: &str) -> Option<Documentation> {
    ...
    let md: &'static str = match key {
        ...
        "drop" => "`drop N, @list` — skip the first N elements.",
        "take" => "`take N, @list` — take at most N elements.",
        "drop_while" => "`drop_while { COND } @list` — skip leading matching elements.",
        "take_while" => "`take_while { COND } @list` — take leading matching elements.",

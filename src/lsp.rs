...
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let md: &'static str = match key {
        ...
        "sys_uptime" => "`sys_uptime` — system uptime in seconds (float). On Linux reads `/proc/uptime`. On macOS uses `kern.boottime` sysctl. This is the wall-clock uptime of the machine, not the perlrs process (see `uptime_secs` for process uptime).\n\n```perl\np sys_uptime                             # 147951.5\nmy $days = int(sys_uptime / 86400)\np \"up $days days\"\n```",

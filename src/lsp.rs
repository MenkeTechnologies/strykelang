//! Language server protocol (stdio) for editors — `fo --lsp`.
...
fn dispatch_request(
    ...
    let resp = Response::new_err(
        ...
        format!("forge LSP: unimplemented request {}", req.method),
    );
    conn.sender.send(resp.into()).expect("lsp channel");
    Ok(())
}
...
/// Raw doc text lookup — single source of truth for both LSP hover and `fo docs`.
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let md: &'static str = match key {
        ...
        // ── Control flow ──
        "if" => "The fundamental conditional construct. Evaluates its condition in boolean context and executes the block if true. forge supports both the block form `if (COND) { BODY }` and the postfix form `EXPR if COND`, which is idiomatic for single-statement guards. The condition can be any expression — numbers (0 is false), strings (empty and `\"0\"` are false), undef (false), and references (always true). Postfix `if` cannot have `elsif`/`else` — use the block form for multi-branch logic.\n\n```perl\nmy $n = 42\nif ($n > 0) { p \"positive\" }\np \"big\" if $n > 10          # postfix — clean one-liner\nmy $label = \"even\" if $n % 2 == 0\np \"got: $n\" if defined $n   # guard against undef\n```",
        "elsif" => "Chain additional conditions after an `if` block without nesting. Each `elsif` is tested in order; the first one whose condition is true has its block executed, and the rest are skipped. There is no limit on the number of `elsif` branches. In forge, prefer `match` for complex multi-branch dispatch since it supports pattern matching, destructuring, and guards — but `elsif` remains the right tool for simple linear condition chains. Note: it is `elsif`, not `elseif` or `else if` — the latter is a syntax error.\n\n```perl\nfn classify($n) {\n    if    ($n < 0)   { \"negative\" }\n    elsif ($n == 0)  { \"zero\" }\n    elsif ($n < 10)  { \"small\" }\n    elsif ($n < 100) { \"medium\" }\n    else             { \"large\" }\n}\n1..200 |> map classify |> frequencies |> dd\n```",
        ...
        "next" => "Skip the rest of the current loop iteration and jump to the next one. The loop condition (for `while`/`until`) or the next element (for `for`/`foreach`) is evaluated immediately. Like `last`, `next` supports labeled loops with `next LABEL` for skipping in nested loops. This is the primary tool for filtering within imperative loops. In forge, consider `grep`/`filter` or `|> reject` for functional-style filtering instead.\n\n```perl\nfor (1..10) {\n    next if $_ % 2        # skip odds\n    p $_\n}   # 2 4 6 8 10\n\nfor my $file (glob \"*\") {\n    next unless -f $file   # skip non-files\n    next if $file =~ /^\\./  # skip hidden\n    p $file\n}\n```",
        ...
        "any" => "`any { COND } @list` — returns true (1) if at least one element satisfies the predicate, false (\"\") if none do. The block receives each element as `$_`. Short-circuits on the first match, making it efficient even on large lists. This is the forge equivalent of Perl's `List::Util::any` and can be used in `|>` pipelines.\n\n```perl\nmy @vals = 1, 3, 5, 8\np any { $_ > 7 } @vals   # 1\n1..1000 |> any { $_ == 42 } |> p  # 1\n```",
        ...
        "min" => "`min @list` — returns the numerically smallest value from a list. Compares all elements using numeric (`<=>`) comparison, so stringy values are coerced to numbers. Returns `undef` for an empty list. In forge, `min` is a built-in that does not require `use List::Util` and works directly in `|>` pipelines.\n\n```perl\np min(5, 3, 9, 1)   # 1\nmy @temps = (72.1, 68.5, 74.3)\n@temps |> min |> p  # 68.5\n```",
        "max" => "`max @list` — returns the numerically largest value from a list. Compares all elements using numeric (`<=>`) comparison. Returns `undef` for an empty list. Like `min`, this is a forge built-in available without imports and works in `|>` pipelines. Combine with `map` to extract max values from complex structures.\n\n```perl\np max(5, 3, 9, 1)   # 9\n1..100 |> map { $_ ** 2 } |> max |> p  # 10000\n```",
        "sum" | "sum0" => "`sum @list` returns the numeric sum of all elements. Returns `undef` for an empty list. `sum0` is identical except it returns `0` for an empty list, which avoids the need for a fallback `// 0` guard. Both are forge built-ins that work in `|>` pipelines. Use `sum0` in contexts where an empty input is expected and you need a safe numeric default.\n\n```perl\np sum(1..100)    # 5050\np sum0()          # 0\n@prices |> map { $_->{amount} } |> sum0 |> p\n```",
        "product" => "`product @list` — returns the product of all numeric elements in the list. Returns `undef` for an empty list. Useful for computing factorials, combinatoric products, and compound multipliers. In forge this is a built-in that composes naturally with `|>` pipelines and `range`.\n\n```perl\np product(1..5)   # 120\nrange(1, 10) |> product |> p  # 3628800\n```",
        ...
        "tail" | "tl" => "`tail N, @list` — returns the last N elements of the list. If N exceeds the list length, the entire list is returned. The alias `tl` is also available. This is the complement of `head`/`take` and mirrors Perl's `List::Util::tail`. In forge, it works both as a function call and in `|>` pipelines.\n\n```perl\nmy @t = tail 2, 1..5\n@t |> e p   # 4 5\n1..100 |> tl 3 |> e p  # 98 99 100\n```",
        ...
        "tee" => "`tee FILE, ITERATOR` — write each element to a file as a side effect while passing it through the pipeline.\n\nEvery element that flows through `tee` is appended as a line to the specified file, and the element itself continues downstream unchanged. The file is opened once on first element and closed when the iterator is exhausted. This is the forge equivalent of the Unix `tee` command, useful for auditing or logging intermediate pipeline results to disk.\n\n```perl\n1..10 |> tee \"/tmp/log.txt\" |> map { $_ * 2 } |> e p\nstdin |> tee \"/tmp/raw.log\" |> grep /ERROR/ |> e p\n```",
        ...
        "words" | "wd" => "`words` (alias `wd`) splits a string on whitespace boundaries and returns the resulting list of words. It handles leading, trailing, and consecutive whitespace gracefully — unlike a naive `split / /`, it never produces empty strings. This is the idiomatic way to tokenize a line of text in forge, and the short alias `wd` keeps pipelines compact. It is equivalent to Perl's `split ' '` behavior.\n\n```perl\nmy @w = wd \"  hello   world  \"\np @w       # hello, world\nmy $line = \"  foo  bar  baz  \"\nmy $count = scalar wd $line\np $count   # 3\nrl(\"data.txt\") |> map { scalar wd $_ } |> e p\n```",
        ...
        "size" => "`size` returns the byte size of a file on disk — equivalent to Perl's `-s FILE` file test. With no arguments, it operates on `$_`; with one argument, it stats the given path; with multiple arguments (or a flattened list), it returns an array of sizes. Paths that can't be stat'd return `undef`. This is a forge extension that makes pipelines over filenames concise.\n\n```perl\np size \"Cargo.toml\"                   # 2013\nf |> map +{ $_ => size } |> tj |> p   # [{name => bytes}, ...]\nf |> filter { size > 1024 } |> e p    # files larger than 1 KiB\n```",
        ...
        "fetch_async_json" => "`fetch_async_json` (alias `ftaj`) initiates a non-blocking HTTP GET request that automatically decodes the JSON response when awaited. This combines `fetch_async` with `json_decode`, making it the ideal choice for concurrent API calls where every response is JSON. The task resolves to the decoded forge data structure — a hashref, arrayref, or scalar depending on the JSON content.\n\n```perl\nmy @urls = map { \"https://api.example.com/item/$_\" } 1..10\nmy @tasks = map fetch_async_json @urls\nmy @results = map await @tasks\n@results |> e { p $_->{name} }\n```",
        ...
        "read_json" | "rj" => "Read a JSON file from disk and parse it into a forge data structure (hash ref or array ref). The short alias `rj` keeps JSON-config one-liners terse. Dies if the file does not exist or contains malformed JSON. This is the complement of `write_json`/`wj`.\n\n```perl\nmy $cfg = rj(\"config.json\")\np $cfg->{database}{host}\nmy @items = @{ rj(\"list.json\") }\n@items |> e { p $_->{name} }$1\n\nNote: numeric strings remain strings; use `+0` to coerce if needed.",
        "write_json" | "wj" => "Serialize a forge data structure (hash ref or array ref) as pretty-printed JSON and write it to a file. Creates or overwrites the target file. The short alias `wj` pairs with `rj` for round-trip JSON workflows. Useful for persisting configuration, caching API responses, or generating fixture data.\n\n```perl\nmy %data = (name => \"Alice\", scores => [98, 87, 95])\nwj(\"out.json\", \\%data)\nmy $back = rj(\"out.json\")\np $back->{name}   # Alice\n```",
        ...
        "squared" | "sq" | "square" => "Return the square of a number (`N * N`). This is a forge convenience function — clearer than writing `$n ** 2` or `$n * $n` in pipelines. The aliases `sq` and `square` are interchangeable.\n\n```perl\np squared(5)        # 25\np sq(12)            # 144\n1..5 |> map sq |> e p    # 1 4 9 16 25\nmy $hyp = sqrt(sq($a) + sq($b))  # Pythagorean theorem\n```",
        "cubed" | "cb" | "cube" => "Return the cube of a number (`N * N * N`). This is a forge convenience function for the common `$n ** 3` operation, useful in math-heavy pipelines. The aliases `cb` and `cube` are interchangeable.\n\n```perl\np cubed(3)          # 27\np cb(10)            # 1000\n1..4 |> map cb |> e p  # 1 8 27 64\nmy $vol = cb($side)             # volume of a cube\n```",
        ...
        // ── forge extensions (syntax / macros) ──
        ...
        "deep_equal" | "deq" => "`deep_equal` (alias `deq`) performs structural equality comparison of two values, recursively descending into array refs, hash refs, and scalar refs. Returns 1 if the structures are identical, 0 otherwise. This is the forge equivalent of Node's `assert.deepStrictEqual`, Lodash `isEqual`, or Python's `==` on nested dicts/lists.\n\n```perl\np deep_equal([1, {a => 2}], [1, {a => 2}])   # 1\np deep_equal([1, {a => 2}], [1, {a => 3}])   # 0\np deq({x => [1,2]}, {x => [1,2]})            # 1\n```",
        ...
        "sys_uptime" => "`sys_uptime` — system uptime in seconds (float). On Linux reads `/proc/uptime`. On macOS uses `kern.boottime` sysctl. This is the wall-clock uptime of the machine, not the forge process (see `uptime_secs` for process uptime).\n\n```perl\np sys_uptime                             # 147951.5\nmy $days = int(sys_uptime / 86400)\np \"up $days days\"\n```",
...
/// Public entry point for `fo docs TOPIC` — returns raw markdown doc text.
...
/// Grouped category list for the `fo docs` book view and the static-site

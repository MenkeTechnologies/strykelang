...
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let md: &'static str = match key {
        ...
        "perlrs::builtins" => "`%perlrs::builtins` (short: `%b`) — every callable name → its category.\n\nValues are real category strings (`\"parallel\"`, `\"string\"`, `\"filesystem\"`, `\"serialization\"`, `\"array / list\"`, …), parsed from the `// ── category ──` section comments in `is_perl5_core` and `perlrs_extension_name`. Replaces the earlier `\"perl\"` / `\"extension\"` tag — `grep`-by-category is the natural query.\n\n```perl\np $b{pmap};                                # \"parallel\"\np $b{map};                                 # \"array / list\"\np $b{to_json};                             # \"serialization\"\np scalar keys %b;                          # total callable names\n\n# every parallel op:\nkeys %b |> grep { $b{$_} eq \"parallel\" } |> sort |> p;\n\n# frequency per category:\nmy %count; $count{$b{$_}}++ for keys %b; dd \\%count;\n```\n\nNames falling through to `\"uncategorized\"` flag a missing section comment, not a real unknown.",
        ...
        "perlrs::descriptions" => "`%perlrs::descriptions` (short: `%d`) — name → one-line summary.\n\nFirst sentence of each LSP hover doc (`doc_for_label_text` in `src/lsp.rs`), harvested at build time. Sparse: only names that have a hover doc appear, so `exists` doubles as \"is this documented?\"\n\n```perl\np $d{pmap};                                # \"Run a block in parallel across the elements of a list…\"\np $d{to_json};                             # \"Serialize a PerlValue to a JSON string.\"\np scalar keys %d;                          # count of documented ops\n\n# find every documented op mentioning \"parallel\":\nkeys %d |> grep { $d{$_} =~ /parallel/i } |> sort |> p;\n```",

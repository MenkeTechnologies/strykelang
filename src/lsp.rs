...
fn doc_for_label(label: &str) -> Option<Documentation> {
    ...
    let md: &'static str = match key {
        ...
        "reduce" => "`reduce { $a OP $b } @list` — sequential left fold.",
        "fold" => "`fold { $a OP $b } INIT, @list` — left fold with initial value.",
        "reductions" => "`reductions { $a OP $b } @list` — running reductions (scan).",
        ...
        "stddev" => "Return the population standard deviation of a numeric list.",
        ...
        "head" => "`head N, @list` — return the first N elements.",
        "tail" => "`tail N, @list` — return the last N elements.",
        ...
        "pairmap" => "`pairmap { $a, $b } @list` — map over key-value pairs.",
        "pairgrep" => "`pairgrep { $a, $b } @list` — filter key-value pairs.",
        "pairfirst" => "`pairfirst { $a, $b } @list` — first matching pair.",
        ...
        "drop" => "`drop N, @list` — skip the first N elements (streaming).",
        "skip" => "`skip N, @list` — skip the first N elements (streaming, alias for `drop`).",
        "take" => "`take N, @list` — take at most N elements (streaming).",
        ...
        "lines" => "`lines STRING` — split string into lines (streaming iterator).",
        "chars" => "`chars STRING` — split string into characters (streaming iterator).",
        "trim" => "`trim STRING` or `trim @list` — strip whitespace (streaming on lists).",
        ...
        "with_index" => "`with_index @list` — pairs each element with its index.",
        "tap" => "`tap { side_effect } @list` — execute block per element, return original list.",
        ...
        // ── File / path utilities ──
        "basename" => "Return the filename part of a path.",
        ...
        "realpath" => "Return the resolved absolute path (following symlinks).",
        ...
        "spurt" | "write_file" => "Write a string to a file: `spurt($path, $content)`.",
        ...
        "gethostname" => "Return the system hostname.",

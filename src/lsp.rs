...
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let md: &'static str = match key {
        ...
        "perlrs::builtins" => "`%perlrs::builtins` (short: `%b`) — every callable name perlrs knows about, mapped to its origin.\n\nKeys are all Perl 5 core keywords plus all perlrs extensions; values are `\"perl\"` or `\"extension\"`. Disjoint from construction, so `keys %b == (keys %perlrs::perl_compats) ∪ (keys %perlrs::extensions)`.\n\n```perl\np $b{map};                   # \"perl\"\np $b{pmap};                  # \"extension\"\np scalar keys %b;            # total callable names\nkeys %b |> grep /^to_/ |> p; # every encoder primary\n```\n\nRegenerated at build time by `build.rs` from `is_perl5_core`, `perlrs_extension_name`, and the `try_builtin` match arms — never hand-maintained.",

        "perlrs::aliases" => "`%perlrs::aliases` (short: `%a`) — alias spelling → canonical primary.\n\nKeys are the 2nd-and-later names in each `try_builtin` match arm; values are the arm's first (primary) name. Use this to resolve short spellings or to find all aliases of a given primary.\n\n```perl\np $a{tj};                                  # \"to_json\"\np $a{bn};                                  # \"basename\"\np scalar keys %a;                          # total alias count\n# reverse: every alias for a primary name\nmy $p = \"basename\";\nkeys %a |> grep { $a{$_} eq $p } |> p;\n```",

        "perlrs::perl_compats" => "`%perlrs::perl_compats` (short: `%pc`) — Perl 5 core keyword/builtin set.\n\nSource of truth: `is_perl5_core` in `parser.rs` — names shipped in stock `perl`. Disjoint from `%perlrs::extensions`. Value is always `1`; use `exists` for the predicate.\n\n```perl\np exists $pc{keys} ? 1 : 0;    # 1 — core\np exists $pc{pmap} ? 1 : 0;    # 0 — perlrs extension, not core\np scalar keys %pc;             # core-only count\n```",

        "perlrs::extensions" => "`%perlrs::extensions` (short: `%e`) — perlrs-only names.\n\nUnion of `perlrs_extension_name` (parser-level, what `--compat` rejects) and `try_builtin` arm primaries that aren't Perl 5 core. Disjoint from `%perlrs::perl_compats`.\n\n```perl\np exists $e{pmap} ? 1 : 0;           # 1\np exists $e{keys} ? 1 : 0;           # 0 — core, not extension\nkeys %e |> grep /^p/ |> sort |> p;   # every p* parallel op\n```",

        "perlrs::callable" => "`%perlrs::callable` (short: `%c`) — every callable spelling → its canonical name.\n\nResolves both aliases (via the `try_builtin` arm map) and bare primaries / core keywords (which map to themselves). One lookup answers both 'is this callable?' (via `exists`) and 'what does it dispatch to?'.\n\n```perl\np $c{tj};             # \"to_json\"   (alias → primary)\np $c{to_json};        # \"to_json\"   (primary → self)\np $c{uc};             # \"uc\"        (core keyword, ExprKind-backed)\np exists $c{xyz} ? 1 : 0;  # 0\n```",

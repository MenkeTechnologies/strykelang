...
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let md: &'static str = match key {
        ...
        "letters" | "lt" => "`letters` (alias `lt`) extracts all alphabetic characters from a string and returns them as a list. Filters out digits, punctuation, whitespace — keeps only letters.\n\n```perl\np join \"\", letters(\"h3ll0 w0rld!\")   # hllwrld\n\"abc123DEF\" |> letters |> cnt |> p   # 6\n```",

...
// `parser.rs` (`is_perl5_core`, `perlrs_extension_name`) and the arms of
// the `try_builtin` match below. `BUILTIN_ARMS` powers the alias → primary
// map; `PERL_COMPATS` and `PERLRS_EXTENSIONS` feed `%perl_compats` and
// `%extensions` (disjoint by construction). `%builtins` is the union —
// `builtins = perl_compat + extensions` literally holds.
include!(concat!(env!("OUT_DIR"), "/reflection.rs"));

/// `%builtins` — every known name with its origin tag: `"perl"` for Perl 5
/// core, `"extension"` for perlrs-only. Extensions are unioned from both
/// `perlrs_extension_name` (parser-level — used for `--compat` rejection)
/// and `try_builtin` arm primaries (runtime-dispatch-level). Many dispatch
/// primaries like `basename`/`copy`/`ddump` aren't listed in
/// `perlrs_extension_name`, but they're still callable extensions, so we
/// pull them in too. Core wins on any overlap (shouldn't happen).
pub fn builtins_hash_map() -> indexmap::IndexMap<String, PerlValue> {
    let core: std::collections::HashSet<&str> = PERL_COMPATS.iter().copied().collect();
    let mut m = indexmap::IndexMap::new();
    for n in PERL_COMPATS {
        m.insert((*n).to_string(), PerlValue::string("perl".into()));
    }
    for n in extension_name_iter() {
        if !core.contains(n) {
            m.insert(n.to_string(), PerlValue::string("extension".into()));
        }
    }
    m
}

/// Iterator over every perlrs extension name — both from
/// `perlrs_extension_name` (parser-level) and the `try_builtin` dispatcher's
/// arm primaries. Produces duplicates; callers dedupe via hash insert.
fn extension_name_iter() -> impl Iterator<Item = &'static str> {
    PERLRS_EXTENSIONS
        .iter()
        .copied()
        .chain(BUILTIN_ARMS.iter().filter_map(|arm| arm.first().copied()))
}
...
/// `%perl_compats` — Perl 5 core keyword → `1`. Source: `is_perl5_core` in
/// `parser.rs`. Disjoint from `%extensions` by construction.
pub fn perl_compats_hash_map() -> indexmap::IndexMap<String, PerlValue> {
    PERL_COMPATS
        .iter()
        .map(|n| ((*n).to_string(), PerlValue::integer(1)))
        .collect()
}

/// `%extensions` — perlrs extension name → `1`. Union of
/// `perlrs_extension_name` (parser-level) and `try_builtin` arm primaries,
/// minus anything that's Perl 5 core. The disjointness invariant
/// `keys %perl_compats ∩ keys %extensions == ∅` holds by construction.
pub fn extensions_hash_map() -> indexmap::IndexMap<String, PerlValue> {
    let core: std::collections::HashSet<&str> = PERL_COMPATS.iter().copied().collect();
    let mut m = indexmap::IndexMap::new();
    for n in extension_name_iter() {
        if !core.contains(n) {
            m.insert(n.to_string(), PerlValue::integer(1));
        }
    }
    m
}

/// `%callable` — every callable name (primary, alias, or core keyword) → the
/// canonical (primary) name. Lets `$callable{$name}` answer both "is this
/// callable?" (via `exists`) and "what's it really called?" (the value).
///
/// Primaries resolve to themselves. Aliases resolve to their primary. Core
/// Perl keywords and extensions that aren't surfaced via `try_builtin` arms
/// still show up keyed to their own name, so `$callable{keys}`, `$callable{uc}`
/// etc. work.
pub fn callable_hash_map() -> indexmap::IndexMap<String, PerlValue> {
    let mut m = indexmap::IndexMap::new();
    for arm in BUILTIN_ARMS {
        let Some(primary) = arm.first() else { continue };
        let primary_val = PerlValue::string((*primary).to_string());
        m.insert((*primary).to_string(), primary_val.clone());
        for alias in &arm[1..] {
            m.insert((*alias).to_string(), primary_val.clone());
        }
    }
    // Fill in names that bypass `try_builtin` (modeled as ExprKinds / parsed
    // directly) — they're still callable, they just have no arm to pull a
    // "primary" from, so the key maps to itself.
    for n in PERL_COMPATS.iter().chain(PERLRS_EXTENSIONS.iter()) {
        m.entry((*n).to_string())
            .or_insert_with(|| PerlValue::string((*n).to_string()));
    }
    m
}

//! Offline generator for `docs/reference.html` — the full `pe docs` corpus
...
//! GitHub Pages.
//!
//! Source of truth: `perlrs::lsp::DOC_CATEGORIES` (chapter → topic names)
//! and `perlrs::lsp::doc_text_for(topic)` (raw markdown). Topics not in
...
use perlrs::lsp::{doc_text_for, doc_topics, DOC_CATEGORIES};
...
/// Read `parity/parity_summary.json` (produced by `parity/run_parity.sh`) and
/// render a one-line badge into the `<p id="perlrsParityBadge">` slot of
/// `docs/index.html`. If the JSON is missing the slot stays empty — users
/// who haven't run the harness see no badge rather than stale numbers.
///
/// Hand-parses the ~6 keys we care about rather than pulling in `serde_json`
/// just for this; the file is always small and machine-written.
fn stamp_parity_badge() {
    let json_path = PathBuf::from("parity/parity_summary.json");
    let Ok(json) = fs::read_to_string(&json_path) else {
        return;
    };
    let total = json_field(&json, "total").unwrap_or_default();
    let passed = json_field(&json, "passed").unwrap_or_default();
    let failed = json_field(&json, "failed").unwrap_or_default();
    let percent = json_field(&json, "percent").unwrap_or_default();
    let version = json_field(&json, "perlrs_version").unwrap_or_default();
    if total.is_empty() || passed.is_empty() || percent.is_empty() {
        return;
    }
    let badge = format!("parity · {passed}/{total} pass ({percent}%) · {failed} fail · v{version}");

    let path = PathBuf::from("docs/index.html");
    let Ok(src) = fs::read_to_string(&path) else {
        return;
    };
    let start_tag = r#"<p class="docs-build-line" id="perlrsParityBadge">"#;
    let end_tag = "</p>";
    let Some(s) = src.find(start_tag) else {
        println!("note: parity badge slot not found in docs/index.html");
        return;
    };
    let after = s + start_tag.len();
    let Some(e_rel) = src[after..].find(end_tag) else {
        return;
    };
    let e = after + e_rel;
    if &src[after..e] == badge {
        return;
    }
    let new_src = format!(
        "{}{}{}{}",
    ...
    println!("stamped parity badge: {}", badge);
}

/// Pluck a bare-JSON field by key. Matches `"key": "value"` or
/// `"key": number` / `"key": 12.34`. Not a full parser — sufficient for the
/// known-shape `parity/parity_summary.json`.
...
fn build_page() -> String {
    ...
    <div class="tutorial-header-inner">
      <div>
        <h1 class="tutorial-brand">// PERLRS — FULL REFERENCE</h1>
      ...
      <div class="tutorial-toolbar">
        ...
        <a class="btn btn-secondary" href="https://github.com/MenkeTechnologies/perlrs" target="_blank" rel="noopener noreferrer">GitHub</a>
      </div>
    </div>
  </header>
...
<head>
  ...
  <meta name="description" content="perlrs full reference — every builtin, keyword, alias, and extension with its LSP hover doc rendered as a static page.">
  <title>perlrs — Reference</title>

...
//! stdout.  `STRYKE_PROGRESS_PLAIN=1` forces one line per tick (CI/logs).
//! `STRYKE_PROGRESS_FULLSCREEN=1` opts in to the alternate-screen mode (PmapProgress only).
...
fn env_force_plain_lines() -> bool {
    match std::env::var("STRYKE_PROGRESS_PLAIN") {
...
fn env_fullscreen_mode() -> bool {
    std::env::var("STRYKE_PROGRESS_FULLSCREEN")

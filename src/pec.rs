...
//! for warm starts without re-parsing or re-compiling when `PERLRS_BC_CACHE=1`.
...
/// `PERLRS_BC_CACHE=1` enables read-through `.pec` in [`cache_dir`] / `<sha256>.pec`.
pub fn cache_enabled() -> bool {
    matches!(
        std::env::var("PERLRS_BC_CACHE").as_deref(),
        Ok("1") | Ok("true") | Ok("yes")
    )
}

/// `~/.cache/perlrs/bc` (or `$PERLRS_BC_DIR` override).
pub fn cache_dir() -> PathBuf {
    if let Ok(p) = std::env::var("PERLRS_BC_DIR") {
        return PathBuf::from(p);
    }
    let home = std::env::var_os("HOME")
        .or_else(|| std::env::var_os("USERPROFILE"))
        .unwrap_or_default();
    PathBuf::from(home).join(".cache").join("perlrs").join("bc")
}

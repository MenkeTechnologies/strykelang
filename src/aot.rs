//! Ahead-of-time build: bake a Perl script into a copy of the running `pe` binary as
//! a compressed trailer, producing a self-contained executable.
//!
//! Layout (little-endian, appended to the end of a copy of the `pe` binary):
//!
//! ```text
//!   [elf/mach-o bytes of pe ...]   (unchanged, still runs as `pe`)
//!   [zstd-compressed payload ...]
//!   [u64 compressed_len]
//!   [u64 uncompressed_len]
//!   [u32 version]
//!   [u32 reserved (0)]
//!   [8 bytes magic  b"PERLRSBN"]
//! ```
//!
...
//! segments, so appending data leaves the original `pe` fully runnable. On macOS the
...
/// 8-byte trailer magic (`b"PERLRSBN"`).
pub const AOT_MAGIC: &[u8; 8] = b"PERLRSBN";
...
/// of the `pe` binary; this function only touches the tail.
...
/// one seek+read+zstd-decode when there is. Safe to call on every `pe` startup.
...
pub fn build(script_path: &Path, out_path: &Path) -> Result<PathBuf, String> {
    let source = fs::read_to_string(script_path)
        .map_err(|e| format!("pe build: cannot read {}: {}", script_path.display(), e))?;
    ...
    let exe = std::env::current_exe()
        .map_err(|e| format!("pe build: locating current executable: {}", e))?;
...
/// from stacking trailers: `pe build a.pl -o a && pe --exe a build b.pl -o b` would otherwise
...
mod tests {
    ...
    fn copy_without_trailer_strips_embedded_script() {
        ...
        fs::write(&src, b"pretend pe binary bytes").unwrap();

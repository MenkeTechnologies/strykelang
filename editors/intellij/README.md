...
| `definition` / `declaration` / `references` / `documentHighlight` | cross-file via the server's `SymbolTable`. **Package-name jump-to-decl** chases `use Foo::Bar` / `Foo::Bar::func()` references through `static_analysis::resolve_require_path_from_file_versioned` (arm 1: project `lib/` → arm 1b: use-site pin `use Foo::Bar 2.13` → arm 2: `stryke.lock` pin → arm 3: store-scan for highest installed, release > pre-release per the runtime's `VersionRank`) so Cmd-B on a package whose decl lives in `~/.stryke/store/<pkg>@<ver>/lib/<Pkg>.stk` lands on the `package Foo::Bar` line. Pinned references chase the EXACT pinned version's file; unpinned references in a standalone script chase the newest installed version on disk. Transitive `use` inside the chased file inherits no pin from the caller — `use Foo 2.13` doesn't constrain `Foo`'s own internal `use Bar`. |

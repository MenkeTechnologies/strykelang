...
| `definition` / `declaration` / `references` / `documentHighlight` | cross-file via the server's `SymbolTable`. **Package-name jump-to-decl** chases `use Foo::Bar` / `Foo::Bar::func()` references through `resolve_require_path_from_file_versioned` (arm 1: project `lib/` → arm 1b: use-site pin `use Foo::Bar 2.13` → arm 2: `stryke.lock` → arm 3: store-scan for highest installed version) so Cmd-B on a package whose decl lives in `~/.stryke/store/<pkg>@<ver>/lib/<Pkg>.stk` lands on the `package Foo::Bar` line. Pinned references chase the EXACT pinned version's file; unpinned references in a standalone script chase the newest installed version on disk. |

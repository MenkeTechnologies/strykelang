...
class StrykeSettingsConfigurable : Configurable {
    ...
    private val lspEnabledBox = JBCheckBox("Enable LSP (uses `st --lsp`)")
    private val extraLspArgsField = JBTextField()
    private val defaultNoInteropBox = JBCheckBox("Default new run configs to `--no-interop`")
    ...
    override fun createComponent(): JComponent {
        val p = FormBuilder.createFormBuilder()
            ...
            .addTooltip("Leave blank to use the first `st` (or `stryke`) on \$PATH.")
            ...
            .addTooltip("`KEY=VAL` pairs, whitespace-separated. e.g. RUST_LOG=info STRYKE_AI=0")
            ...
            .addTooltip("Comma-separated, no leading dot. Default: `stk`")

            .addComponent(sectionHeader("Run configurations"))
            .addComponent(defaultNoInteropBox)
    ...
    override fun isModified(): Boolean {
        val s = StrykeSettings.getInstance()
        return executableField.text != (s.stExecutable ?: "") ||
            lspEnabledBox.isSelected != s.lspEnabled ||
            extraLspArgsField.text != s.extraLspArgs ||
            defaultNoInteropBox.isSelected != s.defaultNoInterop ||
    ...
    override fun apply() {
        val s = StrykeSettings.getInstance()
        s.stExecutable = executableField.text.takeIf { it.isNotBlank() }
        s.lspEnabled = lspEnabledBox.isSelected
        s.extraLspArgs = extraLspArgsField.text
        s.defaultNoInterop = defaultNoInteropBox.isSelected
        s.disableLexerHighlighting = disableLexerBox.isSelected
        s.fileExtensions = fileExtensionsField.text.ifBlank { "stk" }
    ...
    override fun reset() {
        val s = StrykeSettings.getInstance()
        executableField.text = s.stExecutable ?: ""
        lspEnabledBox.isSelected = s.lspEnabled
        extraLspArgsField.text = s.extraLspArgs
        defaultNoInteropBox.isSelected = s.defaultNoInterop
        disableLexerBox.isSelected = s.disableLexerHighlighting

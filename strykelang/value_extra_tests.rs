//! Extra tests for `StrykeValue` to ensure correct Perl-like semantics.
...
fn test_perl_value_truthiness() {
    ...
    // Strings
    assert!(StrykeValue::string("true".into()).is_true());
    assert!(StrykeValue::string("1".into()).is_true());
    assert!(!StrykeValue::string("0".into()).is_true());
    assert!(!StrykeValue::string("".into()).is_true());
    // Perl quirk: "00" is true, but "0" is false.
    assert!(StrykeValue::string("00".into()).is_true());
    assert!(StrykeValue::string("0.0".into()).is_true());

    // Floats
    assert!(StrykeValue::float(1.0).is_true());
...
fn test_numeric_conversions() {
    // String to Int
    assert_eq!(StrykeValue::string("42".into()).to_int(), 42);
    assert_eq!(StrykeValue::string("42.5".into()).to_int(), 42);
    assert_eq!(StrykeValue::string("  42  ".into()).to_int(), 42);
    assert_eq!(StrykeValue::string("42abc".into()).to_int(), 42);
    assert_eq!(StrykeValue::string("abc42".into()).to_int(), 0);

    // Float to Int
    assert_eq!(StrykeValue::float(42.9).to_int(), 42);
    assert_eq!(StrykeValue::float(-42.9).to_int(), -42);

    // Undef to Int
    assert_eq!(StrykeValue::UNDEF.to_int(), 0);
}

#[test]
fn test_string_conversions() {
    assert_eq!(StrykeValue::integer(42).to_string(), "42");
    assert_eq!(StrykeValue::float(42.5).to_string(), "42.5");
    assert_eq!(StrykeValue::UNDEF.to_string(), "");

    // Float that looks like an integer
    assert_eq!(StrykeValue::float(42.0).to_string(), "42");
}

#[test]
fn test_cloning_semantics() {
    // Array (Value type) should deep copy on clone
    let v1 = StrykeValue::array(vec![StrykeValue::integer(10)]);
    let _v2 = v1.clone();

    if let Some(mut arr) = v1.as_array_vec() {
        arr[0] = StrykeValue::integer(20);
...
fn test_comparison_logic() {
    let ten = StrykeValue::integer(10);
    let two_str = StrykeValue::string("2".into());
...
fn test_deep_copy_on_clone() {
    ...
    let arr = vec![StrykeValue::integer(1)];
    let v1 = StrykeValue::array(arr);
    let v2 = v1.clone();
...
fn test_shallow_clone_shares_ptr() {
    let v1 = StrykeValue::array(vec![StrykeValue::integer(1)]);
...
fn test_array_ref_shares_on_clone() {
    ...
    let shared_vec = Arc::new(RwLock::new(vec![StrykeValue::integer(1)]));
    let v1 = StrykeValue::array_ref(shared_vec);
    let v2 = v1.clone();
...
fn test_nested_cloning() {
    ...
    let inner = StrykeValue::array(vec![StrykeValue::integer(1)]);
    let outer = StrykeValue::array(vec![inner]);
...
fn test_hash_cloning() {
    ...
    map.insert("key".to_string(), StrykeValue::integer(1));
    let h1 = StrykeValue::hash(map);
    let h2 = h1.clone();

...
fn b60_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// Curtate life expectancy e_x = (l_{x+1} + l_{x+2} + ...) / l_x. Given a
/// suffix [l_x, l_{x+1}, ...], compute e_x.
fn builtin_life_expectancy_e0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l = b60_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if l.is_empty() || l[0] <= 0.0 { return Ok(PerlValue::float(0.0)); }
    let s: f64 = l.iter().skip(1).sum();
    Ok(PerlValue::float(s / l[0]))
}

/// Force of mortality μ(x) = -d ln l(x) / dx ≈ -(ln l_{x+1} − ln l_{x-1}) / 2.
fn builtin_force_of_mortality(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l_minus = f1(args).max(1e-12);
    let l_plus = args.get(1).map(|v| v.to_number()).unwrap_or(l_minus).max(1e-12);
    Ok(PerlValue::float(-(l_plus.ln() - l_minus.ln()) / 2.0))
}
...
fn builtin_select_ultimate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(alpha * l_select + (1.0 - alpha) * l_ult))
}

/// Annuity-due ä_n = (1 − vⁿ) / d, with v = 1/(1+i), d = i/(1+i).
fn builtin_annuity_due_an(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if i == 0.0 { return Ok(PerlValue::float(n)); }
    let v = 1.0 / (1.0 + i);
    let d = i / (1.0 + i);
    Ok(PerlValue::float((1.0 - v.powf(n)) / d))
}

/// Annuity-immediate a_n = (1 − vⁿ) / i.
fn builtin_annuity_immediate_an(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if i == 0.0 { return Ok(PerlValue::float(n)); }
    let v = 1.0 / (1.0 + i);
    Ok(PerlValue::float((1.0 - v.powf(n)) / i))
}

/// n-year term life A^1_{x:n} = Σ_{k=0}^{n-1} v^{k+1} k_p_x q_{x+k}.
/// Args: i, q_x array of one-year mortality rates (length n).
fn builtin_term_life_a_n_t(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = f1(args);
    let q = b60_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let v = 1.0 / (1.0 + i);
    let mut p_x = 1.0_f64;
    let mut a = 0.0_f64;
...
fn builtin_whole_life_a(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_term_life_a_n_t(args)
}

/// Pure endowment _n E_x = vⁿ · n_p_x.
fn builtin_endowment_pure_e(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = f1(args);
    let q = b60_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let v = 1.0 / (1.0 + i);
    let n = q.len();
...
fn builtin_multi_decrement_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let qs = b60_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let prod_one_minus: f64 = qs.iter().map(|q| 1.0 - q).product();
    Ok(PerlValue::float((1.0 - prod_one_minus).clamp(0.0, 1.0)))
}
...
fn builtin_multi_state_pij(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m = b60_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    if m.len() != 4 { return Ok(PerlValue::float(0.0)); }
    ...
    if i >= 2 || j >= 2 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(acc[i][j]))
}

/// Bühlmann credibility factor Z = n / (n + k), k = E[Var(X|Θ)] / Var(E[X|Θ]).
fn builtin_credibility_buhlmann(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = f1(args);
    ...
    Ok(PerlValue::float(n / (n + k)))
}

/// Lognormal severity loss: density f(x) = (1/(xσ√2π)) exp(−(ln x − μ)²/(2σ²)).
fn builtin_loss_severity_lognormal(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    let z = (x.ln() - mu) / sigma;
    Ok(PerlValue::float((-z * z / 2.0).exp() / (x * sigma * (2.0 * std::f64::consts::PI).sqrt())))
}

/// Poisson loss-frequency PMF: P(N=k) = e^(-λ) λ^k / k!.
fn builtin_loss_frequency_poisson(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lambda = f1(args).max(0.0);
    let k = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    if k < 0 { return Ok(PerlValue::float(0.0)); }
    let mut log_fact = 0.0_f64;
    for j in 1..=k { log_fact += (j as f64).ln(); }
    Ok(PerlValue::float((-lambda + k as f64 * lambda.max(1e-12).ln() - log_fact).exp()))
}

/// Lundberg ruin upper bound: Ψ(u) ≤ exp(-R·u). Args: u (initial surplus), R.
fn builtin_ruin_probability_lundberg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    Ok(PerlValue::float((-r * u).exp()))
}

/// Cramér-Lundberg surplus increment U(t+dt) = U(t) + (1+θ)λμ·dt − loss_increment.
fn builtin_cramer_lundberg_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);

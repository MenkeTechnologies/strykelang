...
fn builtin_molecular_weight_compound(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_molarity_dilution(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m1 = f1(args);
...
fn builtin_gas_constant_value(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(B63_R_GAS))
}

/// Eyring equation: k = (k_B·T/h) · exp(−ΔG‡ / RT). Args: T (K), ΔG‡ (J/mol).
fn builtin_eyring_rate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_van_t_hoff_kp(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_henderson_buffer(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pka = f1(args);
...
fn builtin_titration_ph_endpoint(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_a = f1(args);
...
fn builtin_isoelectric_point_protein(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pka1 = f1(args);
...
/// Ka → pKa.
fn builtin_ka_to_pka(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let ka = f1(args).max(1e-300);
    Ok(StrykeValue::float(-ka.log10()))
}

/// pKb → Kb.
fn builtin_pkb_to_kb(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pkb = f1(args);
...
fn builtin_amphoteric_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_oxidation_number(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let formal = i1(args);
...
fn builtin_half_reaction_balance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_redox_potential_cell(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let e_std = f1(args);
...
fn builtin_electrolysis_mass(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q_coul = f1(args);
...
fn builtin_spectrophotometer_beer_lambert(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let eps = f1(args);
...
fn builtin_epsilon_extinction(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_transmittance_to_a(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// bipy, phen, NO₂⁻, CN⁻).
fn builtin_crystal_field_ligand(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_jahn_teller_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let d = i1(args);
...
fn builtin_vsepr_geometry(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let steric = i1(args);
...
fn builtin_lewis_dot_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_formal_charge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let group_n = i1(args);
    let lone = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
...
fn builtin_resonance_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dbl = i1(args);
...
fn builtin_ramachandran_phi_psi(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let phi = f1(args);
...
/// from centre of mass.
fn builtin_rg_radius_of_gyration(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_spectroscopic_factor(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_avogadro_constant(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {

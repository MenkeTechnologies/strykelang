...
// ══════════════════════════════════════════════════════════════════════
// Probabilistic data structures (Bloom, count-min, HLL, t-digest)
// ══════════════════════════════════════════════════════════════════════

fn hash_bytes(s: &str, seed: u64) -> u64 {
...
pub fn bloom_filter_new(args: &[StrykeValue]) -> StrykeValue {
    ...
    h.insert("size".to_string(), StrykeValue::integer(size as i64));
    h.insert("hashes".to_string(), StrykeValue::integer(hashes as i64));
    h.insert("bits".to_string(), arr(bits));
    StrykeValue::hash_ref(Arc::new(RwLock::new(h)))
}

pub fn bloom_filter_add(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let key = args.get(1).map(|v| v.to_string()).unwrap_or_default();
    let g = f.read();
    let size = g.get("size").map(|v| v.to_int() as usize).unwrap_or(0);
    let hashes = g.get("hashes").map(|v| v.to_int() as usize).unwrap_or(0);
    let bits_v = g.get("bits").cloned().unwrap_or(StrykeValue::UNDEF);
    drop(g);
    let Some(bits) = bits_v.as_array_ref() else {
        return StrykeValue::UNDEF;
    };
    let mut bw = bits.write();
    for i in 0..hashes {
        let h = hash_bytes(&key, i as u64) as usize;
        let idx = h % size;
        if idx < bw.len() {
            bw[idx] = StrykeValue::integer(1);
        }
    }
    StrykeValue::integer(1)
}

pub fn bloom_filter_contains(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let key = args.get(1).map(|v| v.to_string()).unwrap_or_default();
    ...
    for i in 0..hashes {
        let h = hash_bytes(&key, i as u64) as usize;
        let idx = h % size;
        if br.get(idx).map(|v| v.to_int()).unwrap_or(0) == 0 {
            return StrykeValue::integer(0);
        }
    }
    StrykeValue::integer(1)
...
pub fn count_min_sketch_new(args: &[StrykeValue]) -> StrykeValue {
    ...
    let counts: Vec<StrykeValue> = (0..(width * depth))
        ...
        .collect();
    let mut h: IndexMap<String, StrykeValue> = IndexMap::new();
    h.insert("kind".to_string(), StrykeValue::string("cms".to_string()));
    h.insert("width".to_string(), StrykeValue::integer(width as i64));
    h.insert("depth".to_string(), StrykeValue::integer(depth as i64));
    h.insert("counts".to_string(), arr(counts));
    StrykeValue::hash_ref(Arc::new(RwLock::new(h)))
}

pub fn count_min_sketch_add(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let key = args.get(1).map(|v| v.to_string()).unwrap_or_default();
    let g = f.read();
    let width = g.get("width").map(|v| v.to_int() as usize).unwrap_or(0);
    let depth = g.get("depth").map(|v| v.to_int() as usize).unwrap_or(0);
    let counts_v = g.get("counts").cloned().unwrap_or(StrykeValue::UNDEF);
    drop(g);
    let Some(counts) = counts_v.as_array_ref() else {
        return StrykeValue::UNDEF;
    };
    let mut cw = counts.write();
    for i in 0..depth {
        let h = hash_bytes(&key, i as u64) as usize;
        let idx = i * width + h % width;
        if let Some(v) = cw.get(idx) {
            cw[idx] = StrykeValue::integer(v.to_int() + 1);
        }
    }
    StrykeValue::integer(1)
}

pub fn count_min_sketch_query(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let key = args.get(1).map(|v| v.to_string()).unwrap_or_default();
    ...
    for i in 0..depth {
        let h = hash_bytes(&key, i as u64) as usize;
        let idx = i * width + h % width;
        if let Some(v) = cr.get(idx) {
            min = min.min(v.to_int());
        }
    }
    StrykeValue::integer(if min == i64::MAX { 0 } else { min })
}

pub fn hyperloglog_new(args: &[StrykeValue]) -> StrykeValue {
    use indexmap::IndexMap;
    let precision = arg_i64(args, 0).unwrap_or(12).clamp(4, 16) as usize;
...
pub fn hyperloglog_add(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let key = args.get(1).map(|v| v.to_string()).unwrap_or_default();
    ...
    let bucket = (h & (m - 1)) as usize;
    let w = h >> precision;
    let leading = w.leading_zeros() as i64 - precision as i64 + 1;
    let mut rw = registers.write();
    if let Some(cur) = rw.get(bucket) {
        if leading > cur.to_int() {
            rw[bucket] = StrykeValue::integer(leading);
        }
    }
    StrykeValue::integer(1)
}

pub fn hyperloglog_estimate(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let g = f.read();
    ...
    StrykeValue::integer(est as i64)
}

pub fn hyperloglog_merge(args: &[StrykeValue]) -> StrykeValue {
    ...
    let g = a.read();
    let registers_a = g.get("registers").cloned().unwrap_or(StrykeValue::UNDEF);
    drop(g);
    let g2 = b.read();
    let registers_b = g2.get("registers").cloned().unwrap_or(StrykeValue::UNDEF);
    drop(g2);
    let Some(ra) = registers_a.as_array_ref() else {
        return StrykeValue::UNDEF;
    };
    let Some(rb) = registers_b.as_array_ref() else {
        return StrykeValue::UNDEF;
    };
    let mut aw = ra.write();
    let br = rb.read();
    for i in 0..aw.len().min(br.len()) {
        let am = aw[i].to_int();
        let bm = br[i].to_int();
        if bm > am {
            aw[i] = StrykeValue::integer(bm);
        }
    }
    StrykeValue::integer(1)
}
...
pub fn tdigest_add(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let val = arg_f64(args, 1).unwrap_or(0.0);
    let g = f.read();
    let samples_v = g.get("samples").cloned().unwrap_or(StrykeValue::UNDEF);
    drop(g);
    if let Some(samples) = samples_v.as_array_ref() {
        samples.write().push(StrykeValue::float(val));
    }
    StrykeValue::integer(1)
}

pub fn tdigest_quantile(args: &[StrykeValue]) -> StrykeValue {
    let Some(f) = args.first().and_then(|v| v.as_hash_ref()) else {
        return StrykeValue::UNDEF;
    };
    let q = arg_f64(args, 1).unwrap_or(0.5).clamp(0.0, 1.0);
    let g = f.read();
    let samples_v = g.get("samples").cloned().unwrap_or(StrykeValue::UNDEF);
    drop(g);
    let Some(samples) = samples_v.as_array_ref() else {
        return StrykeValue::UNDEF;
    };
    let mut vals: Vec<f64> = samples.read().iter().map(|v| v.to_number()).collect();
    if vals.is_empty() {
        return StrykeValue::UNDEF;
    }
    vals.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    let idx = ((vals.len() - 1) as f64 * q).round() as usize;
    StrykeValue::float(vals[idx])
}

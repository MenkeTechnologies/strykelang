...
fn builtin_tensor_contract_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_tensor_outer_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
...
fn builtin_tensor_trace_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_tensor_contract_two(args)
}

/// Symmetrize T_(ab) = (T_ab + T_ba) / 2
fn builtin_tensor_symmetrize_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_tensor_antisymmetrize_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float((a - b) / 2.0))
}
...
fn builtin_levi_civita_three(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = i1(args);
...
fn builtin_levi_civita_four(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v: Vec<i64> = (0..4)
...
fn builtin_kronecker_three(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = i1(args);
    let j = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
...
fn builtin_kronecker_four(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = i1(args);
    let j = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
...
fn builtin_metric_minkowski_eta_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mu = i1(args);
...
fn builtin_metric_schwarzschild_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
...
fn builtin_metric_kerr_step_simple(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(10.0);
...
fn builtin_metric_frw_lapse(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let gauge = i1(args);
...
fn builtin_christoffel_first_kind_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dg_ab = f1(args);
...
fn builtin_christoffel_second_kind_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g_inv = f1(args);
...
fn builtin_riemann_tensor_step_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dc_g_bd = f1(args);
...
fn builtin_riemann_curvature_normal_form(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k = f1(args);
    Ok(StrykeValue::float(-k / 3.0))
}

/// Ricci R_{ab} = R^c_{acb}: contraction of Riemann over upper index = c. Sum diagonal.
fn builtin_ricci_tensor_step_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_scalar_curvature_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b39_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().sum()))
}

/// Einstein tensor G_{ab} = R_{ab} - ½ g_{ab} R
fn builtin_einstein_tensor_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r_ab = f1(args);
...
fn builtin_weyl_tensor_step_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let riemann = f1(args);
...
fn builtin_schouten_tensor_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r_ab = f1(args);
...
fn builtin_geodesic_equation_step_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let gamma = f1(args);
...
fn builtin_parallel_transport_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v_a = f1(args);
...
fn builtin_covariant_derivative_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dva = f1(args);
    let gamma = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let v_b = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_christoffel_symbol_normalize(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_christoffel_second_kind_step(args)
}

/// Ricci identity [∇_a, ∇_b] V^c = R^c_{dab} V^d → linear contribution
fn builtin_ricci_identity_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_bianchi_first_identity_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r1 = f1(args);
...
fn builtin_bianchi_second_identity_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let nabla_a = f1(args);
...
fn builtin_killing_vector_lie_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let nab = f1(args);
...
fn builtin_lie_derivative_scalar_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x_a = f1(args);
...
fn builtin_lie_derivative_vector_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x_b = f1(args);
...
fn builtin_exterior_derivative_one_form(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dao_b = f1(args);
...
fn builtin_hodge_star_one_form(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w1 = f1(args);
...
fn builtin_codifferential_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_laplace_de_rham_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dd = f1(args);
...
fn builtin_volume_form_riemannian(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let det_g = f1(args);
    Ok(StrykeValue::float(det_g.abs().sqrt()))
}

/// Hodge inner product ⟨α, β⟩ = ∫ α ∧ *β
fn builtin_hodge_inner_product_one(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_sectional_curvature_two_plane(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r_xyxy = f1(args);
...
fn builtin_gauss_codazzi_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_mainardi_codazzi_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lhs = f1(args);
...
fn builtin_weingarten_map_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dn = f1(args);
    Ok(StrykeValue::float(-dn))
}

/// Shape operator eigenvalues — return mean of two principal curvatures
fn builtin_shape_operator_eig(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k1 = f1(args);
...
fn builtin_mean_curvature_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k1 = f1(args);
    let k2 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float((k1 + k2) / 2.0))
}
...
fn builtin_gaussian_curvature_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k1 = f1(args);
    let k2 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_extrinsic_principal_curv(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k1 = f1(args);
    let k2 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
/// Args: E, G, E_vv, G_uu.
fn builtin_intrinsic_principal_curv(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let e = f1(args);
...
fn builtin_geodesic_curvature_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let kappa = f1(args);
...
fn builtin_darboux_frame_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let kappa_g = f1(args);
    let kappa_n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_fermi_normal_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_synge_world_function(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let d = f1(args);
    Ok(StrykeValue::float(0.5 * d * d))
}

/// Raychaudhuri equation step for expansion: dθ/dτ = -⅓θ² - σ² + ω² - R_{ab}u^au^b
fn builtin_raychaudhuri_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta = f1(args);
...
fn builtin_expansion_scalar_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let div_u = f1(args);
...
fn builtin_shear_tensor_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b39_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_twist_tensor_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b39_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_optical_scalars_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta = f1(args);
...
fn builtin_peeling_step_psi4(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let psi4 = f1(args);
...
fn builtin_ads_metric_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_de_sitter_metric_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
    let l = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if l == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(-(1.0 - r * r / (l * l))))
...
fn builtin_warped_product_step_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g_b = f1(args);
...
fn builtin_kaluza_klein_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g4 = f1(args);
...
fn builtin_brans_dicke_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let phi = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_horndeski_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = i1(args);
...
fn builtin_einstein_dilaton_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_gauss_bonnet_term_2d(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_chern_pontryagin_4d_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r2 = f1(args);
    Ok(StrykeValue::float(r2))
}

/// ADM mass M_ADM = (1/16π) ∮ (∂_j h_{ij} - ∂_i h_{jj}) dS^i
fn builtin_adm_mass_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let surface_int = f1(args);
    Ok(StrykeValue::float(surface_int / (16.0 * std::f64::consts::PI)))
}

/// Komar mass M_K = -(1/8π) ∮ ∇^a ξ^b dS_{ab}
fn builtin_komar_mass_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let surface_int = f1(args);
    Ok(StrykeValue::float(-surface_int / (8.0 * std::f64::consts::PI)))
}

/// Bondi mass at null infinity
fn builtin_bondi_mass_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_brown_york_quasilocal(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k_diff = f1(args);
...
fn builtin_isolated_horizon_charge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let area_int = f1(args);
    Ok(StrykeValue::float(area_int / (4.0 * std::f64::consts::PI)))
}

/// Trapped surface check: θ_+ < 0 AND θ_- < 0
fn builtin_trapped_surface_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_apparent_horizon_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta_plus = f1(args);
    Ok(StrykeValue::integer(if theta_plus.abs() < 1e-9 { 1 } else { 0 }))
}

/// Event horizon check at r = 2M for Schwarzschild
fn builtin_event_horizon_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_cosmological_constant_term(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lambda = f1(args);
    let g = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_de_sitter_radius_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lambda = f1(args);
...
fn builtin_anti_de_sitter_radius_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lambda = f1(args);
    if lambda >= 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float((-3.0 / lambda).sqrt()))
}
...
fn builtin_penrose_diagram_factor(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let ct = t.cos();
    let cr = r.cos();
...
fn builtin_conformal_compactification_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
    Ok(StrykeValue::float(1.0 / (1.0 + r * r)))
}

/// Schwarzschild → Kruskal coordinate U = -e^(-u/4M), V = e^(v/4M)
fn builtin_schwarzschild_kruskal_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
    let m = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_gullstrand_painleve_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
    let m = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_kerr_newman_charge_term(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(10.0);
    if r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_boyer_lindquist_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(10.0);
...
fn builtin_hartle_thorne_metric(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(10.0);
...
fn builtin_oppenheimer_volkoff_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rho = f1(args);
...
fn builtin_post_newtonian_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_shapiro_delay_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
...
fn builtin_mercury_perihelion_advance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let a = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_gravitational_wave_quadrupole(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q_dd = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_plus_polarization_amp(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h0 = f1(args);
...
fn builtin_cross_polarization_amp(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h0 = f1(args);
    let theta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_chirp_mass_inspiral_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m1 = f1(args);
...
fn builtin_isco_radius_kerr_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
...
fn builtin_spin_orbit_coupling_term(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = f1(args);
    let m = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
/// Spin-spin coupling
fn builtin_spin_spin_coupling_term(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s1 = f1(args);
...
fn builtin_hawking_area_increase(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let da_dt = f1(args);
    Ok(StrykeValue::integer(if da_dt >= 0.0 { 1 } else { 0 }))
}

/// Unruh temperature T = ℏ a / (2πck_B)
fn builtin_unruh_temperature_full(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_bekenstein_entropy_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let area = f1(args);
    Ok(StrykeValue::float(area / 4.0))
}

/// Holographic entanglement entropy S = (Area_minimal / 4G_N)
fn builtin_holographic_entanglement_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let area = f1(args);
...
fn builtin_ryu_takayanagi_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_holographic_entanglement_step(args)
}

/// Swampland distance conjecture check: |Δφ| < d_c (returns 1 if inside)
fn builtin_swampland_distance_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dphi = f1(args);

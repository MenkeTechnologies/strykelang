...
fn builtin_net_tcp_cwnd_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
...
fn builtin_net_tcp_ssthresh_update(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_tcp_reno_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
...
fn builtin_net_tcp_cubic_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let t = f1(args);
...
fn builtin_net_tcp_bbr_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let bdp = f1(args);
...
fn builtin_net_tcp_vegas_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
...
fn builtin_net_tcp_westwood_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let bw = f1(args);
...
/// Compound TCP step
fn builtin_net_tcp_compound_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let win = f1(args);
...
fn builtin_net_tcp_dctcp_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
...
fn builtin_net_tcp_yeah_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
...
fn builtin_net_tcp_htcp_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_tcp_hybla_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
...
fn builtin_net_tcp_illinois_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args).max(1.0);
...
/// Low-priority TCP-LP
fn builtin_net_tcp_lp_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
...
/// Scalable TCP
fn builtin_net_tcp_scalable_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
...
fn builtin_net_tcp_veno_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args).max(1.0);
    let base_rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.05);
    let cur_rtt = args.get(2).map(|v| v.to_number()).unwrap_or(0.05).max(1e-6);
...
fn builtin_net_aiad_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let w = f1(args);
...
fn builtin_net_aimd_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let w = f1(args);
...
/// MIAD
fn builtin_net_miad_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let w = f1(args);
    let loss = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
/// MIMD
fn builtin_net_mimd_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let w = f1(args);
    let loss = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_net_aqm_red_drop_prob(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let avg_q = f1(args);
...
fn builtin_net_aqm_codel_target(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let _ = args;
    Ok(StrykeValue::float(0.005))
}

/// PIE drop rate increment
fn builtin_net_aqm_pie_drop_rate(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let prev = f1(args);
...
fn builtin_net_aqm_fq_codel_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_aqm_blue_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_net_aqm_choke_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let avg_q = f1(args);
    let min_th = args.get(1).map(|v| v.to_number()).unwrap_or(5.0);
    let max_th = args.get(2).map(|v| v.to_number()).unwrap_or(15.0);
    let max_p = args.get(3).map(|v| v.to_number()).unwrap_or(0.1);
...
fn builtin_net_aqm_sfq_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_aqm_drr_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let deficit = f1(args);
...
/// WRR step
fn builtin_net_aqm_wrr_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let weight = f1(args);
...
fn builtin_net_token_rate_limit(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let tokens = f1(args);
...
/// Traffic shaper step
fn builtin_net_traffic_shaper_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let demand = f1(args);
...
fn builtin_net_priority_queue_index(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let prio = i1(args);
    Ok(StrykeValue::integer(prio.max(0)))
}

/// Packet loss estimate from sample window
fn builtin_net_packet_loss_estimate(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let lost = f1(args);
...
fn builtin_net_jitter_estimate(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let j = f1(args);
...
fn builtin_net_latency_avg(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_rtt_smoothed(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let srtt = f1(args);
...
fn builtin_net_rtt_variation(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let rttvar = f1(args);
...
fn builtin_net_rto_compute(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let srtt = f1(args);
...
fn builtin_net_bandwidth_delay_product(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let bw = f1(args);
...
fn builtin_net_path_capacity_kleinrock(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_net_bandwidth_delay_product(args)
}

/// Loss rate to throughput conversion: 1.22·MSS / (RTT·√p)
fn builtin_net_loss_rate_to_throughput(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let mss = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
...
fn builtin_net_throughput_padhye(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let mss = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    let p = args.get(2).map(|v| v.to_number()).unwrap_or(0.01);
...
/// Mathis model
fn builtin_net_throughput_mathis(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_net_loss_rate_to_throughput(args)
}

/// Throughput response function (steady-state)
fn builtin_net_throughput_response(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
...
fn builtin_net_router_buffer_size(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let bdp = f1(args);
...
fn builtin_net_drop_tail_check(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let q = f1(args);
    let cap = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_net_burst_size_compute(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let rate = f1(args);
...
/// Packet pacing step
fn builtin_net_packet_pacing_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cwnd = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
...
/// Link capacity share
fn builtin_net_link_capacity_share(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_net_aqm_wrr_step(args)
}

/// Proportional fair share
fn builtin_net_proportional_fair_share(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r_i = f1(args);
...
/// Max-min fair step
fn builtin_net_max_min_fair_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cap = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_net_alpha_fair_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let x = f1(args);
...
/// Kelly pricing step
fn builtin_net_kelly_pricing_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let demand = f1(args);
...
fn builtin_net_network_utility_max(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().sum()))
}

/// Lyapunov drift + penalty
fn builtin_net_lyapunov_drift_plus_penalty(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let drift = f1(args);
...
/// Backpressure step
fn builtin_net_backpressure_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let q_a = f1(args);
...
fn builtin_net_max_weight_match(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

/// Q-CSMA propose
fn builtin_net_qcsma_propose(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let q = f1(args);
    Ok(StrykeValue::float(1.0 - (-q).exp()))
}

/// CSMA back-off (binary exponential)
fn builtin_net_csma_back_off(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let collisions = f1(args);
    Ok(StrykeValue::float(2f64.powf(collisions)))
}

/// Pure ALOHA throughput: G·exp(-2G)
fn builtin_net_alohanet_throughput(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let g = f1(args);
    Ok(StrykeValue::float(g * (-2.0 * g).exp()))
}

/// Slotted ALOHA: G·exp(-G)
fn builtin_net_slotted_aloha_throughput(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let g = f1(args);
    Ok(StrykeValue::float(g * (-g).exp()))
}

/// CSMA efficiency = 1 / (1 + a) where a = τ/T
fn builtin_net_csma_efficiency(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_net_token_ring_efficiency(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a = f1(args);
    if 1.0 + a == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 / (1.0 + a)))
}
...
fn builtin_net_polling_efficiency(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let t_data = f1(args);
...
fn builtin_net_radio_path_loss(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let d = f1(args);
...
fn builtin_net_friis_received_power(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let p_t = f1(args);
...
/// Two-ray ground loss
fn builtin_net_two_ray_ground_loss(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let h_t = f1(args);
...
fn builtin_net_okumura_hata_loss(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let f_mhz = f1(args);
...
fn builtin_net_log_distance_path(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let l0 = f1(args);
...
fn builtin_net_shadowing_normal(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let sigma = f1(args);
...
/// Rician K-factor
fn builtin_net_rician_k_factor(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let los = f1(args);
...
fn builtin_net_rayleigh_envelope(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_net_doppler_shift(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_net_capacity_shannon(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let b = f1(args);
...
fn builtin_net_mimo_capacity_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_zero_forcing_beam(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let h = f1(args);
...
fn builtin_net_mmse_beam_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let h = f1(args);
...
fn builtin_net_water_filling_power(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let mu = f1(args);
...
fn builtin_net_amc_threshold_index(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let snr_db = f1(args);
...
fn builtin_net_harq_combining_gain(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let n_tx = f1(args);
    Ok(StrykeValue::float(n_tx))
}

/// Turbo decode iteration step
fn builtin_net_turbo_decode_iter(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let llr = f1(args);
...
fn builtin_net_ldpc_iteration_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
/// Polar decode step
fn builtin_net_polar_decode_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let l = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_net_viterbi_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let m1 = f1(args);
...
fn builtin_net_bcjr_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_net_outage_probability(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
/// Args: N_t, N_r.
fn builtin_net_diversity_gain(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_array_gain(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let n = f1(args).max(1.0);
    Ok(StrykeValue::float(10.0 * n.log10()))
}

/// Multiplexing gain (min of M_t, M_r)
fn builtin_net_multiplexing_gain(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let mt = f1(args);
...
fn builtin_net_coding_gain(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_pruning_gain(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_net_macro_diversity_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

/// Micro-diversity step (combining)
fn builtin_net_micro_diversity_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().sum()))
}

/// Handoff threshold check
fn builtin_net_handoff_threshold(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let rssi_a = f1(args);
...
fn builtin_net_call_admission_check(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let used = f1(args);
    let cap = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_net_blocking_probability(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a = f1(args);
...
/// Erlang B formula
fn builtin_net_erlang_b_formula(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_net_blocking_probability(args)
}

/// Erlang C formula (queuing)
fn builtin_net_erlang_c_formula(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_net_engset_formula(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = i1(args).max(1);
...
/// Little's law: L = λW
fn builtin_net_little_law_l(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let lambda = f1(args);
...
fn builtin_net_throughput_law(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_net_response_time_law(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_net_utilization_law(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_net_forced_flow_law(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let x0 = f1(args);

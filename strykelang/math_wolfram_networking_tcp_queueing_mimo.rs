...
fn builtin_net_tcp_cwnd_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    if cwnd <= 0.0 { return Ok(StrykeValue::float(1.0)); }
    Ok(StrykeValue::float(cwnd + 1.0 / cwnd))
}

/// ssthresh update on loss: cwnd / 2
fn builtin_net_tcp_ssthresh_update(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(f1(args) / 2.0))
}
...
fn builtin_net_tcp_reno_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    ...
    match event {
        0 => {
            if cwnd < ssthresh { Ok(StrykeValue::float(cwnd + smss)) }
            else { Ok(StrykeValue::float(cwnd + smss * smss / cwnd.max(smss))) }
        }
        1 => {
            let new_ssh = (cwnd / 2.0).max(2.0 * smss);
            Ok(StrykeValue::float(new_ssh + 3.0 * smss))
        }
        2 => Ok(StrykeValue::float(smss)),
        _ => Ok(StrykeValue::float(cwnd)),
    }
}

/// Cubic: W(t) = C(t-K)³ + W_max
fn builtin_net_tcp_cubic_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
    ...
    Ok(StrykeValue::float(c * (t - k).powi(3) + w_max))
}

/// BBR step: cwnd = max(BDP, 4·MSS)
fn builtin_net_tcp_bbr_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bdp = f1(args);
    let mss = args.get(1).map(|v| v.to_number()).unwrap_or(1500.0);
    Ok(StrykeValue::float(bdp.max(4.0 * mss)))
}

/// Vegas step (BaseRTT-based)
fn builtin_net_tcp_vegas_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    let base_rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    let cur_rtt = args.get(2).map(|v| v.to_number()).unwrap_or(0.1);
    if base_rtt == 0.0 { return Ok(StrykeValue::float(cwnd)); }
    let diff = cwnd * (1.0 - base_rtt / cur_rtt);
    Ok(StrykeValue::float(cwnd + (1.0_f64).copysign(diff - 2.0)))
}

/// Westwood step (bandwidth-aware ssthresh)
fn builtin_net_tcp_westwood_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bw = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    Ok(StrykeValue::float(bw * rtt))
}

/// Compound TCP step
fn builtin_net_tcp_compound_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let win = f1(args);
    let dwnd = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(win + dwnd))
}

/// DCTCP step (ECN feedback alpha)
fn builtin_net_tcp_dctcp_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    let alpha = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(cwnd * (1.0 - alpha / 2.0)))
}
...
fn builtin_net_tcp_yeah_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    ...
    if cwnd <= 0.0 { return Ok(StrykeValue::float(1.0)); }
    Ok(StrykeValue::float(cwnd + alpha / cwnd))
}
...
fn builtin_net_tcp_htcp_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_net_tcp_hybla_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    let rtt0 = args.get(2).map(|v| v.to_number()).unwrap_or(0.025);
    if rtt0 == 0.0 { return Ok(StrykeValue::float(cwnd)); }
    Ok(StrykeValue::float(cwnd + (rtt / rtt0).powi(2)))
}
...
fn builtin_net_tcp_illinois_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args).max(1.0);
...
/// Low-priority TCP-LP
fn builtin_net_tcp_lp_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    let queueing_delay = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(if queueing_delay > 0.0 { cwnd * 0.5 } else { cwnd + 1.0 }))
}

/// Scalable TCP
fn builtin_net_tcp_scalable_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    Ok(StrykeValue::float(cwnd + 0.01 * cwnd))
}
...
fn builtin_net_tcp_veno_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args).max(1.0);
    let base_rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.05);
    let cur_rtt = args.get(2).map(|v| v.to_number()).unwrap_or(0.05).max(1e-6);
    ...
    Ok(StrykeValue::float(cwnd + increment))
}

/// AIAD: additive increase, additive decrease
fn builtin_net_aiad_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
    ...
    Ok(StrykeValue::float(if loss > 0.0 { w - dec } else { w + inc }))
}

/// AIMD: additive increase, multiplicative decrease (Reno)
fn builtin_net_aimd_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
    let loss = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(if loss > 0.0 { w / 2.0 } else { w + 1.0 }))
}

/// MIAD
fn builtin_net_miad_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
    let loss = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(if loss > 0.0 { w - 1.0 } else { 2.0 * w }))
}

/// MIMD
fn builtin_net_mimd_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
    let loss = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(if loss > 0.0 { w / 2.0 } else { 2.0 * w }))
}

/// RED drop probability: linear between min_th, max_th
fn builtin_net_aqm_red_drop_prob(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let avg_q = f1(args);
    ...
    if avg_q < min_th { return Ok(StrykeValue::float(0.0)); }
    if avg_q >= max_th { return Ok(StrykeValue::float(1.0)); }
    Ok(StrykeValue::float(max_p * (avg_q - min_th) / (max_th - min_th)))
}

/// CoDel target queue delay
fn builtin_net_aqm_codel_target(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let _ = args;
    Ok(StrykeValue::float(0.005))
}

/// PIE drop rate increment
fn builtin_net_aqm_pie_drop_rate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let prev = f1(args);
    let delay_diff = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float((prev + 0.125 * delay_diff).clamp(0.0, 1.0)))
}
...
fn builtin_net_aqm_fq_codel_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let count = f1(args).max(1.0);
    let interval = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    Ok(StrykeValue::float(interval / count.sqrt()))
}

/// BLUE step (mark/drop probability)
fn builtin_net_aqm_blue_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
    let event = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let delta = args.get(2).map(|v| v.to_number()).unwrap_or(0.01);
    Ok(StrykeValue::float((p + event * delta).clamp(0.0, 1.0)))
}
...
fn builtin_net_aqm_choke_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let avg_q = f1(args);
    let min_th = args.get(1).map(|v| v.to_number()).unwrap_or(5.0);
    let max_th = args.get(2).map(|v| v.to_number()).unwrap_or(15.0);
    let max_p = args.get(3).map(|v| v.to_number()).unwrap_or(0.1);
    let same_p = args.get(4).map(|v| v.to_number()).unwrap_or(0.0);
    if avg_q < min_th { return Ok(StrykeValue::integer(0)); }
    if same_p > 0.5 { return Ok(StrykeValue::integer(2)); }
    if avg_q >= max_th { return Ok(StrykeValue::integer(1)); }
    let p = max_p * (avg_q - min_th) / (max_th - min_th);
    Ok(StrykeValue::integer(if p > 0.5 { 1 } else { 0 }))
}

/// SFQ (stochastic fair queueing) step
fn builtin_net_aqm_sfq_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = i1(args) as u64;
    let n = args.get(1).map(|v| v.to_number() as u64).unwrap_or(1024).max(1);
    Ok(StrykeValue::integer((h % n) as i64))
}

/// DRR (deficit round-robin) step
fn builtin_net_aqm_drr_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let deficit = f1(args);
    let quantum = args.get(1).map(|v| v.to_number()).unwrap_or(1500.0);
    Ok(StrykeValue::float(deficit + quantum))
}

/// WRR step
fn builtin_net_aqm_wrr_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let weight = f1(args);
    let total_weight = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total_weight == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(weight / total_weight))
}

/// Token rate limit step
fn builtin_net_token_rate_limit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tokens = f1(args);
...
/// Traffic shaper step
fn builtin_net_traffic_shaper_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let demand = f1(args);
    let cap = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::float(demand.min(cap)))
}

/// Priority queue index
fn builtin_net_priority_queue_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let prio = i1(args);
    Ok(StrykeValue::integer(prio.max(0)))
}

/// Packet loss estimate from sample window
fn builtin_net_packet_loss_estimate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lost = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(lost / total))
}

/// Jitter estimate (RFC 3550): J += (|D(i-1, i)| - J) / 16
fn builtin_net_jitter_estimate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let j = f1(args);
    let d = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(j + (d.abs() - j) / 16.0))
}

/// Average latency from samples
fn builtin_net_latency_avg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_net_rtt_smoothed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let srtt = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let alpha = args.get(2).map(|v| v.to_number()).unwrap_or(0.125);
    Ok(StrykeValue::float((1.0 - alpha) * srtt + alpha * r))
}

/// RTT variation (RTTVAR): (1-β)·RTTVAR + β·|SRTT - R|
fn builtin_net_rtt_variation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rttvar = f1(args);
    let diff = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let beta = args.get(2).map(|v| v.to_number()).unwrap_or(0.25);
    Ok(StrykeValue::float((1.0 - beta) * rttvar + beta * diff.abs()))
}

/// RTO compute: SRTT + 4·RTTVAR
fn builtin_net_rto_compute(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let srtt = f1(args);
    let rttvar = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(srtt + 4.0 * rttvar))
}

/// Bandwidth-delay product
fn builtin_net_bandwidth_delay_product(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bw = f1(args);
...
fn builtin_net_path_capacity_kleinrock(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_net_bandwidth_delay_product(args)
}

/// Loss rate to throughput conversion: 1.22·MSS / (RTT·√p)
fn builtin_net_loss_rate_to_throughput(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mss = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    let p = args.get(2).map(|v| v.to_number()).unwrap_or(0.01);
    if p <= 0.0 || rtt <= 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(1.22 * mss / (rtt * p.sqrt())))
}

/// Padhye throughput model
fn builtin_net_throughput_padhye(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mss = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    let p = args.get(2).map(|v| v.to_number()).unwrap_or(0.01);
    ...
    Ok(StrykeValue::float(mss / denom))
}

/// Mathis model
fn builtin_net_throughput_mathis(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_net_loss_rate_to_throughput(args)
}

/// Throughput response function (steady-state)
fn builtin_net_throughput_response(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    if rtt <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(cwnd / rtt))
}

/// Router buffer size = BDP/√n (Stanford rule)
fn builtin_net_router_buffer_size(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bdp = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if n <= 0.0 { return Ok(StrykeValue::float(bdp)); }
    Ok(StrykeValue::float(bdp / n.sqrt()))
}

/// Drop tail check (queue full)
fn builtin_net_drop_tail_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
    let cap = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::integer(if q >= cap { 1 } else { 0 }))
}

/// Burst size compute (token bucket)
fn builtin_net_burst_size_compute(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rate = f1(args);
    let dt = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::float(rate * dt))
}

/// Packet pacing step
fn builtin_net_packet_pacing_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cwnd = f1(args);
    let rtt = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    if cwnd == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(rtt / cwnd))
}

/// Link capacity share
fn builtin_net_link_capacity_share(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_net_aqm_wrr_step(args)
}

/// Proportional fair share
fn builtin_net_proportional_fair_share(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r_i = f1(args);
    let mean_r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if mean_r == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(r_i / mean_r))
}

/// Max-min fair step
fn builtin_net_max_min_fair_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cap = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if n == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(cap / n))
}

/// α-fair: U(x) = x^(1-α) / (1-α)
fn builtin_net_alpha_fair_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    let alpha = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if (alpha - 1.0).abs() < 1e-9 { return Ok(StrykeValue::float(x.ln())); }
    Ok(StrykeValue::float(x.powf(1.0 - alpha) / (1.0 - alpha)))
}

/// Kelly pricing step
fn builtin_net_kelly_pricing_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let demand = f1(args);
    let capacity = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if capacity == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(demand / capacity))
}

/// Network utility max objective
fn builtin_net_network_utility_max(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().sum()))
}

/// Lyapunov drift + penalty
fn builtin_net_lyapunov_drift_plus_penalty(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let drift = f1(args);
    let penalty = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let v = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::float(drift + v * penalty))
}

/// Backpressure step
fn builtin_net_backpressure_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q_a = f1(args);
    let q_b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(q_a - q_b))
}

/// Max-weight match value
fn builtin_net_max_weight_match(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

/// Q-CSMA propose
fn builtin_net_qcsma_propose(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
    Ok(StrykeValue::float(1.0 - (-q).exp()))
}

/// CSMA back-off (binary exponential)
fn builtin_net_csma_back_off(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let collisions = f1(args);
    Ok(StrykeValue::float(2f64.powf(collisions)))
}

/// Pure ALOHA throughput: G·exp(-2G)
fn builtin_net_alohanet_throughput(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
    Ok(StrykeValue::float(g * (-2.0 * g).exp()))
}

/// Slotted ALOHA: G·exp(-G)
fn builtin_net_slotted_aloha_throughput(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
    Ok(StrykeValue::float(g * (-g).exp()))
}

/// CSMA efficiency = 1 / (1 + a) where a = τ/T
fn builtin_net_csma_efficiency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    if 1.0 + a == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 / (1.0 + a)))
}

/// Token ring efficiency: 1 / (1 + N·a/N)
fn builtin_net_token_ring_efficiency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    if 1.0 + a == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 / (1.0 + a)))
}
...
fn builtin_net_polling_efficiency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t_data = f1(args);
    ...
    Ok(StrykeValue::float(t_data / denom))
}

/// Radio path loss (free-space): L = (4π d / λ)²
fn builtin_net_radio_path_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let d = f1(args);
    let lambda = args.get(1).map(|v| v.to_number()).unwrap_or(0.125);
    if lambda == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float((4.0 * std::f64::consts::PI * d / lambda).powi(2)))
}

/// Friis received power: P_r = P_t G_t G_r λ² / (4πd)²
fn builtin_net_friis_received_power(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_t = f1(args);
    ...
    Ok(StrykeValue::float(p_t * g_t * g_r * lambda * lambda / ((4.0 * std::f64::consts::PI * d).powi(2))))
}

/// Two-ray ground loss
fn builtin_net_two_ray_ground_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h_t = f1(args);
    let h_r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let d = args.get(2).map(|v| v.to_number()).unwrap_or(100.0);
    if h_t == 0.0 || h_r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(d.powi(4) / (h_t * h_t * h_r * h_r)))
}

/// Okumura-Hata loss (urban large-city, simplified)
fn builtin_net_okumura_hata_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f_mhz = f1(args);
    let h_b = args.get(1).map(|v| v.to_number()).unwrap_or(30.0);
    let d_km = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    if d_km <= 0.0 || h_b <= 0.0 || f_mhz <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(69.55 + 26.16 * f_mhz.log10() - 13.82 * h_b.log10() + (44.9 - 6.55 * h_b.log10()) * d_km.log10()))
}

/// Log-distance path loss
fn builtin_net_log_distance_path(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l0 = f1(args);
    ...
    if d0 <= 0.0 || d <= 0.0 { return Ok(StrykeValue::float(l0)); }
    Ok(StrykeValue::float(l0 + 10.0 * n * (d / d0).log10()))
}

/// Log-normal shadowing path-loss component X_σ ~ N(0, σ²) in dB. Convert from
/// uniform U₁, U₂ via Box-Muller: X = σ · √(-2 ln U₁) · cos(2π U₂).
fn builtin_net_shadowing_normal(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sigma = f1(args);
    let u1 = args.get(1).map(|v| v.to_number()).unwrap_or(0.5).max(1e-12);
    let u2 = args.get(2).map(|v| v.to_number()).unwrap_or(0.5);
    Ok(StrykeValue::float(sigma * (-2.0 * u1.ln()).sqrt() * (2.0 * std::f64::consts::PI * u2).cos()))
}

/// Rician K-factor
fn builtin_net_rician_k_factor(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let los = f1(args);
    let scattered = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if scattered == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(los / scattered))
}

/// Rayleigh envelope: f(r) = (r/σ²) exp(-r²/(2σ²))
fn builtin_net_rayleigh_envelope(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
    let sigma2 = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if sigma2 == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float((r / sigma2) * (-r * r / (2.0 * sigma2)).exp()))
}

/// Doppler shift: f_d = v f_c / c
fn builtin_net_doppler_shift(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
    let f_c = args.get(1).map(|v| v.to_number()).unwrap_or(2.4e9);
    let c = 3e8;
    Ok(StrykeValue::float(v * f_c / c))
}

/// Shannon capacity: C = B log₂(1 + SNR)
fn builtin_net_capacity_shannon(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let b = f1(args);
    let snr = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::float(b * (1.0 + snr).log2()))
}
...
fn builtin_net_mimo_capacity_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sigmas = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_net_zero_forcing_beam(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = f1(args);
    if h == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 / h))
}

/// MMSE beamforming step
fn builtin_net_mmse_beam_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = f1(args);
    let noise = args.get(1).map(|v| v.to_number()).unwrap_or(1e-9);
    if h * h + noise == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(h / (h * h + noise)))
}

/// Water-filling power allocation
fn builtin_net_water_filling_power(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mu = f1(args);
    let n_i = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::float((mu - n_i).max(0.0)))
}

/// AMC threshold index (modulation/coding scheme based on SNR)
fn builtin_net_amc_threshold_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let snr_db = f1(args);
    let thresholds = b49_to_floats(args.get(1).unwrap_or(&StrykeValue::array(vec![])));
    let mut idx = 0_i64;
    for (i, &t) in thresholds.iter().enumerate() {
        if snr_db >= t { idx = i as i64 + 1; }
    }
    Ok(StrykeValue::integer(idx))
}

/// HARQ combining gain (chase combining)
fn builtin_net_harq_combining_gain(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_tx = f1(args);
    Ok(StrykeValue::float(n_tx))
}

/// Turbo decode iteration step
fn builtin_net_turbo_decode_iter(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let llr = f1(args);
    let extr = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(llr + extr))
}
...
fn builtin_net_ldpc_iteration_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let llrs = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    if llrs.is_empty() { return Ok(StrykeValue::float(0.0)); }
    ...
    Ok(StrykeValue::float(2.0 * p.atanh()))
}

/// Polar decode step
fn builtin_net_polar_decode_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(l.signum() * r.signum() * l.abs().min(r.abs())))
}

/// Viterbi step (path metric update)
fn builtin_net_viterbi_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m1 = f1(args);
    let m2 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(m1.min(m2)))
}
...
fn builtin_net_bcjr_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    ...
    Ok(StrykeValue::float(a * wa + b * wb))
}

/// Outage probability: P(SNR < γ_th)
fn builtin_net_outage_probability(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let gamma_th = f1(args);
    let mean_snr = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if mean_snr == 0.0 { return Ok(StrykeValue::float(1.0)); }
    Ok(StrykeValue::float(1.0 - (-gamma_th / mean_snr).exp()))
}
...
/// Args: N_t, N_r.
fn builtin_net_diversity_gain(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let nt = f1(args).max(1.0);
    let nr = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(StrykeValue::float(nt * nr))
}

/// Array gain (coherent combining): G_array = N (number of antennas) for
/// matched-filter receiver. In dB: 10·log₁₀(N).
fn builtin_net_array_gain(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args).max(1.0);
    Ok(StrykeValue::float(10.0 * n.log10()))
}

/// Multiplexing gain (min of M_t, M_r)
fn builtin_net_multiplexing_gain(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mt = f1(args);
    let mr = args.get(1).map(|v| v.to_number()).unwrap_or(mt);
    Ok(StrykeValue::float(mt.min(mr)))
}
...
fn builtin_net_coding_gain(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args).max(1e-9);
    let d = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(StrykeValue::float(10.0 * (r * d).log10()))
}
...
fn builtin_net_pruning_gain(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let full = f1(args).max(1.0);
    let pruned = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(StrykeValue::float((full / pruned).log2()))
}

/// Macro-diversity step (best of multiple base stations)
fn builtin_net_macro_diversity_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

/// Micro-diversity step (combining)
fn builtin_net_micro_diversity_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b49_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().sum()))
}

/// Handoff threshold check
fn builtin_net_handoff_threshold(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rssi_a = f1(args);
    let rssi_b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let hyst = args.get(2).map(|v| v.to_number()).unwrap_or(3.0);
    Ok(StrykeValue::integer(if rssi_b > rssi_a + hyst { 1 } else { 0 }))
}

/// Call admission check (capacity left)
fn builtin_net_call_admission_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let used = f1(args);
    let cap = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::integer(if used < cap { 1 } else { 0 }))
}

/// Blocking probability (M/M/c/c) — Erlang B alias
fn builtin_net_blocking_probability(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    ...
    top *= t2;
    if bot == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(top / bot))
}

/// Erlang B formula
fn builtin_net_erlang_b_formula(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_net_blocking_probability(args)
}

/// Erlang C formula (queuing)
fn builtin_net_erlang_c_formula(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let c = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if c <= a { return Ok(StrykeValue::float(1.0)); }
    ...
    Ok(StrykeValue::float(p_b / denom))
}
...
fn builtin_net_engset_formula(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = i1(args).max(1);
...
/// Little's law: L = λW
fn builtin_net_little_law_l(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lambda = f1(args);
    let w = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(lambda * w))
}

/// Throughput law: X = N / R
fn builtin_net_throughput_law(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(n / r))
}

/// Response time law: R = N/X - Z (think time)
fn builtin_net_response_time_law(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    ...
    Ok(StrykeValue::float(n / x - z))
}

/// Utilization law: U = X · S
fn builtin_net_utilization_law(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_net_forced_flow_law(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x0 = f1(args);
    let v_k = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::float(x0 * v_k))
}

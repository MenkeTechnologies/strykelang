...
fn b67_to_floats(v: &PerlValue) -> Vec<f64> {
...
fn builtin_cnf_unit_propagate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let clause = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let assign = b67_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    ...
    for &lit in &clause {
        ...
        if val == -1 { unassigned = lit_i; count += 1; if count > 1 { return Ok(PerlValue::integer(0)); } }
        else if (lit_i > 0 && val == 1) || (lit_i < 0 && val == 0) {
            return Ok(PerlValue::integer(0));
        }
    }
    Ok(PerlValue::integer(if count == 1 { unassigned } else { 0 }))
}
...
/// 0 between clauses.
fn builtin_cnf_pure_literal_elim(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lits = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    let pure: Vec<i64> = pos.iter().filter(|v| !neg.contains(*v)).cloned()
        ...
        .collect();
    Ok(PerlValue::array(pure.into_iter().map(PerlValue::integer).collect()))
}
...
fn builtin_cnf_dpll_branch(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lits = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::integer(counts.into_iter().max_by_key(|(_, c)| *c).map(|(v, _)| v as i64).unwrap_or(0)))
}

/// Conflict-driven clause learning: trace last-uip. Returns size of learned
/// clause. Args: trail of decision levels per literal.
fn builtin_dpll_clause_learning(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let trail = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mut levels = std::collections::HashSet::new();
    for &lvl in &trail { levels.insert(lvl as i64); }
    Ok(PerlValue::integer(levels.len() as i64))
}

/// Two-watched-literals: returns valid (literal_a, literal_b) maintaining the
/// invariant. Args: clause, current assignment.
fn builtin_two_watched_literals(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let clause = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let assign = b67_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::array(watched.into_iter().map(PerlValue::integer).collect()))
}
...
fn builtin_walksat_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
    ...
    Ok(PerlValue::integer(if r < p { idx_random } else { idx_best }))
}

/// Resolution step: clauses {a, x} and {b, ¬x} → {a, b} (size).
fn builtin_resolution_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = b67_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_subsumption_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = b67_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let b_set: std::collections::HashSet<i64> = b.iter().map(|&x| x as i64).collect();
    Ok(PerlValue::integer(if a.iter().all(|&x| b_set.contains(&(x as i64))) { 1 } else { 0 }))
}

/// Tableau branch close check: contains both p and ¬p.
fn builtin_tableau_branch_close(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lits = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::integer(if closed { 1 } else { 0 }))
}
...
fn builtin_sequent_left_intro(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let id = i1(args);
    Ok(PerlValue::integer(if id > 0 { 2 } else { 0 }))
}

/// Sequent-calculus right intro: introduces disjunction on the right.
fn builtin_sequent_right_intro(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let id = i1(args);
    Ok(PerlValue::integer(if id > 0 { 2 } else { 0 }))
}
...
fn builtin_nbe_normalize(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let head = i1(args);
    Ok(PerlValue::integer(head.abs()))
}

/// Church numeral n: encoded as the value of applying λf.λx. f^n x to identity
/// (= n). Returns n.
fn builtin_church_numeral_n(args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::integer(i1(args).max(0)))
}

/// Encode pair: ⟨a, b⟩ → λp. p a b. Returns Cantor pairing (a+b)(a+b+1)/2 + b
/// as a numeric proxy.
fn builtin_encode_pair(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer((a + b) * (a + b + 1) / 2 + b))
}

/// Encode succ: λn.λf.λx. f (n f x). n + 1.
fn builtin_encode_succ(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_simply_typed_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = i1(args);
...
fn builtin_hindley_milner_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lhs = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let rhs = b67_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let n = lhs.len().min(rhs.len());
    let mut subs = 0_i64;
    for i in 0..n { if lhs[i] != rhs[i] { subs += 1; } }
    Ok(PerlValue::integer(subs))
}

/// Robinson unification: returns number of substitutions. Args: pair flat
/// [t1, t2] vectors.
fn builtin_unification_robinson(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_bdd_apply(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a_size = i1(args);
    let b_size = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer((a_size + b_size).max(1) - 1))
}

/// BDD restrict: x_i ← v reduces BDD by 1 variable level.
fn builtin_bdd_restrict(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let size = i1(args);
    Ok(PerlValue::integer((size / 2).max(1)))
}

/// BDD quantification: ∃ x_i. f = restrict(f, 0) ∨ restrict(f, 1).
fn builtin_bdd_quantify(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let size = i1(args);
    Ok(PerlValue::integer(size))
}

/// AIG simplify step: count of nodes after constant propagation.
fn builtin_aig_simplify_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let nodes = i1(args);
    let consts = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer((nodes - consts).max(0)))
}

/// SMT-LIB QF_LIA solver step: returns 1 (sat) if linear constraint a·x ≤ b
/// has a positive solution. Heuristic Bezout: gcd(a) | b.
fn builtin_smt_qf_lia_solve_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let coefs = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    if coefs.is_empty() { return Ok(PerlValue::integer(if b == 0 { 1 } else { 0 })); }
    ...
    if g == 0 { return Ok(PerlValue::integer(if b == 0 { 1 } else { 0 })); }
    Ok(PerlValue::integer(if b % g == 0 { 1 } else { 0 }))
}
...
fn builtin_smt_qf_uf_combine(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let eqs = i1(args);
    let neqs = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer(if eqs > 0 && neqs == 0 { 1 } else { 0 }))
}

/// CTL model checking step: returns 1 if state s satisfies AG p (always
/// globally), given precomputed reachability set.
fn builtin_model_checking_ctl(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_holds = i1(args);
    let reachable = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer(if p_holds == reachable && p_holds == 1 { 1 } else { 0 }))
}

/// LTL model checking: count of satisfying traces in given Büchi automaton run.
fn builtin_model_checking_ltl(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let accepting_runs = i1(args);
    Ok(PerlValue::integer(accepting_runs.max(0)))
}

/// Bisimulation step: refine partition by signature. Args: signature_count.
fn builtin_bisimulation_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let sigs = b67_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_coq_tactic_apply(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let goal_head = i1(args);
    let hyp_head = args.get(1).map(|v| v.to_number() as i64).unwrap_or(goal_head);
    Ok(PerlValue::integer(if goal_head == hyp_head { 1 } else { 0 }))
}

/// Coq term unification: substitution count.
fn builtin_coq_unify_term(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_unification_robinson(args)
}

/// Reflexivity check: a = a.
fn builtin_refl_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(a);
    Ok(PerlValue::integer(if a == b { 1 } else { 0 }))
}
...
/// Symmetry: a = b ↔ b = a.
fn builtin_sym_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(a);
    Ok(PerlValue::integer(if a == b { 1 } else { 0 }))
}
...
fn builtin_trans_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(a);
    let c = args.get(2).map(|v| v.to_number() as i64).unwrap_or(a);
    Ok(PerlValue::integer(if a == b && b == c { 1 } else { 0 }))
}

...
fn builtin_efield_point(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
...
fn builtin_epotential_point(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    let k = 1.0 / (4.0 * std::f64::consts::PI * EPS_0);
...
fn builtin_capacitor_charge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = f1(args);
...
// Ohm's law V = IR
fn builtin_ohm_voltage(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(StrykeValue::float(i * r))
}
// Power = VI
fn builtin_power_vi(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(v * i))
}
// Power dissipation = I²R
fn builtin_power_i2r(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = f1(args);
...
fn builtin_capacitance_parallel_sum(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_bfield_wire(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
// Solenoid B = μ₀nI
fn builtin_bfield_solenoid(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(MU_0 * n * i))
}
// Lorentz force F = qE + qv×B (magnitude only, perpendicular case)
fn builtin_lorentz_force_mag(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
    let v = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_faraday_emf(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_lc_frequency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = f1(args);
...
fn builtin_lc_omega(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = f1(args);
    let c = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if l * c <= 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_rc_tau(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
    let c = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(r * c))
}
// RL time constant τ = L/R
fn builtin_rl_tau(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if r == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_poynting_magnitude(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let e = f1(args);
    Ok(StrykeValue::float(e * e / (MU_0 * C_LIGHT)))
}
// Intensity from amplitude in vacuum: I = ½ε₀cE₀²
fn builtin_em_intensity(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let e0 = f1(args);
    Ok(StrykeValue::float(0.5 * EPS_0 * C_LIGHT * e0 * e0))
}
// Radiation pressure p = I/c
fn builtin_radiation_pressure(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = f1(args);
    Ok(StrykeValue::float(i / C_LIGHT))
}
// EM wavelength λ = c/f
fn builtin_em_wavelength(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f = f1(args);
    if f == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
    Ok(StrykeValue::float(C_LIGHT / f))
}
// EM frequency f = c/λ
fn builtin_em_frequency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lambda = f1(args);
...
fn builtin_snell_theta2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n1 = f1(args);
...
fn builtin_index_from_speed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_fresnel_reflection_normal(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n1 = f1(args);
...
fn builtin_fresnel_rs(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n1 = f1(args);
    let n2 = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_fresnel_rp(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n1 = f1(args);
    let n2 = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let theta_i = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let arg = n1 / n2 * theta_i.sin();
...
fn builtin_lensmaker(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_thin_lens_v(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f = f1(args);
...
fn builtin_mirror_equation_v(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_thin_lens_v(args)
}
// Magnification m = -v/u
fn builtin_lens_magnification(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_diffraction_grating_angle(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
...
fn builtin_single_slit_min(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let lambda = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_rayleigh_resolution(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lambda = f1(args);
...
fn builtin_lorentz_gamma(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_rel_momentum(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let v = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let beta_sq = (v / C_LIGHT).powi(2);
    if beta_sq >= 1.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_rel_ke(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let v = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let beta_sq = (v / C_LIGHT).powi(2);
    if beta_sq >= 1.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_rel_total_energy(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let v = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let beta_sq = (v / C_LIGHT).powi(2);
    if beta_sq >= 1.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_rel_energy_pm(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_relativistic_doppler(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f = f1(args);
    let v = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let beta = v / C_LIGHT;
...
fn builtin_rel_velocity_add(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
    let v = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_wave_string_speed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
...
fn builtin_sound_solid(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_sound_gas(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let gamma = f1(args);
...
fn builtin_doppler_classical(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f = f1(args);
...
fn builtin_standing_wave_fundamental(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_open_pipe_harmonic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_closed_pipe_harmonic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    let v = args.get(1).map(|v| v.to_number()).unwrap_or(343.0);
    let l = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    if l == 0.0 { return Ok(StrykeValue::float(f64::INFINITY)); }
...
fn builtin_sound_db(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_alfven_speed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let b = f1(args);
    let rho = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_grav_time_dilation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let g = 6.674e-11;
...
fn builtin_grav_redshift(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let g = 6.674e-11;
    let factor = 1.0 - 2.0 * g * m / (r * C_LIGHT * C_LIGHT);

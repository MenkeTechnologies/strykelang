...
fn builtin_sparse_csr_build(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::array(vec![
        StrykeValue::array(values.into_iter().map(StrykeValue::float).collect()),
        StrykeValue::array(col_idx.into_iter().map(StrykeValue::integer).collect()),
...
fn builtin_sparse_csr_mul_vec(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let col_idx: Vec<usize> = arg_to_vec(&csr.get(1).cloned().unwrap_or(StrykeValue::UNDEF))
        .iter().map(|v| v.to_number() as usize).collect();
    let row_ptr: Vec<usize> = arg_to_vec(&csr.get(2).cloned().unwrap_or(StrykeValue::UNDEF))
        .iter().map(|v| v.to_number() as usize).collect();
...
fn builtin_lower_triangular_q(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    for i in 0..n { for j in i + 1..m[i].len() {
        if m[i][j].abs() > 1e-12 { return Ok(StrykeValue::integer(0)); }
    }}
...
fn builtin_diagonal_dominance_q(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    for i in 0..n {
        ...
        if m[i][i].abs() <= off_sum { return Ok(StrykeValue::integer(0)); }
    }
    Ok(StrykeValue::integer(1))
}
...
fn builtin_matrix_identity_q(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if m[0].len() != n { return Ok(StrykeValue::integer(0)); }
    for i in 0..n { for j in 0..n {
        let target = if i == j { 1.0 } else { 0.0 };
        if (m[i][j] - target).abs() > 1e-12 { return Ok(StrykeValue::integer(0)); }
    }}
    Ok(StrykeValue::integer(1))
}
...
fn builtin_andrew_monotone_chain(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    lower.pop(); upper.pop();
    lower.extend(upper);
    Ok(StrykeValue::array(lower.into_iter().map(|(x, y)| {
...
fn builtin_polygon_convex_q(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let n = pts.len(); if n < 3 { return Ok(StrykeValue::integer(0)); }
    let mut sign = 0_i32;
    for i in 0..n {
        ...
        if s != 0 {
            if sign == 0 { sign = s; } else if sign != s { return Ok(StrykeValue::integer(0)); }
        }
    }
    Ok(StrykeValue::integer(1))
}
...
fn builtin_hausdorff_distance_2d(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let b: Vec<(f64, f64)> = arg_to_vec(&args.get(1).cloned().unwrap_or(StrykeValue::UNDEF))
        .iter().map(|p| { let v = arg_to_vec(p); (
            v.first().map(|x| x.to_number()).unwrap_or(0.0),
            v.get(1).map(|x| x.to_number()).unwrap_or(0.0),
        )}).collect();
...
fn builtin_minkowski_sum_simple(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    for p in &a { for q in &b {
        ...
        out.push(StrykeValue::array(vec![StrykeValue::float(x), StrykeValue::float(y)]));
    }}
...
fn builtin_circle_3_points(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if d.abs() < 1e-30 { return Ok(StrykeValue::array(vec![])); }
    let u = (ax * ax + ay * ay) / d;
    let v = (bx * bx + by * by) / d;
...
fn builtin_segment_length(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let a = to_pair(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
...
fn builtin_segments_perpendicular_q(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::integer(if dot.abs() < 1e-12 { 1 } else { 0 }))
}
...
fn builtin_inverse_chi2_pdf(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(pre * x.powf(-nu / 2.0 - 1.0) * (-1.0 / (2.0 * x)).exp()))
}

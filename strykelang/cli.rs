...
fn print_cyberpunk_help() {
    ...
    // Was `env!("CARGO_BIN_NAME")`, which a library has no such thing as. The
    // name to print is the one the user typed — `s -h` should say `s` — so it
    // comes from argv[0] now. See [`invoked_as`].
    let bin = invoked_as();
    let bin = bin.as_str();
        ...
        for arg in &argv {
            if switches_done || !arg.starts_with('-') || arg == "--" {
                ...
                if let Some((name, val)) = switch.split_once('=') {
                    let _ = interp
                        .scope
                        .set_scalar(name, crate::value::StrykeValue::string(val.to_string()));
                } else {
                    let _ = interp
                        .scope
                        .set_scalar(switch, crate::value::StrykeValue::integer(1));
                }
            }
        }
        argv = remaining;
...
/// Run one `stryke` invocation inside a host process and return its exit
/// status.
///
/// This is the entry point for a host with no fork to spend — the `stryke`
/// shell builtin in zshrs-native, where strykelang is linked into the shell and
/// a one-liner costs no process at all. `argv` is the whole command line,
...
/// shell down, a panic becomes a status, and the working directory the host had
/// is restored.
...
thread_local! {
    ...
    ///
    /// Replaces `env!("CARGO_BIN_NAME")`, which existed only while the command
    /// line was a binary target. Thread-local rather than a `OnceLock` because
    /// a host dispatches many invocations in one process and each may have been
    /// typed under a different name: `s`, `st`, `stryke`.
    static INVOKED_AS: std::cell::RefCell<String> =
        const { std::cell::RefCell::new(String::new()) };
}

/// The name this invocation was made under, defaulting to `stryke`.
...
fn run(full_argv: &[String]) -> i32 {
    ...
    //
    // Not hosted: the disposition belongs to the process, and the host is not
    // ours to change. A shell that linked stryke in would inherit SIG_DFL and
    // die on the next write to a closed pipe — the shell's own SIGPIPE policy
    // stands.
    #[cfg(unix)]
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(payload) = crate::aot::try_load_embedded(&exe) {
            let argv: Vec<String> = full_argv.iter().skip(1).cloned().collect();
            match payload {
                crate::aot::EmbeddedPayload::Script(embedded) => {
                    crate::hosted::exit(run_embedded_script(embedded, argv));
                }
                crate::aot::EmbeddedPayload::Bundle(bundle) => {
                    crate::hosted::exit(run_embedded_bundle(bundle, argv));
                }
            }
        }
    }
    ...
    let mut args = expand_perl_bundled_argv(full_argv.to_vec());
    ...
    if record_requested || crate::perf_recorder::recording_enabled_in_env() {
        let path = crate::perf_recorder::classify_invocation(&args);
        crate::perf_recorder::install(path, args.clone());
    }
    ...
    if args.len() == 2 && args[1] == "--test-worker" {
        crate::hosted::exit(crate::cli_runners::run_test_worker_loop());
    }
    ...
    if args.len() == 2 && args[1] == "--remote-worker" {
        ...
        crate::hosted::exit(crate::remote_wire::run_remote_worker_session());
    }
    if args.len() == 2 && args[1] == "--remote-worker-v1" {
        crate::hosted::exit(crate::remote_wire::run_remote_worker_stdio());
    }

    if args.len() == 2 && args[1] == "--lsp" {
        crate::hosted::exit(crate::run_lsp_stdio());
    }
    ...
    if args.len() >= 2 && args[1] == "agent" {
        ...
        crate::hosted::exit(crate::agent::run_agent_with_overrides(
            config_path,
    ...
    if args.len() >= 2 && args[1] == "controller" {
        ...
        crate::hosted::exit(crate::controller::run_controller(bind, port));
    }
    ...
    if args.len() >= 2 && args[1] == "ai" {
        crate::hosted::exit(run_ai_subcommand(&args[2..]));
    }
    ...
    if args.len() >= 2 && (args[1] == "docs" || args[1] == "help" || args[1] == "h") {
        crate::hosted::exit(run_doc_subcommand(&args[2..]));
    }

    // `stryke fmt [-i] FILE...` — format stryke source files.
    if args.len() >= 2 && args[1] == "fmt" {
        crate::hosted::exit(run_fmt_subcommand(&args[2..]));
    }

    if args.len() >= 2 && args[1] == "minify" {
        crate::hosted::exit(run_minify_subcommand(&args[2..]));
    }

    // `stryke bench [FILE|DIR]` — discover and run benchmark files.
    if args.len() >= 2 && args[1] == "bench" {
        crate::hosted::exit(run_bench_subcommand(&args[0], &args[2..]));
    }

    // `stryke init [NAME]` — scaffold a new stryke project (with stryke.toml).
    if args.len() >= 2 && args[1] == "init" {
        crate::hosted::exit(crate::pkg::commands::cmd_init(
    ...
    if args.len() >= 2 && args[1] == "new" {
        ...
        crate::hosted::exit(crate::pkg::commands::cmd_new(&args[2]));
    }

    // Package manager subcommands. See docs/PACKAGE_REGISTRY.md.
    if args.len() >= 2 && args[1] == "add" {
        crate::hosted::exit(crate::pkg::commands::cmd_add(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "remove" {
        crate::hosted::exit(crate::pkg::commands::cmd_remove(&args[2..]));
    }
    if args.len() >= 2 && matches!(args[1].as_str(), "install" | "i") {
        ...
        if rest.iter().any(|a| a == "-g" || a == "--global") {
            let filtered: Vec<String> = rest
                ...
                .cloned()
                .collect();
            crate::hosted::exit(crate::pkg::commands::cmd_install_global(&filtered));
        } else {
            crate::hosted::exit(crate::pkg::commands::cmd_install(rest));
        }
    }
    if args.len() >= 2 && args[1] == "tree" {
        crate::hosted::exit(crate::pkg::commands::cmd_tree(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "info" {
        crate::hosted::exit(crate::pkg::commands::cmd_info(&args[2..]));
    }
    if args.len() >= 2 && matches!(args[1].as_str(), "update" | "up" | "upgrade") {
        if args[2..].iter().any(|a| a == "-g" || a == "--global") {
            let filtered: Vec<String> = args[2..]
                ...
                .cloned()
                .collect();
            crate::hosted::exit(crate::pkg::commands::cmd_upgrade_global(&filtered));
        }
        if args[1] == "upgrade" {
            crate::hosted::exit(crate::pkg::commands::cmd_upgrade_project(&args[2..]));
        }
        crate::hosted::exit(crate::pkg::commands::cmd_update(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "outdated" {
        crate::hosted::exit(crate::pkg::commands::cmd_outdated(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "audit" {
        crate::hosted::exit(crate::pkg::commands::cmd_audit(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "vendor" {
        crate::hosted::exit(crate::pkg::commands::cmd_vendor(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "clean" {
        crate::hosted::exit(crate::pkg::commands::cmd_clean(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "search" {
        crate::hosted::exit(crate::pkg::commands::cmd_search(&args[2..]));
    }
    if args.len() >= 2 && matches!(args[1].as_str(), "publish" | "pub") {
        crate::hosted::exit(crate::pkg::commands::cmd_publish(&args[2..]));
    }
    if args.len() >= 2 && args[1] == "yank" {
        crate::hosted::exit(crate::pkg::commands::cmd_yank(&args[2..]));
    }
    ...
    if args.len() >= 2 && matches!(args[1].as_str(), "uninstall" | "un") {
        ...
        if rest.iter().any(|a| a == "-g" || a == "--global") {
            let filtered: Vec<String> = rest
                ...
                .cloned()
                .collect();
            crate::hosted::exit(crate::pkg::commands::cmd_uninstall_global(&filtered));
        } else {
    ...
    if args.len() >= 2 && matches!(args[1].as_str(), "list" | "ls") {
        ...
        if rest.iter().any(|a| a == "-g" || a == "--global") {
            let filtered: Vec<String> = rest
                ...
                .cloned()
                .collect();
            crate::hosted::exit(crate::pkg::commands::cmd_list_global(&filtered));
        } else {
    ...
    if args.len() >= 2 && args[1] == "pkg" {
        crate::hosted::exit(crate::pkg::commands::dispatch(&args[2..]));
    }

    // `stryke repl [--load FILE]` — explicit REPL entry.
    if args.len() >= 2 && args[1] == "repl" {
        crate::hosted::exit(run_repl_subcommand(&args[2..]));
    }
    ...
    if args.len() >= 2 && args[1] == "run" {
        ...
        if args.len() >= 3 && !args[2].starts_with('-') && !args[2].ends_with(".stk") {
            ...
            if let Ok(cwd) = std::env::current_dir() {
                if let Some(root) = crate::pkg::commands::find_project_root(&cwd) {
                    let mp = root.join(crate::pkg::commands::MANIFEST_FILE);
                    if let Ok(m) = crate::pkg::manifest::Manifest::from_path(&mp) {
                        if m.scripts.contains_key(candidate) {
                            crate::hosted::exit(crate::pkg::commands::cmd_run_script(&args[2..]));
                        }
                    }
                }
            }
        ...
        crate::hosted::exit(status.code().unwrap_or(1));
    }

    // `stryke prun FILE...` — run multiple files in parallel.
    if args.len() >= 2 && args[1] == "prun" {
        crate::hosted::exit(run_prun_subcommand(&args[0], &args[2..]));
    }
    ...
    if is_test_subcmd {
        ...
        if want_inproc {
            crate::hosted::exit(crate::cli_runners::run_tests_with_mode(
                &raw_targets,
                j_threads.as_deref(),
                no_interop,
                quiet,
        ...
        // Default = pool.
        crate::hosted::exit(crate::cli_runners::run_tests_pool(
            &raw_targets,
            j_threads.as_deref(),
            no_interop,
            quiet,
    ...
    if args.len() >= 2 && args[1] == "completions" {
        crate::hosted::exit(run_completions_subcommand(&args[2..]));
    }

    // `stryke ast FILE` — dump AST as JSON.
    if args.len() >= 2 && args[1] == "ast" {
        crate::hosted::exit(run_ast_subcommand(&args[2..]));
    }
    ...
    if args.len() >= 2 && args[1] == "gen-docs" {
        crate::hosted::exit(run_gen_docs_subcommand(&args[2..]));
    }
    ...
    if args.len() >= 2
        && !args[1].starts_with('-')
        && !force_script
        && crate::builtins::is_builtin(&args[1])
        && !Path::new(&args[1]).exists()
    {
        ...
        crate::hosted::exit(run_builtin_subcommand(builtin_name, &builtin_args));
    }
    ...
    if cli.compat {
        crate::set_compat_mode(true);
    }
    if cli.no_interop {
        crate::set_no_interop_mode(true);
    }
    if cli.static_typing {
        crate::set_static_mode(true);
    }
    ...
    if let Some(code) = &cli.explain {
        match crate::error::explain_error(code) {
    ...
    if let Some(script) = cli.script.as_ref() {
            ...
            && !cli.args.is_empty()
        {
            ...
            if is_existing_script(script) && cli.args.iter().all(|a| is_existing_script(a)) {
                ...
                crate::hosted::exit(if failed > 0 { 1 } else { 0 });
            }
        }
    }
    ...
    let runtime_bin_name = args
        ...
        .unwrap_or_default();
    // Was `matches!(env!("CARGO_BIN_NAME"), …) || matches!(runtime_bin_name, …)`.
    // The compile-time half cannot exist in a library — and never added
    // anything, since the runtime half already answers the same question from
    // argv[0], which is also what a host passes when it dispatches `stryke` as
    // a builtin.
    let is_stryke_bin = matches!(runtime_bin_name.as_str(), "stryke" | "st" | "s");
    ...
    if cli.from_zsh {
        let ns = crate::zsh_convert::namespace_from_path(&filename);
        let (stryke_src, warnings) = crate::zsh_convert::convert_zsh(&code, &ns);
        for w in &warnings {
    ...
    if cli.dump_tokens {
        let desugared = if crate::compat_mode() {
            full_code.clone()
        } else {
            let s = crate::rust_sugar::desugar_rust_blocks(&full_code);
            crate::ai_sugar::desugar(&s)
        };
        let mut lexer = crate::lexer::Lexer::new_with_file(&desugared, &filename);
    ...
    if cli.tiers {
        match crate::tiers::report(&full_code) {
            Ok(r) => println!("{r}"),
            Err(e) => {
    ...
    let cached = if cache_eligible && script_path.exists() {
        crate::script_cache::try_load(script_path)
    } else {
        None
    };
    ...
    let (program, cached_chunk, needs_cache_save) = if let Some(c) = cached {
        ...
        let parsed = match crate::parse_with_file(&full_code, &filename) {
            Ok(p) => p,
            Err(e) => {
                eprintln!("{}", e);
                crate::hosted::exit(255);
    ...
    if cli.dump_bytecode {
        ...
        let comp = crate::compiler::Compiler::new().with_source_file(filename.clone());
    ...
    if cli.format_source {
        ...
        println!("{}", crate::convert::convert_program(&program));
        return 0;
    }
    ...
    if cli.profile || cli.flame {
        interp.profiler = Some(crate::profiler::Profiler::new(filename.clone()));
    }
    ...
    if cli.line_mode || cli.print_mode {
        ...
        match interp.execute(&program) {
            Ok(_) => {
                ...
                // `test_run` no longer calls `crate::hosted::exit(1)` from inside the
    ...
    // Every branch above leaves through `hosted::exit`; reaching here means
    // the run finished without one, which is success.
    0
}

/// Run an [`crate::aot::EmbeddedScript`] as if it were the primary program. Minimal
...
fn run_embedded_script(embedded: crate::aot::EmbeddedScript, argv: Vec<String>) -> i32 {
    // AOT binaries don't use the rkyv cache — they're self-contained and parse/compile on every run.
    let program = match crate::parse_with_file(&embedded.source, &embedded.name) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("{}", e);
            return 255;
    ...
    interp.scope.declare_array(
        "INC",
        vec![crate::value::StrykeValue::string(".".to_string())],
    );
    match interp.execute(&program) {
        Ok(_) => {
            let _ = interp.run_global_teardown();
...
fn run_embedded_bundle(bundle: crate::aot::EmbeddedBundle, argv: Vec<String>) -> i32 {
    ...
    let program = match crate::parse_with_file(&entry_source, &bundle.entry) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("{}", e);
            return 255;
...
fn run_builtin_subcommand(name: &str, argv: &[String]) -> i32 {
    ...
    let program = match crate::parse_with_file(&code, "-e") {
        Ok(p) => p,
        Err(e) => {
            eprintln!("{}", e);
            return 255;
...
fn run_disasm_subcommand(args: &[String]) -> i32 {
    ...
    let comp = crate::compiler::Compiler::new().with_source_file(file.clone());
...
fn run_gen_docs_subcommand(args: &[String]) -> i32 {
    ...
    for src in &sources {
        ...
        let program = match crate::parse_with_file(&source, &src.to_string_lossy()) {
            Ok(p) => p,
            Err(e) => {
        ...
        let md = crate::docs::generate_markdown(&src.to_string_lossy(), &source, &program);
...
fn run_ai_subcommand(args: &[String]) -> i32 {
    ...
    let mut interp = VMHelper::new();
    match crate::parse_and_run_string(&final_script, &mut interp) {
        Ok(_) => 0,
        Err(e) => {
...
fn run_ai_cli_script(script: &str) -> i32 {
    let mut interp = VMHelper::new();
    match crate::parse_and_run_string(script, &mut interp) {
        Ok(_) => 0,
        Err(e) => {
            eprintln!("stryke ai: {}", e);
            1
...
fn run_build_subcommand(args: &[String]) -> i32 {
    ...
    if let Some(dir) = project_dir {
        ...
        match crate::aot::build_project(&project_path, &out_path) {
            Ok(p) => {
        ...
        let result = if native {
            ...
            crate::aot_native::build_native(&script_path, &out_path)
        } else {
            crate::aot::build(&script_path, &out_path)
        };
        match result {
            Ok(p) => {
                eprintln!("stryke build: wrote {}", p.display());
...
fn run_convert_subcommand(args: &[String]) -> i32 {
    ...
    for f in &files {
        ...
        let program = match crate::parse_with_file(&code, f) {
            Ok(p) => p,
            Err(e) => {
                eprintln!("stryke convert: {}: {}", f, e);
                errors += 1;
        ...
        let converted = crate::convert_to_stryke_with_options(&program, &opts);
        if in_place {
...
"#
    ...
    match crate::parse_and_run_string(&code, &mut interp) {
        Ok(_) => 0,
        Err(e) => {
            if let crate::error::ErrorKind::Exit(code) = e.kind {
                return code;
            }
            eprintln!("{}", e);
            255
...
fn run_doc_subcommand(args: &[String]) -> i32 {
    ...
    for &(chapter, topics) in crate::lsp::DOC_CATEGORIES {
        for &t in topics {
            if seen.contains(t) {
                continue;
            }
            if let Some(text) = crate::lsp::doc_text_for(t) {
    ...
    let mut leftover: Vec<(&'static str, &'static str)> = crate::builtins::category_map_iter()
    ...
    for (chapter, name) in leftover {
        ...
        if let Some(text) = crate::lsp::doc_text_for(name) {
    ...
    for topic in crate::lsp::doc_topics() {
        ...
        if crate::builtins::is_callable_spelling(topic) {
            continue;
        }
        ...
        if let Some(text) = crate::lsp::doc_text_for(topic) {
...
"
        ...
        || args.first().map(|s| s.as_str()) == Some("-L")
    {
        ...
        for (name, _) in crate::builtins::category_map_iter() {
            all.insert(name.to_string());
        }
        for entry in crate::builtins::aliases_hash_map().keys() {
            all.insert(entry.clone());
        }
        // CORE:: + main:: qualified spellings of every callable.
        let bare_callables: Vec<String> = crate::builtins::category_map_iter()
            .map(|(n, _)| n.to_string())
            .chain(
                crate::builtins::aliases_hash_map()
                    .keys()
                    .cloned()
                    .collect::<Vec<_>>(),
            )
        ...
        for line in include_str!("lsp_completion_words.txt").lines() {
            ...
            // main::, crate::) flow through here unconditionally —
    ...
    if !args.is_empty() {
        ...
        if let Ok(n) = arg.parse::<usize>() {
            ...
            let resolved: String = if let Some(rest) = arg.strip_prefix("CORE::") {
                ...
                rest.to_string()
            } else if let Some(rest) = arg.strip_prefix("crate::") {
                // `crate::builtins`, `crate::all`, `crate::aliases`,
                // … all have hand-written hover entries under the
                // sigil-prefixed `%crate::NAME` spelling — that's the
                ...
                // crate::all` mis-resolved to the `all` builtin (a
                // separate primary that happens to share the suffix).
                let sigil = format!("%crate::{}", rest);
            ...
            let entry_idx = entries
                ...
                .or_else(|| {
                    ...
                    // `%crate::aliases` and try the primary.
                    if let Some(primary) = crate::builtins::primary_for_alias(&resolved) {
                ...
                .or_else(|| {
                    ...
                    // crate::builtins` should resolve to the
                    // `%crate::builtins` page.
            ...
            match entry_idx {
                ...
                None => {
                    ...
                    if let Some(text) = crate::lsp::doc_text_for(&resolved) {
    ...
    // The TUI is only useful for *browsing* without a target — when the
    // caller named a specific topic or page number, dump that page and
    // exit (`man pmap` semantics). Otherwise gate on real TTYs for
    // both stdin and stdout, since headless wrappers (CI, AI agents
    // like Gemini's exec, `s docs foo | cat`) often keep stdout as a
    // tty while piping stdin and the read loop would block forever.
...
fn run_deconvert_subcommand(args: &[String]) -> i32 {
    ...
    let opts = crate::deconvert::DeconvertOptions { output_delim };
    let mut errors = 0;
    for f in &files {
        let code = match std::fs::read_to_string(f) {
            Ok(c) => c,
        ...
        let deconverted = crate::deconvert_to_perl_with_options(&program, &opts);
        if in_place {
...
fn run_fmt_subcommand(args: &[String]) -> i32 {
    ...
    for f in &expanded {
        ...
        let formatted = crate::convert_to_stryke(&program);
        if in_place {
            if formatted == code {
...
fn run_minify_subcommand(args: &[String]) -> i32 {
    ...
    for f in &expanded {
        ...
        let minified = match crate::minify::minify_source(&code) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("stryke minify: {}: {}", f, e);
                errors += 1;

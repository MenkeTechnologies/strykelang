...
fn builtin_sk_logistic_predict(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let z = f1(args);
    Ok(StrykeValue::float(1.0 / (1.0 + (-z).exp())))
}

/// `sk_logistic_fit` — gradient descent step: w ← w − η ∇L.
fn builtin_sk_logistic_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
...
fn builtin_sk_random_forest_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args).max(0);
...
fn builtin_sk_gbt_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g_sum = f1(args);
...
fn builtin_sk_xgb_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g_l = f1(args);
...
fn builtin_sk_lightgbm_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g_l = f1(args);
...
fn builtin_sk_svm_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_sk_kmeans_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_dbscan_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_agglomerative_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let d_sum = f1(args);
...
fn builtin_sk_pca_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_tsne_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dist_sq = f1(args).max(0.0);
    Ok(StrykeValue::float(1.0 / (1.0 + dist_sq)))
}

/// `sk_umap_fit` — UMAP edge weight = exp(−(d − ρ) / σ).
fn builtin_sk_umap_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let d = f1(args);
...
fn builtin_sk_isolation_forest_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_lof_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lrd_avg = f1(args);
...
fn builtin_sk_kfold_split(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args).max(0);
...
fn builtin_sk_stratified_kfold(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_class = f1(args);
...
fn builtin_sk_cross_val_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_grid_search(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_random_search(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_iter = f1(args);
    let n_folds = args.get(1).map(|v| v.to_number()).unwrap_or(5.0);
...
fn builtin_sk_bayes_search(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mu = f1(args);
...
fn builtin_sk_pipeline_fit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(args.len() as i64))
}

/// `sk_standard_scaler` — z-score: (x − μ) / σ.
fn builtin_sk_standard_scaler(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_sk_min_max_scaler(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_sk_robust_scaler(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_sk_quantile_transform(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rank = f1(args);
...
fn builtin_sk_power_transform(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_one_hot(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cat = i1(args);
...
fn builtin_sk_ordinal_encode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(i1(args)))
}

/// `sk_label_encode` — assign new label = current count.
fn builtin_sk_label_encode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(i1(args)))
}

/// `sk_tfidf` — TF-IDF: tf · log(N / df).
fn builtin_sk_tfidf(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tf = f1(args);
...
fn builtin_sk_count_vectorize(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_silhouette(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_sk_calinski_harabasz(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let b = f1(args);
...
fn builtin_sk_davies_bouldin(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_adjusted_rand(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let ri = f1(args);
...
fn builtin_sk_mutual_info(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_lda_topic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_kw = f1(args);
...
fn builtin_sk_nmf_topic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_word2vec_train(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dot = f1(args);
...
fn builtin_sk_doc2vec_train(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_naive_bayes_predict(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let log_pri = f1(args);
...
fn builtin_sk_knn_predict(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sk_decision_tree_split(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {

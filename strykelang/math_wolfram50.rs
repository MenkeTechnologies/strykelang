...
fn builtin_os_priority_aging_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(p - factor * age))
}

/// MLFQ demote step (move down one level)
fn builtin_os_mlfq_demote_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let level = i1(args);
    let max_level = args.get(1).map(|v| v.to_number() as i64).unwrap_or(7);
    Ok(PerlValue::integer((level + 1).min(max_level)))
}
...
fn builtin_os_mlfq_promote_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let level = i1(args);
    let elapsed = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let s = args.get(2).map(|v| v.to_number()).unwrap_or(1000.0);
    if elapsed >= s { return Ok(PerlValue::integer(0)); }
    Ok(PerlValue::integer(level.max(0)))
}

/// Round-robin quantum (ms scaled by priority)
fn builtin_os_round_robin_quantum(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let prio = f1(args);
    let base = args.get(1).map(|v| v.to_number()).unwrap_or(10.0);
    Ok(PerlValue::float(base * (1.0 + prio / 40.0)))
}

/// Linux CFS vruntime increment
fn builtin_os_completely_fair_vruntime(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if weight == 0.0 { return Ok(PerlValue::float(vruntime)); }
    Ok(PerlValue::float(vruntime + delta / weight))
}

/// Lottery scheduler ticket count
fn builtin_os_lottery_ticket_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_os_stride_pass_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pass = f1(args);
    let stride = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(pass + stride))
}

/// EEVDF eligibility (vruntime > virtual_eligible)
fn builtin_os_eevdf_eligible(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v_runtime = f1(args);
    let v_eligible = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::integer(if v_runtime >= v_eligible { 1 } else { 0 }))
}

/// CFS load balance step
fn builtin_os_cfs_load_balance_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let busy = f1(args);
    let idle = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((busy - idle) / 2.0))
}

/// EAS energy estimate
fn builtin_os_eas_energy_estimate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(p_static + p_dynamic * load))
}

/// SMT threading share (per logical CPU)
fn builtin_os_smt_threading_share(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = f1(args);
...
fn builtin_os_numa_node_distance(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer(if a == b { 10 } else { 20 }))
}

/// CPU affinity score (matching mask bit count)
fn builtin_os_cpu_affinity_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mask = i1(args) as u64;
    Ok(PerlValue::integer(mask.count_ones() as i64))
}

/// Thread migration cost (cache miss penalty)
fn builtin_os_thread_migration_cost(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cache_size = f1(args);
    Ok(PerlValue::float(cache_size * 0.001))
}

/// Load average decay (1, 5, 15-min EWMAs): newL = e^(-1/N) · oldL + (1 - e^(-1/N)) · n
fn builtin_os_load_average_decay(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let old_l = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let tau = args.get(2).map(|v| v.to_number()).unwrap_or(60.0);
    if tau == 0.0 { return Ok(PerlValue::float(n)); }
    let alpha = (-1.0 / tau).exp();
    Ok(PerlValue::float(alpha * old_l + (1.0 - alpha) * n))
}
...
fn builtin_os_runqueue_depth(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let states = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let depth = states.iter().filter(|s| {
        let v = s.to_number() as i64;
        v == 0 || v == 1
    }).count();
    Ok(PerlValue::integer(depth as i64))
}

/// Deadline I/O scheduler check
fn builtin_os_io_scheduler_deadline(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let deadline = f1(args);
    let now = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::integer(if now >= deadline { 1 } else { 0 }))
}
...
fn builtin_os_io_scheduler_cfq_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let service = f1(args);
    let weight = args.get(1).map(|v| v.to_number()).unwrap_or(500.0).max(1.0);
    Ok(PerlValue::float(service / weight))
}
...
fn builtin_os_io_scheduler_noop_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut sectors = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    sectors.sort_by(|a, b| a.to_number().partial_cmp(&b.to_number()).unwrap_or(std::cmp::Ordering::Equal));
    if sectors.is_empty() { return Ok(PerlValue::integer(0)); }
    let mut merged = 1_i64;
    for w in sectors.windows(2) {
        if (w[1].to_number() - w[0].to_number()).abs() > 1.0 { merged += 1; }
    }
    Ok(PerlValue::integer(merged))
}

/// BFQ I/O scheduler step (budget)
fn builtin_os_io_scheduler_bfq_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let budget = f1(args);
    let consumed = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((budget - consumed).max(0.0)))
}
...
fn builtin_os_io_scheduler_kyber_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let depth = f1(args);
    ...
    Ok(PerlValue::integer(new_depth.clamp(1.0, 256.0).round() as i64))
}

/// MQ-deadline scheduler step
fn builtin_os_io_scheduler_mq_deadline(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_os_io_scheduler_deadline(args)
}

/// Anticipation window (anticipatory I/O)
fn builtin_os_anticipation_window(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let last_read_time = f1(args);
    let dt = args.get(1).map(|v| v.to_number()).unwrap_or(0.001);
    Ok(PerlValue::float(last_read_time + dt))
}

/// Elevator (SCAN) step
fn builtin_os_elevator_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pos = f1(args);
    let direction = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(pos + direction))
}

/// Disk seek time (linear in distance)
fn builtin_os_disk_seek_time(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist = f1(args);
    let speed = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if speed == 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(dist / speed))
}

/// Disk rotational latency (avg = 1/(2·rpm/60))
fn builtin_os_disk_rotational_lat(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rpm = f1(args);
    if rpm <= 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(60.0 / (2.0 * rpm)))
}

/// Disk transfer time = bytes / bandwidth
fn builtin_os_disk_transfer_time(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = f1(args);
    let bw = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if bw == 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(bytes / bw))
}
...
fn builtin_os_pre_fetch_window(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(win))
}

/// Buffer cache pages
fn builtin_os_buffer_cache_pages(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = f1(args);
    let page_size = args.get(1).map(|v| v.to_number()).unwrap_or(4096.0);
    if page_size == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(bytes / page_size))
}

/// Dirty page threshold
fn builtin_os_dirty_page_threshold(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mem = f1(args);
    let pct = args.get(1).map(|v| v.to_number()).unwrap_or(0.2);
    Ok(PerlValue::float(mem * pct))
}

/// Writeback step
fn builtin_os_writeback_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dirty = f1(args);
    let rate = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float((dirty - rate).max(0.0)))
}

/// Swappiness factor (linux: 0..200)
fn builtin_os_swappiness_factor(args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::float(f1(args).clamp(0.0, 200.0)))
}
...
fn builtin_os_kswapd_wake_threshold(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(if free_pages < low_wmark { 1 } else { 0 }))
}

/// OOM score step (badness)
fn builtin_os_oom_score_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rss = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(1000.0 * rss / total))
}
...
fn builtin_os_page_replacement_lru(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let times = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    if times.is_empty() { return Ok(PerlValue::integer(-1)); }
...
fn builtin_os_page_replacement_clock(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pos = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1).max(1);
    Ok(PerlValue::integer((pos + 1) % n))
}
...
fn builtin_os_page_replacement_2q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if hit_a1out != 0 { return Ok(PerlValue::integer(0)); }
    if a1in >= kin_limit { return Ok(PerlValue::integer(2)); }
    Ok(PerlValue::integer(1))
}

/// Working set W(t, τ) = pages referenced in window [t-τ, t]. Compute from
/// access trace + timestamp + window.
fn builtin_os_working_set_size(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let trace = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let now = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_os_thrashing_threshold(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pf_rate = f1(args);
    let threshold = args.get(1).map(|v| v.to_number()).unwrap_or(0.1);
    Ok(PerlValue::integer(if pf_rate > threshold { 1 } else { 0 }))
}
...
fn builtin_os_demand_paging_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float((1.0 - p) * mem + p * fault))
}

/// Copy-on-write check
fn builtin_os_copy_on_write_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let writable = i1(args);
    let shared = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer(if shared != 0 && writable != 0 { 1 } else { 0 }))
}

/// Zero-page optimization (free if all-zero)
fn builtin_os_zero_page_optimization(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let nonzero = i1(args);
    Ok(PerlValue::integer(if nonzero == 0 { 1 } else { 0 }))
}

/// Huge page threshold
fn builtin_os_huge_page_threshold(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pages = f1(args);
    let huge_size = args.get(1).map(|v| v.to_number()).unwrap_or(512.0);
    Ok(PerlValue::integer(if pages >= huge_size { 1 } else { 0 }))
}
...
fn builtin_os_transparent_hugepage(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mode = i1(args);
    let madvised = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    let aligned = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0);
    if mode == 2 || aligned == 0 { return Ok(PerlValue::integer(0)); }
    if mode == 0 { return Ok(PerlValue::integer(1)); }
    Ok(PerlValue::integer(if madvised != 0 { 1 } else { 0 }))
}

/// KASAN shadow offset
fn builtin_os_kasan_shadow_offset(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let addr = i1(args) as u64;
    let offset = args.get(1).map(|v| v.to_number() as u64).unwrap_or(0xdfff_e000_0000_0000);
    Ok(PerlValue::integer((addr.wrapping_shr(3).wrapping_add(offset)) as i64))
}

/// KFENCE check (sample 1 in N allocations)
fn builtin_os_kfence_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.5);
    Ok(PerlValue::integer(if r * n < 1.0 { 1 } else { 0 }))
}

/// KFENCE alloc index
fn builtin_os_kfence_alloc_index(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(255);
    Ok(PerlValue::integer(i % (n + 1).max(1)))
}

/// SLUB object size round (next power of 2)
fn builtin_os_slub_object_size_round(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(1) as u64;
    Ok(PerlValue::integer(n.next_power_of_two() as i64))
}

/// Slab color offset (cache line distribution)
fn builtin_os_slab_color_offset(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let slot = i1(args);
    let line_size = args.get(1).map(|v| v.to_number() as i64).unwrap_or(64);
    Ok(PerlValue::integer(slot * line_size))
}
...
fn builtin_os_per_cpu_cache_size(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(limit.min(n / 8.0).max(1.0).round() as i64))
}

/// Buddy allocator order pick: ⌈log2(pages)⌉
fn builtin_os_buddy_order_pick(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pages = f1(args);
    if pages <= 0.0 { return Ok(PerlValue::integer(0)); }
    Ok(PerlValue::integer(pages.log2().ceil() as i64))
}
...
fn builtin_os_compact_memory_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let prev = i1(args);
    ...
    Ok(PerlValue::integer((prev + migratable - rejected).max(0)))
}

/// KVM VMCS field offset
fn builtin_os_kvm_vmcs_field_offset(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let field_id = i1(args);
    Ok(PerlValue::integer(field_id * 8))
}

/// APIC IRQ priority class
fn builtin_os_apic_irq_priority(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let irq = i1(args);
    Ok(PerlValue::integer(irq >> 4))
}
...
fn builtin_os_iommu_domain_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(((iova >> shift) & 0x1ff) as i64))
}

/// PCI bus address
fn builtin_os_pci_bus_address(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bus = i1(args);
    let dev = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    let func = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer((bus << 8) | (dev << 3) | func))
}

/// ACPI state transition (S0..S5 cost)
fn builtin_os_acpi_state_transition(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let from = i1(args);
    let to = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer((to - from).abs()))
}

/// CPUFreq governor step
fn builtin_os_cpufreq_governor_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let load = f1(args);
    let f_max = args.get(1).map(|v| v.to_number()).unwrap_or(3000.0);
    Ok(PerlValue::float(load * f_max))
}
...
fn builtin_os_intel_pstate_target(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(raw.clamp(min_perf, max_perf)))
}
...
fn builtin_os_amd_pstate_target(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let busy = f1(args).clamp(0.0, 1.0);
    let max_perf = args.get(1).map(|v| v.to_number()).unwrap_or(3000.0);
    let min_perf = args.get(2).map(|v| v.to_number()).unwrap_or(800.0);
    let epp = args.get(3).map(|v| v.to_number()).unwrap_or(128.0).clamp(0.0, 255.0);
    let alpha = busy * (1.0 - epp / 255.0);
    Ok(PerlValue::float(min_perf + alpha * (max_perf - min_perf)))
}

/// Thermal zone trip point
fn builtin_os_thermal_zone_trip(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let temp = f1(args);
    let trip = args.get(1).map(|v| v.to_number()).unwrap_or(85.0);
    Ok(PerlValue::integer(if temp >= trip { 1 } else { 0 }))
}

/// Throttle temperature (clamps frequency)
fn builtin_os_throttle_temperature(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f = f1(args);
    let throttle = args.get(1).map(|v| v.to_number()).unwrap_or(0.5);
    Ok(PerlValue::float(f * throttle.clamp(0.0, 1.0)))
}

/// Battery capacity percent
fn builtin_os_battery_capacity_pct(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cur = f1(args);
    let max = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if max == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(100.0 * cur / max))
}

/// Powertop wakeup score = wakeups_per_second · power_per_wakeup_J. Higher is
/// worse. Args: wakeups, total_seconds, power_watts.
fn builtin_os_powertop_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let wakeups = f1(args);
    let secs = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-6);
    let p_watts = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(wakeups / secs * p_watts))
}

/// Idle state select (deepest below latency budget)
fn builtin_os_idle_state_select(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let predicted_idle = f1(args);
    let depths = arg_to_vec(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let mut best = -1_i64;
...
fn builtin_os_c_state_residency(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let in_state = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(in_state / total))
}

/// P-state voltage at frequency f: V(f) = V_min + k·(f - f_min). Linear DVFS curve.
fn builtin_os_p_state_voltage(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f = f1(args);
...
fn builtin_os_inotify_event_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mask = i1(args) as u64;
    Ok(PerlValue::integer(mask.count_ones() as i64))
}

/// epoll_ctl total ops = ADD + MOD + DEL counts. Args: array [add, mod, del].
fn builtin_os_epoll_ctl_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(v.iter().map(|x| x.to_number() as i64).sum()))
}

/// io_uring SQE count = head - tail in submission queue ring. Args: head, tail, mask.
fn builtin_os_io_uring_sqe_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let head = i1(args) as u32;
    let tail = args.get(1).map(|v| v.to_number() as u32).unwrap_or(0);
    Ok(PerlValue::integer(tail.wrapping_sub(head) as i64))
}

/// io_uring CQE count = tail - head in completion queue ring.
fn builtin_os_io_uring_cqe_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let head = i1(args) as u32;
    let tail = args.get(1).map(|v| v.to_number() as u32).unwrap_or(0);
    Ok(PerlValue::integer(tail.wrapping_sub(head) as i64))
}
...
fn builtin_os_kqueue_event_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let flags = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_os_systemd_journal_size(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let files = f1(args);
    let max_mb = args.get(1).map(|v| v.to_number()).unwrap_or(128.0);
    Ok(PerlValue::float(files * max_mb * 1024.0 * 1024.0))
}

/// dmesg severity level (RFC 5424: 0=emerg..7=debug)
fn builtin_os_dmesg_severity_level(args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::integer(i1(args).clamp(0, 7)))
}
...
fn builtin_os_audit_event_priority(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let t = i1(args);
    if (1100..1200).contains(&t) { Ok(PerlValue::integer(1)) }
    else if (1400..1500).contains(&t) { Ok(PerlValue::integer(2)) }
    else if (1500..1600).contains(&t) { Ok(PerlValue::integer(3)) }
    else if (1300..1400).contains(&t) { Ok(PerlValue::integer(4)) }
    else { Ok(PerlValue::integer(5)) }
}
...
fn builtin_os_apparmor_profile_active(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mode = i1(args);
    Ok(PerlValue::integer(if mode == 1 || mode == 2 { 1 } else { 0 }))
}

/// SELinux context match
fn builtin_os_selinux_context_match(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
...
fn builtin_os_smack_label_compare(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if exact != 0 { return Ok(PerlValue::integer(if subj == obj { 1 } else { 0 })); }
    if obj == 0 || subj == 2 { return Ok(PerlValue::integer(1)); }
    if subj == 3 { return Ok(PerlValue::integer(0)); }
    Ok(PerlValue::integer(if subj == obj || (subj == 1 && obj != 3) { 1 } else { 0 }))
}

/// Capability check (linux capabilities bitmask)
fn builtin_os_capability_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mask = i1(args) as u64;
    let cap = args.get(1).map(|v| v.to_number() as u64).unwrap_or(0);
    Ok(PerlValue::integer(if mask & (1u64 << cap) != 0 { 1 } else { 0 }))
}

/// Seccomp filter step
fn builtin_os_seccomp_filter_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let action = i1(args);
...
fn builtin_os_namespace_isolation(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_os_cgroup_v1_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mask = i1(args) as u64;
    Ok(PerlValue::integer((mask & 0xfff).count_ones() as i64))
}

/// cgroup v2 controller count: 7 standard v2 controllers (cpu, memory, io,
/// pids, cpuset, hugetlb, rdma). Args: bitmask.
fn builtin_os_cgroup_v2_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mask = i1(args) as u64;
    Ok(PerlValue::integer((mask & 0x7f).count_ones() as i64))
}

/// pid_max value
fn builtin_os_pid_max_value(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::integer(4_194_304))
}

/// kernel.threads-max default: max(20, RAM_KB / (8 · THREAD_SIZE_KB)) per
/// fork.c. Args: RAM in KB, optional thread-stack size in KB.
fn builtin_os_thread_max_value(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ram_kb = f1(args);
    let stack_kb = args.get(1).map(|v| v.to_number()).unwrap_or(16.0).max(1.0);
    Ok(PerlValue::integer((ram_kb / (8.0 * stack_kb)).max(20.0).floor() as i64))
}

/// fs.file-max default: NR_FILE_STAT pages → max(8192, RAM_pages / 10) per
/// kernel/sysctl.c. Args: RAM in pages.
fn builtin_os_file_max_value(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ram_pages = f1(args);
    Ok(PerlValue::integer((ram_pages / 10.0).max(8192.0).floor() as i64))
}

/// Open files count: sum across [pid].nr_open arrays.
fn builtin_os_open_files_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(v.iter().map(|x| x.to_number() as i64).sum()))
}
...
fn builtin_os_socket_max_value(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let want = i1(args);
    let hard_max = args.get(1).map(|v| v.to_number() as i64).unwrap_or(65535);
    Ok(PerlValue::integer(want.min(hard_max).max(0)))
}

/// inotify watches per user max: fs.inotify.max_user_watches; default
/// max(8192, ram_pages / 32). Args: RAM in pages.
fn builtin_os_inotify_max_watches(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ram_pages = f1(args);
    Ok(PerlValue::integer((ram_pages / 32.0).max(8192.0).floor() as i64))
}
...
fn builtin_os_oom_kill_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rss = f1(args);
    ...
    if is_kthread != 0 || is_init != 0 || adj <= -1000 { return Ok(PerlValue::integer(0)); }
    let mem_score = (1000.0 * (rss + swap + pgt) / total).round() as i64;
    let total_score = (mem_score + adj).clamp(1, 1000);
    Ok(PerlValue::integer(total_score))
}

/// zswap compress ratio
fn builtin_os_zswap_compress_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let raw = f1(args);
    let compressed = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if compressed == 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(raw / compressed))
}
...
fn builtin_os_zram_compress_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let raw = f1(args);
    let compressed = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    ...
    Ok(PerlValue::float(raw / denom))
}

/// Swap pressure score = swap_used / swap_total + (1 - free_ram / total_ram).
fn builtin_os_swap_pressure_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(swap_used / swap_total + 1.0 - free_ram / total_ram))
}

/// PSI pressure stall step
fn builtin_os_pressure_stall_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let stall_time = f1(args);
    let total_time = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total_time == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(stall_time / total_time))
}
...
fn builtin_os_psi_avg10_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let prev = f1(args);
    let sample = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let exp_n = 1677.0_f64 / 2048.0;
    Ok(PerlValue::float(prev * exp_n + sample * (1.0 - exp_n)))
}

fn builtin_os_psi_avg60_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let prev = f1(args);
    let sample = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_os_psi_avg300_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let prev = f1(args);
    let sample = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_os_load_proc_avg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let active = f1(args);
    let prev = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let exp1 = (-5.0_f64 / 60.0).exp();
    Ok(PerlValue::float(prev * exp1 + active * (1.0 - exp1)))
}

/// User CPU% load: user_jiffies / total_jiffies · 100.
fn builtin_os_load_user_avg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let user = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(100.0 * user / total))
}

/// I/O wait fraction: iowait_jiffies / total_jiffies · 100.
fn builtin_os_load_iowait_avg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let iowait = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(100.0 * iowait / total))
}

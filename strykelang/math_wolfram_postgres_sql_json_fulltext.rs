...
fn builtin_btrim(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_translate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = b70_to_codepoints(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_ascii(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = b70_to_codepoints(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_regexp_split(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let hits = i1(args);
    Ok(StrykeValue::integer(hits + 1))
}

/// regexp_matches — count of matches found.
fn builtin_regexp_matches(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let hits = i1(args);
    Ok(StrykeValue::integer(hits.max(0)))
}

/// regexp_replace — count of replacements made.
fn builtin_regexp_replace(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let hits = i1(args);
...
fn builtin_json_build_object(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_jsonb_set(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// json_array_length.
fn builtin_json_array_length(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_json_extract_path(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_json_strip_nulls(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let nulls = i1(args);
...
fn builtin_jsonb_pretty(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tokens = i1(args);
...
fn builtin_jsonb_path_query(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_json_each(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer((v.len() / 2) as i64))
}

/// jsonb_array_length.
fn builtin_jsonb_array_length(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_json_array_length(args)
}

/// jsonb_object_keys — count of keys.
fn builtin_jsonb_object_keys(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer((v.len() / 2) as i64))
}
...
/// 4 array, 5 object.
fn builtin_jsonb_typeof(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let id = i1(args).clamp(0, 5);
    Ok(StrykeValue::integer(id))
}

/// array_to_jsonb — count of elements emitted.
fn builtin_array_to_jsonb(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.len() as i64))
}

/// ts_match @@ — boolean: does tsvector match tsquery (1 if any term overlaps).
fn builtin_ts_match(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_ts_rank(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_ts_headline(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_substring_similarity(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_levenshtein_dist(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_word_similarity(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_substring_similarity(args)
}

/// strict_word_similarity — same but with word boundaries (boolean for hit).
fn builtin_strict_word_similarity(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sim = f1(args);
    Ok(StrykeValue::float(if sim >= 0.5 { sim } else { 0.0 }))
}

/// hstore_to_array — flatten {k=>v, ...} into [k1, v1, k2, v2, ...].
fn builtin_hstore_to_array(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::array(v))
}

/// array_to_hstore — fold flat array back into key-count.
fn builtin_array_to_hstore(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer((v.len() / 2) as i64))
}

/// string_agg — total characters in concatenation with separator length s.
fn builtin_string_agg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_array_agg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.len() as i64))
}

/// corr_agg — Pearson correlation aggregate.
fn builtin_corr_agg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_covar_pop(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let xs = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let ys = args.get(1).map(b70_to_floats).unwrap_or_default();
...
fn builtin_covar_samp(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let xs = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let ys = args.get(1).map(b70_to_floats).unwrap_or_default();
    let n = xs.len().min(ys.len());
...
fn builtin_regr_slope(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let xs = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let ys = args.get(1).map(b70_to_floats).unwrap_or_default();
    let n = xs.len().min(ys.len());
    if n < 2 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_regr_intercept(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let xs = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let ys = args.get(1).map(b70_to_floats).unwrap_or_default();
    let n = xs.len().min(ys.len());
    if n == 0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_regr_r2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let xs = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let ys = args.get(1).map(b70_to_floats).unwrap_or_default();
    let n = xs.len().min(ys.len());
    if n < 2 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_percentile_cont(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_percentile_disc(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mut v = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let p = args.get(1).map(|x| x.to_number()).unwrap_or(0.5).clamp(0.0, 1.0);
    if v.is_empty() { return Ok(StrykeValue::float(0.0)); }
    v.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
...
fn builtin_mode_agg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    if v.is_empty() { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_array_to_string(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_string_agg(args)
}

/// array_position — 1-based index of v in array (0 if not found, Postgres style).
fn builtin_array_position(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_array_positions(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let needle = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
...
fn builtin_array_remove(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let needle = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::integer(v.iter().filter(|&&x| x != needle).count() as i64))
}
...
fn builtin_array_replace(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let needle = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::integer(v.iter().filter(|&&x| x == needle).count() as i64))
}
...
fn builtin_xmlforest(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer((v.len() / 2) as i64))
}

/// xmlagg — Σ child-node counts.
fn builtin_xmlagg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b70_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));

...
fn builtin_euler_char_complex(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_betti_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_betti_one(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = i1(args);
...
fn builtin_hodge_diamond_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = i1(args);
...
fn builtin_poincare_duality(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bk = f1(args);
...
fn builtin_fundamental_group_zn(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = i1(args);
    Ok(StrykeValue::integer(p.abs().max(1)))
}

/// Rank of the kth homology group from boundary map ranks
fn builtin_homology_rank(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dim_ck = f1(args);
...
fn builtin_cohomology_rank(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_homology_rank(args)
}

/// πₖ(Sⁿ): canonical small cases (only the obvious ones)
fn builtin_homotopy_group_sphere_pi(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k = i1(args);
...
fn builtin_torsion_coefficient(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer(n.abs()))
}

/// Volume of standard n-simplex of side 1: √(n+1) / (n! 2^(n/2))
fn builtin_simplex_volume_n(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_simplicial_volume(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
    Ok(StrykeValue::float(((4.0 * g - 4.0).max(0.0)) * 1.0))
}

/// Number of simplices in a nerve complex with k cover sets, all intersecting
fn builtin_nerve_complex_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k = i1(args);
...
fn builtin_cech_zero_cohomology(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_de_rham_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_poincare_polynomial_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_chromatic_homology_rank(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_khovanov_q_grading(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_plus = i1(args);
...
fn builtin_hochschild_zero(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_cyclic_homology_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let hh_n = f1(args);
...
fn builtin_group_cohomology_dim(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = i1(args).max(1);
...
fn builtin_group_homology_dim(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = i1(args).max(1);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    if n == 0 { return Ok(StrykeValue::float(1.0)); }
    if m == 1 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_abelianization_quotient(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_free_group_rank_lower(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let gens = i1(args);
...
fn builtin_nilpotency_class_lower(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_solvable_length_upper(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_schreier_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_todd_genus_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c1 = f1(args);
    Ok(StrykeValue::float(c1 / 2.0))
}

/// Hirzebruch signature for 4k-manifold: ⟨L_k, [M]⟩ ≈ p₁/3 in dim 4
fn builtin_hirzebruch_signature(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p1 = f1(args);
    Ok(StrykeValue::float(p1 / 3.0))
}

/// Chern-Simons action level k SU(2) on S³: k/(4π²)·∫ tr(A∧dA + 2A³/3)
fn builtin_chern_simons_action(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k = f1(args);
...
fn builtin_gauss_bonnet_total(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let chi = f1(args);
    Ok(StrykeValue::float(2.0 * std::f64::consts::PI * chi))
}

/// Lower bound on Seifert genus from Alexander polynomial degree / 2
fn builtin_seifert_genus_lower(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_alexander_polynomial_at_one(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(if (i1(args) % 2).abs() == 0 { 1 } else { -1 }))
}

/// Jones polynomial at q = -1 gives (-1)^c · Δ(-1)
fn builtin_jones_polynomial_at_minus_one(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let determinant = i1(args);
    Ok(StrykeValue::integer(determinant.abs()))
}

/// Jones polynomial at q = i gives ±(√2)^(c-1)
fn builtin_jones_polynomial_at_i(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = i1(args);
    Ok(StrykeValue::float(2f64.sqrt().powi((c - 1) as i32)))
}

/// HOMFLY polynomial evaluation at (l, m)
fn builtin_homfly_evaluation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = f1(args);
...
fn builtin_kauffman_bracket_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    Ok(StrykeValue::float(-(a * a) - 1.0 / (a * a)))
}

/// Cabling pair signature: (p, q) torus knot signature ≈ -(p-1)(q-1)
fn builtin_cabling_pair_signature(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = i1(args);
...
fn builtin_seifert_form_2x2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_turaev_alexander_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let z = f1(args);
    Ok(StrykeValue::float(z * z - 1.0))
}

/// V polynomial (Jones-style) evaluated at q
fn builtin_v_polynomial_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
    Ok(StrykeValue::float(-q.powi(2) - q.powi(-2)))
}

/// Skein relation step for Jones polynomial: q V₊ - q⁻¹ V₋ = (q^(1/2) - q^(-1/2)) V₀
fn builtin_polynomial_jones_skein(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
...
fn builtin_delta_complex_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().map(|x| x.to_number()).sum()))
}

/// Zeta function of a 2-element poset evaluated at s
fn builtin_poset_zeta_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = f1(args);
    Ok(StrykeValue::float(1.0 + s))
}

/// Möbius function of a 2-element poset
fn builtin_mobius_poset_two(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(-1))
}
...
fn builtin_mobius_inversion_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mu_val = f1(args);
...
fn builtin_incidence_algebra_dim(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer(n * (n + 1) / 2))
}

/// Number of paths of length k in a quiver with adjacency matrix sum a (scalar approx)
fn builtin_quiver_path_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_representation_dim_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_weyl_group_order(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let mut p = 1_i64;
...
fn builtin_root_system_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer(n * (n + 1)))
}

/// Determinant of Cartan matrix A₂ = 3
fn builtin_cartan_determinant_a2(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(3))
}

/// Cartan matrix B₂ entry (i, j)
fn builtin_cartan_matrix_b2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = i1(args);
...
fn builtin_killing_form_su2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_casimir_eigenvalue_su2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let j = f1(args);
    Ok(StrykeValue::float(j * (j + 1.0)))
}

/// Universal enveloping algebra dimension in degree ≤ k for sl₂: (k+1)(k+2)(k+3)/6
fn builtin_universal_enveloping_dim(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let k = i1(args);
    Ok(StrykeValue::integer((k + 1) * (k + 2) * (k + 3) / 6))
}

/// Verma module character step ch L_λ at q
fn builtin_verma_character_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lam = f1(args);
...
fn builtin_plethystic_substitution_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f_val = f1(args);
...
fn builtin_schur_polynomial_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_hall_inner_product_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_plactic_class_size(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let k = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
...
fn builtin_robinson_schensted_pair(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let mut f = 1_i64;
...
fn builtin_yamanouchi_word_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_rsk_size(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let mut f = 1_i64;
    for k in 2..=n { f = f.saturating_mul(k); }
    Ok(StrykeValue::integer(f))
...
fn builtin_character_su2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let j = f1(args);
...
fn builtin_character_sun(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_quantum_dimension_su2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let j = f1(args);
...
fn builtin_quantum_dimension_q(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_fusion_rule_su2_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = i1(args);
...
fn builtin_modular_data_s_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_modular_data_t_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = f1(args);
...
fn builtin_verlinde_count_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = i1(args).max(0);
...
fn builtin_quantum_invariant_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let coefs = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_operad_count_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_moduli_dimension_curves(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
...
fn builtin_hodge_polynomial_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().map(|x| x.to_number()).sum()))
}

/// Mirror symmetry check: h^{p,q}(M) == h^{n-p,q}(M̃)
fn builtin_mirror_symmetry_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = f1(args);
...
fn builtin_gromov_witten_invariant(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_donaldson_invariant(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q = f1(args);
...
fn builtin_seiberg_witten_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c1_sq = f1(args);
...
fn builtin_floer_homology_rank(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mtype = i1(args);
...
fn builtin_khovanov_rasmussen_s(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let kind = i1(args);
    if kind == 1 {
...
fn builtin_ozsvath_szabo_tau(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let kind = i1(args);
    if kind == 1 {
        let p = args.get(1).map(|v| v.to_number() as i64).unwrap_or(2).max(2);
        let q = args.get(2).map(|v| v.to_number() as i64).unwrap_or(3).max(2);
...
fn builtin_heegaard_genus_lower(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_fintushel_stern_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sw_x = f1(args);
...
fn builtin_bauer_furuta_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let b2 = f1(args);
    let sigma = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_geometric_intersection_number(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_algebraic_intersection_number(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let b = arg_to_vec(args.get(1).unwrap_or(&StrykeValue::array(vec![])));

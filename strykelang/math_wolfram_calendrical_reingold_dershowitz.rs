...
fn builtin_fixed_from_gregorian(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
fn builtin_gregorian_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let d0 = n - 1;
    ...
    let prior_days = n - builtin_fixed_from_gregorian(&[
        StrykeValue::integer(year), StrykeValue::integer(1), StrykeValue::integer(1)
    ]).unwrap().to_number() as i64;
    let correction = if n < builtin_fixed_from_gregorian(&[
        StrykeValue::integer(year), StrykeValue::integer(3), StrykeValue::integer(1)
    ...
    let day = n - builtin_fixed_from_gregorian(&[
        StrykeValue::integer(year), StrykeValue::integer(month), StrykeValue::integer(1)
    ]).unwrap().to_number() as i64 + 1;
    Ok(StrykeValue::integer(year * 10000 + month * 100 + day))
}

/// Julian: leap if y mod 4 == 0 (no century rule). Epoch JD 0001-01-01 = RD -1.
fn builtin_fixed_from_julian(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let d = args.get(2).map(|v| v.to_number() as i64).unwrap_or(1);
...
/// `julian_from_fixed`
fn builtin_julian_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let approx = ((4 * (n + 1) + 1464) / 1461) - 1;
    let y = if approx <= 0 { approx - 1 } else { approx };
    let prior_days = n - builtin_fixed_from_julian(&[
        StrykeValue::integer(y), StrykeValue::integer(1), StrykeValue::integer(1)
    ]).unwrap().to_number() as i64;
    let leap = y.rem_euclid(4) == 0;
    let correction = if n < builtin_fixed_from_julian(&[
        StrykeValue::integer(y), StrykeValue::integer(3), StrykeValue::integer(1)
    ]).unwrap().to_number() as i64 { 0 }
    else if leap { 1 } else { 2 };
    let month = (12 * (prior_days + correction) + 373) / 367;
    let day = n - builtin_fixed_from_julian(&[
        StrykeValue::integer(y), StrykeValue::integer(month), StrykeValue::integer(1)
    ]).unwrap().to_number() as i64 + 1;
    Ok(StrykeValue::integer(y * 10000 + month * 100 + day))
}

/// ISO week date: returns iso_year*10000 + week*100 + day.
fn builtin_iso_week_date(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    ...
    Ok(StrykeValue::integer(year * 10000 + week * 100 + day))
}

fn gregorian_from_fixed_year(n: i64) -> i64 {
    let v = builtin_gregorian_from_fixed(&[StrykeValue::integer(n)]).unwrap().to_number() as i64;
    v / 10000
}

fn fixed_iso_year_start(year: i64) -> i64 {
    let jan4 = builtin_fixed_from_gregorian(&[
        StrykeValue::integer(year), StrykeValue::integer(1), StrykeValue::integer(4)
    ]).unwrap().to_number() as i64;
    jan4 - (jan4 - 1).rem_euclid(7)
}

/// Hebrew calendar: leap year if year mod 19 ∈ {0, 3, 6, 8, 11, 14, 17}.
fn builtin_hebrew_leap_year(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let r = y.rem_euclid(19);
    let leap = matches!(r, 0 | 3 | 6 | 8 | 11 | 14 | 17);
    Ok(StrykeValue::integer(if leap { 1 } else { 0 }))
}

/// Hebrew year length: 353, 354, 355 (common); 383, 384, 385 (leap).
fn builtin_hebrew_year_length(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let elapsed = hebrew_elapsed_days(y + 1) - hebrew_elapsed_days(y);
    Ok(StrykeValue::integer(elapsed))
}
...
/// `fixed_from_hebrew`
fn builtin_fixed_from_hebrew(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let d = args.get(2).map(|v| v.to_number() as i64).unwrap_or(1);
    ...
    Ok(StrykeValue::integer(hebrew_epoch_rd + hebrew_elapsed_days(y) + sum_months + d - 1))
}
...
fn builtin_islamic_leap_year(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let r = (14 + 11 * y).rem_euclid(30);
    Ok(StrykeValue::integer(if r < 11 { 1 } else { 0 }))
}

/// `fixed_from_islamic`
fn builtin_fixed_from_islamic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let d = args.get(2).map(|v| v.to_number() as i64).unwrap_or(1);
...
fn builtin_persian_arithmetic_leap(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
/// `fixed_from_persian`
fn builtin_fixed_from_persian(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let d = args.get(2).map(|v| v.to_number() as i64).unwrap_or(1);
...
fn builtin_coptic_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_ethiopic_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_french_revolutionary_leap(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let leap = y.rem_euclid(4) == 0
...
/// `fixed_from_french`
fn builtin_fixed_from_french(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let d = args.get(2).map(|v| v.to_number() as i64).unwrap_or(1);
...
fn builtin_chinese_year_zodiac(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    Ok(StrykeValue::integer((y - 4).rem_euclid(12)))
}
...
fn builtin_chinese_lunation_winter(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let solstice_1900: f64 = 693595.0;
    let synodic_month: f64 = 29.530_588_853;
    Ok(StrykeValue::integer(((n as f64 - solstice_1900) / synodic_month).floor() as i64))
}
...
fn builtin_hindu_solar_year(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let saka_epoch_rd: f64 = 84371.0;
    let mean_year: f64 = 365.258_756_481_481;
    Ok(StrykeValue::integer(((n as f64 - saka_epoch_rd) / mean_year).floor() as i64 + 1))
}

/// Hindu lunisolar tabular month index 1..12 within current solar year.
fn builtin_hindu_lunisolar_month(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let synodic_month: f64 = 29.530_588_853;
    let saka_epoch_rd: f64 = 84371.0;
    let lunations = ((n as f64 - saka_epoch_rd) / synodic_month).floor() as i64;
    Ok(StrykeValue::integer(lunations.rem_euclid(12) + 1))
}
...
fn builtin_maya_long_count_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_mayan_haab_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let mlc_epoch: i64 = -1137142;
...
fn builtin_mayan_tzolkin_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let mlc_epoch: i64 = -1137142;
    let count = n - mlc_epoch;
    let number = (count + 4).rem_euclid(13) + 1;
    let name = (count + 19).rem_euclid(20) + 1;
    Ok(StrykeValue::integer(number * 100 + name))
}
...
fn builtin_badi_year_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let badi_epoch_rd: i64 = 673222;
    Ok(StrykeValue::integer((n - badi_epoch_rd).div_euclid(365) + 1))
}

/// `bahai_from_fixed`
fn builtin_bahai_from_fixed(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    let badi_epoch_rd: i64 = 673222;
...
fn builtin_easter_gregorian_year(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
fn builtin_easter_orthodox_year(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    ...
    let julian_n = builtin_fixed_from_julian(&[
        StrykeValue::integer(y), StrykeValue::integer(month_julian), StrykeValue::integer(day_julian)
    ]).unwrap().to_number() as i64;
    let greg = builtin_gregorian_from_fixed(&[StrykeValue::integer(julian_n)])
        .unwrap().to_number() as i64;
    Ok(StrykeValue::integer((greg / 100) % 10000))
}

/// Easter on the Julian (old-calendar) date: month*100 + day in JULIAN.
fn builtin_easter_julian_year(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let a = y.rem_euclid(4);
    let b = y.rem_euclid(7);
    let c = y.rem_euclid(19);
...
/// 1 = Sunday, ..., 6 = Friday.
fn builtin_day_of_week_zeller(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    ...
    Ok(StrykeValue::integer(h))
}

/// ISO 8601 day number in the week (Mon=1, ..., Sun=7).
fn builtin_iso_day_number(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer((n - 1).rem_euclid(7) + 1))
}

/// English short weekday name index: 0..6 → Sun..Sat.
fn builtin_weekday_name_short(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_leap_year_gregorian(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    Ok(StrykeValue::integer(if leap_gregorian(y) { 1 } else { 0 }))
}

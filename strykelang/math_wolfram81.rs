...
fn b81_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

fn b81_to_bytes(v: &PerlValue) -> Vec<u8> {
...
fn builtin_mirr_excel(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float((pos_fv / neg_pv).powf(1.0 / (n - 1.0)) - 1.0))
}

/// `accrint` — Accrued interest on a security: par · rate · (days/freq_basis).
fn builtin_accrint(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let par = f1(args);
    ...
    Ok(PerlValue::float(par * rate * days / basis))
}
...
fn builtin_cumipmt(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rate = f1(args);
    ...
    if rate.abs() < 1e-15 { return Ok(PerlValue::float(0.0)); }
    ...
    Ok(PerlValue::float(-interest))
}

/// `cumprinc` — cumulative principal paid between periods s and e.
fn builtin_cumprinc(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rate = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let pv = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let s = args.get(3).map(|v| v.to_number() as i64).unwrap_or(1).max(1);
    let e = args.get(4).map(|v| v.to_number() as i64).unwrap_or(1).max(s);
    if rate.abs() < 1e-15 || n < 1.0 { return Ok(PerlValue::float(0.0)); }
    ...
    Ok(PerlValue::float(principal))
}

/// `dollarde` — convert fractional dollar (1.04 = 1+4/32) to decimal.
fn builtin_dollarde(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(whole + frac * pow10 / frac_denom))
}

/// `dollarfr` — convert decimal dollar back to fractional notation.
fn builtin_dollarfr(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dec = f1(args);
    let frac_denom = args.get(1).map(|v| v.to_number()).unwrap_or(8.0).max(1.0);
    let whole = dec.trunc();
    let frac = dec - whole;
    let pow10 = 10_f64.powi((frac_denom.log10().ceil() as i32).max(1));
    Ok(PerlValue::float(whole + frac * frac_denom / pow10))
}

/// `received` — amount received at maturity = par / (1 − discount · days/basis).
fn builtin_received(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let par = f1(args);
    let discount = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let days = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let basis = args.get(3).map(|v| v.to_number()).unwrap_or(360.0).max(1.0);
    Ok(PerlValue::float(par / (1.0 - discount * days / basis).max(1e-15)))
}

/// `yieldmat` — Yield of security paying interest at maturity.
fn builtin_yieldmat(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rate = f1(args);
    ...
    Ok(PerlValue::float(num * basis / dsm))
}

/// `yielddisc` — Yield on discounted security.
fn builtin_yielddisc(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let basis = args.get(3).map(|v| v.to_number()).unwrap_or(360.0).max(1.0);
    Ok(PerlValue::float((redemption - pr) / pr * basis / dsm))
}

/// `duration_macaulay` — Macaulay duration: weighted-average time to cashflows.
fn builtin_duration_macaulay(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cfs = b81_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::float(if pv_total > 0.0 { pv_t_total / pv_total } else { 0.0 }))
}

/// `mduration` — modified duration = Macaulay / (1 + y/freq).
fn builtin_mduration(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mac_dur = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.05);
    let freq = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(mac_dur / (1.0 + y / freq)))
}

/// `odddyield` — odd-period yield iteration: solves price = Σ cf / (1+y)^t.
fn builtin_odddyield(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pr = f1(args);
...
fn builtin_disc_excel(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pr = f1(args);
    let redemption = args.get(1).map(|v| v.to_number()).unwrap_or(100.0).max(1e-15);
    let dsm = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    let basis = args.get(3).map(|v| v.to_number()).unwrap_or(360.0).max(1.0);
    Ok(PerlValue::float((redemption - pr) / redemption * basis / dsm))
}

/// `effect` — effective annual rate from nominal: (1 + nom/n)^n − 1.
fn builtin_effect(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let nom = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float((1.0 + nom / n).powf(n) - 1.0))
}

/// `nominal` — inverse of `effect`: nom = n · ((1+eff)^(1/n) − 1).
fn builtin_nominal(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let eff = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(n * ((1.0 + eff).powf(1.0 / n) - 1.0)))
}

/// `intrate` — interest rate of fully invested security.
fn builtin_intrate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let invest = f1(args).max(1e-15);
    let redemption = args.get(1).map(|v| v.to_number()).unwrap_or(100.0);
    let dsm = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    let basis = args.get(3).map(|v| v.to_number()).unwrap_or(360.0).max(1.0);
    Ok(PerlValue::float((redemption - invest) / invest * basis / dsm))
}

/// `price_disc` — price of discounted security: redemption · (1 − disc · dsm/basis).
fn builtin_price_disc(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let discount = f1(args);
    let redemption = args.get(1).map(|v| v.to_number()).unwrap_or(100.0);
    let dsm = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    let basis = args.get(3).map(|v| v.to_number()).unwrap_or(360.0).max(1.0);
    Ok(PerlValue::float(redemption * (1.0 - discount * dsm / basis)))
}

// ───── Hash families (non-cryptographic) ─────

/// `cityhash64` — Google CityHash64 8-byte input fast path: rotate-and-mix.
fn builtin_cityhash64(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_farmhash_64(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_metro_hash_64(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::integer((h ^ (h >> 33)) as i64))
}

/// `spookyhash_128` — SpookyHash V2 128-bit: returns lo64.
fn builtin_spookyhash_128(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_t1ha(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_highway_hash(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::integer((v0 ^ v1) as i64))
}

/// `fnv0_32` — FNV-0 (offset 0); foundation for FNV-1.
fn builtin_fnv0_32(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mut h: u32 = 0;
...
fn builtin_lose_lose(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_oat_hash(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mut h: u32 = 0;
    for &b in &bytes {
...
fn builtin_lz4_encode_block(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(1 + lit_overflow + lit_len + 2 + match_overflow))
}

/// `snappy_encode` — Snappy literal token: tag = (len-1) << 2 if len ≤ 60,
/// else tag = (60 + size_bytes) << 2 with size in following bytes.
fn builtin_snappy_encode(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lit_len = i1(args).max(1);
    if lit_len <= 60 { return Ok(PerlValue::integer((lit_len - 1) << 2)); }
...
fn builtin_lzma_encode_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(shifts))
}

/// `bz2_encode_step` — BZ2 RLE pre-pass: count of run characters emitted.
fn builtin_bz2_encode_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_gzip_encode_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(10 + fname_len + 1 + fcomment_len + 1 + extra_len + (if extra_len > 0 { 2 } else { 0 })))
}

// ───── URI / URL operations ─────

/// `uri_template_expand` — expand RFC 6570 template variable count: each {var} = 1 substitution.
fn builtin_uri_template_expand(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mut count = 0_i64;
    let mut depth = 0_i64;
    for &b in &bytes {
...
fn builtin_uri_resolve(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    if bytes.starts_with(b"//") { return Ok(PerlValue::integer(2)); }
    if bytes.starts_with(b"/") { return Ok(PerlValue::integer(3)); }
    if bytes.iter().take_while(|&&b| b != b'/').any(|&b| b == b':') { return Ok(PerlValue::integer(1)); }
    Ok(PerlValue::integer(4))
}
...
fn builtin_uri_normalize(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b81_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::integer(changes))
}

/// `percent_decode_url` — decode %XX sequences. Returns decoded byte for given hex pair.
fn builtin_percent_decode_url(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_url_encode_form(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let b = i1(args) as u8;
    if b == b' ' { return Ok(PerlValue::integer(b'+' as i64)); }
    if b.is_ascii_alphanumeric() || b == b'-' || b == b'_' || b == b'.' || b == b'~' {
        return Ok(PerlValue::integer(b as i64));
    }
    Ok(PerlValue::integer(0x25_00_00 | ((nibble_to_hex(b >> 4) as i64) << 8) | nibble_to_hex(b & 0xF) as i64))
}
...
fn builtin_url_decode_form(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let b = i1(args) as u8;
    Ok(PerlValue::integer(if b == b'+' { b' ' as i64 } else { b as i64 }))
}

/// `punycode_decode_step` — decode one Punycode digit: 'a'-'z' → 0-25, '0'-'9' → 26-35.
fn builtin_punycode_decode_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let b = i1(args) as u8;
...
fn builtin_url_origin(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(scheme_len + host_len + port + 3))
}
...
fn builtin_etag_validate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(bodies_match))
}
...
/// max-age value.
fn builtin_cache_control_parse(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let flags = i1(args);
    let max_age = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    if flags & 2 != 0 { return Ok(PerlValue::integer(-1)); }
    if flags & 1 != 0 { return Ok(PerlValue::integer(0)); }
    Ok(PerlValue::integer(max_age))
}

/// `vary_match` — RFC 7234: do request headers in the Vary list match cached values?
fn builtin_vary_match(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mismatches = i1(args);
    Ok(PerlValue::integer(if mismatches == 0 { 1 } else { 0 }))
}

/// `content_negotiate` — Accept q-value picker: returns index of best match.
fn builtin_content_negotiate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let q_values = b81_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if q_values.is_empty() { return Ok(PerlValue::integer(-1)); }
...
fn builtin_accept_lang_pick(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let q_supported = b81_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_range_header_parse(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let start = i1(args);
    let end = args.get(1).map(|v| v.to_number() as i64).unwrap_or(-1);
    let total = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0);
    if end < 0 { return Ok(PerlValue::integer(total - 1)); }
    Ok(PerlValue::integer(end.min(total - 1).max(start)))
}

/// `if_match_check` — If-Match: return 1 if any quoted ETag matches, 0 if none.
fn builtin_if_match_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n_match = i1(args);
    Ok(PerlValue::integer(if n_match > 0 { 1 } else { 0 }))
}

/// `if_none_match_check` — If-None-Match: 1 if zero matches, else 0 (i.e. send 304).
fn builtin_if_none_match_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n_match = i1(args);
    Ok(PerlValue::integer(if n_match == 0 { 1 } else { 0 }))
}

/// `digest_auth_quote` — H(A1) = MD5(user:realm:password) — returns 32-hex digest length 32.
fn builtin_digest_auth_quote(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(user_len + realm_len + pass_len + 2))
}

/// `www_auth_parse` — challenge selector: 0=Basic, 1=Digest, 2=Bearer, 3=Negotiate.
fn builtin_www_auth_parse(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let scheme = i1(args).clamp(0, 3);
    Ok(PerlValue::integer(scheme))
}

...
fn builtin_nlp_bm25_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tf = f1(args);
...
/// TF-IDF
fn builtin_nlp_tf_idf_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tf = f1(args);
    let idf = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_nlp_okapi_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tf = f1(args);
    let idf = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let k1 = args.get(2).map(|v| v.to_number()).unwrap_or(1.5);
    let b = args.get(3).map(|v| v.to_number()).unwrap_or(0.75);
...
fn builtin_nlp_word_freq_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let count = f1(args);
...
fn builtin_nlp_doc_freq_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let prev = f1(args);
...
/// IDF = log(N / df)
fn builtin_nlp_inverse_doc_freq(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_nlp_cosine_similarity_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dot = f1(args);
...
fn builtin_nlp_jaccard_similarity_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_overlap_coefficient(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let intersection = f1(args);
...
fn builtin_nlp_dice_coefficient_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let inter = f1(args);
...
fn builtin_nlp_simpson_coefficient(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_overlap_coefficient(args)
}

/// Levenshtein distance scalar from precomputed table cell
fn builtin_nlp_levenshtein_dist(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_damerau_levenshtein(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lev = f1(args);
...
/// Jaro distance
fn builtin_nlp_jaro_distance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
...
fn builtin_nlp_jaro_winkler(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let jaro = f1(args);
...
fn builtin_nlp_hamming_distance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// LCS length
fn builtin_nlp_lcs_length(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dp_diag = f1(args);
...
fn builtin_nlp_lcs_ratio(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lcs = f1(args);
...
fn builtin_nlp_meteor_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_nlp_bleu_score_n(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_rouge_score_n(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// chrF: char F-score
fn builtin_nlp_chrf_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_nlp_ter_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let edits = f1(args);
...
fn builtin_nlp_wer_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_ter_score(args)
}

/// Character error rate
fn builtin_nlp_cer_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_ter_score(args)
}

/// Perplexity = exp(avg negative log likelihood)
fn builtin_nlp_perplexity_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let avg_nll = f1(args);
    Ok(StrykeValue::float(avg_nll.exp()))
}

/// Bits per character
fn builtin_nlp_bits_per_character(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let nll = f1(args);
    Ok(StrykeValue::float(nll / 2f64.ln()))
}

/// Character n-gram count: (L - n + 1)
fn builtin_nlp_char_ngram_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let len = i1(args);
...
/// Word n-gram count
fn builtin_nlp_word_ngram_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_char_ngram_count(args)
}

/// Skip-gram count: (L - n) C(n+k, k)
fn builtin_nlp_skip_gram_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
...
fn builtin_nlp_byte_pair_merge_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_wordpiece_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_w = f1(args);
...
/// Unigram LM score
fn builtin_nlp_unigram_lm_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_kneser_ney_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = f1(args);
...
fn builtin_nlp_witten_bell_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_nlp_good_turing_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = f1(args);
...
fn builtin_nlp_laplace_smoothing(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = f1(args);
...
fn builtin_nlp_lidstone_smoothing(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let v = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_nlp_jelinek_mercer(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_high = f1(args);
...
fn builtin_nlp_dirichlet_smoothing(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let c = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_nlp_query_likelihood_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_kl_lm_div(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_nlp_pmi_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_xy = f1(args);
...
/// Normalized PMI
fn builtin_nlp_npmi_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pmi = f1(args);
...
/// χ² collocation test
fn builtin_nlp_chi2_collocation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_loglikelihood_collocation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let obs = f1(args);
...
fn builtin_nlp_t_score_collocation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let obs = f1(args);
    let exp_v = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_nlp_dunning_log_likelihood(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_loglikelihood_collocation(args)
}

/// LDA α update step
fn builtin_nlp_lda_alpha_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_lda_beta_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_wz = f1(args);
...
fn builtin_nlp_lda_topic_dist(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_zd = f1(args);
...
fn builtin_nlp_plsa_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_z_d = f1(args);
...
fn builtin_nlp_word2vec_skipgram_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dot = f1(args);
...
fn builtin_nlp_word2vec_cbow_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let avg_dot = f1(args);
    Ok(StrykeValue::float((1.0 + (-avg_dot).exp()).ln()))
}

/// GloVe loss: f(x) (w_i · w_j + b_i + b_j - log x)²
fn builtin_nlp_glove_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dot_b = f1(args);
...
fn builtin_nlp_fasttext_subword_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = i1(args);
...
/// Byte-level BPE step
fn builtin_nlp_byte_level_bpe_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_byte_pair_merge_step(args)
}

/// SentencePiece score (likelihood of segmentation)
fn builtin_nlp_sentencepiece_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_unigram_lm_score(args)
}

/// Unigram subword loss
fn builtin_nlp_unigram_subword_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_unigram_lm_score(args)
}

/// Subword regularization: sample from top-k segmentations
fn builtin_nlp_subword_regularization(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_nlp_pointwise_attn_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q_dot_k = f1(args);
...
fn builtin_nlp_relative_position_bias(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dist = f1(args);
...
fn builtin_nlp_alibi_position_bias(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dist = f1(args);
...
fn builtin_nlp_rope_rotary_angle(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pos = f1(args);
...
fn builtin_nlp_rope_apply_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_nlp_position_encoding_sin(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pos = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let d = args.get(2).map(|v| v.to_number()).unwrap_or(64.0);
    if d == 0.0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_nlp_position_encoding_cos(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pos = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let d = args.get(2).map(|v| v.to_number()).unwrap_or(64.0);
...
fn builtin_nlp_pe_freq_band(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = f1(args);
...
fn builtin_nlp_max_seq_len_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let len = f1(args);
...
/// Token drop rate
fn builtin_nlp_token_drop_rate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dropped = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_nlp_byte_frequency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_char_frequency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_byte_frequency(args)
}

/// Punctuation ratio
fn builtin_nlp_punct_ratio(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let punct = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(StrykeValue::float(0.0)); }
...
/// Uppercase ratio
fn builtin_nlp_uppercase_ratio(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_punct_ratio(args)
}

/// Digit ratio
fn builtin_nlp_digit_ratio(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_punct_ratio(args)
}

/// Emoji ratio
fn builtin_nlp_emoji_ratio(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_punct_ratio(args)
}

/// URL count: occurrences of "://" trigram in text given as code-point array.
fn builtin_nlp_url_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_nlp_email_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let mut n = 0_i64;
...
fn builtin_nlp_phone_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let mut n = 0_i64;
    let mut run = 0_usize;
    for &c in &s {
...
fn builtin_nlp_hashtag_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let mut n = 0_i64;
    for i in 0..s.len() {
...
fn builtin_nlp_mention_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let mut n = 0_i64;
    for i in 0..s.len() {
        if s[i] as i64 != '@' as i64 { continue; }
...
fn builtin_nlp_token_overlap_two(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_jaccard_similarity_two(args)
}

/// Word Mover's distance step
fn builtin_nlp_word_mover_dist(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().sum()))
}

/// SIF weight a / (a + p(w))
fn builtin_nlp_sif_weight_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_w = f1(args);
...
fn builtin_nlp_doc_embedding_avg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_nlp_attention_pool_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let alpha = f1(args);
...
/// Max pool step
fn builtin_nlp_max_pool_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

/// Avg pool step
fn builtin_nlp_avg_pool_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_doc_embedding_avg(args)
}

/// Sum pool step
fn builtin_nlp_sum_pool_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().sum()))
}

/// Self-attention compute step
fn builtin_nlp_self_attn_compute_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_pointwise_attn_score(args)
}

/// Cross attention compute step
fn builtin_nlp_cross_attn_compute_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_nlp_pointwise_attn_score(args)
}

/// Window attention step (within radius w)
fn builtin_nlp_window_attn_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dist = f1(args);
...
fn builtin_nlp_strided_attn_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dist = i1(args);
...
fn builtin_nlp_block_attn_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let block_i = i1(args);
...
fn builtin_nlp_sliding_window_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
...
/// keys-attended-to.
fn builtin_nlp_local_attn_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
...
fn builtin_nlp_dilated_attn_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dist = i1(args);
...
fn builtin_nlp_global_attn_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = b46_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_nlp_sparse_attn_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mask = i1(args);
...
fn builtin_nlp_linformer_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    let d = args.get(1).map(|v| v.to_number()).unwrap_or(64.0);
...
fn builtin_nlp_performer_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    let d = args.get(1).map(|v| v.to_number()).unwrap_or(64.0);
...
fn builtin_nlp_reformer_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    let d = args.get(1).map(|v| v.to_number()).unwrap_or(64.0);
...
fn builtin_nlp_longformer_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_nlp_bigbird_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
    let w = args.get(1).map(|v| v.to_number()).unwrap_or(64.0);
...
fn builtin_nlp_routing_attn_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);

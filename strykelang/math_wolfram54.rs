...
fn b54_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// iota_n: APL ⍳N → [0, 1, ..., N-1].
fn builtin_iota_n(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0);
    let v: Vec<PerlValue> = (0..n).map(PerlValue::integer).collect();
    Ok(PerlValue::array(v))
}

/// reduce_axis: f/array along last axis with op id (0=add, 1=mul, 2=max, 3=min, 4=or, 5=and).
fn builtin_reduce_axis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b54_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let op = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    if v.is_empty() {
        return Ok(PerlValue::float(match op { 1 => 1.0, 2 => f64::NEG_INFINITY, 3 => f64::INFINITY, _ => 0.0 }));
    }
    Ok(PerlValue::float(match op {
        0 => v.iter().sum(),
        1 => v.iter().product(),
...
fn builtin_scan_axis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b54_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let op = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    if v.is_empty() { return Ok(PerlValue::array(vec![])); }
    ...
    for &x in &v {
        ...
        out.push(PerlValue::float(acc));
    }
    Ok(PerlValue::array(out))
}

/// fold_axis: like reduce but takes an explicit initial value.
fn builtin_fold_axis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let init = f1(args);
    let v = b54_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_rotate_axis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let mut out: Vec<PerlValue> = v[k..].to_vec();
...
fn builtin_transpose_axis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let rows = args.get(1).map(|v| v.to_number() as usize).unwrap_or(1).max(1);
    let cols = v.len() / rows;
    if cols == 0 { return Ok(PerlValue::array(v)); }
...
fn builtin_reshape_dim(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = args.get(1).map(|v| v.to_number() as usize).unwrap_or(0);
    if v.is_empty() { return Ok(PerlValue::array(vec![PerlValue::integer(0); n])); }
    let out: Vec<PerlValue> = (0..n).map(|i| v[i % v.len()].clone()).collect();
    Ok(PerlValue::array(out))
}

/// encode_base: APL ⊤ → digits of n in mixed radix [b1, b2, ...]. Returns
/// digits little-endian.
fn builtin_encode_base(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut n = i1(args);
    let radix = arg_to_vec(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let mut out: Vec<PerlValue> = Vec::with_capacity(radix.len());
    for r in &radix {
        let b = r.to_number() as i64;
        if b == 0 { out.push(PerlValue::integer(n)); n = 0; }
        else { out.push(PerlValue::integer(n.rem_euclid(b))); n = n.div_euclid(b); }
    }
    Ok(PerlValue::array(out))
}
...
/// little-endian).
fn builtin_decode_base(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let digits = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let radix = arg_to_vec(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let mut acc = 0_i64;
    let mut place = 1_i64;
...
fn builtin_nub_list(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_nub_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mut seen = std::collections::HashSet::new();
...
fn builtin_membership_idx(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let y = arg_to_vec(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let yset: std::collections::HashSet<String> = y.iter().map(|v| format!("{:?}", v)).collect();
    let out: Vec<PerlValue> = x.iter().map(|v| {
        PerlValue::integer(if yset.contains(&format!("{:?}", v)) { 1 } else { 0 })
    }).collect();
    Ok(PerlValue::array(out))
}
...
fn builtin_deal_n_k(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    for i in 0..k {
        ...
        pool.swap(i, r);
        out.push(PerlValue::integer(pool[i]));
    }
    Ok(PerlValue::array(out))
}

/// roll_n: APL ?n — single uniform draw from [0, n) given seed.
fn builtin_roll_n(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(1);
    let mut seed = args.get(1).map(|v| v.to_number() as u64).unwrap_or(0xdeadbeef);
    seed ^= seed << 13; seed ^= seed >> 7; seed ^= seed << 17;
    Ok(PerlValue::integer((seed % n as u64) as i64))
}

/// permute_idx: APL x⌷y — gather y at indices x.
fn builtin_permute_idx(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let idx = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let src = arg_to_vec(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let n = src.len();
    let out: Vec<PerlValue> = idx.iter().map(|i| {
        let k = i.to_number() as i64;
        if k < 0 || (k as usize) >= n { PerlValue::UNDEF } else { src[k as usize].clone() }
    }).collect();
    Ok(PerlValue::array(out))
}

/// invert_perm: given perm π, return σ with σ[π[i]] = i.
fn builtin_invert_perm(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = p.len();
    let mut out = vec![PerlValue::integer(0); n];
    for (i, v) in p.iter().enumerate() {
        let j = v.to_number() as usize;
        if j < n { out[j] = PerlValue::integer(i as i64); }
    }
    Ok(PerlValue::array(out))
}

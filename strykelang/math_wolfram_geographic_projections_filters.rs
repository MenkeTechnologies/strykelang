...
fn builtin_mollweide_project(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_robinson_project(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sinusoidal_project(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let r = args.get(2).map(|v| v.to_number()).unwrap_or(6378137.0);
    Ok(StrykeValue::array(vec![StrykeValue::float(r * lon * lat.cos()), StrykeValue::float(r * lat)]))
}
fn builtin_equirectangular_project(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
...
fn builtin_lambert_azimuthal_project(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
...
fn builtin_albers_conic_project(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
...
fn builtin_geohash_encode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_geohash_decode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_gabor_kernel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_unsharp_mask_kernel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_emboss_kernel(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(matrix_to_value(&[vec![-2.0, -1.0, 0.0], vec![-1.0, 1.0, 1.0], vec![0.0, 1.0, 2.0]]))
}
fn builtin_box_blur_kernel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = args.first().map(|v| v.to_number() as usize).unwrap_or(1);
    let n = 2 * r + 1;
    let v = 1.0 / (n * n) as f64;
    Ok(matrix_to_value(&vec![vec![v; n]; n]))
}
fn builtin_motion_blur_kernel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_sharpen_kernel(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(matrix_to_value(&[vec![0.0, -1.0, 0.0], vec![-1.0, 5.0, -1.0], vec![0.0, -1.0, 0.0]]))
}
fn builtin_edge_detect_kernel(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(matrix_to_value(&[vec![-1.0, -1.0, -1.0], vec![-1.0, 8.0, -1.0], vec![-1.0, -1.0, -1.0]]))
}
fn builtin_sobel_diagonal_kernel(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(matrix_to_value(&[vec![0.0, 1.0, 2.0], vec![-1.0, 0.0, 1.0], vec![-2.0, -1.0, 0.0]]))
}
fn builtin_haar_2d_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db4_coeffs(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = vec![0.6830127, 1.1830127, 0.3169873, -0.1830127];
    let s = 1.0 / 2.0_f64.sqrt();
    Ok(StrykeValue::array(h.into_iter().map(|x| StrykeValue::float(x * s)).collect()))
}
fn builtin_db6_coeffs(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::array(vec![
...
fn builtin_sym4_coeffs(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::array(vec![
...
fn builtin_coif1_coeffs(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::array(vec![
...
fn builtin_aes_sbox_byte(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_aes_inv_sbox_byte(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = (i1(args) & 0xff) as usize;
    static INV: [u8; 256] = [
...
fn builtin_chacha20_qround(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_xtea_round(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_speck_round(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_simon_round(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_geohash_neighbor(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_geohash_bbox(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
    let base32 = "0123456789bcdefghjkmnpqrstuvwxyz";
    let mut lat_range = (-90.0_f64, 90.0_f64);
    let mut lon_range = (-180.0_f64, 180.0_f64);

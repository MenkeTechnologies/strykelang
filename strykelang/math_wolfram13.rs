...
fn builtin_bfs_distances(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_dfs_preorder(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let s = args.get(1).map(|v| v.to_number() as usize).unwrap_or(0);
    let n = adj.len();
...
fn builtin_connected_components(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
...
fn builtin_graph_is_tree(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
...
fn builtin_graph_density(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
...
fn builtin_graph_average_degree(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len() as f64;
...
fn builtin_graph_max_degree(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    Ok(StrykeValue::integer(adj.iter().map(|n| n.len()).max().unwrap_or(0) as i64))
}
fn builtin_graph_min_degree(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    Ok(StrykeValue::integer(adj.iter().map(|n| n.len()).min().unwrap_or(0) as i64))
}
fn builtin_graph_complement(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
...
fn builtin_in_degree_directed(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
...
fn builtin_out_degree_directed(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    Ok(StrykeValue::array(adj.iter().map(|n| StrykeValue::integer(n.len() as i64)).collect()))
}
fn builtin_graph_eccentricity_all(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
...
fn builtin_is_connected(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
    if n == 0 { return Ok(StrykeValue::integer(1)); }
    let comp = builtin_connected_components(args)?;
...
fn builtin_articulation_points(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
    let mut visited = vec![false; n];
...
fn builtin_bridges_edges(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
    let mut visited = vec![false; n];
...
fn builtin_eulerian_path_q(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let odd_count = adj.iter().filter(|n| n.len() % 2 == 1).count();
    Ok(StrykeValue::integer(if odd_count == 0 || odd_count == 2 { 1 } else { 0 }))
}
fn builtin_hamiltonian_brute(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let adj = parse_adj_list(&args.first().cloned().unwrap_or(StrykeValue::UNDEF));
    let n = adj.len();
...
fn builtin_string_to_charcodes(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
    Ok(StrykeValue::array(s.chars().map(|c| StrykeValue::integer(c as i64)).collect()))
}
fn builtin_charcodes_to_string(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_string_xor(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_string_camel_to_snake(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_string_snake_to_camel(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_string_kebab_to_snake(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    Ok(StrykeValue::string(args.first().map(|v| v.to_string()).unwrap_or_default().replace('-', "_")))
}
fn builtin_string_snake_to_kebab(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    Ok(StrykeValue::string(args.first().map(|v| v.to_string()).unwrap_or_default().replace('_', "-")))
}
fn builtin_palindromic_q(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_substring_count(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_string_truncate_ellipsis(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_string_expand_tabs(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_string_normalize_spaces(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_days_in_year(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let y = i1(args);
    let leap = (y % 4 == 0 && y % 100 != 0) || y % 400 == 0;
    Ok(StrykeValue::integer(if leap { 366 } else { 365 }))
}
fn builtin_quarter_of_year(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let m = i1(args);
    Ok(StrykeValue::integer(((m - 1) / 3) + 1))
}
fn builtin_zeller_day_of_week(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_age_from_birthdate(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_business_days_between(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_unix_epoch_to_iso(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let secs = i1(args);
...
fn builtin_loan_payment_pmt(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_loan_balance(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_amortization_total_interest(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let principal = f1(args); let rate = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
...
fn builtin_simple_interest_compute(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_perpetuity_value(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cash = f1(args); let r = args.get(1).map(|x| x.to_number()).unwrap_or(0.0).max(1e-30);
    Ok(StrykeValue::float(cash / r))
}
fn builtin_growing_perpetuity(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_annuity_present_value(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cash = f1(args); let r = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
...
fn builtin_annuity_future_value(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cash = f1(args); let r = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    let n = args.get(2).map(|x| x.to_number() as i32).unwrap_or(0);
    if r.abs() < 1e-15 { return Ok(StrykeValue::float(cash * n as f64)); }
    Ok(StrykeValue::float(cash * ((1.0 + r).powi(n) - 1.0) / r))
}
fn builtin_capm_expected_return(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_treynor_ratio(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_jensens_alpha(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let rp = f1(args); let rf = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
...
fn builtin_information_ratio(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_friction_factor_laminar(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let re = f1(args).max(1e-30);
    Ok(StrykeValue::float(64.0 / re))
}
fn builtin_swamee_jain_factor(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_pipe_pressure_drop(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_orifice_velocity(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let dp = f1(args); let rho = args.get(1).map(|x| x.to_number()).unwrap_or(1000.0).max(1e-30);
    Ok(StrykeValue::float((2.0 * dp / rho).sqrt()))
}
fn builtin_chezy_velocity(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_manning_velocity(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let n = f1(args).max(1e-30); let r = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    let s = args.get(2).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(r.powf(2.0 / 3.0) * s.sqrt() / n))
}
fn builtin_froude_number(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_nusselt_dittus_boelter(args: &[StrykeValue]) -> PerlResult<StrykeValue> {

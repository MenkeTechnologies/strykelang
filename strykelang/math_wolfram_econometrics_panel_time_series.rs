...
fn builtin_arch_lm_test(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let vx = b36_var(&x);
    let vy = b36_var(&y);
    if vx <= 0.0 || vy <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    let r_sq = (cv * cv) / (vx * vy);
    Ok(StrykeValue::float(r_sq * (n - 1) as f64))
}
...
fn builtin_breusch_pagan_test(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let vx = b36_var(&x[..n]);
    let vr = b36_var(&r2);
    if vx <= 0.0 || vr <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    let r_sq = (cv * cv) / (vx * vr);
    Ok(StrykeValue::float(n as f64 * r_sq))
}
...
fn builtin_white_robust_se(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if xx == 0.0 { return Ok(StrykeValue::float(0.0)); }
    let mid: f64 = x.iter().zip(e.iter()).take(n).map(|(xi, ei)| xi * xi * ei * ei).sum();
    Ok(StrykeValue::float((mid / (xx * xx)).sqrt()))
}
...
fn builtin_newey_west_se(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float((s / (xx * xx)).sqrt()))
}
...
fn builtin_hausman_test(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let dv = v_fe - v_re;
    if dv.abs() < 1e-12 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float((b_fe - b_re).powi(2) / dv))
}
...
fn builtin_adf_test_stat(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let vyl = b36_var(&yl);
    if vyl <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    let gamma = cv / vyl;
    ...
    if se == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(gamma / se))
}
...
fn builtin_pp_test_stat(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let n = y.len();
    if n < 3 { return Ok(StrykeValue::float(0.0)); }
    let adf = builtin_adf_test_stat(&[StrykeValue::array(y.iter().copied().map(StrykeValue::float).collect())])?.to_number();
    let dy: Vec<f64> = (1..n).map(|i| y[i] - y[i - 1]).collect();
    ...
    let v = b36_var(&dy);
    if v <= 0.0 { return Ok(StrykeValue::float(adf)); }
    Ok(StrykeValue::float(adf * (v / s2).sqrt()))
}
...
fn builtin_engle_granger_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let vx = b36_var(&x[..n]);
    if vx <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    let beta = cv / vx;
    let alpha = b36_mean(&y[..n]) - beta * b36_mean(&x[..n]);
    let resid: Vec<f64> = (0..n).map(|i| y[i] - alpha - beta * x[i]).collect();
    builtin_adf_test_stat(&[StrykeValue::array(resid.into_iter().map(StrykeValue::float).collect())])
}
...
fn builtin_johansen_trace_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let vy = b36_var(&yl);
    let vd = b36_var(&dy);
    if vy <= 0.0 || vd <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    let r2 = cv * cv / (vy * vd);
    Ok(StrykeValue::float(-(n as f64) * (1.0 - r2).max(1e-12).ln()))
}
...
fn builtin_panel_random_effects(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if denom == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 - s_eps / denom))
}
...
fn builtin_ols_residual_variance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(sse / (n - k) as f64))
}
...
fn builtin_ols_r_squared(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if sst == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 - ssr / sst))
}
...
fn builtin_ols_adjusted_r2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if n - k - 1.0 <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 - (1.0 - r2) * (n - 1.0) / (n - k - 1.0)))
}
...
fn builtin_bayesian_info_crit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(k * n.ln() - 2.0 * ll))
}
...
fn builtin_hannan_quinn_ic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let k = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let n = args.get(2).map(|v| v.to_number()).unwrap_or(100.0);
    Ok(StrykeValue::float(2.0 * k * n.ln().ln() - 2.0 * ll))
}
...
fn builtin_f_statistic_pooled(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if ssr_u <= 0.0 || (n - k) <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(((ssr_r - ssr_u) / q) / (ssr_u / (n - k))))
}
...
fn builtin_breusch_pagan_lm(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(0.5 * ess / sigma2.powi(2)))
}
...
fn builtin_ramsey_reset_test(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let k = args.get(4).map(|v| v.to_number()).unwrap_or(2.0);
    if 1.0 - r2_u <= 0.0 || n - k <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(((r2_u - r2_r) / q) / ((1.0 - r2_u) / (n - k))))
}
...
fn builtin_chow_test_stat(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if den <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(((ssr - ssr1 - ssr2) / k) / den))
}
...
fn builtin_white_test_stat(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let vr = b36_var(&r2);
    if vr <= 0.0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_goldfeld_quandt(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(ssr2 / ssr1))
}
...
fn builtin_wald_test_stat(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(diff * diff / var))
}
...
fn builtin_two_sls_iv(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if zx == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(zy / zx))
}
...
fn builtin_mle_normal_log_lik(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let n = args.get(2).map(|v| v.to_number()).unwrap_or(100.0);
    if sigma2 <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(-0.5 * n * (2.0 * std::f64::consts::PI * sigma2).ln() - sse / (2.0 * sigma2)))
}
...
fn builtin_mle_exponential_log_lik(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if lambda <= 0.0 { return Ok(StrykeValue::float(f64::NEG_INFINITY)); }
    Ok(StrykeValue::float(n * lambda.ln() - lambda * sum))
}
...
fn builtin_mle_poisson_log_lik(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if lambda <= 0.0 { return Ok(StrykeValue::float(f64::NEG_INFINITY)); }
    Ok(StrykeValue::float(sum_x * lambda.ln() - n * lambda - sum_lnxfact))
}
...
fn builtin_pooling_test_stat(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if ssr_u <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float((ssr_r - ssr_u) / ssr_u * df))
}
...
fn builtin_heckman_correction(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if big_phi == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(phi / big_phi))
}
...
fn builtin_tobit_log_likelihood(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if sigma <= 0.0 { return Ok(StrykeValue::float(f64::NEG_INFINITY)); }
    if y > 0.0 {
        let z = (y - xb) / sigma;
        Ok(StrykeValue::float(-0.5 * z * z - sigma.ln() - 0.5 * (2.0 * std::f64::consts::PI).ln()))
    } else {
        let z = -xb / sigma;
        let big_phi = 0.5 * (1.0 + libm::erf(z / std::f64::consts::SQRT_2));
        Ok(StrykeValue::float(big_phi.max(1e-300).ln()))
    }
}
...
fn builtin_probit_log_likelihood(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(p.max(1e-300).ln()))
}
...
fn builtin_logit_log_likelihood(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(pp.max(1e-300).ln()))
}
...
fn builtin_multinomial_logit_prob(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if denom == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float((xb[j] - max).exp() / denom))
}
...
fn builtin_ordered_probit_threshold(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(0.5 * (1.0 + libm::erf(z))))
}
...
fn builtin_panel_var_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let v = b36_var(&yl);
    if v == 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(cv / v))
}
...
fn builtin_variance_decomposition(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let j = args.get(1).map(|v| v.to_number() as usize).unwrap_or(0);
    if j >= psi.len() { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(psi[j] * psi[j] / total))
}
...
fn builtin_granger_causality_chi2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let n = args.get(2).map(|v| v.to_number()).unwrap_or(100.0);
    if ssr_u <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(n * (ssr_r - ssr_u) / ssr_u))
}
...
fn builtin_error_correction_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(alpha * (y_lag - beta * x_lag)))
}
...
fn builtin_random_walk_drift_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(y_lag + mu + eps))
}
...
fn builtin_ar_model_likelihood(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    let sigma2 = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let n = r.len() as f64;
    if sigma2 <= 0.0 { return Ok(StrykeValue::float(f64::NEG_INFINITY)); }
    let sse: f64 = r.iter().map(|v| v * v).sum();
    Ok(StrykeValue::float(-0.5 * n * (2.0 * std::f64::consts::PI * sigma2).ln() - sse / (2.0 * sigma2)))
}
...
fn builtin_ma_model_likelihood(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    if sigma2 <= 0.0 || y.is_empty() { return Ok(StrykeValue::float(f64::NEG_INFINITY)); }
    let q = theta.len();
    let n = y.len();
    ...
    let nf = n as f64;
    Ok(StrykeValue::float(-0.5 * nf * (2.0 * std::f64::consts::PI * sigma2).ln()
        - sse / (2.0 * sigma2)))
}
...
fn builtin_arma_model_innovation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(y - phi * y_lag - theta * eps_lag))
}

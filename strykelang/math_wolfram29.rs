...
fn builtin_hyper1f1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
...
fn builtin_hyper0f1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let b = f1(args);
...
fn builtin_pochhammer(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
...
fn builtin_mathieu_ce0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(1.0 + 0.5 * q * (2.0 * z).cos()))
}
fn builtin_mathieu_se1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let z = f1(args);
    let q = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(z.sin() + (q / 8.0) * (3.0 * z).sin()))
}

// Parabolic cylinder D_n(x) for n=0,1
fn builtin_parabolic_d0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    Ok(PerlValue::float((-x * x / 4.0).exp()))
}
fn builtin_parabolic_d1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    Ok(PerlValue::float(x * (-x * x / 4.0).exp()))
}

// Whittaker M(a, b, z) (in terms of 1F1): M_{k,μ}(z) = z^{μ+1/2} e^{-z/2} M(μ-k+1/2, 2μ+1, z)
fn builtin_whittaker_m(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = f1(args);
    ...
    Ok(PerlValue::float(z.powf(mu + 0.5) * (-z / 2.0).exp() * m_val))
}

// Struve function H₀(x) — series form
fn builtin_struve_h0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_struve_h1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let pi = std::f64::consts::PI;
    let mut sum = 0.0;
...
fn builtin_lambert_w0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x < -1.0 / std::f64::consts::E { return Ok(PerlValue::float(f64::NAN)); }
    ...
    for _ in 0..50 {
        ...
        let w_new = w - dw;
        if (w_new - w).abs() < 1e-12 { return Ok(PerlValue::float(w_new)); }
        w = w_new;
    }
...
fn builtin_wright_omega(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let z = f1(args);
    builtin_lambert_w0(&[PerlValue::float(z.exp())])
}

// Sinhc(x) = sinh(x)/x
fn builtin_sinhc(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x.abs() < 1e-9 { return Ok(PerlValue::float(1.0)); }
    Ok(PerlValue::float(x.sinh() / x))
}
// Coshc(x) = cosh(x)/x for x≠0 (only defined as scaled cosh / use cosh-1/x²)
fn builtin_cosh_minus1_over_x2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x.abs() < 1e-9 { return Ok(PerlValue::float(0.5)); }
    Ok(PerlValue::float((x.cosh() - 1.0) / (x * x)))
}

// Sici (sine integral) Si(x) — series for moderate |x|
fn builtin_sine_integral_si(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_cosine_integral_ci(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x <= 0.0 { return Ok(PerlValue::float(f64::NAN)); }
    ...
    Ok(PerlValue::float(euler_gamma + x.ln() + sum))
}

// Exponential integral E1(x) for x>0 (series for small, asymptotic for large)
fn builtin_exp_integral_e1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    if x < 1.0 {
        ...
        Ok(PerlValue::float((-x).exp() * sum / x))
    }
}
...
fn builtin_dawson_function(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if x.abs() < 4.0 {
        ...
        Ok(PerlValue::float((-x * x).exp() * sum))
    } else {
        // Asymptotic
        let mut sum = 1.0_f64;
        let mut term = 1.0_f64;
        ...
        Ok(PerlValue::float(0.5 * sum / x))
    }
}

// Owen's T function approximation (4-term Patefield-Tandy)
fn builtin_owen_t(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let h = f1(args);
    let a = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if a == 0.0 { return Ok(PerlValue::float(0.0)); }
    let pi = std::f64::consts::PI;
    let mut sum = 0.0;
    let mut term = a;
    for n in 0..50 {
    ...
    Ok(PerlValue::float((1.0 / (2.0 * pi)) * (-(h * h) / 2.0).exp() * sum))
}

// Spherical Bessel j₀
fn builtin_spherical_bessel_j0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x.abs() < 1e-9 { return Ok(PerlValue::float(1.0)); }
    Ok(PerlValue::float(x.sin() / x))
}
// Spherical Bessel j₁
fn builtin_spherical_bessel_j1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x.abs() < 1e-9 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((x.sin() - x * x.cos()) / (x * x)))
}
// Spherical Bessel y₀
fn builtin_spherical_bessel_y0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x == 0.0 { return Ok(PerlValue::float(f64::NEG_INFINITY)); }
    Ok(PerlValue::float(-x.cos() / x))
}
// Spherical Bessel y₁
fn builtin_spherical_bessel_y1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x == 0.0 { return Ok(PerlValue::float(f64::NEG_INFINITY)); }
    Ok(PerlValue::float(-(x.cos() / (x * x) + x.sin() / x)))
}

// Modified spherical Bessel i₀
fn builtin_mod_sph_bessel_i0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x.abs() < 1e-9 { return Ok(PerlValue::float(1.0)); }
    Ok(PerlValue::float(x.sinh() / x))
}
fn builtin_mod_sph_bessel_i1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x.abs() < 1e-9 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((x.cosh() - x.sinh() / x) / x))
}
fn builtin_mod_sph_bessel_k0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    if x <= 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    let pi = std::f64::consts::PI;
    Ok(PerlValue::float(pi / 2.0 * (-x).exp() / x))
}

// Coulomb wave function F_L (L=0, simplified)
fn builtin_coulomb_f0(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let eta = f1(args);
    ...
    Ok(PerlValue::float(amplitude.sqrt() * (rho - eta * rho.ln().max(1e-30)).sin()))
}

// Polylogarithm Li₂ (dilogarithm) — series
fn builtin_polylog_li2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let z = f1(args);
    if z.abs() >= 1.0 { return Ok(PerlValue::float(f64::NAN)); }
    let mut sum = 0.0;
    let mut term = z;
...
fn builtin_polylog_n(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_clausen_cl2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let theta = f1(args);
    let mut sum = 0.0;
    for n in 1..1000 {
...
fn builtin_bose_einstein_g(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_polylog_n(args)
}

// Fermi-Dirac integral F_n(x) — numerical via simpson approx
fn builtin_fermi_dirac_int(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = f1(args);
...
fn builtin_theta3(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let z = f1(args);
    let q = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let mut sum = 1.0;
    for n in 1..50 {
...
fn builtin_theta2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let z = f1(args);
    let q = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let mut sum = 0.0;
    for n in 0..50 {
...
fn builtin_jacobi_sn_small_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    let q = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(u.sin() + q * u.sin().powi(3)))
}
fn builtin_jacobi_cn_small_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    let q = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(u.cos() - q * u.cos() * u.sin().powi(2)))
}
fn builtin_jacobi_dn_small_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    let q = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(1.0 - 0.5 * q * (1.0 - (2.0 * u).cos())))
}

// Riemann ξ(s) (with completed gamma factor — simplified)
fn builtin_riemann_xi(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s = f1(args);
    ...
    Ok(PerlValue::float(0.5 * s * (s - 1.0) * pi.powf(-s / 2.0) * libm::tgamma(s / 2.0) * zeta))
}

// Bessel J of arbitrary integer order n via series
fn builtin_bessel_jn_general(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = args.first().map(|v| v.to_number() as i32).unwrap_or(0);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if n < 0 { return Ok(PerlValue::float(f64::NAN)); }
    let mut sum = 0.0;
...
fn builtin_bessel_in_general(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = args.first().map(|v| v.to_number() as i32).unwrap_or(0);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if n < 0 { return Ok(PerlValue::float(f64::NAN)); }
    let mut sum = 0.0;

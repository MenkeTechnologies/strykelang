...
fn b76_to_floats(v: &PerlValue) -> Vec<f64> {
...
fn builtin_df_groupby(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_df_aggregate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let agg = args.get(1).map(|x| x.to_number() as i64).unwrap_or(0);
    if v.is_empty() { return Ok(PerlValue::float(f64::NAN)); }
    let n = v.len() as f64;
...
fn builtin_df_apply(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let a = args.get(1).map(|x| x.to_number()).unwrap_or(1.0);
    let b = args.get(2).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(v.iter().map(|x| a * x + b).sum::<f64>()))
}

/// `df_transform` — broadcast group statistic; returns mean of group.
fn builtin_df_transform(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
/// flat-values array.
fn builtin_df_pivot(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let idx = i * n_cols + j;
    Ok(PerlValue::float(values.get(idx).copied().unwrap_or(f64::NAN)))
}

/// `df_pivot_table` — accepts duplicate keys; aggregates with sum/mean (returns mean).
fn builtin_df_pivot_table(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if v.is_empty() { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(v.iter().sum::<f64>() / v.len() as f64))
}
...
fn builtin_df_melt(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_df_stack(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let values = args.get(3).map(b76_to_floats).unwrap_or_default();
    Ok(PerlValue::float(values.get(col * n_rows + row).copied().unwrap_or(f64::NAN)))
}

/// `df_unstack` — long → wide; rows = unique index level count.
fn builtin_df_unstack(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let total = i1(args).max(0);
    let level_size = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1).max(1);
    Ok(PerlValue::integer((total + level_size - 1) / level_size))
}

/// `df_explode` — each list value contributes len(list) rows.
fn builtin_df_explode(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lengths = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(lengths.iter().sum::<f64>() as i64))
}

/// `df_get_dummies` — one-hot indicator: 1 if `value == category`, else 0.
fn builtin_df_get_dummies(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let value = i1(args);
    let category = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer(if value == category { 1 } else { 0 }))
}
...
fn builtin_df_crosstab(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let row = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_df_merge(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = args.get(1).map(b76_to_floats).unwrap_or_default();
    let set_a: std::collections::HashSet<u64> = a.iter().map(|x| x.to_bits()).collect();
    Ok(PerlValue::integer(b.iter().filter(|x| set_a.contains(&x.to_bits())).count() as i64))
}

/// `df_join` — left-join row count, accounting for one-to-many matches:
/// for each key in `a` count how many times it appears in `b`, sum.
fn builtin_df_join(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = args.get(1).map(b76_to_floats).unwrap_or_default();
...
fn builtin_df_concat(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = args.get(1).map(b76_to_floats).unwrap_or_default();
    let i = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0).max(0) as usize;
    if i < a.len() { return Ok(PerlValue::float(a[i])); }
    let j = i - a.len();
    Ok(PerlValue::float(b.get(j).copied().unwrap_or(f64::NAN)))
}

// ───── time series ─────

/// `df_resample` — output rows for given source period and target rule.
fn builtin_df_resample(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let src_periods = i1(args).max(0);
    let ratio = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    Ok(PerlValue::integer((src_periods as f64 / ratio).ceil() as i64))
}

/// `df_rolling` — rolling window mean step value.
fn builtin_df_rolling(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if v.is_empty() { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(v.iter().sum::<f64>() / v.len() as f64))
}
...
fn builtin_df_expanding(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_df_rolling(args)
}

/// `df_ewm` — exponentially weighted moving average step: y_t = α·x + (1-α)·y_{t-1}.
fn builtin_df_ewm(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y_prev = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let alpha = args.get(2).map(|v| v.to_number()).unwrap_or(0.3).clamp(0.0, 1.0);
    Ok(PerlValue::float(alpha * x + (1.0 - alpha) * y_prev))
}

/// `df_shift` — n-th lag value.
fn builtin_df_shift(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let i = (v.len() as i64) + n;
    if i < 0 || i >= v.len() as i64 { return Ok(PerlValue::float(f64::NAN)); }
    Ok(PerlValue::float(v[i as usize]))
}

/// `df_diff` — first-difference value at index i.
fn builtin_df_diff(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if v.len() < 2 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(v[v.len() - 1] - v[v.len() - 2]))
}

/// `df_pct_change` — percentage change between successive elements.
fn builtin_df_pct_change(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if v.len() < 2 || v[v.len() - 2] == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((v[v.len() - 1] - v[v.len() - 2]) / v[v.len() - 2]))
}

// ───── statistics ─────

/// `df_corr` — Pearson correlation across two columns.
fn builtin_df_corr(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xs = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_df_cov(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xs = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let ys = args.get(1).map(b76_to_floats).unwrap_or_default();
    let n = xs.len().min(ys.len());
    if n < 2 { return Ok(PerlValue::float(0.0)); }
...
fn builtin_df_corrwith(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_df_corr(args)
}

/// `df_describe` — returns N count.
fn builtin_df_describe(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(v.len() as i64))
}

/// `df_kurtosis` — sample excess kurtosis.
fn builtin_df_kurtosis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = v.len() as f64;
    if n < 4.0 { return Ok(PerlValue::float(0.0)); }
    let mean = v.iter().sum::<f64>() / n;
    let m2: f64 = v.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / n;
    let m4: f64 = v.iter().map(|x| (x - mean).powi(4)).sum::<f64>() / n;
    if m2 == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(m4 / (m2 * m2) - 3.0))
}

/// `df_skew` — sample skewness.
fn builtin_df_skew(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = v.len() as f64;
    ...
    if m2 == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(m3 / m2.powf(1.5)))
}

/// `df_sem` — standard error of the mean: σ / √n.
fn builtin_df_sem(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = v.len() as f64;
    if n < 2.0 { return Ok(PerlValue::float(0.0)); }
    let mean = v.iter().sum::<f64>() / n;
    let var: f64 = v.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / (n - 1.0);
    Ok(PerlValue::float(var.sqrt() / n.sqrt()))
}

/// `df_mad` — mean absolute deviation from the mean.
fn builtin_df_mad(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = v.len() as f64;
    if n == 0.0 { return Ok(PerlValue::float(0.0)); }
    let mean = v.iter().sum::<f64>() / n;
    Ok(PerlValue::float(v.iter().map(|x| (x - mean).abs()).sum::<f64>() / n))
}

// ───── NA / replace ─────

/// `df_dropna` — count of non-null rows.
fn builtin_df_dropna(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(v.iter().filter(|x| !x.is_nan()).count() as i64))
}

/// `df_fillna` — count of rows that received a fill value.
fn builtin_df_fillna(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(v.iter().filter(|x| x.is_nan()).count() as i64))
}

/// `df_interpolate` — linearly interpolate NaNs; count of filled.
fn builtin_df_interpolate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_df_fillna(args)
}

/// `df_replace` — count of replacements.
fn builtin_df_replace(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_df_isnull(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_df_fillna(args)
}

/// `df_notnull` — non-null count.
fn builtin_df_notnull(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_df_sort_values(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let s: std::collections::HashSet<u64> = v.iter().map(|x| x.to_bits()).collect();
    Ok(PerlValue::integer(if s.len() == v.len() { 1 } else { 0 }))
}

/// `df_rank` — average rank of value within array.
fn builtin_df_rank(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::float(less + (eq + 1.0) / 2.0))
}

/// `df_quantile` — q-th sample quantile via linear interpolation.
fn builtin_df_quantile(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_df_value_counts(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_df_groupby(args)
}

/// `df_sample` — sample n with replacement; deterministic given seed.
fn builtin_df_sample(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0);
    let seed = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer(n.wrapping_mul(seed.max(1)).rem_euclid(i64::MAX)))
}

/// `df_nlargest` — n-th largest value.
fn builtin_df_nlargest(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = args.get(1).map(|x| x.to_number() as usize).unwrap_or(1).max(1);
    if v.is_empty() { return Ok(PerlValue::float(0.0)); }
    v.sort_by(|a, b| b.partial_cmp(a).unwrap_or(std::cmp::Ordering::Equal));
    Ok(PerlValue::float(v[(n - 1).min(v.len() - 1)]))
}

/// `df_nsmallest` — n-th smallest value.
fn builtin_df_nsmallest(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = args.get(1).map(|x| x.to_number() as usize).unwrap_or(1).max(1);
    if v.is_empty() { return Ok(PerlValue::float(0.0)); }
    v.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
...
fn builtin_df_idxmax(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if v.is_empty() { return Ok(PerlValue::integer(-1)); }
...
fn builtin_df_idxmin(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if v.is_empty() { return Ok(PerlValue::integer(-1)); }
    let mut best = (0_usize, v[0]);
    for (i, &x) in v.iter().enumerate() { if x < best.1 { best = (i, x); } }
...
fn builtin_df_to_datetime(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
    ...
    Ok(PerlValue::integer(jdn))
}

/// `df_to_timedelta` — convert (days, hours, minutes, seconds) → total seconds.
fn builtin_df_to_timedelta(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let days = f1(args);
    ...
    Ok(PerlValue::float(days * 86_400.0 + hours * 3_600.0 + mins * 60.0 + secs))
}

/// `df_to_numeric` — coerce string-as-bytes to f64 via simple parser; on
/// failure return NaN.
fn builtin_df_to_numeric(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(s.trim().parse::<f64>().unwrap_or(f64::NAN)))
}

/// `df_eval` — evaluate `a*x + b*y + c` on three columns; returns row sum.
fn builtin_df_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xs = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let ys = args.get(1).map(b76_to_floats).unwrap_or_default();
    ...
    let n = xs.len().min(ys.len());
    Ok(PerlValue::float((0..n).map(|i| a * xs[i] + b * ys[i] + c).sum::<f64>()))
}

/// `df_query` — count of rows matching predicate (predicate args = booleans).
fn builtin_df_query(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(v.iter().filter(|&&x| x != 0.0).count() as i64))
}

/// `df_filter` — count of items matching label-set.
fn builtin_df_filter(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let labels = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let allowed: std::collections::HashSet<u64> = args.get(1)
        .map(b76_to_floats).unwrap_or_default()
        .iter().map(|x| x.to_bits()).collect();
    Ok(PerlValue::integer(labels.iter().filter(|x| allowed.contains(&x.to_bits())).count() as i64))
}

/// `df_drop_duplicates` — count of unique rows.
fn builtin_df_drop_duplicates(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_df_groupby(args)
}

/// `df_duplicated` — count of duplicated rows.
fn builtin_df_duplicated(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let s: std::collections::HashSet<u64> = v.iter().map(|x| x.to_bits()).collect();
    Ok(PerlValue::integer((v.len() - s.len()) as i64))
}

/// `df_set_index` — given a key column, find row index of target key (linear
/// scan). Returns -1 on miss.
fn builtin_df_set_index(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let keys = b76_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let target = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    keys.iter().position(|&k| k == target)
        .map(|i| Ok(PerlValue::integer(i as i64)))
        .unwrap_or_else(|| Ok(PerlValue::integer(-1)))
}
...
fn builtin_df_reset_index(args: &[PerlValue]) -> PerlResult<PerlValue> {

...
fn builtin_nfa_to_dfa(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_subset_construction(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_dfa_minimize_hopcroft(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_regex_to_nfa_thompson(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let nodes = i1(args);
    Ok(StrykeValue::integer(2 * nodes))
}

/// Glushkov construction: |Σ_p| + 1 states for regex with p positions.
fn builtin_glushkov_construction(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_brzozowski_derivative(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_ll1_first_set(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_ll1_follow_set(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_ll1_first_set(args)
}

/// LL(1) predict-table cell: returns 1 if (nonterm, term) has a rule.
fn builtin_ll1_predict_table(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let nt = i1(args);
...
fn builtin_lr0_items_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cur = i1(args);
...
fn builtin_lalr_lookahead_compute(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_shunting_yard_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_regex_compile_thompson(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_regex_to_nfa_thompson(args)
}

/// Regex match via DFA: returns 1 if accepting state reached.
fn builtin_regex_match_dfa(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_lex_keyword_classify(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_peg_seq(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_peg_choice(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let consumed = b68_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
/// PEG repeat: greedy.
fn builtin_peg_repeat(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_peg_lookahead(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let inner = i1(args);
    Ok(StrykeValue::integer(if inner >= 0 { 0 } else { -1 }))
}

/// DFA simulate one step: δ(q, a) = q'.
fn builtin_dfa_simulate_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let q = i1(args);
...
fn builtin_bytecode_disasm_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let pc = i1(args);
...
fn builtin_ssa_phi_insert(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_dom_tree_idom(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let node = i1(args);
...
fn builtin_dominance_frontier(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let preds = i1(args);

...
fn b82_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

fn b82_to_bytes(v: &PerlValue) -> Vec<u8> {
...
fn builtin_iso8601_duration_parse(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    if bytes.is_empty() || bytes[0] != b'P' { return Ok(PerlValue::float(f64::NAN)); }
...
fn builtin_iso8601_duration_to_seconds(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = f1(args);
    ...
    Ok(PerlValue::float(y * 365.25 * 86400.0 + mo * 30.4375 * 86400.0
...
fn builtin_rrule_next_occurrence(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if now < start { return Ok(PerlValue::float(start)); }
    let elapsed = now - start;
    let n_intervals = (elapsed / step).floor() + 1.0;
    Ok(PerlValue::float(start + n_intervals * step))
}
...
fn builtin_week_number_iso(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(week.max(1)))
}

/// `fiscal_year_us` — US federal fiscal year: starts October 1.
fn builtin_fiscal_year_us(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    Ok(PerlValue::integer(if m >= 10 { y + 1 } else { y }))
}

/// `age_at_date` — full years between two (year, month, day) tuples.
fn builtin_age_at_date(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let by = i1(args);
...
fn builtin_easter_western(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
...
fn builtin_easter_orthodox_year_2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
    let a = y.rem_euclid(19);
...
fn builtin_chinese_new_year(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
    let cycle = (y - 1900).rem_euclid(19);
    let day_offset = (cycle * 11) % 30;
    Ok(PerlValue::integer(122 + day_offset))
}

/// `solstice_winter` — winter solstice approx (December 21 ± 1 by year mod 4).
fn builtin_solstice_winter(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
    let leap = y.rem_euclid(4);
    Ok(PerlValue::integer(if leap == 0 { 1221 } else { 1222 }))
}

/// `equinox_spring` — vernal equinox: March 20 ± 1 by year leap-cycle.
fn builtin_equinox_spring(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
    let leap = y.rem_euclid(4);
    Ok(PerlValue::integer(if leap == 0 { 320 } else { 321 }))
}
...
fn builtin_rgb_to_oklab(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let m = mp.cbrt();
    let s = sp.cbrt();
    Ok(PerlValue::float(0.2104542553 * l + 0.7936177850 * m - 0.0040720468 * s))
}

/// `oklab_to_rgb` — OkLab L (with a=b=0) → linear-RGB → sRGB R channel.
fn builtin_oklab_to_rgb(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l = f1(args);
    ...
    Ok(PerlValue::float((r * 255.0).clamp(0.0, 255.0)))
}

/// `rgb_to_cmyk` — return K channel: K = 1 − max(R, G, B) / 255.
fn builtin_rgb_to_cmyk(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args) / 255.0;
    let g = args.get(1).map(|v| v.to_number()).unwrap_or(0.0) / 255.0;
    let b = args.get(2).map(|v| v.to_number()).unwrap_or(0.0) / 255.0;
    Ok(PerlValue::float(1.0 - r.max(g).max(b)))
}

/// `cmyk_to_rgb` — return R channel: R = 255 (1 − C)(1 − K).
fn builtin_cmyk_to_rgb(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args).clamp(0.0, 1.0);
    let k = args.get(1).map(|v| v.to_number()).unwrap_or(0.0).clamp(0.0, 1.0);
    Ok(PerlValue::float(255.0 * (1.0 - c) * (1.0 - k)))
}

/// `rgb_to_xyz` — sRGB → CIE 1931 XYZ Y channel (D65).
fn builtin_rgb_to_xyz(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args) / 255.0;
    let g = args.get(1).map(|v| v.to_number()).unwrap_or(0.0) / 255.0;
    let b = args.get(2).map(|v| v.to_number()).unwrap_or(0.0) / 255.0;
    let lin = |c: f64| if c >= 0.04045 { ((c + 0.055) / 1.055).powf(2.4) } else { c / 12.92 };
    Ok(PerlValue::float(0.21263901 * lin(r) + 0.71516868 * lin(g) + 0.07219232 * lin(b)))
}

/// `xyz_to_rgb` — XYZ → linear-RGB R channel via M⁻¹.
fn builtin_xyz_to_rgb(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float((r_srgb * 255.0).clamp(0.0, 255.0)))
}

/// `rgb_to_yuv` — sRGB → YUV (BT.601) Y channel.
fn builtin_rgb_to_yuv(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
    let g = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let b = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(0.299 * r + 0.587 * g + 0.114 * b))
}

/// `yuv_to_rgb` — YUV → R channel: R = Y + 1.13983·V.
fn builtin_yuv_to_rgb(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = f1(args);
    let v = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((y + 1.13983 * v).clamp(0.0, 255.0)))
}

/// `luminance_relative` — WCAG relative luminance Y of sRGB (D65).
fn builtin_luminance_relative(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = (f1(args) / 255.0).clamp(0.0, 1.0);
    let g = (args.get(1).map(|v| v.to_number()).unwrap_or(0.0) / 255.0).clamp(0.0, 1.0);
    let b = (args.get(2).map(|v| v.to_number()).unwrap_or(0.0) / 255.0).clamp(0.0, 1.0);
    let lin = |c: f64| if c <= 0.03928 { c / 12.92 } else { ((c + 0.055) / 1.055).powf(2.4) };
    Ok(PerlValue::float(0.2126 * lin(r) + 0.7152 * lin(g) + 0.0722 * lin(b)))
}

/// `contrast_ratio` — WCAG contrast: (L_max + 0.05) / (L_min + 0.05).
fn builtin_contrast_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l1 = f1(args);
...
fn builtin_wcag_pass(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ratio = f1(args);
    let level = if ratio >= 7.0 { 4 }
                else if ratio >= 4.5 { 2 }
                else if ratio >= 3.0 { 1 }
                else { 0 };
    Ok(PerlValue::integer(level))
}

/// `color_temperature_kelvin` — chromaticity (x, y) → CCT via McCamy approx.
fn builtin_color_temperature_kelvin(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let n = (x - 0.3320) / (0.1858 - y);
    Ok(PerlValue::float(449.0 * n.powi(3) + 3525.0 * n.powi(2) + 6823.3 * n + 5520.33))
}

/// `delta_e76` — CIE76 ΔE: Euclidean distance in Lab.
fn builtin_delta_e76(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l1 = f1(args);
...
fn builtin_delta_e94(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l1 = f1(args);
    let a1 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let b1 = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let l2 = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_color_blend_alpha(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(alpha_s * cs + (1.0 - alpha_s) * alpha_d * cd))
}

// ───── Validation checksums ─────

/// `isbn10_check` — ISBN-10: Σ d_i · (10−i) ≡ 0 (mod 11), 'X' = 10.
fn builtin_isbn10_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_eth_addr_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let addr = b82_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    let hash = args.get(1).map(b82_to_bytes).unwrap_or_default();
    if addr.len() != 40 || hash.len() < 40 { return Ok(PerlValue::integer(0)); }
    for i in 0..40 {
        let c = addr[i];
        if !c.is_ascii_hexdigit() { return Ok(PerlValue::integer(0)); }
        if c.is_ascii_alphabetic() {
            let want_upper = hash[i] >= 8;
            if want_upper && !c.is_ascii_uppercase() { return Ok(PerlValue::integer(0)); }
            if !want_upper && !c.is_ascii_lowercase() { return Ok(PerlValue::integer(0)); }
        }
    }
...
fn builtin_btc_addr_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let claimed = b82_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    let computed = args.get(1).map(b82_to_bytes).unwrap_or_default();
    if claimed.len() != 4 || computed.len() != 4 { return Ok(PerlValue::integer(0)); }
    Ok(PerlValue::integer(if claimed == computed { 1 } else { 0 }))
}

/// `ssn_check` — US SSN basic validity: not 000, 666, 9XX area; not 00 group; not 0000 serial.
fn builtin_ssn_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let area = i1(args);
    ...
    Ok(PerlValue::integer(if valid { 1 } else { 0 }))
}

/// `vin_check` — VIN: weighted sum mod 11 (chars I, O, Q forbidden).
fn builtin_vin_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    if bytes.len() != 17 { return Ok(PerlValue::integer(0)); }
    ...
    for (i, &c) in bytes.iter().enumerate() {
        let v = val(c);
        if v < 0 { return Ok(PerlValue::integer(0)); }
        sum += v * weights[i];
    }
    ...
    Ok(PerlValue::integer(if check == claimed { 1 } else { 0 }))
}

/// `imei_check` — IMEI Luhn 15-digit checksum.
fn builtin_imei_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    let digits: Vec<i64> = bytes.iter().filter_map(|&b|
        if b.is_ascii_digit() { Some((b - b'0') as i64) } else { None }).collect();
    if digits.len() != 15 { return Ok(PerlValue::integer(0)); }
    let mut sum = 0_i64;
    ...
    Ok(PerlValue::integer(if sum % 10 == 0 { 1 } else { 0 }))
}
...
fn builtin_iban_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let digits = b82_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mut rem = 0_i64;
    for d in digits { rem = (rem * 10 + d as i64) % 97; }
    Ok(PerlValue::integer(if rem == 1 { 1 } else { 0 }))
}

/// `cusip_check` — CUSIP modulo-10 with weighted alpha mapping.
fn builtin_cusip_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&PerlValue::array(vec![])));
    if bytes.len() != 9 { return Ok(PerlValue::integer(0)); }
    let val = |c: u8| -> i64 {
        match c.to_ascii_uppercase() {
            b'0'..=b'9' => (c - b'0') as i64,
...
fn builtin_kde_silverman_bw(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let sigma = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(1.06 * sigma * n.powf(-0.2)))
}

/// `kde_scott_bw` — Scott's rule: h = σ · n^(−1/(d+4)) for d-dim data.
fn builtin_kde_scott_bw(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let sigma = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    let d = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(sigma * n.powf(-1.0 / (d + 4.0))))
}

/// `kde_bandwidth_lscv` — least-squares cross-validation score (lower = better).
fn builtin_kde_bandwidth_lscv(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(r_int / h - 2.0 * leave_one / (n * h)))
}

/// `kde_epanechnikov` — K(u) = ¾(1 − u²) for |u| ≤ 1, else 0.
fn builtin_kde_epanechnikov(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    Ok(PerlValue::float(if u.abs() <= 1.0 { 0.75 * (1.0 - u * u) } else { 0.0 }))
}

/// `kde_gaussian_2d` — 2-D Gaussian kernel: (1/(2πh²)) exp(−(x² + y²)/(2h²)).
fn builtin_kde_gaussian_2d(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let h = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    Ok(PerlValue::float((-((x * x + y * y) / (2.0 * h * h))).exp()
        / (2.0 * std::f64::consts::PI * h * h)))
}

/// `kde_uniform` — K(u) = ½ for |u| ≤ 1.
fn builtin_kde_uniform(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    Ok(PerlValue::float(if u.abs() <= 1.0 { 0.5 } else { 0.0 }))
}

/// `kde_triangular` — K(u) = (1 − |u|) for |u| ≤ 1.
fn builtin_kde_triangular(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    Ok(PerlValue::float((1.0 - u.abs()).max(0.0)))
}

/// `kde_biweight` — K(u) = (15/16)(1 − u²)² for |u| ≤ 1.
fn builtin_kde_biweight(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    if u.abs() > 1.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(15.0 / 16.0 * (1.0 - u * u).powi(2)))
}

/// `kde_triweight` — K(u) = (35/32)(1 − u²)³ for |u| ≤ 1.
fn builtin_kde_triweight(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    if u.abs() > 1.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(35.0 / 32.0 * (1.0 - u * u).powi(3)))
}

/// `kde_cosine` — K(u) = (π/4) cos(π u/2) for |u| ≤ 1.
fn builtin_kde_cosine(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    if u.abs() > 1.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(std::f64::consts::PI / 4.0 * (std::f64::consts::PI * u / 2.0).cos()))
}

/// `kde_logistic_kernel` — K(u) = 1 / (e^u + 2 + e^(−u)).
fn builtin_kde_logistic_kernel(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
    Ok(PerlValue::float(1.0 / (u.exp() + 2.0 + (-u).exp())))
}

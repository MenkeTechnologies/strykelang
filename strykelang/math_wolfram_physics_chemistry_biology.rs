...
fn builtin_relativistic_kinetic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_lorentz_factor_v(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args); let c = 2.997_924_58e8_f64;
    Ok(StrykeValue::float(1.0 / (1.0 - (v / c).powi(2)).sqrt()))
}
fn builtin_doppler_relativistic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f = f1(args); let v = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    let c = 2.997_924_58e8_f64;
    Ok(StrykeValue::float(f * ((1.0 + v / c) / (1.0 - v / c)).sqrt()))
}
fn builtin_drag_force_quadratic(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_terminal_velocity(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_carnot_efficiency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tc = f1(args); let th = args.get(1).map(|x| x.to_number()).unwrap_or(0.0).max(1e-30);
    Ok(StrykeValue::float(1.0 - tc / th))
}
fn builtin_otto_efficiency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args); let g = args.get(1).map(|x| x.to_number()).unwrap_or(1.4);
    Ok(StrykeValue::float(1.0 - 1.0 / r.powf(g - 1.0)))
}
fn builtin_brayton_efficiency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args); let g = args.get(1).map(|x| x.to_number()).unwrap_or(1.4);
    Ok(StrykeValue::float(1.0 - 1.0 / r.powf((g - 1.0) / g)))
}
fn builtin_diesel_efficiency(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_specific_heat_const_v(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let dof = f1(args); let r = 8.314462618_f64;
    Ok(StrykeValue::float(0.5 * dof * r))
}
fn builtin_speed_of_sound_ideal(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_kepler_period_au(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a_au = f1(args);
    Ok(StrykeValue::float(a_au.powf(1.5))) // years for solar mass
}
fn builtin_synodic_period(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_hill_radius(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_jeans_length(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_chandrasekhar_mass(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(1.4))  // solar masses
}
fn builtin_eddington_luminosity(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_kg = f1(args);
    Ok(StrykeValue::float(1.26e31 * m_kg / 1.989e30))
}
fn builtin_schwarzschild_radius_m(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args); let g = 6.674e-11_f64; let c = 2.997_924_58e8_f64;
    Ok(StrykeValue::float(2.0 * g * m / (c * c)))
}
fn builtin_gravity_at_radius(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args); let r = args.get(1).map(|x| x.to_number()).unwrap_or(0.0).max(1e-30);
    Ok(StrykeValue::float(6.674e-11 * m / (r * r)))
}
fn builtin_gravitational_pe(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_centripetal_accel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args); let r = args.get(1).map(|x| x.to_number()).unwrap_or(0.0).max(1e-30);
    Ok(StrykeValue::float(v * v / r))
}
fn builtin_lens_focal_length(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_avogadros_number(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(6.022_140_76e23))
}
fn builtin_boltzmann_const(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(1.380_649e-23))
}
fn builtin_planck_const_h(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(6.626_070_15e-34))
}
fn builtin_gas_constant_r(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(8.314_462_618))
}
fn builtin_concentration_dilute(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
#[allow(dead_code)]
fn builtin_buffer_capacity(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_titration_volume(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_atomic_radius_pm(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(53.0))  // Bohr radius pm
}
fn builtin_de_broglie_wavelength_kg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
#[allow(dead_code)]
fn builtin_logistic_growth(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_lotka_volterra_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_michaelis_menten(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_hill_equation(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v_max = f1(args); let s = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
...
fn builtin_lineweaver_burk(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let inv_v = f1(args); let inv_s = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(inv_v + inv_s))
}
fn builtin_eadie_hofstee_y(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v_max = f1(args); let v_s = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(v_max - v_s))
}
fn builtin_arrhenius_temp_q10(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_body_surface_area_dubois(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_bmr_harris_benedict_male(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_bmr_harris_benedict_female(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w_kg = f1(args); let h_cm = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    let age = args.get(2).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(447.593 + 9.247 * w_kg + 3.098 * h_cm - 4.330 * age))
}
fn builtin_max_heart_rate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let age = f1(args);
    Ok(StrykeValue::float(220.0 - age))
}
fn builtin_target_heart_rate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_vo2_max_estimate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let age = f1(args); let resting = args.get(1).map(|x| x.to_number()).unwrap_or(60.0);
    Ok(StrykeValue::float(15.3 * (220.0 - age) / resting))
}
fn builtin_pulse_pressure(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sys = f1(args); let dia = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(sys - dia))
}
fn builtin_mean_arterial_pressure(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sys = f1(args); let dia = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
...
fn builtin_dew_point_magnus(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_heat_index_celsius(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_wind_chill_celsius(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_pressure_altitude_m(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_hpa = f1(args);
    Ok(StrykeValue::float(44330.0 * (1.0 - (p_hpa / 1013.25).powf(0.1903))))
}
fn builtin_density_altitude_m(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let pa = f1(args); let temp_c = args.get(1).map(|x| x.to_number()).unwrap_or(15.0);
    Ok(StrykeValue::float(pa + 120.0 * (temp_c - 15.0)))
}
fn builtin_saturation_vapor_pressure(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t_c = f1(args);
    Ok(StrykeValue::float(6.112 * (17.67 * t_c / (t_c + 243.5)).exp()))
}
fn builtin_humidex(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_universal_thermal_climate_index_simple(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_resistance_parallel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_resistance_series(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rs: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(StrykeValue::UNDEF))
        .iter().map(|v| v.to_number()).collect();
    Ok(StrykeValue::float(rs.iter().sum()))
}
fn builtin_capacitance_parallel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cs: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(StrykeValue::UNDEF))
        .iter().map(|v| v.to_number()).collect();
    Ok(StrykeValue::float(cs.iter().sum()))
}
fn builtin_capacitance_series(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cs: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(StrykeValue::UNDEF))
        .iter().map(|v| v.to_number()).collect();
    let s: f64 = cs.iter().filter(|&&c| c.abs() > 1e-30).map(|c| 1.0 / c).sum();
    Ok(StrykeValue::float(if s.abs() < 1e-30 { f64::INFINITY } else { 1.0 / s }))
}
fn builtin_inductance_parallel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_resistance_parallel(args)
}
fn builtin_inductance_series(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_resistance_series(args)
}
fn builtin_voltage_divider(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_current_divider(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i_in = f1(args); let r1 = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    let r2 = args.get(2).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(i_in * r2 / (r1 + r2)))
}
fn builtin_lc_resonant(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_q_factor_rlc(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = f1(args); let c = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    let r = args.get(2).map(|x| x.to_number()).unwrap_or(0.0).max(1e-30);
    Ok(StrykeValue::float((l / c).sqrt() / r))
}
fn builtin_skin_depth(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_wire_resistance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
#[allow(non_snake_case)]
fn builtin_dB_power(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {

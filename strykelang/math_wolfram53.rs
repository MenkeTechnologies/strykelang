...
fn b53_to_floats(v: &PerlValue) -> Vec<f64> {
...
fn builtin_dnorm(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let mu = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let sigma = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-300);
    let z = (x - mu) / sigma;
    Ok(PerlValue::float((-z * z / 2.0).exp() / (sigma * (2.0 * std::f64::consts::PI).sqrt())))
}

/// Student t pdf with ν degrees of freedom.
fn builtin_dt(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float((log_norm + log_kernel).exp()))
}

/// F distribution pdf with d1, d2 degrees of freedom.
fn builtin_df_dist(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_dchisq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_glm(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mu = b53_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let sigma = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-300);
...
fn builtin_aov(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let group_means = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let group_sizes = b53_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let group_ss = b53_to_floats(args.get(2).unwrap_or(&PerlValue::array(vec![])));
    let k = group_means.len();
    if k < 2 { return Ok(PerlValue::float(0.0)); }
    ...
    if ms_within == 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(ms_between / ms_within))
}

/// Shapiro-Wilk W statistic (small-n approximation, n ≤ 50). Uses Royston's
/// formula for a-coefficients. Args: sorted sample.
fn builtin_shapiro_wilk(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = x.len();
    if n < 3 { return Ok(PerlValue::float(1.0)); }
    let mean: f64 = x.iter().sum::<f64>() / n as f64;
    let ss: f64 = x.iter().map(|v| (v - mean).powi(2)).sum();
    if ss == 0.0 { return Ok(PerlValue::float(1.0)); }
    ...
    Ok(PerlValue::float((num / ss).clamp(0.0, 1.0)))
}
...
fn builtin_anderson_darling(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = x.len();
    ...
    Ok(PerlValue::float(-(n as f64) - s / n as f64))
}

/// Kolmogorov-Smirnov two-sample D = max |F1(x) − F2(x)| from sorted samples.
fn builtin_kolmogorov_smirnov(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut a = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let mut b = b53_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    if a.is_empty() || b.is_empty() { return Ok(PerlValue::float(0.0)); }
...
fn builtin_spearmanr(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let y = b53_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    ...
    if dx <= 0.0 || dy <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(num / (dx * dy).sqrt()))
}
...
fn builtin_kendalltau(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let y = b53_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let n = x.len().min(y.len());
    if n < 2 { return Ok(PerlValue::float(0.0)); }
    ...
    Ok(PerlValue::float((conc - disc) as f64 / denom))
}

/// Pearson r, the canonical correlation.
fn builtin_pearsonr(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let y = b53_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let n = x.len().min(y.len());
    if n < 2 { return Ok(PerlValue::float(0.0)); }
...
fn builtin_mannwhitneyu(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let y = b53_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let n_x = x.len();
    if n_x == 0 || y.is_empty() { return Ok(PerlValue::float(0.0)); }
    ...
    Ok(PerlValue::float(r_x - n_x as f64 * (n_x as f64 + 1.0) / 2.0))
}

/// Wilcoxon signed-rank statistic W = sum of positive signed ranks.
fn builtin_wilcoxon(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let d = b53_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let pairs: Vec<(f64, f64)> = d.iter().filter(|&&x| x != 0.0)
        .map(|&x| (x.abs(), x.signum())).collect();
    if pairs.is_empty() { return Ok(PerlValue::float(0.0)); }
    ...
    let w_pos: f64 = (0..pairs.len()).filter(|&i| pairs[i].1 > 0.0)
        .map(|i| ranks[i]).sum();
    Ok(PerlValue::float(w_pos))
}

/// Kruskal-Wallis H = (12 / N(N+1)) Σ T_g²/n_g − 3(N+1).
fn builtin_kruskal_h(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let groups = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let group_data: Vec<Vec<f64>> = groups.iter().map(b53_to_floats).collect();
    if group_data.len() < 2 { return Ok(PerlValue::float(0.0)); }
    ...
    if big_n < 2 { return Ok(PerlValue::float(0.0)); }
    combined.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap_or(std::cmp::Ordering::Equal));

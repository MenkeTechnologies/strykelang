...
fn builtin_iso8601_duration_parse(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_iso8601_duration_to_seconds(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_rrule_next_occurrence(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_cron_next_fire(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cur = i1(args);
...
/// 2=hour, 3=day).
fn builtin_date_round_iso(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
...
fn builtin_week_number_iso(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_fiscal_year_us(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
fn builtin_age_at_date(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let by = i1(args);
...
fn builtin_easter_western(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
fn builtin_easter_orthodox_year_2(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let a = y.rem_euclid(19);
...
fn builtin_chinese_new_year(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
fn builtin_solstice_winter(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
fn builtin_equinox_spring(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
    let leap = y.rem_euclid(4);
...
fn builtin_rgb_to_oklab(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_oklab_to_rgb(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l = f1(args);
...
fn builtin_rgb_to_cmyk(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args) / 255.0;
...
fn builtin_cmyk_to_rgb(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_rgb_to_xyz(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args) / 255.0;
    let g = args.get(1).map(|v| v.to_number()).unwrap_or(0.0) / 255.0;
    let b = args.get(2).map(|v| v.to_number()).unwrap_or(0.0) / 255.0;
    let lin = |c: f64| if c >= 0.04045 { ((c + 0.055) / 1.055).powf(2.4) } else { c / 12.92 };
...
fn builtin_xyz_to_rgb(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_rgb_to_yuv(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_yuv_to_rgb(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_luminance_relative(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = (f1(args) / 255.0).clamp(0.0, 1.0);
    let g = (args.get(1).map(|v| v.to_number()).unwrap_or(0.0) / 255.0).clamp(0.0, 1.0);
    let b = (args.get(2).map(|v| v.to_number()).unwrap_or(0.0) / 255.0).clamp(0.0, 1.0);
...
fn builtin_contrast_ratio(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l1 = f1(args);
...
fn builtin_wcag_pass(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let ratio = f1(args);
...
fn builtin_color_temperature_kelvin(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_delta_e76(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l1 = f1(args);
...
fn builtin_delta_e94(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let l1 = f1(args);
    let a1 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let b1 = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let l2 = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_delta_e2000(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_delta_e94(args)
}

/// `color_blend_alpha` — Porter-Duff "over": c = α_s c_s + (1 − α_s) α_d c_d.
fn builtin_color_blend_alpha(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cs = f1(args);
...
fn builtin_isbn10_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_isbn13_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let digits: Vec<i64> = bytes.iter().filter_map(|&b| match b {
        b'0'..=b'9' => Some((b - b'0') as i64),
        _ => None,
...
fn builtin_ean13_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_isbn13_check(args)
}

/// `upc_check` — UPC-A: 11 digits + check; Σ odd · 3 + Σ even ≡ 0 (mod 10).
fn builtin_upc_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_eth_addr_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_btc_addr_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_ssn_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let area = i1(args);
...
fn builtin_vin_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_imei_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let digits: Vec<i64> = bytes.iter().filter_map(|&b|
        if b.is_ascii_digit() { Some((b - b'0') as i64) } else { None }).collect();
...
fn builtin_iban_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_cusip_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = b82_to_bytes(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_kde_silverman_bw(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sigma = f1(args);
...
fn builtin_kde_scott_bw(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sigma = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
...
fn builtin_kde_bandwidth_lscv(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_kde_epanechnikov(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
    Ok(StrykeValue::float(if u.abs() <= 1.0 { 0.75 * (1.0 - u * u) } else { 0.0 }))
}

/// `kde_gaussian_2d` — 2-D Gaussian kernel: (1/(2πh²)) exp(−(x² + y²)/(2h²)).
fn builtin_kde_gaussian_2d(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_kde_uniform(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
    Ok(StrykeValue::float(if u.abs() <= 1.0 { 0.5 } else { 0.0 }))
}

/// `kde_triangular` — K(u) = (1 − |u|) for |u| ≤ 1.
fn builtin_kde_triangular(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
    Ok(StrykeValue::float((1.0 - u.abs()).max(0.0)))
}

/// `kde_biweight` — K(u) = (15/16)(1 − u²)² for |u| ≤ 1.
fn builtin_kde_biweight(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
...
fn builtin_kde_triweight(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
    if u.abs() > 1.0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_kde_cosine(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);
    if u.abs() > 1.0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_kde_logistic_kernel(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let u = f1(args);

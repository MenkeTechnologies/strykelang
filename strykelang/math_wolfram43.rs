...
fn b43_to_floats(v: &PerlValue) -> Vec<f64> {
...
fn builtin_nash_equilibrium_pair(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let denom = a - b - c + d;
    if denom == 0.0 { return Ok(PerlValue::float(0.5)); }
...
fn builtin_mixed_strategy_value(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let d = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
    let denom = a - b - c + d;
    if denom == 0.0 { return Ok(PerlValue::float((a + d) / 2.0)); }
...
fn builtin_prisoners_dilemma_payoff(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    match (action_i, action_j) {
        (0, 0) => Ok(PerlValue::float(3.0)),
        (0, 1) => Ok(PerlValue::float(0.0)),
        (1, 0) => Ok(PerlValue::float(5.0)),
        (1, 1) => Ok(PerlValue::float(1.0)),
...
fn builtin_chicken_game_payoff(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let j = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    match (i, j) {
        (0, 0) => Ok(PerlValue::float(0.0)),
        (0, 1) => Ok(PerlValue::float(-1.0)),
        (1, 0) => Ok(PerlValue::float(1.0)),
        (1, 1) => Ok(PerlValue::float(-10.0)),
        _ => Ok(PerlValue::float(0.0)),
    }
}
...
fn builtin_stag_hunt_payoff(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let j = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    match (i, j) {
        (0, 0) => Ok(PerlValue::float(4.0)),
        (0, 1) => Ok(PerlValue::float(0.0)),
        (1, 0) => Ok(PerlValue::float(3.0)),
        (1, 1) => Ok(PerlValue::float(3.0)),
        _ => Ok(PerlValue::float(0.0)),
    }
}
...
fn builtin_battle_sexes_payoff(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    match (i, j, role) {
        (0, 0, 0) => Ok(PerlValue::float(2.0)),
        (1, 1, 0) => Ok(PerlValue::float(1.0)),
        (0, 0, 1) => Ok(PerlValue::float(1.0)),
        (1, 1, 1) => Ok(PerlValue::float(2.0)),
        _ => Ok(PerlValue::float(0.0)),
    }
}
...
fn builtin_tragedy_commons_metric(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if n == 0.0 { return Ok(PerlValue::float(resource)); }
...
fn builtin_kalai_smorodinsky_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if u_max1 + u_max2 == 0.0 { return Ok(PerlValue::float(0.0)); }
...
fn builtin_quantal_response_logit(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if u.is_empty() || idx >= u.len() { return Ok(PerlValue::float(0.0)); }
...
fn builtin_repeated_game_avg_payoff(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if (1.0 - delta).abs() < 1e-12 { return Ok(PerlValue::float(stage)); }

...
fn b74_to_floats(v: &PerlValue) -> Vec<f64> {
...
fn builtin_hann_w(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    Ok(PerlValue::float(0.5 * (1.0 - (2.0 * std::f64::consts::PI * n / (big_n - 1.0)).cos())))
}

/// Hamming window w[n] = 0.54 − 0.46 cos(2π n / (N−1)).
fn builtin_hamming_w(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    Ok(PerlValue::float(0.54 - 0.46 * (2.0 * std::f64::consts::PI * n / (big_n - 1.0)).cos()))
}

/// Blackman window w[n] = 0.42 − 0.5 cos(2π n / (N−1)) + 0.08 cos(4π n / (N−1)).
fn builtin_blackman_w(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    let arg = 2.0 * std::f64::consts::PI * n / (big_n - 1.0);
    Ok(PerlValue::float(0.42 - 0.5 * arg.cos() + 0.08 * (2.0 * arg).cos()))
}

/// Bartlett-Hann window: 0.62 − 0.48 |n/(N−1) − 0.5| − 0.38 cos(2π n / (N−1)).
fn builtin_barthann_w(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    let r = n / (big_n - 1.0);
    Ok(PerlValue::float(0.62 - 0.48 * (r - 0.5).abs()
        - 0.38 * (2.0 * std::f64::consts::PI * r).cos()))
}

/// Nuttall window: 4-term, sidelobe −98 dB.
fn builtin_nuttall_w(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    let arg = 2.0 * std::f64::consts::PI * n / (big_n - 1.0);
    Ok(PerlValue::float(0.355768
        - 0.487396 * arg.cos()
...
fn builtin_flattop_w(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    let arg = 2.0 * std::f64::consts::PI * n / (big_n - 1.0);
    Ok(PerlValue::float(0.21557895
        - 0.41663158 * arg.cos()
...
fn builtin_parzen_window(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
...
fn builtin_tukey_w(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
...
fn builtin_taylor_window(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    ...
    Ok(PerlValue::float((std::f64::consts::PI * beta * (1.0 - r * r).sqrt()).cosh()
        / (std::f64::consts::PI * beta).cosh()))
}

/// DPSS (Slepian) window approximation via Kaiser with matched β.
fn builtin_dpss_window(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args) as f64;
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(2.0);
    ...
    Ok(PerlValue::float(b74_io_bessel_zero(arg) / b74_io_bessel_zero(beta)))
}
...
fn builtin_kaiserord_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args).abs();
    let beta = if a > 50.0 {
        0.1102 * (a - 8.7)
    } else if a >= 21.0 {
...
fn builtin_butter_lp_re(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let order = i1(args).max(1) as f64;
    let k = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(-((2.0 * k + 1.0) * std::f64::consts::PI / (2.0 * order)).sin()))
}

/// Butterworth high-pass analog prototype magnitude at ω: |H|² = 1 / (1 + (ωc/ω)^{2N}).
fn builtin_butter_hp_mag(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(1.0 / (1.0 + (omega_c / omega).powf(2.0 * n)).sqrt()))
}

/// Chebyshev type-I low-pass magnitude |H|² = 1 / (1 + ε² T_n²(ω/ωc)).
fn builtin_cheby1_lp(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float((1.0 / (1.0 + eps_sq * tn * tn)).sqrt()))
}

/// Chebyshev type-II low-pass magnitude.
fn builtin_cheby2_lp(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(1) as i32;
    let omega = args.get(1).map(|v| v.to_number()).unwrap_or(0.0).max(1e-15);
    ...
    Ok(PerlValue::float((1.0 / (1.0 + 1.0 / (eps_sq_inv * tn * tn))).sqrt()))
}

/// Elliptic (Cauer) low-pass magnitude approximation via Chebyshev-of-rational.
fn builtin_ellip_lp(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(1) as i32;
    let omega = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let omega_c = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
...
fn builtin_bessel_lp(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    if n == 0 { return Ok(PerlValue::float(t_prev)); }
    if n == 1 { return Ok(PerlValue::float(t_cur)); }
    for k in 2..=n {
        let t_next = (2.0 * k as f64 - 1.0) * t_cur + s * s * t_prev;
        t_prev = t_cur;
        t_cur = t_next;
    }
    Ok(PerlValue::float(t_cur))
}

/// Notch filter (IIR, single zero on unit circle): H(z) = (1 − 2 cos ω₀ z⁻¹ + z⁻²) /
///                                                       (1 − 2 r cos ω₀ z⁻¹ + r² z⁻²).
/// Returns magnitude at frequency ω.
fn builtin_notch_filter(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let omega = f1(args);
    ...
    Ok(PerlValue::float((num.powi(2) / den.powi(2).max(1e-300)).sqrt()))
}
...
fn builtin_sosfilt_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float(b0 * x + b1 * x1 + b2 * x2 - a1 * y1 - a2 * y2))
}

/// Direct-form-II IIR filter zero-input response initial condition.
fn builtin_lfilter_zi_init(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let b_sum = f1(args);
    let a_sum = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    Ok(PerlValue::float(b_sum / a_sum))
}

/// filtfilt zero-phase preprocessing: required reflection padding length
/// = 3 · max(|a|, |b|).
fn builtin_filtfilt_pad(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let na = i1(args).max(0);
    let nb = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0).max(0);
    Ok(PerlValue::integer(3 * na.max(nb)))
}

/// freqz evaluation: H(e^{jω}) = Σ b_k e^{−jωk} / Σ a_k e^{−jωk}. Returns |H|.
fn builtin_freqz_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let b = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::float((num_mag_sq / denom).sqrt()))
}
...
fn builtin_freqs_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let b = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let a = args.get(1).map(b74_to_floats).unwrap_or_else(|| vec![1.0]);
    let omega = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_group_delay_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(-(phi_plus - phi_minus) / (2.0 * dw)))
}

/// Impulse-response truncation length n at which |h[n]| < tol; returns n_max.
fn builtin_impulse_response_n(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pole_rad = f1(args).abs().min(0.999_999);
    let tol = args.get(1).map(|v| v.to_number()).unwrap_or(1e-6);
    if pole_rad <= 0.0 { return Ok(PerlValue::integer(1)); }
    let n = (tol.ln() / pole_rad.ln()).ceil() as i64;
    Ok(PerlValue::integer(n.max(1)))
}

// ───── transform conversions ─────

/// TF → ZPK conversion: count of poles equals deg(a) − [leading-zeros].
fn builtin_tf2zpk_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let leading_zeros = a.iter().take_while(|&&v| v == 0.0).count();
    Ok(PerlValue::integer((a.len().saturating_sub(leading_zeros)) as i64 - 1))
}

/// ZPK → TF: produce one b-coefficient via Vieta. Args: zeros array, k index.
fn builtin_zpk2tf_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let zeros = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let k = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0).max(0) as usize;
    let n = zeros.len();
    if k > n { return Ok(PerlValue::float(0.0)); }
    let sign = if k.is_multiple_of(2) { 1.0 } else { -1.0 };
    Ok(PerlValue::float(sign * elementary_symmetric(&zeros, k)))
}
...
fn builtin_tf2sos_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let order = i1(args).max(1);
    Ok(PerlValue::integer((order + 1) / 2))
}

/// ZPK → SOS step: pair conjugates, return biquad count.
fn builtin_zpk2sos_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pole_count = i1(args).max(0);
    let zero_count = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0).max(0);
    Ok(PerlValue::integer((pole_count.max(zero_count) + 1) / 2))
}

/// SOS → TF step: deg = 2 · n_biquads.
fn builtin_sos2tf_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let nb = i1(args).max(0);
    Ok(PerlValue::integer(2 * nb))
}

/// Bilinear transform s → 2/T · (z−1)/(z+1): map analog pole p to digital
/// p_d = (2/T + p) / (2/T − p).
fn builtin_bilinear_xform(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let t = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    let two_over_t = 2.0 / t;
    Ok(PerlValue::float((two_over_t + p) / (two_over_t - p)))
}

/// Bilinear transform on ZPK: scale gain by ∏(2/T − z_k) / ∏(2/T − p_k).
fn builtin_bilinear_zpk_xform(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(k * zeros_prod / poles_prod))
}
...
fn builtin_firwin_lowpass(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args);
...
fn builtin_firwin_highpass(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args);
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(11.0).max(2.0);
    let omega_c = args.get(2).map(|v| v.to_number()).unwrap_or(0.5);
    let m = (big_n - 1.0) / 2.0;
    ...
    Ok(PerlValue::float(delta - h_lp))
}

/// firwin bandpass: difference of two lowpass impulse responses.
fn builtin_firwin_bandpass(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args);
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(11.0).max(2.0);
    ...
    Ok(PerlValue::float(s_h - s_l))
}

/// firwin bandstop: 1 − bandpass.
fn builtin_firwin_bandstop(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args);
    let big_n = args.get(1).map(|v| v.to_number()).unwrap_or(11.0).max(2.0);
    let omega_l = args.get(2).map(|v| v.to_number()).unwrap_or(0.25);
    let omega_h = args.get(3).map(|v| v.to_number()).unwrap_or(0.75);
    ...
    let bp = s_h - s_l;
    let delta = if (n as f64 - m).abs() < 1e-9 { 1.0 } else { 0.0 };
    Ok(PerlValue::float(delta - bp))
}

/// firwin2 (frequency-sampling design): inverse-DFT of |H(ω_k)| samples.
fn builtin_firwin2_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mag = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::float(h_n / big_n))
}

/// Remez exchange step: error-norm convergence test.
fn builtin_remez_design(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cur_err = f1(args);
    let prev_err = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let tol = args.get(2).map(|v| v.to_number()).unwrap_or(1e-6);
    Ok(PerlValue::integer(if (cur_err - prev_err).abs() < tol { 1 } else { 0 }))
}

// ───── spectral analysis ─────

/// STFT step: one-frame DFT of windowed signal at frequency bin k.
fn builtin_stft_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::float((re * re + im * im).sqrt()))
}

/// Inverse STFT step: overlap-add reconstruction; returns sum of cos·X_k.
fn builtin_istft_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mags = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_cwt_morlet(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(envelope * (omega0 * u).cos()))
}

/// Ricker (Mexican hat 2D version): ψ(t) = (1 − t²) e^{−t²/2}.
fn builtin_ricker_wavelet(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let t = f1(args);
    let sigma = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    let u = t / sigma;
    Ok(PerlValue::float((1.0 - u * u) * (-u * u / 2.0).exp()))
}
...
fn builtin_mexican_hat_wavelet(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float(norm * (1.0 - r_sq / 2.0) * (-r_sq / 2.0).exp()))
}

/// Magnitude-squared coherence γ_xy(ω) = |S_xy|² / (S_xx · S_yy).
fn builtin_coherence_xy(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s_xy_re = f1(args);
    ...
    Ok(PerlValue::float((s_xy_re * s_xy_re + s_xy_im * s_xy_im) / (s_xx * s_yy)))
}

/// Cross-spectral density: average of conjugate-multiplied DFT bins.
fn builtin_csd_xy(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xr = f1(args);
    ...
    Ok(PerlValue::float(xr * yr + xi * yi))
}

/// Welch periodogram: average of |X_k|² across overlapping frames.
fn builtin_welch_psd_avg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let frames = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if frames.is_empty() { return Ok(PerlValue::float(0.0)); }
    let s: f64 = frames.iter().map(|x| x * x).sum();
    Ok(PerlValue::float(s / frames.len() as f64))
}

/// Basic periodogram: |X(ω)|² / N.
fn builtin_periodogram_basic(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xr = f1(args);
    let xi = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let n = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float((xr * xr + xi * xi) / n))
}

/// Lomb-Scargle periodogram contribution at frequency ω for sample (t, x).
fn builtin_lombscargle_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let t = f1(args);
...
fn builtin_oaconvolve_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m = i1(args).max(1);
    let target = 4 * m;
    let mut p = 1_i64;
    while p < target { p *= 2; }
...
fn builtin_upfirdn_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer((n_filtered + m_down - 1) / m_down))
}

/// resample_poly: polyphase filter output length = ⌈n · up / down⌉.
fn builtin_resample_poly_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0);
    ...
    Ok(PerlValue::integer((n * up + down - 1) / down))
}

/// Decimate by integer factor M: output length = ⌈n / M⌉.
fn builtin_decimate_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0);
    let m = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1).max(1);
    Ok(PerlValue::integer((n + m - 1) / m))
}
...
fn builtin_savgol_coef(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = f1(args);
    let m = args.get(1).map(|v| v.to_number()).unwrap_or(2.0).max(1.0);
    let scale = 1.0 / (2.0 * m + 1.0);
    Ok(PerlValue::float(scale * (-(k * k) / (2.0 * m * m).max(1e-15)).exp()))
}

/// Linear detrend: subtract least-squares-fit line. Returns slope.
fn builtin_detrend_linear(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xs = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = xs.len() as f64;
...
fn builtin_wiener_filter(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let sxx = f1(args).max(0.0);
    let noise = args.get(1).map(|v| v.to_number()).unwrap_or(0.0).max(0.0);
    Ok(PerlValue::float(sxx / (sxx + noise).max(1e-300)))
}

/// 1-D median filter: median of (2k+1)-sized window centred at i.
fn builtin_medfilt_1d(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut win = b74_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if win.is_empty() { return Ok(PerlValue::float(0.0)); }
    win.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    let mid = win.len() / 2;
    Ok(PerlValue::float(if win.len() % 2 == 1 { win[mid] }
                        else { (win[mid - 1] + win[mid]) / 2.0 }))
}

/// Peak-width-at-half-prominence estimation given prominence and slope.
fn builtin_peak_widths_at(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(prominence / 2.0 / slope_left + prominence / 2.0 / slope_right))
}

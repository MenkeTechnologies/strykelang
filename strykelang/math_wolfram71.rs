...
fn builtin_zadd(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let existed = i1(args);
    Ok(StrykeValue::integer(if existed != 0 { 0 } else { 1 }))
}

/// ZREM — count removed.
fn builtin_zrem(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let removed = i1(args);
    Ok(StrykeValue::integer(removed.max(0)))
}

/// ZRANGEBYSCORE — count in [min, max] inclusive.
fn builtin_zrangebyscore(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_zrank(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_zrevrank(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let mut scores = b71_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let target = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_zincrby(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cur = f1(args);
...
/// ZCARD — cardinality.
fn builtin_zcard(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let scores = b71_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(scores.len() as i64))
}

/// ZCOUNT — same as ZRANGEBYSCORE count.
fn builtin_zcount(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_zrangebyscore(args)
}

/// ZLEXCOUNT — count of members between two lex bounds (treat as ints here).
fn builtin_zlexcount(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let scores = b71_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let min = args.get(1).map(|v| v.to_number()).unwrap_or(f64::NEG_INFINITY);
    let max = args.get(2).map(|v| v.to_number()).unwrap_or(f64::INFINITY);
    Ok(StrykeValue::integer(scores.iter().filter(|&&s| s >= min && s < max).count() as i64))
...
/// LPUSH — new length.
fn builtin_lpush(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let len = i1(args);
...
/// RPUSH — new length.
fn builtin_rpush(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_lpush(args)
}

/// LRANGE — number of elements in [start, stop] inclusive after wrap.
fn builtin_lrange(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_lrem(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_hset(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let existed = i1(args);
    Ok(StrykeValue::integer(if existed != 0 { 0 } else { 1 }))
}

/// HGET — return existence marker (1 if present).
fn builtin_hget(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let exists = i1(args);
    Ok(StrykeValue::integer(if exists != 0 { 1 } else { 0 }))
}

/// HGETALL — count of (k, v) pairs.
fn builtin_hgetall(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer((v.len() / 2) as i64))
}

/// HLEN — number of fields.
fn builtin_hlen(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_hgetall(args)
}

/// HKEYS — count.
fn builtin_hkeys(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_hgetall(args)
}

/// HVALS — count.
fn builtin_hvals(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_hgetall(args)
}

/// HMSET — number of fields written.
fn builtin_hmset(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer((v.len() / 2) as i64))
}

/// HINCRBY — new field value.
fn builtin_hincrby(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cur = i1(args);
...
fn builtin_sadd(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let already = i1(args);
...
/// SREM — number removed.
fn builtin_srem(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let removed = i1(args);
    Ok(StrykeValue::integer(removed.max(0)))
}

/// SMEMBERS — cardinality.
fn builtin_smembers(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.len() as i64))
}

/// SINTER — |A ∩ B|.
fn builtin_sinter(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
/// SUNION — |A ∪ B|.
fn builtin_sunion(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a: std::collections::HashSet<u64> =
        b71_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])))
            .iter().map(|x| x.to_bits()).collect();
    let b: std::collections::HashSet<u64> = args.get(1).map(b71_to_floats).unwrap_or_default()
...
/// SDIFF — |A \ B|.
fn builtin_sdiff(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a: std::collections::HashSet<u64> =
        b71_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])))
            .iter().map(|x| x.to_bits()).collect();
    let b: std::collections::HashSet<u64> = args.get(1).map(b71_to_floats).unwrap_or_default()
...
/// SCARD — cardinality.
fn builtin_scard(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    builtin_smembers(args)
}

/// SISMEMBER — 1/0.
fn builtin_sismember(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_spop(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_setex(_args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    Ok(StrykeValue::integer(1))
}
/// `setnx`
fn builtin_setnx(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let exists = i1(args);
    Ok(StrykeValue::integer(if exists != 0 { 0 } else { 1 }))
}

/// EXPIRE — 1 if applied.
fn builtin_expire(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let exists = i1(args);
    Ok(StrykeValue::integer(if exists != 0 { 1 } else { 0 }))
}

/// TTL — remaining seconds (-2 missing, -1 no expire).
fn builtin_ttl(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let secs = i1(args);
    Ok(StrykeValue::integer(secs))
}
/// `pttl`
fn builtin_pttl(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let secs = i1(args);
    Ok(StrykeValue::integer(secs * 1000))
}

/// PERSIST — 1 if expiration removed.
fn builtin_persist(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let had_ttl = i1(args);
    Ok(StrykeValue::integer(if had_ttl != 0 { 1 } else { 0 }))
}

/// INCR / DECR / INCRBY / DECRBY.
fn builtin_incr(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    Ok(StrykeValue::integer(i1(args) + 1))
}
/// `decr`
fn builtin_decr(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    Ok(StrykeValue::integer(i1(args) - 1))
}
/// `incrby`
fn builtin_incrby(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cur = i1(args);
    let inc = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(StrykeValue::integer(cur + inc))
}
/// `decrby`
fn builtin_decrby(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cur = i1(args);
...
fn builtin_setrange(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_append_redis(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cur = i1(args);
...
fn builtin_bitcount(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_bitop(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let op = i1(args);
...
fn builtin_bitpos(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let bytes = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_pfadd(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let novel = i1(args);
    Ok(StrykeValue::integer(if novel != 0 { 1 } else { 0 }))
}

/// PFCOUNT — α_m · m² / Σ 2^{-M[j]}; we use raw register sum.
fn builtin_pfcount(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_geoadd(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let added = i1(args);
    Ok(StrykeValue::integer(added.max(0)))
}

/// GEODIST — Haversine in metres.
fn builtin_geodist(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_geohash(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let lat = f1(args);
...
fn builtin_xadd(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let last_id = i1(args);
    Ok(StrykeValue::integer(last_id + 1))
}

/// XLEN — entry count.
fn builtin_xlen(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let v = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.len() as i64))
}

/// XRANGE — count of entries in [start, end].
fn builtin_xrange(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_object_encoding(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let id = i1(args).clamp(0, 8);
    Ok(StrykeValue::integer(id))
}

/// DEBUG OBJECT — return refcount.
fn builtin_debug_object(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let refcount = i1(args).max(1);
    Ok(StrykeValue::integer(refcount))
}

/// CLUSTER SLOTS — slot index from CRC16(key) mod 16384.
fn builtin_cluster_slots(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let bytes = arg_to_vec(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let mut crc: u16 = 0;
    for byte in bytes {
        let b = byte.to_number() as u8;

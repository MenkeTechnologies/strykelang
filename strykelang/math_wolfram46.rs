...
fn b46_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

// BM25 score: IDF·(tf(k1+1))/(tf + k1(1 - b + b·dl/avgdl))
fn builtin_nlp_bm25_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tf = f1(args);
    ...
    Ok(PerlValue::float(idf * tf * (k1 + 1.0) / denom))
}

// TF-IDF
fn builtin_nlp_tf_idf_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tf = f1(args);
    let idf = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_nlp_doc_freq_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let prev = f1(args);
...
// IDF = log(N / df)
fn builtin_nlp_inverse_doc_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = f1(args);
    let df = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if df <= 0.0 || n <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((n / df).ln()))
}

// Cosine similarity (precomputed dot, norms)
fn builtin_nlp_cosine_similarity_two(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dot = f1(args);
    let na = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let nb = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    if na == 0.0 || nb == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(dot / (na * nb)))
}

// Jaccard similarity |A∩B|/|A∪B|
fn builtin_nlp_jaccard_similarity_two(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_nlp_overlap_coefficient(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let intersection = f1(args);
    let min_size = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if min_size == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(intersection / min_size))
}

// Dice coefficient 2|A∩B|/(|A|+|B|)
fn builtin_nlp_dice_coefficient_two(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let inter = f1(args);
    let a_plus_b = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if a_plus_b == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(2.0 * inter / a_plus_b))
}

// Simpson coefficient |A∩B|/min
fn builtin_nlp_simpson_coefficient(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_overlap_coefficient(args)
}

// Levenshtein distance scalar from precomputed table cell
fn builtin_nlp_levenshtein_dist(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float((prev_diag + cost).min((prev_left + 1.0).min(prev_up + 1.0))))
}

// Damerau-Levenshtein step (with transposition)
fn builtin_nlp_damerau_levenshtein(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lev = f1(args);
    let trans = args.get(1).map(|v| v.to_number()).unwrap_or(f64::INFINITY);
    Ok(PerlValue::float(lev.min(trans)))
}

// Jaro distance
fn builtin_nlp_jaro_distance(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m = f1(args);
    ...
    Ok(PerlValue::float((m / s1 + m / s2 + (m - t) / m) / 3.0))
}

// Jaro-Winkler: jaro + l·p·(1 - jaro)
fn builtin_nlp_jaro_winkler(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let jaro = f1(args);
...
// LCS length
fn builtin_nlp_lcs_length(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dp_diag = f1(args);
    let same = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    let max_lr = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    if same > 0 { Ok(PerlValue::float(dp_diag + 1.0)) } else { Ok(PerlValue::float(max_lr)) }
}

// LCS ratio = LCS / max(|s1|, |s2|)
fn builtin_nlp_lcs_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lcs = f1(args);
    let max_len = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if max_len == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(lcs / max_len))
}

// METEOR (precision, recall, fragmentation form)
fn builtin_nlp_meteor_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let frag = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    if p + 9.0 * r == 0.0 { return Ok(PerlValue::float(0.0)); }
    let f_mean = 10.0 * p * r / (p + 9.0 * r);
    Ok(PerlValue::float(f_mean * (1.0 - 0.5 * frag.powi(3))))
}

// BLEU n-gram with brevity penalty (sentence-level)
fn builtin_nlp_bleu_score_n(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_n = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let bp = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if p_n.is_empty() { return Ok(PerlValue::float(0.0)); }
    let log_sum: f64 = p_n.iter().filter(|&&x| x > 0.0).map(|x| x.ln()).sum();
    Ok(PerlValue::float(bp * (log_sum / p_n.len() as f64).exp()))
}

// ROUGE-N (precision form)
fn builtin_nlp_rouge_score_n(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let match_count = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(match_count / total))
}

// chrF: char F-score
fn builtin_nlp_chrf_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let beta = args.get(2).map(|v| v.to_number()).unwrap_or(2.0);
    if beta * beta * p + r == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((1.0 + beta * beta) * p * r / (beta * beta * p + r)))
}

// Translation edit rate
fn builtin_nlp_ter_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let edits = f1(args);
    let ref_len = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if ref_len == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(edits / ref_len))
}

// Word error rate = (S+D+I)/N
fn builtin_nlp_wer_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_ter_score(args)
}

// Character error rate
fn builtin_nlp_cer_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_ter_score(args)
}

// Perplexity = exp(avg negative log likelihood)
fn builtin_nlp_perplexity_value(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let avg_nll = f1(args);
    Ok(PerlValue::float(avg_nll.exp()))
}

// Bits per character
fn builtin_nlp_bits_per_character(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let nll = f1(args);
    Ok(PerlValue::float(nll / 2f64.ln()))
}

// Character n-gram count: (L - n + 1)
fn builtin_nlp_char_ngram_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let len = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    Ok(PerlValue::integer((len - n + 1).max(0)))
}

// Word n-gram count
fn builtin_nlp_word_ngram_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_char_ngram_count(args)
}

// Skip-gram count: (L - n) C(n+k, k)
fn builtin_nlp_skip_gram_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let k = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0);
    let mut bin = 1_i64;
    for i in 0..k.min(50) { bin = bin.saturating_mul(n + i + 1) / (i + 1).max(1); }
    Ok(PerlValue::integer((l - n).max(0).saturating_mul(bin)))
}

// BPE merge step (simulated count of merges)
fn builtin_nlp_byte_pair_merge_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

// WordPiece score: P(w|context) / P(token1)·P(token2)
fn builtin_nlp_wordpiece_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_w = f1(args);
    let p_split = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if p_split == 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(p_w / p_split))
}

// Unigram LM score
fn builtin_nlp_unigram_lm_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let probs = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(probs.iter().filter(|&&p| p > 0.0).map(|p| p.ln()).sum()))
}

// Kneser-Ney step: P_KN(w|context) = max(c(context, w) - δ, 0) / Σ + λ(context) P_KN(w | shorter)
fn builtin_nlp_kneser_ney_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args);
    let delta = args.get(1).map(|v| v.to_number()).unwrap_or(0.75);
    let total = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((c - delta).max(0.0) / total))
}

// Witten-Bell smoothing step
fn builtin_nlp_witten_bell_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let unique = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    if total + unique == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(c / (total + unique)))
}

// Good-Turing count: c* = (c+1)·N_{c+1}/N_c
fn builtin_nlp_good_turing_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args);
    let n_c = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let n_c_plus_1 = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    if n_c == 0.0 { return Ok(PerlValue::float(c)); }
    Ok(PerlValue::float((c + 1.0) * n_c_plus_1 / n_c))
}

// Laplace smoothing: (c+1)/(N+V)
fn builtin_nlp_laplace_smoothing(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let v = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    if n + v == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((c + 1.0) / (n + v)))
}

// Lidstone smoothing: (c+λ)/(N+λV)
fn builtin_nlp_lidstone_smoothing(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let v = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    let lambda = args.get(3).map(|v| v.to_number()).unwrap_or(0.5);
    if n + lambda * v == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((c + lambda) / (n + lambda * v)))
}

// Jelinek-Mercer interpolation: λ P_higher + (1 - λ) P_lower
fn builtin_nlp_jelinek_mercer(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_high = f1(args);
    let p_low = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let lambda = args.get(2).map(|v| v.to_number()).unwrap_or(0.5);
    Ok(PerlValue::float(lambda * p_high + (1.0 - lambda) * p_low))
}

// Dirichlet smoothing: (c + μ P_collection) / (N + μ)
fn builtin_nlp_dirichlet_smoothing(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let mu = args.get(2).map(|v| v.to_number()).unwrap_or(2000.0);
    let p_coll = args.get(3).map(|v| v.to_number()).unwrap_or(1e-6);
    if n + mu == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((c + mu * p_coll) / (n + mu)))
}

// Query likelihood: P(Q|D) = Π P(q_i|D)
fn builtin_nlp_query_likelihood_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_q = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(p_q.iter().filter(|&&p| p > 0.0).map(|p| p.ln()).sum()))
}

// KL between language models
fn builtin_nlp_kl_lm_div(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
...
fn builtin_nlp_pmi_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_xy = f1(args);
    let p_x = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let p_y = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    if p_x * p_y == 0.0 || p_xy <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((p_xy / (p_x * p_y)).log2()))
}

// Normalized PMI
fn builtin_nlp_npmi_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pmi = f1(args);
    let p_xy = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if p_xy <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(pmi / (-p_xy.log2())))
}

// χ² collocation test
fn builtin_nlp_chi2_collocation(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let observed = f1(args);
    let expected = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if expected == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((observed - expected).powi(2) / expected))
}

// Log-likelihood collocation = 2·Σ obs · ln(obs/exp)
fn builtin_nlp_loglikelihood_collocation(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let obs = f1(args);
    let exp_v = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if exp_v <= 0.0 || obs <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(2.0 * obs * (obs / exp_v).ln()))
}

// t-score collocation = (mean - μ) / σ
fn builtin_nlp_t_score_collocation(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let obs = f1(args);
    let exp_v = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if obs <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((obs - exp_v) / obs.sqrt()))
}

// Dunning log-likelihood (alias)
fn builtin_nlp_dunning_log_likelihood(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_loglikelihood_collocation(args)
}

// LDA α update step
fn builtin_nlp_lda_alpha_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_nlp_lda_topic_dist(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n_zd = f1(args);
...
fn builtin_nlp_word2vec_skipgram_loss(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dot = f1(args);
...
fn builtin_nlp_glove_loss_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dot_b = f1(args);
    let log_x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let f_x = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(f_x * (dot_b - log_x).powi(2)))
}

// FastText subword count: L - n + 1 + (L - n + 1 if both ends bounded)
fn builtin_nlp_fasttext_subword_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(3);
    Ok(PerlValue::integer((l + 2 - n + 1).max(0)))
}

// Byte-level BPE step
fn builtin_nlp_byte_level_bpe_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_byte_pair_merge_step(args)
}

// SentencePiece score (likelihood of segmentation)
fn builtin_nlp_sentencepiece_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_unigram_lm_score(args)
}

// Unigram subword loss
fn builtin_nlp_unigram_subword_loss(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_unigram_lm_score(args)
}

// Subword regularization: sample from top-k segmentations
fn builtin_nlp_subword_regularization(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args);
    let alpha = args.get(1).map(|v| v.to_number()).unwrap_or(0.5);
    Ok(PerlValue::float(alpha * n as f64))
}

// Pointwise attention score: q · k / √d (no batch)
fn builtin_nlp_pointwise_attn_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let q_dot_k = f1(args);
...
fn builtin_nlp_relative_position_bias(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist = f1(args);
    let max_dist = args.get(1).map(|v| v.to_number()).unwrap_or(128.0);
    if max_dist == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(-dist.abs() / max_dist))
}

// ALiBi position bias = -|i - j|·m
fn builtin_nlp_alibi_position_bias(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist = f1(args);
    let m = args.get(1).map(|v| v.to_number()).unwrap_or(0.0625);
    Ok(PerlValue::float(-dist.abs() * m))
}

// Rotary position angle θ_i = pos / 10000^(2i/d)
fn builtin_nlp_rope_rotary_angle(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pos = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let d = args.get(2).map(|v| v.to_number()).unwrap_or(64.0);
    if d == 0.0 { return Ok(PerlValue::float(pos)); }
    Ok(PerlValue::float(pos / 10000f64.powf(2.0 * i / d)))
}

// RoPE apply: rotate (x, y) by θ
fn builtin_nlp_rope_apply_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let theta = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(x * theta.cos() - y * theta.sin()))
}

// Positional encoding sin: sin(pos/10000^(2i/d))
fn builtin_nlp_position_encoding_sin(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pos = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let d = args.get(2).map(|v| v.to_number()).unwrap_or(64.0);
    if d == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((pos / 10000f64.powf(2.0 * i / d)).sin()))
}

// Positional encoding cos
fn builtin_nlp_position_encoding_cos(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pos = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let d = args.get(2).map(|v| v.to_number()).unwrap_or(64.0);
    if d == 0.0 { return Ok(PerlValue::float(1.0)); }
    Ok(PerlValue::float((pos / 10000f64.powf(2.0 * i / d)).cos()))
}

// PE frequency band: 1 / 10000^(2i/d)
fn builtin_nlp_pe_freq_band(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = f1(args);
    let d = args.get(1).map(|v| v.to_number()).unwrap_or(64.0);
    if d == 0.0 { return Ok(PerlValue::float(1.0)); }
    Ok(PerlValue::float(1.0 / 10000f64.powf(2.0 * i / d)))
}

// Max sequence length check
fn builtin_nlp_max_seq_len_check(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let len = f1(args);
    let max = args.get(1).map(|v| v.to_number()).unwrap_or(2048.0);
    Ok(PerlValue::integer(if len <= max { 1 } else { 0 }))
}

// Token drop rate
fn builtin_nlp_token_drop_rate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dropped = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(PerlValue::float(0.0)); }
...
// Punctuation ratio
fn builtin_nlp_punct_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let punct = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if total == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(punct / total))
}

// Uppercase ratio
fn builtin_nlp_uppercase_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_punct_ratio(args)
}

// Digit ratio
fn builtin_nlp_digit_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_punct_ratio(args)
}

// Emoji ratio
fn builtin_nlp_emoji_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_punct_ratio(args)
}
...
fn builtin_nlp_token_overlap_two(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_jaccard_similarity_two(args)
}

// Word Mover's distance step
fn builtin_nlp_word_mover_dist(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(v.iter().sum()))
}

// SIF weight a / (a + p(w))
fn builtin_nlp_sif_weight_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_w = f1(args);
    let a = args.get(1).map(|v| v.to_number()).unwrap_or(1e-3);
    if a + p_w == 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(a / (a + p_w)))
}

// Doc embedding average
fn builtin_nlp_doc_embedding_avg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_nlp_attention_pool_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(alpha * x))
}

// Max pool step
fn builtin_nlp_max_pool_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

// Avg pool step
fn builtin_nlp_avg_pool_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_doc_embedding_avg(args)
}

// Sum pool step
fn builtin_nlp_sum_pool_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b46_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(v.iter().sum()))
}

// Self-attention compute step
fn builtin_nlp_self_attn_compute_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_pointwise_attn_score(args)
}

// Cross attention compute step
fn builtin_nlp_cross_attn_compute_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_nlp_pointwise_attn_score(args)
}

// Window attention step (within radius w)
fn builtin_nlp_window_attn_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist = f1(args);
    let w = args.get(1).map(|v| v.to_number()).unwrap_or(64.0);
    let score = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(if dist.abs() <= w { score } else { f64::NEG_INFINITY }))
}

// Strided attention step
fn builtin_nlp_strided_attn_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist = i1(args);
    let stride = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let score = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    if stride == 0 { return Ok(PerlValue::float(score)); }
    Ok(PerlValue::float(if dist % stride == 0 { score } else { f64::NEG_INFINITY }))
}

// Block attention step (block-diagonal mask)
fn builtin_nlp_block_attn_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let block_i = i1(args);
...
fn builtin_nlp_dilated_attn_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist = i1(args);
    let dilation = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    let score = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    if dilation == 0 { return Ok(PerlValue::float(score)); }
...
fn builtin_nlp_sparse_attn_score(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mask = i1(args);

...
fn b80_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// `qubit_x` — Pauli-X (NOT) gate: |0⟩↔|1⟩. Returns flipped amplitude.
fn builtin_qubit_x(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let want_alpha = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::float(if want_alpha == 0 { beta } else { alpha }))
}

/// `qubit_y` — Pauli-Y gate. Apply: α' = -i β, β' = i α. Returns magnitude.
fn builtin_qubit_y(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((alpha * alpha + beta * beta).sqrt()))
}

/// `qubit_z` — Pauli-Z phase flip: |1⟩ → −|1⟩.
fn builtin_qubit_z(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let which = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::float(if which == 0 { alpha } else { -beta }))
}

/// `qubit_h` — Hadamard: H|0⟩ = (|0⟩+|1⟩)/√2, H|1⟩ = (|0⟩−|1⟩)/√2.
fn builtin_qubit_h(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let want_alpha = args.get(2).map(|v| v.to_number() as i64).unwrap_or(0);
    let inv_sqrt2 = 1.0 / 2_f64.sqrt();
    Ok(PerlValue::float(inv_sqrt2 * if want_alpha == 0 { alpha + beta } else { alpha - beta }))
}

/// `qubit_s` — phase gate S: |1⟩ → i|1⟩.
fn builtin_qubit_s(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let beta = f1(args);
    Ok(PerlValue::float(beta))
}

/// `qubit_t` — π/8 gate T: |1⟩ → e^{iπ/4}|1⟩. Returns real part.
fn builtin_qubit_t(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let beta = f1(args);
    Ok(PerlValue::float(beta * (std::f64::consts::PI / 4.0).cos()))
}

/// `qubit_rx` — rotation about X: cos(θ/2) I − i sin(θ/2) X.
fn builtin_qubit_rx(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let theta = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let c = (theta / 2.0).cos();
    let s = (theta / 2.0).sin();
    Ok(PerlValue::float(c * alpha + s * beta))
}

/// `qubit_ry` — rotation about Y.
fn builtin_qubit_ry(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let theta = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let c = (theta / 2.0).cos();
...
fn builtin_qubit_rz(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let theta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(alpha * (theta / 2.0).cos()))
}

/// `qubit_u3` — universal single-qubit gate U3(θ, φ, λ).
fn builtin_qubit_u3(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let theta = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    ...
    let c = (theta / 2.0).cos();
    let s = (theta / 2.0).sin();
    Ok(PerlValue::float(c * alpha - s * beta * (phi + lambda).cos()))
}

/// `qubit_u2` — U2(φ, λ) = U3(π/2, φ, λ).
fn builtin_qubit_u2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let inv_sqrt2 = 1.0 / 2_f64.sqrt();
    Ok(PerlValue::float(inv_sqrt2 * (alpha - beta)))
}

/// `qubit_u1` — phase gate U1(λ) = diag(1, e^{iλ}).
fn builtin_qubit_u1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let beta = f1(args);
    let lambda = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(beta * lambda.cos()))
}
...
fn builtin_qubit_reset(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p1 = f1(args).clamp(0.0, 1.0);
    Ok(PerlValue::float((1.0 - p1) + p1))
}

/// `bell_state` — return amplitude of basis state |b₀b₁⟩ in Bell pair index k
/// (k = 0..3). Args: k (0=Φ⁺, 1=Φ⁻, 2=Ψ⁺, 3=Ψ⁻), b0, b1.
fn builtin_bell_state(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(amp))
}

/// `ghz_state` — amplitude of basis state |b₀…b_{N-1}⟩ in N-qubit GHZ:
/// (|0…0⟩ + |1…1⟩)/√2. Args: bit-array.
fn builtin_ghz_state(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bits = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    if bits.is_empty() { return Ok(PerlValue::float(0.0)); }
    let all_zero = bits.iter().all(|b| b.to_number() == 0.0);
    let all_one = bits.iter().all(|b| b.to_number() != 0.0);
    let inv_sqrt2 = 1.0 / 2_f64.sqrt();
    Ok(PerlValue::float(if all_zero || all_one { inv_sqrt2 } else { 0.0 }))
}

/// `w_state` — amplitude of |b₀…b_{N-1}⟩ in W: 1/√N if exactly one bit set.
fn builtin_w_state(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bits = arg_to_vec(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = bits.len();
    if n == 0 { return Ok(PerlValue::float(0.0)); }
    let ones = bits.iter().filter(|b| b.to_number() != 0.0).count();
    if ones == 1 { Ok(PerlValue::float(1.0 / (n as f64).sqrt())) }
    else { Ok(PerlValue::float(0.0)) }
}

/// `qft` — Quantum Fourier Transform amplitude on basis state |k⟩ at output |j⟩.
fn builtin_qft(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = i1(args);
    ...
    Ok(PerlValue::float(phi.cos() / n.sqrt()))
}

/// `inverse_qft` — adjoint of QFT.
fn builtin_inverse_qft(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = i1(args);
    let j = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    let n = args.get(2).map(|v| v.to_number() as i64).unwrap_or(2).max(2) as f64;
    let phi = -2.0 * std::f64::consts::PI * (j as f64) * (k as f64) / n;
...
fn builtin_grover_iter(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = f1(args);
    let m = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::integer((std::f64::consts::PI / 4.0 * (n / m).sqrt()).floor() as i64))
}

/// `shor_period` — period r given quantum measurement outcome m, N.
fn builtin_shor_period(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m = i1(args).max(1);
    let q = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1).max(1);
    Ok(PerlValue::float(q as f64 / m as f64))
}

/// `vqe_step` — variational quantum eigensolver expectation ⟨ψ(θ)|H|ψ(θ)⟩.
fn builtin_vqe_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pauli_terms = b80_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let coefs = args.get(1).map(b80_to_floats).unwrap_or_default();
    let n = pauli_terms.len().min(coefs.len());
    Ok(PerlValue::float((0..n).map(|i| pauli_terms[i] * coefs[i]).sum()))
}

/// `qaoa_step` — QAOA cost: ⟨ψ|C|ψ⟩ at angles (γ, β).
fn builtin_qaoa_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(cost_expectation * gamma.cos() * beta.cos()))
}

/// `qpe_iteration` — quantum phase estimation k-th controlled phase.
fn builtin_qpe_iteration(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let phase = f1(args);
    let k = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((2_f64.powf(k) * phase * 2.0 * std::f64::consts::PI).cos()))
}

/// `pauli_string_expect` — ⟨ψ|σ_a ⊗ σ_b ⊗ ...|ψ⟩ approximate expectation.
fn builtin_pauli_string_expect(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let amps = b80_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_circuit_depth(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n_gates = i1(args).max(0);
    let width = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1).max(1);
    Ok(PerlValue::integer((n_gates + width - 1) / width))
}

/// `circuit_width` — number of qubits (rows in circuit diagram).
fn builtin_circuit_width(args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::integer(i1(args).max(1)))
}

/// `gate_decompose` — single-qubit decomposition: U = e^{iα} R_z(β) R_y(γ) R_z(δ).
fn builtin_gate_decompose(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let gamma = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let delta = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(alpha + beta + gamma + delta))
}

/// `ancilla_alloc` — ancilla qubit allocation: returns next free index.
fn builtin_ancilla_alloc(args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::integer(i1(args) + 1))
}

/// `bloch_sphere_x` — Bloch x-coordinate: 2 Re(α* β).
fn builtin_bloch_sphere_x(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(2.0 * alpha * beta))
}

/// `bloch_sphere_z` — Bloch z = |α|² − |β|².
fn builtin_bloch_sphere_z(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(alpha * alpha - beta * beta))
}

/// `density_matrix_purity_q` — Tr(ρ²) for quantum state with eigenvalues λ_i;
/// 1.0 for pure state, 1/d for maximally mixed.
fn builtin_density_matrix_purity_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lambdas = b80_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(lambdas.iter().map(|x| x * x).sum::<f64>()))
}

/// `entanglement_entropy` — von Neumann S(ρ_A) = −Σ λ_i log λ_i.
fn builtin_entanglement_entropy(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lambdas = b80_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_quantum_teleportation(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m1 = i1(args);
    let m2 = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::float(if (m1 == 0) ^ (m2 == 0) { -1.0 } else { 1.0 }))
}
...
fn builtin_superdense_coding(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = i1(args).clamp(0, 3);

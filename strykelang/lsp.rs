...
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let md: &'static str = match key {
        ...
        "_<" => "`_<FILENAME` — the per-file source line array used by the debugger / profiler. For every source file Perl / stryke loads, the global variable `@{\"_<$filename\"}` holds one element per source line (line N at index N; index 0 is unused). The hash `%{\"_<$filename\"}` carries metadata. Debugger / profiler tooling reads these to display source context next to breakpoints, stack frames, and hot lines.\n\nNot a variable you'd write by hand — the syntax `_<` is just the prefix used to construct the symbol-table key. Use the stryke debugger or profiler builtins to interact with this state.\n\n```perl\nmy @lines = @{ \"main::_<$0\" }\np \"source has \" . scalar(@lines) . \" lines\"\n```\n\nSee also: stryke debugger (`zdb`, `--debug`), profiler (`--flame`, `zpwrFlame`).",

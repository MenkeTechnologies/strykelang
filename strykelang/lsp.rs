...
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let presigil: Option<&'static str> = match key {
        ...
        "$" => Some("**Scalar sigil.** `$name` accesses a scalar variable. `$ref` holds references. `$h{key}` accesses one hash value; `$a[idx]` accesses one array element (sigil reflects the *value being accessed*, not the container). For postfix-deref: `$ref->$*` (in stryke contexts that accept it).\n\n```perl\nmy $x = 42                    # scalar declaration\nmy @arr = (1, 2, 3)\np $arr[0]                     # 1 — scalar context on @arr\nmy %h = (k => \"v\")\np $h{k}                       # one value\nmy $ref = \\@arr               # reference, scalar holds it\n```\n\nSee also: `@` (array sigil), `%` (hash sigil), `\\` (reference), `->`."),
    ...
    let md: &'static str = match key {
        ...
        "_0" => "`_0` (or `$_0`) — the current value being threaded through a `|>` pipeline, equivalent to `$_` and bare `_`. Use the explicit `0` form for clarity when reading code that also references positional 1, 2, 3, … via `_1`, `_2`, ….\n\n```perl\n1:5 |> map _ * 2     |> e p       # implicit topic\n1:5 |> map _0 * 2    |> e p       # explicit positional 0 — same meaning\n```\n\nSee also: `$_`, `_`, `_1`..`_4`, `_<N` (indexed ascent past depth 2).",
        "_1" => "`_1` (or `$_1`) — the **first additional** positional bound in a `|>` pipeline stage. When a HOF takes a block plus extra arguments, the extras are bound as `_1`, `_2`, …; the current value stays in `_` / `_0` / `$_`. This is what makes EXPR-form HOFs (`grep EXPR, LIST`) and brace-form HOFs (`grep { BLOCK } LIST`) share the same scope binding.\n\n```perl\n# grep EXPR, LIST — $_ is each element, $_1 is the predicate coderef\nmy $pred = fn ($x) { $x > 10 }\nmy @big = grep _1, $pred, (5, 12, 8, 20)\n```\n\nSee also: `_0`, `_2`..`_4`, `_<N` (indexed ascent).",
        "_2" => "`_2` (or `$_2`) — the second additional positional in a `|>` pipeline stage. See `_1` for full semantics.",
        "_3" => "`_3` (or `$_3`) — the third additional positional in a `|>` pipeline stage. See `_1` for full semantics.",
        "_4" => "`_4` (or `$_4`) — the fourth additional positional in a `|>` pipeline stage. See `_1` for full semantics. Past depth 2 also consider the indexed-ascent shortcut `_<N`.",
        "_<" | "$_<" | "_0<" | "$_0<" | "_<1" | "$_<1" | "_0<1" | "$_0<1" => "**Topic ascent, 1 frame up.** Eight equivalent spellings of the same value — the topic from the *enclosing* block one frame out.\n\n```text\nspellings (all identical): _<  $_<  _0<  $_0<  _<1  $_<1  _0<1  $_0<1\n```\n\nEvery `|>` stage, `map`/`grep`/`fore` block, and `sort` block opens a new topic frame. Inside that frame the *current* topic is `_`/`$_`/`_0`/`$_0`; one chevron `<` walks up one frame to the surrounding block's topic. Indexed form `_<N` is sugar for `N` chevrons — `_<1` ≡ `_<`, more readable as ascent depth grows. Runtime cap: 5 frames (`parser.rs:1473`).\n\nAt the outermost iter (no enclosing topic frame), `_<` is **undef** — strict ascent semantics, no fallback to current `_`. If you need the current frame's topic, write `_` directly.\n\n```perl\n@rows |> map {\n    my @cells = @$_                # $_ is current row\n    @cells |> map { uc($_) . \":\" . len($_<) }\n    #                ^^ inner topic   ^^^ outer row arrayref\n}\n```\n\nThe four-spelling matrix lets you choose readability per context: bare `_<` in pipe chains, sigiled `$_<` when interpolating, slot-explicit `_0<` when slot 1, 2, … positionals are also in scope.\n\nSee also: `_<<` (2 up), `_<<<` (3 up), `_<<<<` (4 up), `_<<<<<` (5 up), `_<N`/`_0<N` (indexed ascent), `_`/`$_`/`_0`/`$_0` (current frame).",
        ...
        "+" => "**Addition / numeric coercion.** `$a + $b` adds two numbers. Operands are coerced to numeric context — strings parse as `\\d+(\\.\\d+)?` until a non-numeric character, so `\"5 apples\" + 3 == 8`. Use `.` (dot) for string concatenation; using `+` on intended-strings is a common bug since the strings parse to 0 silently under `no warnings`. As a unary prefix, `+EXPR` forces numeric context without changing value.\n\n```perl\np 2 + 3                # 5\np \"10x\" + 5            # 15 — leading digits parse\np \"x10\" + 5            # 5  — leading non-digit → 0\nmy $sum = sum(1:100)   # 5050\n```\n\nSee also: `-`, `*`, `/`, `+=` (compound assign), `.` (string concat).",
        "-" => "**Subtraction / unary minus / file-test operator.** Binary: `$a - $b` subtracts numbers (same coercion rules as `+`). Unary: `-EXPR` negates. Special: `-X FILE` where `X` is a single letter is a file-test operator (`-e` exists, `-f` regular file, `-d` directory, `-r` readable, `-w` writable, `-x` executable, `-s` size in bytes, `-M` age in days, etc.).\n\n```perl\np 10 - 3              # 7\np -5 * 2               # -10\nif (-e $path) { p \"exists\" }\nif (-f $path && -r _) { p \"readable file\" }   # _ reuses last stat\n```\n\nSee also: `+`, `-=`, file-test reference.",
        "*" => "**Multiplication / glob deref.** Binary: `$a * $b` multiplies numbers. In expression context (`*name`), references the typeglob for that name — rarely used directly; assigning `*alias = \\&func` installs an alias for the sub.\n\n```perl\np 6 * 7              # 42\nmy $area = $w * $h\n*shortcut = \\&long_function_name   # sub alias\n```\n\nSee also: `*=`, `**` (exponent), `x` (string/list repeat — NOT multiplication).",
        "/" => "**Division / regex delimiter.** Binary: `$a / $b` divides as floats. Stryke uses true division — `5 / 2 == 2.5`, not 2. Use `int($a / $b)` or `intdiv($a, $b)` for integer division. In expression context, `/PATTERN/` opens a regex — context-dependent: after an operator or at statement start, `/` parses as regex; after a variable or value, it's division.\n\n```perl\np 10 / 4               # 2.5\np int(10 / 4)          # 2\nif ($s =~ /^\\d+$/) { p \"all digits\" }\n```\n\nSee also: `*`, `%` (modulo), `/=`, regex syntax, `intdiv`.",
        "%" => "**Modulo / hash sigil.** Binary: `$a % $b` returns the remainder of integer division. Result has the sign of the right operand in Perl semantics. As a sigil `%name` accesses the entire hash; `$h{k}` accesses one value.\n\n```perl\np 10 % 3               # 1\np -7 % 3               # 2 (Perl semantics: sign matches RHS)\nfor (1:20) { p _ if _ % 2 == 0 }   # even numbers\nmy %h = (a => 1, b => 2)\np keys %h\n```\n\nSee also: `%=`, hash declarations.",
        "**" => "**Exponentiation.** `$a ** $b` raises `$a` to the `$b`-th power. Floating-point throughout — `2 ** 10 == 1024`, `2 ** 0.5 == 1.414...`. Right-associative: `2 ** 3 ** 2 == 2 ** 9 == 512`. Negative bases with fractional exponents return NaN.\n\n```perl\np 2 ** 10              # 1024\np 2 ** 0.5             # 1.4142135623731\np 2 ** 3 ** 2          # 512 (right-assoc)\nmy $sq = $r ** 2\n```\n\nSee also: `**=`, `sqrt`, `exp`, `log`.",
        ...
        "." => "**String concatenation.** `$a . $b` glues two strings together. Both operands are coerced to string context. For multi-arg joins prefer `join \"\", @parts` (faster, less typing); for building large strings prefer interpolation or `sprintf`. The `.` operator does **not** insert separators — `\"hello\" . \"world\" eq \"helloworld\"`.\n\n```perl\nmy $name = \"world\"\np \"hello \" . $name . \"!\"        # hello world!\nmy $full = $first . \" \" . $last\nmy $line = sprintf \"%-20s %d\", $name, $count   # often clearer\n```\n\nSee also: `.=` (append assign), `x` (repeat), `join`, `sprintf`, interpolation.",
        ...
        "==" => "**Numeric equality.** Coerces both operands to numbers and compares. Use `eq` for strings — `\"10\" == \"10.0\"` is true, but `\"10\" eq \"10.0\"` is false. Float comparison with `==` is dangerous due to rounding; for tolerances use `abs($a - $b) < $epsilon`.\n\n```perl\np 5 == 5.0                # 1\np \"10\" == 10              # 1 (numeric coercion)\np \"foo\" == \"bar\"          # 1 (both → 0)  — common bug, use eq!\nif ($n == 0) { ... }\n```\n\nSee also: `!=`, `<=>`, `eq` (string equality), `near_eq` builtin for float tolerance.",
        ...
        "xor" => "**Low-precedence logical XOR.** Returns true exactly when one operand is truthy and the other is falsy. No short-circuit (both sides always evaluated). Rarely used; if you find yourself wanting `xor` consider `($a ? 1 : 0) != ($b ? 1 : 0)` for clarity.\n\n```perl\nif ($admin xor $guest) { p \"exactly one role\" }\n```\n\nSee also: `&&`, `||`, `^` (bitwise xor).",
        ...
        // ── Bitwise ──
        "&" => "**Bitwise AND** (and prototype/sub-deref). On numbers, `$a & $b` computes the bitwise AND. As a sigil prefix `&name` calls a sub (in non-bareword contexts) or takes a coderef. With `\\` makes a coderef: `\\&sub`.\n\n```perl\np 0b1100 & 0b1010            # 8 (0b1000)\nmy $mask = $flags & 0xFF\nmy $code = \\&my_sub          # coderef\n```\n\nSee also: `|`, `^`, `~`, `&&`, `\\&` (coderef).",
        "|" => "**Bitwise OR.** `$a | $b` ORs the bits. Also a regex alternation inside `/.../`. As a stream operator `||` is logical OR, `|>` is pipe-forward.\n\n```perl\np 0b1100 | 0b1010           # 14 (0b1110)\nmy $flags = OPEN_READ | OPEN_NONBLOCK\n```\n\nSee also: `&`, `^`, `~`, `||`, `|>`, regex alternation.",
        "^" => "**Bitwise XOR.** `$a ^ $b` XORs the bits.\n\n```perl\np 0b1100 ^ 0b1010           # 6 (0b0110)\nmy $toggled = $bits ^ $mask\n```\n\nSee also: `&`, `|`, `~`, `xor` (logical).",
        ...
        "<<" => "**Left bit-shift / heredoc.** As `$a << $b`, shifts bits left by `$b` positions. As `<<TAG`, opens a heredoc (multi-line string literal terminated by `TAG` at start of line).\n\n```perl\np 1 << 10                    # 1024\nmy $kb = $bytes << 10\nmy $sql = <<END\n    SELECT *\n    FROM users\nEND\n```\n\nSee also: `>>`, `<<=`, heredoc syntax.",
        ">>" => "**Right bit-shift / file redirect.** As `$a >> $b`, shifts bits right by `$b` positions. As a mode in `open`, `>>` opens for append.\n\n```perl\np 1024 >> 2                  # 256\nmy $exit_code = $? >> 8       # extract exit code from $?\nopen my $fh, '>>', 'log.txt' or die\n```\n\nSee also: `<<`, `>>=`, `open` modes.",
        ...
        // ── Assignment family ──
        "=" => "**Assignment.** Returns the assigned value (so chained `$a = $b = 5` works). Context matters: `my @a = $scalar` copies the scalar's value into a one-element list; `my @a = @b` does list copy; `my $s = @a` gives the array's length in scalar context. The single most-used operator after `;`.\n\n```perl\nmy $x = 42\nmy ($a, $b, $c) = (1, 2, 3)        # list-destructure\nmy ($first, @rest) = split /,/\n($a, $b) = ($b, $a)                  # swap\nmy $len = @items                     # scalar context — count\n```\n\nSee also: `+=`, `-=`, `.=`, `//=`, `||=`, `&&=`, `my`, `our`, `local`, `state`.",
        ...
        "ai_compare" => "`ai_compare($a, $b, criteria => \"factual accuracy\", scale => 5)` → `+{ winner, reason, scores => +{a, b}, raw }`. Single-call structured comparison. `winner` is `\"a\"`, `\"b\"`, or `\"tie\"`.",

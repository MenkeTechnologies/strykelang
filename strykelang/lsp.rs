...
fn doc_for_label_text(label: &str) -> Option<&'static str> {
    ...
    let md: &'static str = match key {
        ...
        "ls" => "`ls` / `ls(DIR)` — long directory listing in the style of `ls -l`. With no arguments, lists the process current working directory (not `$_`). With one or more directory paths, lists each directory; if more than one path is given, each block is prefixed with `DIR:` like GNU `ls`. Returns a single multiline string (suitable for `p` or `spurt`). On Unix, includes a `total` line (1024-byte block units), mode bits, link count, numeric uid/gid, size, mtime, and name; symbolic links show `-> target`. On Windows, lines use file attributes, size, and a simplified timestamp.\n\n```perl\np ls()           # cwd\np ls(\"/tmp\")\n```",

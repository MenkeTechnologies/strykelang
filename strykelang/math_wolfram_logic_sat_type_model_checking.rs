...
fn builtin_cnf_unit_propagate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// 0 between clauses.
fn builtin_cnf_pure_literal_elim(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_cnf_dpll_branch(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lits = b67_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_dpll_clause_learning(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_two_watched_literals(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let clause = b67_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let assign = b67_to_floats(args.get(1).unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_walksat_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_resolution_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_subsumption_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = b67_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let b = b67_to_floats(args.get(1).unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_tableau_branch_close(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lits = b67_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_sequent_left_intro(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let id = i1(args);
    Ok(StrykeValue::integer(if id > 0 { 2 } else { 0 }))
}

/// Sequent-calculus right intro: introduces disjunction on the right.
fn builtin_sequent_right_intro(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let id = i1(args);
    Ok(StrykeValue::integer(if id > 0 { 2 } else { 0 }))
}
...
fn builtin_nbe_normalize(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let head = i1(args);
...
/// (= n). Returns n.
fn builtin_church_numeral_n(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(i1(args).max(0)))
}

/// Encode pair: ⟨a, b⟩ → λp. p a b. Returns Cantor pairing (a+b)(a+b+1)/2 + b
/// as a numeric proxy.
fn builtin_encode_pair(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = i1(args);
...
fn builtin_encode_succ(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_simply_typed_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = i1(args);
...
fn builtin_hindley_milner_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// [t1, t2] vectors.
fn builtin_unification_robinson(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_bdd_apply(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a_size = i1(args);
...
fn builtin_bdd_restrict(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let size = i1(args);
    Ok(StrykeValue::integer((size / 2).max(1)))
}

/// BDD quantification: ∃ x_i. f = restrict(f, 0) ∨ restrict(f, 1).
fn builtin_bdd_quantify(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let size = i1(args);
    Ok(StrykeValue::integer(size))
}

/// AIG simplify step: count of nodes after constant propagation.
fn builtin_aig_simplify_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let nodes = i1(args);
...
fn builtin_smt_qf_lia_solve_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_smt_qf_uf_combine(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let eqs = i1(args);
...
fn builtin_model_checking_ctl(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p_holds = i1(args);
...
fn builtin_model_checking_ltl(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_bisimulation_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_coq_tactic_apply(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_coq_unify_term(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_unification_robinson(args)
}

/// Reflexivity check: a = a.
fn builtin_refl_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(a);
    Ok(StrykeValue::integer(if a == b { 1 } else { 0 }))
}
...
/// Symmetry: a = b ↔ b = a.
fn builtin_sym_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(a);
    Ok(StrykeValue::integer(if a == b { 1 } else { 0 }))
}
...
fn builtin_trans_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = i1(args);
    let b = args.get(1).map(|v| v.to_number() as i64).unwrap_or(a);

...
fn b56_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// Elo expected score: E = 1 / (1 + 10^((Rb − Ra) / 400)).
fn builtin_elo_expected(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r_a = f1(args);
    let r_b = args.get(1).map(|v| v.to_number()).unwrap_or(r_a);
    Ok(PerlValue::float(1.0 / (1.0 + 10f64.powf((r_b - r_a) / 400.0))))
}

/// Elo update: Ra' = Ra + K (S − E), where S ∈ {0, 0.5, 1}.
fn builtin_elo_update(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r_a = f1(args);
    let r_b = args.get(1).map(|v| v.to_number()).unwrap_or(r_a);
...
fn builtin_glicko_rating(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
    ...
    Ok(PerlValue::float(new_r))
}

/// TrueSkill rating update (1v1, no draw). μ' = μ + σ²/c · v(t, ε).
fn builtin_trueskill_update(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mu = f1(args);
    ...
    Ok(PerlValue::float(mu + sigma * sigma / c * v))
}

/// TrueSkill match quality q ≈ 2β / sqrt(c²) · exp(−(μ_a − μ_b)² / (2c²)).
fn builtin_trueskill_match_quality(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mu_a = f1(args);
    ...
    Ok(PerlValue::float(prefac * kernel))
}

/// Pythagorean expectation: W% = R^x / (R^x + RA^x). x = 1.83 for MLB.
fn builtin_pythagorean_expectation(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(rf / (rf + ra)))
}

/// WAR scaffold: WAR = (player_runs - replacement_runs) / runs_per_win.
fn builtin_war_above_replacement(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float((player_runs - repl_runs) / runs_per_win))
}
...
fn builtin_woba_weight(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b56_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_wrc_plus(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(100.0 * (woba / lg_woba) / park_factor))
}

/// OPS+ = 100·(OBP/lgOBP + SLG/lgSLG − 1) / parkFactor.
fn builtin_ops_plus(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let obp = f1(args);
    ...
    Ok(PerlValue::float(100.0 * (obp / lg_obp + slg / lg_slg - 1.0) / pf))
}

/// ERA+ = 100·lgERA / (ERA · parkFactor).
fn builtin_era_plus(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(100.0 * lg_era / (era * pf)))
}

/// FIP = ((13·HR + 3·(BB+HBP) − 2·K) / IP) + lgFIPconst. Constants per FanGraphs.
fn builtin_fip(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let hr = f1(args);
    ...
    Ok(PerlValue::float((13.0 * hr + 3.0 * (bb + hbp) - 2.0 * k) / ip + fip_const))
}

/// xFIP: same as FIP but normalises HR rate to league-average HR/FB ratio.
/// Args: FB, BB, HBP, K, IP, lgHR/FB, fipConst.
fn builtin_xfip(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let fb = f1(args);
    let bb = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let hbp = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let k = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_goals_above_avg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(player_goals - exp_share * team_goals))
}

/// Tackle efficiency: tackles / total_tackle_attempts.
fn builtin_tackle_efficiency(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let made = f1(args);
    let att = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-9);
    Ok(PerlValue::float(made / att))
}
...
fn builtin_yards_per_attempt(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let yards = f1(args);
    let att = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-9);
    Ok(PerlValue::float(yards / att))
}

/// QBR (ESPN composite, simplified). 0–100 scale.
fn builtin_qbr_metric(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let raw = (comp_pct - 0.30) * 5.0
        ...
        - (int_rate - 0.05) * 25.0
        + 2.375;
    Ok(PerlValue::float((raw * 100.0 / 6.0).clamp(0.0, 100.0)))
}

/// EPA per play: average expected points added across an array of EPA values.
fn builtin_epa_per_play(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b56_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));

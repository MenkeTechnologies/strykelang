...
fn b55_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// Julian Day from proleptic Gregorian (Y, M, D, hour). Meeus §7.
fn builtin_julian_day(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = i1(args);
...
fn builtin_jd_to_calendar(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let jd = f1(args) + 0.5;
    ...
    Ok(PerlValue::float(year as f64 * 10000.0 + month as f64 * 100.0 + day))
}
...
fn builtin_ra_dec_to_alt_az(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(az_deg * 1000.0 + alt_deg))
}

/// Horizontal → equatorial inverse.
fn builtin_alt_az_to_ra_dec(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(ra * 1000.0 + dec.to_degrees()))
}
...
fn builtin_aberration_annual(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let kappa = 20.495_52;
    Ok(PerlValue::float(-kappa * (sun_lon - lambda).cos() / beta.cos()))
}

/// Apply proper motion μ_α, μ_δ (mas/yr) over Δt years to (RA, Dec) in deg.
fn builtin_proper_motion_apply(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ra = f1(args);
    let dec = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    ...
    Ok(PerlValue::float(new_ra * 1000.0 + new_dec))
}

/// Annual parallax shift: π·sin(λ_sun − λ) for object at distance d_pc.
/// Returns offset in arcseconds.
fn builtin_parallax_correction(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let d_pc = f1(args);
    let lambda_diff_deg = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if d_pc <= 0.0 { return Ok(PerlValue::float(0.0)); }
    let parallax_arcsec = 1.0 / d_pc;
    Ok(PerlValue::float(parallax_arcsec * lambda_diff_deg.to_radians().sin()))
}

/// Sun's geocentric ecliptic longitude (low precision, Meeus §25). Returns deg.
fn builtin_sun_position_low(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let jd = f1(args);
    let n = jd - B55_J2000;
    ...
    Ok(PerlValue::float(lambda.rem_euclid(360.0)))
}

/// Earth-Sun distance in AU (Meeus §25 low precision).
fn builtin_sun_distance_au(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let jd = f1(args);
    let n = jd - B55_J2000;
    let g = (357.528 + 0.985_600_28 * n).to_radians();
...
fn builtin_moon_phase_age(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let jd = f1(args);
    let known_new = 2_451_550.1; // 2000-01-06 New Moon
    Ok(PerlValue::float(((jd - known_new) % 29.530_589 + 29.530_589) % 29.530_589))
}

/// Lunation index (count of new moons since 2000-01-06).
fn builtin_lunation_index(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let jd = f1(args);
    let known_new = 2_451_550.1;
    Ok(PerlValue::integer(((jd - known_new) / 29.530_589).floor() as i64))
}
...
fn builtin_eclipse_magnitude(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let sep = f1(args).abs();
    let r_sun = args.get(1).map(|v| v.to_number()).unwrap_or(960.0);
    let r_moon = args.get(2).map(|v| v.to_number()).unwrap_or(933.0);
    if sep >= r_sun + r_moon { return Ok(PerlValue::float(0.0)); }
    if sep <= (r_sun - r_moon).abs() {
        return Ok(PerlValue::float(if r_moon >= r_sun { 1.0 } else { r_moon / r_sun }));
    }
    let mag = (r_sun + r_moon - sep) / (2.0 * r_sun);
    Ok(PerlValue::float(mag.clamp(0.0, 1.0)))
}
...
fn builtin_metonic_cycle(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let year = i1(args);
    Ok(PerlValue::integer((year - 1).rem_euclid(19) + 1))
}

/// Kepler's 3rd law: T² = a³ / M_sun  → T_years = sqrt(a_AU³ / M_solar).
fn builtin_orbit_kepler3(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a_au = f1(args).max(0.0);
    let m_sun = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-12);
    Ok(PerlValue::float((a_au.powi(3) / m_sun).sqrt()))
}

/// Orbital period in years given semi-major axis in AU.
fn builtin_orbital_period_au(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_orbit_kepler3(args)
}

/// Eccentric anomaly E from M, e via Newton-Raphson on M = E - e sin E.
fn builtin_orbit_eccentric_anomaly(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m = f1(args);
    ...
    Ok(PerlValue::float(big_e))
}

/// Escape velocity: v = sqrt(2 G M / r). Args: M_kg, r_m.
fn builtin_escape_velocity_body(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-9);
    let big_g = 6.674_30e-11;
    Ok(PerlValue::float((2.0 * big_g * m / r).sqrt()))
}

/// Hill sphere radius r_H = a(1-e)·(m/(3M))^(1/3). Args: a, e, m, M.
fn builtin_hill_sphere_radius(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
    ...
    Ok(PerlValue::float(a * (1.0 - e) * (m / (3.0 * big_m)).cbrt()))
}

/// Tisserand parameter T_J = a_J/a + 2·sqrt((a/a_J)(1-e²))·cos i.
fn builtin_tisserand_param(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
    ...
    if a <= 0.0 || a_j <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(a_j / a + 2.0 * ((a / a_j) * (1.0 - e * e)).sqrt() * i_deg.to_radians().cos()))
}
...
fn builtin_sgp4_propagate_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m0 = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let dt = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((m0 + n * dt).rem_euclid(2.0 * std::f64::consts::PI)))
}

/// Airy disk radius for first dark ring: 1.22 λ / D in radians. Args: λ_m, D_m.
fn builtin_airy_disk_radius(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lambda = f1(args);
...
fn builtin_rayleigh_criterion(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_airy_disk_radius(args)
}

/// Strehl ratio S ≈ exp(-σ²) for RMS wavefront error σ in waves.
fn builtin_strehl_ratio(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let sigma_waves = f1(args);
    let sigma_rad = 2.0 * std::f64::consts::PI * sigma_waves;
    Ok(PerlValue::float((-sigma_rad * sigma_rad).exp()))
}

/// AU → km conversion factor (used here once to ensure constant is alive).
fn builtin_au_to_km(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let _ = b55_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(B55_AU_KM))
}

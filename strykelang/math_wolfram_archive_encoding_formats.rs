...
fn builtin_tar_header_checksum(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_tar_pad_512(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer((512 - n.rem_euclid(512)).rem_euclid(512)))
}

/// TAR member record total size = 512 (header) + ⌈data/512⌉·512.
fn builtin_tar_member_record(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_zip_local_header(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_zip_central_dir(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let name_len = i1(args).max(0);
    let extra_len = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0).max(0);
...
fn builtin_zip_eocd(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_gzip_member_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_gzip_crc32_init(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::integer(0xFFFFFFFF_i64))
}

/// GZIP ISIZE field: original-size mod 2³² (little-endian).
fn builtin_gzip_isize(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_deflate_dynamic_huffman(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_deflate_static_block(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let kind = i1(args);
...
fn builtin_lz4_block_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_lz4_match_offset(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_zstd_frame_header(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_brotli_huffman_table(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_brotli_meta_block(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_lzma_range_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let range = f1(args);
...
fn builtin_quoted_printable_encode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_uuencode_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_modhex_encode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_percent_encode_full(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let b = (i1(args) & 0xFF) as u8;
...
fn builtin_punycode_encode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_idn_to_ascii(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = b62_to_bytes(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_idn_to_unicode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// as prefix*100 + size.
fn builtin_msgpack_pack_int(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_msgpack_pack_str(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let len = i1(args);
...
fn builtin_cbor_encode_uint(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args).max(0);
...
fn builtin_cbor_encode_str(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {

...
fn b72_to_floats(v: &PerlValue) -> Vec<f64> {
...
fn builtin_argpartition(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let k = args.get(1).map(|x| x.to_number() as i64).unwrap_or(0).max(0) as usize;
    if v.is_empty() { return Ok(PerlValue::integer(0)); }
    let mut idx: Vec<usize> = (0..v.len()).collect();
    idx.sort_by(|&a, &b| v[a].partial_cmp(&v[b]).unwrap_or(std::cmp::Ordering::Equal));
    Ok(PerlValue::integer(idx[k.min(v.len() - 1)] as i64))
}

/// bincount — count occurrences of each non-negative int.
fn builtin_bincount(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_nonzero_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::integer(v.iter().filter(|&&x| x != 0.0).count() as i64))
}

/// flatnonzero — first nonzero index, or -1.
fn builtin_flatnonzero(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    for (i, &x) in v.iter().enumerate() { if x != 0.0 { return Ok(PerlValue::integer(i as i64)); } }
    Ok(PerlValue::integer(-1))
}

/// searchsorted — leftmost insertion point for v in a sorted array.
fn builtin_searchsorted(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let needle = args.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    let pos = v.partition_point(|&x| x < needle);
    Ok(PerlValue::integer(pos as i64))
}

/// digitize — bin index for value in monotone bin edges.
fn builtin_digitize(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let edges = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_histogram_bin_edges(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n_bins = i1(args).max(1);
    Ok(PerlValue::integer(n_bins + 1))
}

/// unique_count — distinct value count.
fn builtin_unique_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_polyfit_rmse(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xs = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::float(mse.sqrt()))
}

/// Complete elliptic integral K(m): AGM iteration.
fn builtin_ellipk(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_ellipe(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(k * (1.0 - c2_sum / 2.0)))
}

/// Hypergeometric ₁F₁(a; b; z) via series.
fn builtin_hyp1f1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
...
fn builtin_hyp2f1(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
...
fn builtin_mathieu_b(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(1) as f64;
    let q = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(n * n - q * q / (2.0 * (n * n - 1.0).max(1e-15))))
}

/// Spherical Bessel jₙ(x): recursive (downward unstable; use upward for small n).
fn builtin_spherical_jn(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0) as i32;
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if x.abs() < 1e-15 { return Ok(PerlValue::float(if n == 0 { 1.0 } else { 0.0 })); }
    let mut j_prev = x.cos() / x;
    let mut j_cur = x.sin() / x;
    if n == 0 { return Ok(PerlValue::float(j_cur)); }
    for k in 1..=n as usize {
        let j_next = (2.0 * k as f64 - 1.0) / x * j_cur - j_prev;
        j_prev = j_cur;
        j_cur = j_next;
    }
    Ok(PerlValue::float(j_cur))
}

/// Spherical Bessel yₙ(x).
fn builtin_spherical_yn(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0) as i32;
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if x.abs() < 1e-15 { return Ok(PerlValue::float(f64::NEG_INFINITY)); }
    let mut y_prev = x.sin() / x;
    let mut y_cur = -x.cos() / x;
    if n == 0 { return Ok(PerlValue::float(y_cur)); }
    for k in 1..=n as usize {
        let y_next = (2.0 * k as f64 - 1.0) / x * y_cur - y_prev;
        y_prev = y_cur;
        y_cur = y_next;
    }
    Ok(PerlValue::float(y_cur))
}

/// Bessel Jν(x) via series for ν integer.
fn builtin_jv(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let nu = i1(args);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_yn(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0) as i32;
    ...
    if n == 0 { return Ok(PerlValue::float(y0)); }
    if n == 1 { return Ok(PerlValue::float(y1)); }
    let mut a = y0;
    let mut b = y1;
    for k in 1..n {
...
fn builtin_iv(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let nu = i1(args);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let n = nu.unsigned_abs() as i64;
    let mut term = (x / 2.0).powi(n as i32) / (1..=n).fold(1_f64, |a, k| a * k as f64);
...
fn builtin_kv(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let _nu = i1(args);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0).max(1e-15);
    Ok(PerlValue::float((std::f64::consts::PI / (2.0 * x)).sqrt() * (-x).exp()))
}

/// Airy Ai(x) via series for |x|<5.
fn builtin_airyai(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float(c1 * f_sum - c2 * g_sum))
}

/// Airy Bi(x): same series with + sign.
fn builtin_airybi(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_polygamma(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0);
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if n == 0 {
        return Ok(PerlValue::float(libm::tgamma(x).ln()));
    }
    let mut s = 0.0;
    for k in 0..1000 { s += 1.0 / (x + k as f64).powi((n + 1) as i32); }
    let sign = if n % 2 == 0 { -1.0 } else { 1.0 };
    let factn = (1..=n).fold(1_f64, |a, k| a * k as f64);
    Ok(PerlValue::float(sign * factn * s))
}

/// trigamma ψ'(x).
fn builtin_trigamma(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let mut s = 0.0;
...
fn builtin_loggamma(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    Ok(PerlValue::float(libm::lgamma(x)))
}

/// factorial2 — double factorial n!! = n·(n-2)·...
fn builtin_factorial2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0);
    let mut acc = 1_i64;
    let mut k = n;
...
fn builtin_factorialk(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0);
...
fn builtin_owens_t(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let h = f1(args);
    ...
    Ok(PerlValue::float(sum * dx / (2.0 * std::f64::consts::PI)))
}

/// Marcum Q-function Q_M(a, b): tail of noncentral chi.
fn builtin_marcum_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(0.5 * libm::erfc((b - a) / 2_f64.sqrt())))
}

/// Voigt profile = real part of Faddeeva w(z), z = (x + iγ) / (σ √2).
fn builtin_voigt_profile(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float((1.0 - eta) * g + eta * l))
}

/// Chebyshev T_n(x).
fn builtin_chebyt(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0) as i32;
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
/// Chebyshev U_n(x).
fn builtin_chebyu(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0) as i32;
    let x = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if n == 0 { return Ok(PerlValue::float(1.0)); }
...
fn builtin_sph_harm(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(norm * p))
}

/// Faddeeva w(z) — magnitude only via Hermite-Gauss approximation.
fn builtin_wofz(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let denom = ((x * x + y * y).sqrt()).max(1e-15);
    Ok(PerlValue::float((-x * x + y * y).exp() / (denom * std::f64::consts::PI.sqrt())))
}

/// erfcx(x) = exp(x²) erfc(x).
fn builtin_erfcx(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    Ok(PerlValue::float((x * x).exp() * libm::erfc(x)))
}

/// erfi(x) = -i erf(ix) (real expansion).
fn builtin_erfi(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let mut term = x;
    let mut sum = term;
    for k in 1..200 {
    ...
    Ok(PerlValue::float(2.0 / std::f64::consts::PI.sqrt() * sum))
}

/// Dawson's F(x) = e^{-x²} ∫₀ˣ e^{t²} dt.
fn builtin_dawsn(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let mut term = x;
    let mut sum = term;
    for k in 1..200 {
...
fn builtin_interp1d(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xs = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let ys = args.get(1).map(b72_to_floats).unwrap_or_default();
    let x = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    if xs.is_empty() || ys.is_empty() { return Ok(PerlValue::float(0.0)); }
    let pos = xs.partition_point(|&xi| xi < x);
    if pos == 0 { return Ok(PerlValue::float(ys[0])); }
    if pos >= xs.len() { return Ok(PerlValue::float(ys[ys.len() - 1])); }
    let (x0, x1) = (xs[pos - 1], xs[pos]);
    let (y0, y1) = (ys[pos - 1], ys[pos.min(ys.len() - 1)]);
    let t = (x - x0) / (x1 - x0).max(1e-300);
    Ok(PerlValue::float(y0 + t * (y1 - y0)))
}

/// convolve_full — output length = m + n - 1.
fn builtin_convolve_full(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = args.get(1).map(b72_to_floats).unwrap_or_default();
    let n = a.len() + b.len();
    Ok(PerlValue::integer(n.saturating_sub(1) as i64))
}

/// convolve_valid — only fully overlapping window.
fn builtin_convolve_valid(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = args.get(1).map(b72_to_floats).unwrap_or_default();
...
fn builtin_correlate_full(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_convolve_full(args)
}

/// kron_product — output length m·n.
fn builtin_kron_product(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let b = args.get(1).map(b72_to_floats).unwrap_or_default();
    Ok(PerlValue::integer((a.len() * b.len()) as i64))
}

// ───── quadrature / ODE ─────

/// Composite Simpson rule on n+1 sample points (n even).
fn builtin_simpson_rule(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ys = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let h = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    if ys.len() < 3 { return Ok(PerlValue::float(0.0)); }
    let n = ys.len() - 1;
    let mut sum = ys[0] + ys[n];
    for i in 1..n { sum += ys[i] * if i % 2 == 0 { 2.0 } else { 4.0 }; }
    Ok(PerlValue::float(sum * h / 3.0))
}

/// Romberg quadrature step: T_{n,m} = (4^m T_{n,m-1} - T_{n-1,m-1}) / (4^m - 1).
fn builtin_romberg_quad(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let t_n_mm1 = f1(args);
    let t_nm1_mm1 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let m = args.get(2).map(|v| v.to_number() as i32).unwrap_or(1);
    let p = 4_f64.powi(m);
    Ok(PerlValue::float((p * t_n_mm1 - t_nm1_mm1) / (p - 1.0)))
}

/// Fixed-order Gauss-Legendre quadrature: returns Σ wᵢ f(xᵢ).
fn builtin_fixed_quad(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f_vals = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_ode45_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = f1(args);
    ...
    Ok(PerlValue::float(y + h * (k1 + 2.0 * k2 + 2.0 * k3 + k4) / 6.0))
}

/// LSODA-style adaptive step result: doubles or halves h on success/fail.
fn builtin_ode_lsoda(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let h = f1(args);
    let success = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1);
    Ok(PerlValue::float(if success != 0 { h * 1.5 } else { h * 0.5 }))
}

/// solve_ivp generic step (delegates to RK4).
fn builtin_solve_ivp_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    builtin_ode45_step(args)
}

/// Brent's method bracket halving step.
fn builtin_root_brentq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let fa = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let fb = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
    if (fb - fa).abs() < 1e-300 { return Ok(PerlValue::float((a + b) / 2.0)); }
    Ok(PerlValue::float(b - fb * (b - a) / (fb - fa)))
}

/// Newton's method step: x - f/f'.
fn builtin_root_newton(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let f = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let fp = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(x - f / fp.max(1e-300)))
}

/// Secant method step.
fn builtin_root_secant(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x0 = f1(args);
    ...
    Ok(PerlValue::float(x1 - f1v * (x1 - x0) / (f1v - f0).max(1e-300)))
}

/// Powell line-search step: f(x + α·p) — returns α minimising parabolic fit.
fn builtin_fmin_powell(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f0 = f1(args);
    ...
    if denom.abs() < 1e-300 { return Ok(PerlValue::float(0.5)); }
    Ok(PerlValue::float(0.5 + (f0 - f2) / denom))
}

/// COBYLA constraint slack: returns minimum of constraint values.
fn builtin_fmin_cobyla(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b72_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let m = v.iter().cloned().fold(f64::INFINITY, f64::min);
    Ok(PerlValue::float(if m.is_finite() { m } else { 0.0 }))
}

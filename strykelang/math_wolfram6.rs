...
fn builtin_gibbs_sampler_step(
    ...
    let conds = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
    let mut x: Vec<f64> = arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
...
fn builtin_euler_maruyama(
    ...
    use rand::Rng;
    let mu = args.first().cloned().unwrap_or(PerlValue::UNDEF);
    let sigma = args.get(1).cloned().unwrap_or(PerlValue::UNDEF);
    ...
    let call = |interp: &mut VMHelper, sub: &_, x: f64, t: f64| -> PerlResult<f64> {
        let r = exec_to_perl_result(
            interp.call_sub(
                sub,
                vec![PerlValue::float(x), PerlValue::float(t)],
                WantarrayCtx::Scalar,
                line,
            ),
            "callback",
...
fn builtin_milstein(
    ...
    let mu = args.first().cloned().unwrap_or(PerlValue::UNDEF);
    let sigma = args.get(1).cloned().unwrap_or(PerlValue::UNDEF);
    let sigma_x = args.get(2).cloned().unwrap_or(PerlValue::UNDEF);
...
fn builtin_ornstein_uhlenbeck_path(args: &[PerlValue]) -> PerlResult<PerlValue> {
    use rand::Rng;
...
fn builtin_hmm_forward(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pi: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    let a = matrix_from_value(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF));
    let b = matrix_from_value(&args.get(2).cloned().unwrap_or(PerlValue::UNDEF));
    let obs: Vec<usize> = arg_to_vec(&args.get(3).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
    ...
    Ok(PerlValue::float(log_p))
}

/// Viterbi decoding: most likely hidden-state path.
fn builtin_hmm_viterbi(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pi: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    ...
    Ok(PerlValue::array(
        path.into_iter().map(|v| PerlValue::integer(v as i64)).collect(),
    ))
}

/// Backward algorithm. Returns the β matrix at each time step.
fn builtin_hmm_backward(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pi: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
...
fn builtin_kaplan_meier(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut data: Vec<(f64, i64)> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .zip(arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF)).iter())
...
fn builtin_log_rank_test(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let t1: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    let e1: Vec<i64> = arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number() as i64)
        .collect();
    let t2: Vec<f64> = arg_to_vec(&args.get(2).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    let e2: Vec<i64> = arg_to_vec(&args.get(3).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number() as i64)
        .collect();
    ...
    Ok(PerlValue::array(vec![PerlValue::float(chi2), PerlValue::float(p)]))
}

// ─── 5. Sequence alignment (bioinformatics) ──────────────────────────────────

/// Needleman-Wunsch global alignment score (linear gap penalty).
fn builtin_needleman_wunsch(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(dp[m][n]))
}

/// Smith-Waterman local alignment best score.
fn builtin_smith_waterman(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = args.first().map(|v| v.to_string()).unwrap_or_default();
    let b = args.get(1).map(|v| v.to_string()).unwrap_or_default();
...
fn builtin_gibbs_free_energy(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dh = f1(args);
...
fn builtin_pid_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let state = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
    ...
    Ok(PerlValue::array(vec![
        PerlValue::array(vec![PerlValue::float(integral), PerlValue::float(e)]),
        PerlValue::float(u),
    ]))
}
...
fn builtin_transfer_function_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let num: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    let den: Vec<f64> = arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    ...
    Ok(PerlValue::array(vec![PerlValue::float(re), PerlValue::float(im)]))
}

/// `bode_magnitude_db` — Bode magnitude db. Returns a float.
fn builtin_bode_magnitude_db(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(20.0 * (re * re + im * im).sqrt().log10()))
}

/// `bode_phase_deg` — Bode phase deg. Returns a float.
fn builtin_bode_phase_deg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let h = builtin_transfer_function_eval(args)?;
    let v = arg_to_vec(&h);
    let re = v.first().map(|x| x.to_number()).unwrap_or(0.0);
    let im = v.get(1).map(|x| x.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(im.atan2(re).to_degrees()))
}
...
fn builtin_lqr_2x2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let q = matrix_from_value(&args.get(2).cloned().unwrap_or(PerlValue::UNDEF));
    let r = matrix_from_value(&args.get(3).cloned().unwrap_or(PerlValue::UNDEF));
...
fn builtin_nash_eq_2x2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p1 = matrix_from_value(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
    let p2 = matrix_from_value(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF));
...
fn builtin_shapley_value(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = args.first().map(|v| v.to_number() as usize).unwrap_or(0);
    let v_table: Vec<f64> = arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
...
fn builtin_expected_utility(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let probs: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    let payoffs: Vec<f64> = arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    Ok(PerlValue::float(
...
fn builtin_hungarian_assignment(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut c = matrix_from_value(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
    let n = c.len();
    if n == 0 {
        return Ok(PerlValue::array(vec![PerlValue::array(vec![]), PerlValue::float(0.0)]));
    }
    ...
    let original = matrix_from_value(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
    for i in 0..n {
        if row_assign[i] == -1 {
            for j in 0..n {
                if !col_used[j] {
    ...
    let total: f64 = (0..n)
        ...
        .sum();
    Ok(PerlValue::array(vec![
        PerlValue::array(row_assign.into_iter().map(PerlValue::integer).collect()),
        PerlValue::float(total),
    ]))
}

/// TSP nearest-neighbour heuristic. Returns [tour, length].
fn builtin_tsp_nearest_neighbor(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist = matrix_from_value(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
    let n = dist.len();
    if n == 0 {
        return Ok(PerlValue::array(vec![PerlValue::array(vec![]), PerlValue::float(0.0)]));
    }
    ...
    tour.push(0);
    Ok(PerlValue::array(vec![
        PerlValue::array(tour.into_iter().map(PerlValue::integer).collect()),
        PerlValue::float(total),
    ]))
}

/// 2-approximation vertex cover (greedy edge picking).
fn builtin_vertex_cover_2approx(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_heat_eq_1d(
    ...
    Ok(PerlValue::array(u.into_iter().map(PerlValue::float).collect()))
}
...
fn builtin_wave_eq_1d(
    ...
    let f0 = args.first().cloned().unwrap_or(PerlValue::UNDEF);
    let a = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let b = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    ...
    Ok(PerlValue::array(u_curr.into_iter().map(PerlValue::float).collect()))
}

/// 2-D Laplace equation Jacobi smoother on a grid with given Dirichlet boundary.
fn builtin_laplace_2d_jacobi(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let g = matrix_from_value(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
...
fn builtin_beta_binomial_update(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![
        PerlValue::float(alpha + k),
        PerlValue::float(beta + n - k),
    ]))
}

/// Normal-Normal (known variance): prior N(μ₀, σ₀²) with n samples mean ȳ,
/// known data variance σ². Returns [μ_post, σ²_post].
fn builtin_normal_normal_update(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![
        PerlValue::float(mu_post),
        PerlValue::float(1.0 / prec_post),
    ]))
}

/// Gamma-Poisson update: prior Gamma(α, β) (shape, rate), observed
/// counts (n total trials, k events). Returns [α_post, β_post].
fn builtin_gamma_poisson_update(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha = args.first().map(|v| v.to_number()).unwrap_or(1.0);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let n = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let total_events = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::array(vec![
        PerlValue::float(alpha + total_events),
        PerlValue::float(beta + n),
    ]))
}

/// Dirichlet-Multinomial update.
fn builtin_dirichlet_multinomial_update(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let alpha: Vec<f64> = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    let counts: Vec<f64> = arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    let new_alpha: Vec<f64> = alpha
        ...
        .map(|(a, c)| a + c)
        .collect();
    Ok(PerlValue::array(new_alpha.into_iter().map(PerlValue::float).collect()))
}

// ─── 12. Quantum gates (real / complex 2-vector) ─────────────────────────────

/// `hadamard_gate` — Hadamard gate.
fn builtin_hadamard_gate(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s = 1.0 / 2.0_f64.sqrt();
    Ok(matrix_to_value(&[vec![s, s], vec![s, -s]]))
}

/// `cnot_gate` — Cnot gate.
fn builtin_cnot_gate(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(matrix_to_value(&[
        vec![1.0, 0.0, 0.0, 0.0],
        vec![0.0, 1.0, 0.0, 0.0],
        vec![0.0, 0.0, 0.0, 1.0],
...
/// `swap_gate` — Swap gate.
fn builtin_swap_gate(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(matrix_to_value(&[
        vec![1.0, 0.0, 0.0, 0.0],
        vec![0.0, 0.0, 1.0, 0.0],
        vec![0.0, 1.0, 0.0, 0.0],
...
/// `cz_gate` — Cz gate.
fn builtin_cz_gate(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(matrix_to_value(&[
        vec![1.0, 0.0, 0.0, 0.0],
        vec![0.0, 1.0, 0.0, 0.0],
        vec![0.0, 0.0, 1.0, 0.0],
...
fn builtin_qft_matrix(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![matrix_to_value(&re), matrix_to_value(&im)]))
}

/// Phase-shift gate diag(1, e^iφ) returned as `[re, im]` 2×2 blocks.
fn builtin_phase_gate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let phi = f1(args);
    Ok(PerlValue::array(vec![
...
/// S gate = phase(π/2).
fn builtin_s_gate(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::array(vec![
...
/// T gate = phase(π/4).
fn builtin_t_gate(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s = 1.0 / 2.0_f64.sqrt();
    Ok(PerlValue::array(vec![
...
fn builtin_bezier_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(
        q[0].iter().copied().map(PerlValue::float).collect(),
    ))
}

/// Catmull-Rom spline evaluation (uniform parameterisation).
fn builtin_catmull_rom_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p0 = arg_to_vec(&args.first().cloned().unwrap_or(PerlValue::UNDEF));
    let p1 = arg_to_vec(&args.get(1).cloned().unwrap_or(PerlValue::UNDEF));
    let p2 = arg_to_vec(&args.get(2).cloned().unwrap_or(PerlValue::UNDEF));
    let p3 = arg_to_vec(&args.get(3).cloned().unwrap_or(PerlValue::UNDEF));
        ...
        let p = 0.5
            * (2.0 * p1[d].to_number()
                ...
                + (-p0[d].to_number() + 3.0 * p1[d].to_number() - 3.0 * p2[d].to_number()
                    + p3[d].to_number())
                    * t3);
        out.push(PerlValue::float(p));
    }
    Ok(PerlValue::array(out))
}

/// Cubic-Hermite basis evaluation: H(t) on (p0, m0, p1, m1).
fn builtin_cubic_hermite_eval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p0 = f1(args);
    ...
    let h11 = t3 - t2;
    Ok(PerlValue::float(h00 * p0 + h10 * m0 + h01 * p1 + h11 * m1))
}

/// Cox-de Boor B-spline basis function value N_{i,k}(t) on knots.
fn builtin_bspline_basis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = args.first().map(|v| v.to_number() as usize).unwrap_or(0);
    let k = args.get(1).map(|v| v.to_number() as usize).unwrap_or(1).max(1);
    let t = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let knots: Vec<f64> = arg_to_vec(&args.get(3).cloned().unwrap_or(PerlValue::UNDEF))
        .iter()
        .map(|v| v.to_number())
        .collect();
    ...
    Ok(PerlValue::float(n_basis(i, k, t, &knots)))
}

// ─── 14. Music / audio ───────────────────────────────────────────────────────

/// `freq_to_midi` — Freq to midi. Returns a float.
fn builtin_freq_to_midi(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f = f1(args).max(1e-9);
    Ok(PerlValue::float(69.0 + 12.0 * (f / 440.0).log2()))
}

/// `midi_to_freq` — Midi to freq. Returns a float.
fn builtin_midi_to_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let m = f1(args);
    Ok(PerlValue::float(440.0 * 2.0_f64.powf((m - 69.0) / 12.0)))
}

/// `equal_temperament_freq` — Equal temperament freq. Returns a float.
fn builtin_equal_temperament_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let semitones_above_a4 = args.first().map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(440.0 * 2.0_f64.powf(semitones_above_a4 / 12.0)))
}

/// `cents_difference` — Cents difference. Returns a float.
fn builtin_cents_difference(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f1_v = f1(args).max(1e-9);
    let f2_v = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-9);
    Ok(PerlValue::float(1200.0 * (f2_v / f1_v).log2()))
}

// ─── 15. Astronomy ───────────────────────────────────────────────────────────

/// Cosmological redshift z from observed and emitted wavelengths.
fn builtin_redshift_z(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lambda_obs = f1(args);
    let lambda_emit = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-30);
    Ok(PerlValue::float(lambda_obs / lambda_emit - 1.0))
}

/// Hubble distance d_H = c / H₀ (Mpc if c in km/s and H₀ in km/s/Mpc).
fn builtin_hubble_distance(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let h0 = f1(args);
    if h0.abs() < 1e-30 {
        return Ok(PerlValue::float(f64::INFINITY));
    }
    let c_kms = 299792.458_f64;
    Ok(PerlValue::float(c_kms / h0))
}

/// Luminosity distance for a flat ΛCDM cosmology integrated numerically.
fn builtin_luminosity_distance(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let z = f1(args);
    ...
    Ok(PerlValue::float((1.0 + z) * c_kms / h0 * sum * h))
}

// ─── 16. Fluid dynamics ──────────────────────────────────────────────────────

/// `reynolds_number` — Reynolds number. Returns a float.
fn builtin_reynolds_number(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rho = f1(args);
    ...
    Ok(PerlValue::float(rho * u * l / mu))
}

/// `mach_number` — Mach number. Returns a float.
fn builtin_mach_number(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let u = f1(args);
...
fn builtin_prandtl_number(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cp = f1(args);
    let mu = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let k = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-30);
    Ok(PerlValue::float(cp * mu / k))
}

/// Bernoulli speed: v = √(2(p₁ - p₂)/ρ) for incompressible flow at same height.
fn builtin_bernoulli_velocity(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p1 = f1(args);
...
fn builtin_negative_binomial_pmf(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = i1(args);
...
/// Hypergeometric PMF.
fn builtin_hypergeometric_pmf(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
/// Beta-binomial PMF.
fn builtin_beta_binomial_pmf(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    let _ = beta;
    Ok(PerlValue::float((log_binom + log_b1 + log_b2).exp()))
}

/// Von Mises PDF on the circle.
fn builtin_von_mises_pdf(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let theta = f1(args);
    let mu = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let kappa = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    let denom = 2.0 * std::f64::consts::PI * bessel_i0_real(kappa);
    Ok(PerlValue::float((kappa * (theta - mu).cos()).exp() / denom))
}

// ─── 18. Random graphs ───────────────────────────────────────────────────────

/// `erdos_renyi_random` — Erdos renyi random. Returns an integer.
fn builtin_erdos_renyi_random(args: &[PerlValue]) -> PerlResult<PerlValue> {
    use rand::Rng;
    let n = args.first().map(|v| v.to_number() as usize).unwrap_or(0);
...
fn builtin_barabasi_albert_random(args: &[PerlValue]) -> PerlResult<PerlValue> {
    use rand::Rng;
    let n = args.first().map(|v| v.to_number() as usize).unwrap_or(0);
...
fn builtin_watts_strogatz_random(args: &[PerlValue]) -> PerlResult<PerlValue> {
    use rand::Rng;
    let n = args.first().map(|v| v.to_number() as usize).unwrap_or(0);
    ...
    Ok(PerlValue::array(
        adj.into_iter()
            .map(|nbrs| {
                ...
                v.sort();
                PerlValue::array(v.into_iter().map(|x| PerlValue::integer(x as i64)).collect())
            })
            .collect(),
    ))
}
...
fn builtin_rgb_to_lab(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_lab_to_rgb(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let l = f1(args);
    let a = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    ...
    Ok(PerlValue::array(vec![
        PerlValue::float(to_srgb(rl)),
        PerlValue::float(to_srgb(gl)),
        PerlValue::float(to_srgb(bl)),
    ]))
}

/// Approximate Kelvin → sRGB (Tanner Helland). Returns [R, G, B] in 0..255.
fn builtin_kelvin_to_rgb(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![
        PerlValue::float(clip(r)),
        PerlValue::float(clip(g)),
        PerlValue::float(clip(b)),
    ]))
}
...
fn builtin_bell_triangle(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(
        rows.into_iter()
            .map(|r| PerlValue::array(r.into_iter().map(PerlValue::integer).collect()))
            .collect(),
    ))
}
...
fn builtin_surjection_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = args.first().map(|v| v.to_number() as i64).unwrap_or(0);
    ...
    Ok(PerlValue::integer(sum as i64))
}

/// Number of partitions of n into distinct parts (q-Pochhammer expansion).
fn builtin_distinct_partition_count(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args).max(0) as usize;
...
fn builtin_fibonacci_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = i1(args);
    let is_square = |x: i64| {
        if x < 0 {
            return false;

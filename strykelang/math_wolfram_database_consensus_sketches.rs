...
fn builtin_db_b_tree_split(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer((n - 1) / 2))
}

/// B-tree merge: combined node fill
fn builtin_db_b_tree_merge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = i1(args);
...
fn builtin_db_lsm_compaction_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_skiplist_height_pick(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_db_bloom_filter_bit_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_cuckoo_filter_fingerprint(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = i1(args) as u64;
    Ok(StrykeValue::integer((h & 0xff) as i64))
}

/// Quotient filter canonical slot
fn builtin_db_quotient_filter_canonical(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = i1(args) as u64;
...
fn builtin_db_count_min_sketch_bin(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = i1(args) as u64;
...
fn builtin_db_hyperloglog_register_max(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = i1(args) as u64;
    Ok(StrykeValue::integer(h.leading_zeros() as i64 + 1))
}

/// MinHash value: min(h(x_i)) over set
fn builtin_db_min_hash_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// SimHash bit index
fn builtin_db_simhash_bit(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
    Ok(StrykeValue::integer(if v >= 0.0 { 1 } else { 0 }))
}

/// Consistent hash bucket
fn builtin_db_consistent_hash_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_rendezvous_hash_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let key = f1(args);
...
fn builtin_db_jump_hash_bucket(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// Maglev hash step
fn builtin_db_maglev_hash_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_lru_cache_eviction_age(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let now = f1(args);
...
fn builtin_db_lfu_cache_decay(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let freq = f1(args);
...
fn builtin_db_arc_cache_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lru = f1(args);
...
fn builtin_db_clock_cache_hand(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let h = i1(args);
    let n = args.get(1).map(|v| v.to_number() as i64).unwrap_or(1).max(1);
...
fn builtin_db_tinylfu_admit_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_w_tinylfu_freq(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b48_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::float(v.iter().cloned().fold(f64::INFINITY, f64::min)))
}

/// Buffer pool score (clock-pro / 2Q proxy)
fn builtin_db_buffer_pool_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let recency = f1(args);
...
fn builtin_db_query_plan_cost_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let card = f1(args);
...
fn builtin_db_join_selectivity_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let d_a = f1(args);
...
fn builtin_db_index_seek_cost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_db_seq_scan_cost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rows = f1(args);
...
fn builtin_db_index_scan_cost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_db_sort_cost_estimate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
/// Hash join cost: |A| + |B|
fn builtin_db_hash_join_cost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_db_merge_join_cost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_db_nested_loop_cost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(a * b))
}
...
fn builtin_db_query_cardinality(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_db_histogram_bucket_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_db_quantile_estimate_p99(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_t_digest_centroid(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// KLL sketch quantile
fn builtin_db_kll_quantile_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_db_dd_sketch_bin(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_db_reservoir_sample_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let i = f1(args);
...
fn builtin_db_chao_estimator_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_obs = f1(args);
...
fn builtin_db_jaccard_minhash_estimate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let matches = f1(args);
...
fn builtin_db_distinct_estimate_lpc(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
...
fn builtin_db_distinct_estimate_hll(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m = f1(args);
...
/// Throttle token step
fn builtin_db_throttle_token_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tokens = f1(args);
...
/// Leaky bucket step
fn builtin_db_leaky_bucket_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let level = f1(args);
...
fn builtin_db_token_bucket_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let tokens = f1(args);
...
fn builtin_db_circuit_breaker_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let errors = f1(args);
...
fn builtin_db_two_phase_commit_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_three_phase_commit_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cur = i1(args);
...
fn builtin_db_paxos_propose_id(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let epoch = i1(args);
...
/// Raft term advance
fn builtin_db_raft_term_advance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let term = i1(args);
    Ok(StrykeValue::integer(term + 1))
}

/// Raft log match check (consistency)
fn builtin_db_raft_log_match_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_zab_epoch_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_chubby_lease_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lease = f1(args);
...
fn builtin_db_logical_clock_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let local = i1(args);
...
fn builtin_db_lamport_timestamp(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_db_logical_clock_step(args)
}

/// Vector clock merge (max-wise)
fn builtin_db_vector_clock_merge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_db_hybrid_logical_clock(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let phys = f1(args);
...
fn builtin_db_crdt_g_counter_merge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(a.max(b)))
}
...
/// CRDT PN-Counter: P - N
fn builtin_db_crdt_pn_counter_merge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_db_crdt_lww_register_merge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v_a = f1(args);
...
fn builtin_db_crdt_set_or_merge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(StrykeValue::float(a + b))
}
...
fn builtin_db_consensus_quorum_size(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer(n / 2 + 1))
}

/// Replication lag step
fn builtin_db_replication_lag_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let primary = f1(args);
...
fn builtin_db_partitions_for_n(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = f1(args);
...
fn builtin_db_consistent_lookup_id(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_db_consistent_hash_index(args)
}

/// Chord finger index = (n + 2^i) mod 2^m
fn builtin_db_chord_finger_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
...
fn builtin_db_kademlia_xor_distance(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_pastry_routing_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let target = f1(args);
...
fn builtin_db_dht_replicate_factor(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_partition_failure_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let alive = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_db_byzantine_quorum_size(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args);
    Ok(StrykeValue::integer(2 * n / 3 + 1))
}

/// PBFT view change step
fn builtin_db_pbft_view_change(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let view = i1(args);
...
fn builtin_db_honey_badger_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let batch = f1(args);
...
fn builtin_db_avalanche_query_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let yes = f1(args);
...
fn builtin_db_quorum_intersection_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
...
fn builtin_db_anti_entropy_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(f1(args) + 1.0))
}

/// Merkle node hash combine
fn builtin_db_merkle_node_hash(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_merkle_path_verify(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// Gossip fanout step
fn builtin_db_gossip_fanout_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let f = f1(args);
    Ok(StrykeValue::float(f.max(1.0)))
}

/// Anti-entropy pull step
fn builtin_db_anti_entropy_pull_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_db_anti_entropy_step(args)
}

/// Split-brain check
fn builtin_db_split_brain_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
/// Clock skew estimate
fn builtin_db_clock_skew_estimate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t1 = f1(args);
...
fn builtin_db_freshness_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lag = f1(args);
...
fn builtin_db_read_repair_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v_local = f1(args);
...
/// Hinted handoff step
fn builtin_db_hinted_handoff_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let queue = f1(args);
...
/// Compaction score
fn builtin_db_compaction_score(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let level_size = f1(args);
...
fn builtin_db_levelled_compaction_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_db_lsm_compaction_step(args)
}

/// Size-tiered compaction
fn builtin_db_size_tiered_compaction(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b48_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_db_universal_compaction_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let nfiles = i1(args);
...
/// Write amplification
fn builtin_db_write_amplification(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_read_amplification(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_space_amplification(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args).max(1.0001);
    Ok(StrykeValue::float(t / (t - 1.0)))
}

/// Block cache hit rate = hits / (hits + misses)
fn builtin_db_block_cache_hit_rate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let hits = f1(args);
...
fn builtin_db_page_cache_eviction_age(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_wal_fsync_cost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let bytes = f1(args);
...
fn builtin_db_group_commit_count(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_replica_lag_threshold(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lag = f1(args);
...
fn builtin_db_synchronous_commit_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let acks = f1(args);
...
fn builtin_db_async_commit_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let leader_ack = i1(args);
    Ok(StrykeValue::integer(leader_ack))
}

/// Eventual consistency check (just returns 1 if propagation begun)
fn builtin_db_eventual_consistency_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let propagating = i1(args);
    Ok(StrykeValue::integer(propagating))
}

/// Strong consistency check (W + R > N)
fn builtin_db_strong_consistency_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_db_linearizability_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let rt = i1(args);
...
fn builtin_db_causal_consistency_check(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {

...
fn b78_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// `arima_fit` — AR(p) coefficient via Yule-Walker (return ρ_1 from autocov).
fn builtin_arima_fit(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = v.len();
    ...
    Ok(PerlValue::float(if c0 > 0.0 { c1 / c0 } else { 0.0 }))
}

/// `arima_forecast` — one-step forecast: ŷ = c + φ y_{t-1} + θ ε_{t-1}.
fn builtin_arima_forecast(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let c = f1(args);
    ...
    Ok(PerlValue::float(c + phi * y_prev + theta * eps_prev))
}

/// `arma_order_select` — Akaike Information Criterion = −2 ℓ + 2 k.
fn builtin_arma_order_select(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let log_lik = f1(args);
    let k = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(-2.0 * log_lik + 2.0 * k))
}

/// `sarimax_fit` — seasonal ARMA log-likelihood term.
fn builtin_sarimax_fit(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let resid = f1(args);
    let sigma2 = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    Ok(PerlValue::float(-0.5 * (sigma2.ln() + resid * resid / sigma2)))
}

/// `garch_fit` — GARCH(1,1) variance update: σ²_t = ω + α ε² + β σ²_{t-1}.
fn builtin_garch_fit(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let omega = f1(args);
    ...
    Ok(PerlValue::float(omega + alpha * eps_sq + beta * sigma_sq_prev))
}

/// `ewma_smooth` — exponentially-weighted moving average y_t = α x + (1-α) y_{t-1}.
fn builtin_ewma_smooth(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_holt_winters_additive(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float(alpha * (x - s_prev) + (1.0 - alpha) * (l_prev + b_prev)))
}

/// `holt_winters_multiplicative` — multiplicative seasonality variant.
fn builtin_holt_winters_multiplicative(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_kalman_filter_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x_pred = f1(args);
    ...
    Ok(PerlValue::float(x_pred + k * (z - x_pred)))
}

/// `kalman_smoother_step` — RTS smoother gain: A_k = P_k F' / P_{k+1|k}.
fn builtin_kalman_smoother_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p_k = f1(args);
    let f = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let p_k1_pred = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    Ok(PerlValue::float(p_k * f / p_k1_pred))
}

/// `var_fit` — VAR(1) coefficient via OLS on lagged regressor.
fn builtin_var_fit(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let xs = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let n = xs.len();
    if n < 2 { return Ok(PerlValue::float(0.0)); }
    let mut num = 0.0;
    let mut den = 0.0;
...
fn builtin_vecm_fit(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = f1(args);
...
fn builtin_kpss_test(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(cum_sq_sum / (n * n * s_lr_sq)))
}

/// `breusch_godfrey` — LM test for serial correlation: T R².
fn builtin_breusch_godfrey(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r_sq = f1(args).clamp(0.0, 1.0);
    let t = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(t * r_sq))
}

/// `ljung_box_q` — Ljung-Box Q = T(T+2) Σ ρ²/k for k=1..h.
fn builtin_ljung_box_q(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rho_sq = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let t = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let s: f64 = rho_sq.iter().enumerate()
        .map(|(i, r)| r / (i as f64 + 1.0)).sum();
    Ok(PerlValue::float(t * (t + 2.0) * s))
}

/// `durbin_watson_d` — DW = Σ (e_t − e_{t-1})² / Σ e_t².
fn builtin_durbin_watson_d(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let e = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if e.len() < 2 { return Ok(PerlValue::float(0.0)); }
...
fn builtin_granger_causality(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rss_r = f1(args);
    ...
    Ok(PerlValue::float(((rss_r - rss_u) / q) / (rss_u / n_k)))
}
...
fn builtin_cointegration_eg(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cov_xy = f1(args);
    ...
    let _ = beta_hat;
    Ok(PerlValue::float((phi_resid - 1.0) / se_resid))
}

/// `seasonal_decompose` — additive decomposition residual: x − trend − seasonal.
fn builtin_seasonal_decompose(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let trend = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let seasonal = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(x - trend - seasonal))
}

/// `stl_decompose` — STL trend update via LOESS-style weighted local fit.
fn builtin_stl_decompose(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let weights = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    if w_sum <= 0.0 { return Ok(PerlValue::float(0.0)); }
    let s: f64 = (0..n).map(|i| weights[i] * values[i]).sum();
    Ok(PerlValue::float(s / w_sum))
}

/// `acf_basis` — sample autocorrelation at lag k.
fn builtin_acf_basis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    Ok(PerlValue::float(if c0 > 0.0 { ck / c0 } else { 0.0 }))
}

/// `pacf_basis` — partial autocorrelation via Durbin-Levinson recursion.
fn builtin_pacf_basis(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let acf = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let k = args.get(1).map(|v| v.to_number() as usize).unwrap_or(1);
    if k == 0 || k >= acf.len() { return Ok(PerlValue::float(0.0)); }
    ...
    Ok(PerlValue::float(phi[k][k]))
}

/// `moving_average_filter` — simple moving average value.
fn builtin_moving_average_filter(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b78_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_exp_smooth_simple(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let resid_dot = f1(args);
    let resid_var = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    Ok(PerlValue::float((resid_dot / resid_var).clamp(0.0, 1.0)))
}
...
/// horizon h.
fn builtin_exp_smooth_double(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    ...
    Ok(PerlValue::float(l_new + h * b_new))
}

/// `markov_switching_ar` — regime-switching transition step.
fn builtin_markov_switching_ar(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(p_stay * prior + p_switch * (1.0 - prior)))
}
...
fn builtin_arch_lm(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let t = f1(args);
...
fn builtin_pivoted_cholesky_var(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pivot = f1(args);
    let sigma_sq = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(sigma_sq / pivot.max(1e-15)))
}

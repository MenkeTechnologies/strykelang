...
fn b61_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// R₀ basic reproduction number from contact rate β, infectious period 1/γ.
fn builtin_r_naught_basic(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let beta = f1(args);
...
fn builtin_r_effective_t(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r0 = f1(args);
...
fn builtin_doubling_time_growth(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
    if r <= 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(2f64.ln() / r))
}

/// SIRS one-step Euler: S' = -βSI/N + ξR, I' = βSI/N - γI, R' = γI - ξR.
fn builtin_sirs_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s = f1(args);
    ...
    Ok(PerlValue::float(new_s.max(0.0) + new_i.max(0.0) / 1e9))
}

/// SEIRS one-step: adds exposed compartment with rate σ (E → I), waning ξ.
fn builtin_seirs_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s = f1(args);
    ...
    Ok(PerlValue::float(new_s.max(0.0) + (new_e + new_i).max(0.0) / 1e9))
}

/// Force of infection λ = β·I/N (S → I rate per susceptible).
fn builtin_susceptible_to_infected(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let beta = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let n = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-12);
    Ok(PerlValue::float(beta * i / n))
}

/// Final attack rate (SIR endemic): solve A = 1 - exp(-R₀·A) numerically.
fn builtin_attack_rate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r0 = f1(args);
    ...
    for _ in 0..200 {
        ...
        let new_a = a - f / fp;
        if (new_a - a).abs() < 1e-12 { return Ok(PerlValue::float(new_a.clamp(0.0, 1.0))); }
        a = new_a;
    }
    Ok(PerlValue::float(a.clamp(0.0, 1.0)))
}

/// Vaccination coverage required: 1 − 1/R₀ for sterilising vaccine.
fn builtin_vaccination_coverage_required(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r0 = f1(args);
    if r0 <= 1.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((1.0 - 1.0 / r0).clamp(0.0, 1.0)))
}

/// Case fatality rate: deaths / cases.
fn builtin_cfr_case_fatality(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let deaths = f1(args);
    let cases = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(deaths / cases))
}

/// Infection fatality rate: deaths / total infections (incl. asymptomatic).
fn builtin_ifr_infection_fatality(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let deaths = f1(args);
    let infections = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1.0);
    Ok(PerlValue::float(deaths / infections))
}

/// DALYs = YLL + YLD, weighted by disability weight d. Returns YLD increment.
fn builtin_dalys_disability_weight(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(prevalence * d * duration_years))
}

/// QALY remaining = years_remaining · health_utility (0..1).
fn builtin_qaly_lifetime(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let yrs = f1(args);
    let utility = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).clamp(0.0, 1.0);
    Ok(PerlValue::float(yrs * utility))
}

/// YLL (years of life lost): sum over deaths of (life_expectancy − age_at_death).
fn builtin_ylll_pml(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ages = b61_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_rt_serial_interval(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let inc = b61_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    if prior <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((cur / prior).powf(w)))
}

/// Generation time step (gamma-distributed): mean increment.
fn builtin_generation_time_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mean = f1(args);
    Ok(PerlValue::float(mean))
}

/// Gini coefficient for health-inequality (Lorenz-curve area). Args: sorted shares.
fn builtin_gini_inequality_health(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut shares = b61_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if shares.is_empty() { return Ok(PerlValue::float(0.0)); }
    ...
    Ok(PerlValue::float((2.0 * cumulative / (n * total)) - (n + 1.0) / n))
}

/// Standardised mortality ratio: observed / expected.
fn builtin_standardized_mortality_smr(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let observed = f1(args);
    let expected = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-12);
    Ok(PerlValue::float(observed / expected))
}

/// Indirect age-adjusted rate: (SMR × standard rate).
fn builtin_indirect_age_adjusted(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let smr = f1(args);
    let std_rate = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(smr * std_rate))
}

/// Direct age-adjusted: Σ w_i · r_i with reference population weights.
fn builtin_direct_age_adjusted(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let rates = b61_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let weights = b61_to_floats(args.get(1).unwrap_or(&PerlValue::array(vec![])));
    let n = rates.len().min(weights.len());
    let total_w: f64 = weights.iter().take(n).sum();
    if total_w <= 0.0 { return Ok(PerlValue::float(0.0)); }
    let s: f64 = (0..n).map(|i| rates[i] * weights[i]).sum();
    Ok(PerlValue::float(s / total_w))
}

/// Odds ratio for 2×2 contingency: (a·d) / (b·c).
fn builtin_odds_ratio_2x2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
    ...
    Ok(PerlValue::float(a * d / denom))
}

/// Risk ratio: incidence_exposed / incidence_unexposed = (a/(a+b)) / (c/(c+d)).
fn builtin_risk_ratio_2x2(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let c = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let d = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
    let p_exp = if a + b > 0.0 { a / (a + b) } else { return Ok(PerlValue::float(f64::INFINITY)); };
    let p_un = if c + d > 0.0 { c / (c + d) } else { return Ok(PerlValue::float(f64::INFINITY)); };
    if p_un <= 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(p_exp / p_un))
}

/// Number Needed to Treat: 1 / (CER − EER).
fn builtin_number_needed_to_treat(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cer = f1(args);
    let eer = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let arr = cer - eer;
    if arr <= 0.0 { return Ok(PerlValue::float(f64::INFINITY)); }
    Ok(PerlValue::float(1.0 / arr))
}

/// Population-attributable fraction: PAF = (R − R₀) / R for the unexposed risk R₀.
fn builtin_attributable_fraction_pop(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
    let r0 = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if r <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(((r - r0) / r).clamp(0.0, 1.0)))
}

/// Preventive (preventable) fraction: PF = (R₀ − R) / R₀.
fn builtin_preventive_fraction(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r0 = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    if r0 <= 0.0 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float(((r0 - r) / r0).clamp(0.0, 1.0)))
}

/// Contact tracing effectiveness: (cases prevented / total cases). Args: traced
/// secondary cases blocked, total secondary cases.
fn builtin_contact_tracing_eff(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let blocked = f1(args);
    let total = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-12);
    Ok(PerlValue::float((blocked / total).clamp(0.0, 1.0)))
}

/// Cluster attack rate: cases / exposed within outbreak cluster.
fn builtin_cluster_attack_rate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cases = f1(args);
    let exposed = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-12);
    Ok(PerlValue::float(cases / exposed))
}

/// Transmission pair index for case (i, j): max p with j on i's onset trajectory.
fn builtin_transmission_pair_index(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let onset_i = f1(args);
    ...
    Ok(PerlValue::float((-z * z / 2.0).exp()))
}

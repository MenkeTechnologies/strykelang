...
fn builtin_r_naught_basic(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let beta = f1(args);
...
fn builtin_r_effective_t(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r0 = f1(args);
...
fn builtin_doubling_time_growth(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_sirs_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = f1(args);
...
fn builtin_seirs_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let s = f1(args);
...
fn builtin_susceptible_to_infected(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let beta = f1(args);
    let i = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let n = args.get(2).map(|v| v.to_number()).unwrap_or(1.0).max(1e-12);
...
fn builtin_attack_rate(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r0 = f1(args);
...
fn builtin_vaccination_coverage_required(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r0 = f1(args);
    if r0 <= 1.0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_cfr_case_fatality(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let deaths = f1(args);
...
fn builtin_ifr_infection_fatality(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let deaths = f1(args);
...
fn builtin_dalys_disability_weight(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_qaly_lifetime(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let yrs = f1(args);
...
fn builtin_ylll_pml(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_rt_serial_interval(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_generation_time_step(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let mean = f1(args);
    Ok(StrykeValue::float(mean))
}

/// Gini coefficient for health-inequality (Lorenz-curve area). Args: sorted shares.
fn builtin_gini_inequality_health(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_standardized_mortality_smr(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_indirect_age_adjusted(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let smr = f1(args);
...
fn builtin_direct_age_adjusted(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
...
fn builtin_odds_ratio_2x2(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a = f1(args);
...
fn builtin_risk_ratio_2x2(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let c = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let d = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_number_needed_to_treat(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cer = f1(args);
...
fn builtin_attributable_fraction_pop(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_preventive_fraction(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let r0 = f1(args);
...
fn builtin_contact_tracing_eff(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let blocked = f1(args);
...
fn builtin_cluster_attack_rate(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let cases = f1(args);
...
fn builtin_transmission_pair_index(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    let onset_i = f1(args);

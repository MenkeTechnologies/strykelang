...
pub struct Compiler {
    ...
    /// Qualified stash keys of every `sub` declared in this program, collected by the
    /// pass-1 walk in [`Self::compile_program`]. Consulted only by
    /// [`Self::compat_user_sub_wins`], which lets `--compat` give a user sub a name
    /// that a stryke *extension* builtin also claims.
    declared_subs: HashSet<String>,
}
...
impl Compiler {
    ...
    fn try_emit_goto_label(&mut self, target: &Expr, line: usize) -> bool {
        let name = match &target.kind {
            ...
            // Parser may produce a zero-arg FuncCall for a bare label name
            ExprKind::FuncCall { name, args } if args.is_empty() => name.clone(),
            _ => return false,
        };
    ...
    /// Under `--compat`, does a user `sub NAME` take precedence over the same-named
    /// stryke builtin at this call site?
    ///
    /// `--compat` is documented as "disable all stryke extensions", so a program that
    /// defines `sub all { … }` and calls `all()` must reach its own sub. Without this,
    /// the `FuncCall` dispatch arms below lower the call straight to `Op::CallBuiltin`,
    /// which `patch_static_sub_calls` never revisits, so the user sub is unreachable.
    ///
    /// Scope: this covers names dispatched through `ExprKind::FuncCall`. Names the
    /// *parser* lowers to a dedicated `ExprKind` instead (`f`/`filesf` →
    /// `ExprKind::Filesf`, `d`/`dirs` → `ExprKind::Dirs`, `p`, `rev`, …) never reach
    /// this function, so a user sub with one of those names is still shadowed — that
    /// fix belongs at the parser's bareword-dispatch match, not here.
    ///
    /// The precedence flips only for stryke's **own** extension names. Perl 5 core
    /// operators (`print`, `length`, `push`, `sort`, …) keep winning, because that is
    /// what Perl itself does: a plain `sub print { … }` does not override the named
    /// operator (overriding those requires `use subs` / `CORE::GLOBAL`). The
    /// `CORE_CATEGORY_MAP` / `EXT_CATEGORY_MAP` split generated by `build.rs` is
    /// exactly that partition — 215 core names vs. the stryke-only remainder.
    ///
    /// An explicit `CORE::name` call always means the builtin and is never redirected.
    fn compat_user_sub_wins(&self, name: &str, dispatch_name: &str) -> bool {
        if !crate::compat_mode() || name.starts_with("CORE::") {
            return false;
        }
    ...
    fn compile_expr_ctx(&mut self, root: &Expr, ctx: WantarrayCtx) -> Result<(), CompileError> {
        ...
        match &root.kind {
            ...
            ExprKind::FuncCall { name, args } => {
                ...
                // `--compat`: a user `sub` beats a stryke *extension* builtin of the same
                // name. Handled before the dispatch table because the arms below lower
                ...
                match dispatch_name {
                    ...
                    "cluster" => {
                        // Each arg pushed in list context so an `@hosts`
                        // operand flattens into individual slot specs and
                        // a single bareword `cluster("host1:4")` arrives
                        // as one string. `ClusterNew` mirrors the
                        // tree-walker arm in `vm_helper::call_named_sub`.
                        if args.is_empty() {
                            return Err(CompileError::Unsupported(
                                "cluster() expects at least one host/slot specifier".into(),
                            ));
                        }
                        for arg in args {
                            self.compile_expr_ctx(arg, WantarrayCtx::List)?;
                        ...
                        self.emit_op(
                            Op::CallBuiltin(BuiltinId::ClusterNew as u16, args.len() as u8),
                            line,
                            Some(root),
                        );
                    }
            ...
            | ExprKind::DistReduceExpr { .. } => {
                // `par { BLOCK }`, `par_reduce { extract } [{ merge }]`, and
                // `~d>` (distributed thread macro) — bytecode VM has no
                // dedicated opcodes; defer AST eval to the interpreter via
                // `Op::EvalAstExpr`.
            ...
            ExprKind::Swallow(e) => {
                self.compile_expr(e)?;
                self.emit_op(
                    Op::CallBuiltin(BuiltinId::Swallow as u16, 1),
                    line,
                    Some(root),
                );
            }

...
/// ReLU(x) = max(0, x)
fn builtin_ml_relu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(f1(args).max(0.0)))
}

/// Leaky ReLU(x) = x if x > 0 else α x
fn builtin_ml_leaky_relu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_elu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_selu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_gelu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
/// Swish(x) = x · σ(x)
fn builtin_ml_swish_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    Ok(StrykeValue::float(x / (1.0 + (-x).exp())))
}

/// Mish(x) = x · tanh(softplus(x))
fn builtin_ml_mish_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    Ok(StrykeValue::float(x * (1.0 + x.exp()).ln().tanh()))
}

/// Softplus(x) = ln(1 + e^x)
fn builtin_ml_softplus_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    Ok(StrykeValue::float((1.0 + x.exp()).ln()))
}

/// Softsign(x) = x / (1 + |x|)
fn builtin_ml_softsign_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    Ok(StrykeValue::float(x / (1.0 + x.abs())))
}

/// Hard sigmoid: max(0, min(1, 0.2x + 0.5))
fn builtin_ml_hard_sigmoid(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    Ok(StrykeValue::float((0.2 * x + 0.5).clamp(0.0, 1.0)))
}

/// Hard tanh: clamp(x, -1, 1)
fn builtin_ml_hard_tanh(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(f1(args).clamp(-1.0, 1.0)))
}

/// PReLU(x) = x if x > 0 else a·x (parametric)
fn builtin_ml_prelu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_ml_leaky_relu_step(args)
}

/// CELU(x) = max(0, x) + min(0, α(exp(x/α) - 1))
fn builtin_ml_celu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    let alpha = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
/// SiLU = swish
fn builtin_ml_silu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_ml_swish_step(args)
}

/// LogSumExp(x) = a + ln Σ exp(xᵢ - a)
fn builtin_ml_logsumexp_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_ml_log_softmax_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b45_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
/// log σ(x) = -softplus(-x)
fn builtin_ml_log_sigmoid(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
    Ok(StrykeValue::float(-((1.0 + (-x).exp()).ln())))
}

/// GLU(a, b) = a · σ(b)
fn builtin_ml_glu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
...
/// GeGLU(a, b) = a · GELU(b)
fn builtin_ml_geglu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_ml_swiglu_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = f1(args);
    let b = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_ml_attention_score_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q_dot_k = f1(args);
...
fn builtin_ml_scaled_dot_product(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let q_k = f1(args);
...
fn builtin_ml_multihead_avg(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b45_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_ml_softmax_temperature(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b45_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_ml_dropout_mask_prob(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
/// LayerNorm: (x - μ) / √(σ² + ε)
fn builtin_ml_layer_norm_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_batch_norm_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_group_norm_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
/// RMSNorm: x / √(mean(x²) + ε)
fn builtin_ml_rms_norm_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_instance_norm_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
/// WeightNorm: w / |w|
fn builtin_ml_weight_norm_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
...
fn builtin_ml_spectral_norm_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let w = f1(args);
...
/// L2 normalize: x / √(Σx²)
fn builtin_ml_l2_normalize_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b45_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let i = args.get(1).map(|v| v.to_number() as usize).unwrap_or(0);
...
fn builtin_ml_huber_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_smooth_l1_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_focal_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_ml_dice_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// IoU loss: 1 - |A∩B|/|A∪B|
fn builtin_ml_iou_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let inter = f1(args);
...
fn builtin_ml_giou_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let iou = f1(args);
...
fn builtin_ml_diou_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let iou = f1(args);
...
fn builtin_ml_ciou_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let diou = f1(args);
...
fn builtin_ml_contrastive_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_ml_triplet_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let d_ap = f1(args);
...
/// ArcFace: cos(θ + m)
fn builtin_ml_arcface_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// Center loss: ½|x - c|²
fn builtin_ml_center_loss_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_ml_kl_divergence_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
...
fn builtin_ml_cross_entropy_loss(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_ml_binary_cross_entropy(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_ml_label_smoothing(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = f1(args);
...
fn builtin_ml_mixup_lambda(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let alpha = f1(args);
...
/// CutMix box IoU
fn builtin_ml_cutmix_box_iou(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_ml_iou_loss_step(args)
}

/// Random erasing step (apply probability)
fn builtin_ml_random_erasing_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let p = f1(args);
    let r = args.get(1).map(|v| v.to_number()).unwrap_or(0.5);
...
fn builtin_ml_cosine_lr_schedule(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
...
fn builtin_ml_warmup_lr_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
...
/// Step LR schedule
fn builtin_ml_step_lr_schedule(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lr0 = f1(args);
...
/// Exponential LR
fn builtin_ml_exponential_lr(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lr0 = f1(args);
...
fn builtin_ml_polynomial_lr(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let lr0 = f1(args);
...
fn builtin_ml_one_cycle_lr(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
...
fn builtin_ml_inverse_sqrt_lr(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
...
fn builtin_ml_cyclic_lr_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t = f1(args);
...
/// SGD step: θ ← θ - η g
fn builtin_ml_sgd_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta = f1(args);
...
fn builtin_ml_momentum_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_ml_nesterov_momentum(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
    let mu = args.get(1).map(|v| v.to_number()).unwrap_or(0.9);
    let g = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
...
/// AdaGrad step
fn builtin_ml_adagrad_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta = f1(args);
    let eta = args.get(1).map(|v| v.to_number()).unwrap_or(0.01);
    let g = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_ml_rmsprop_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta = f1(args);
...
/// Adam step
fn builtin_ml_adam_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_hat = f1(args);
...
/// AdamW step
fn builtin_ml_adamw_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_hat = f1(args);
    let v_hat = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let eta = args.get(2).map(|v| v.to_number()).unwrap_or(0.001);
...
/// Adamax step (∞-norm)
fn builtin_ml_adamax_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_hat = f1(args);
...
fn builtin_ml_nadam_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_hat = f1(args);
    let v_hat = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    let g = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_ml_radam_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_hat = f1(args);
    let v_hat = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_ml_lookahead_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let slow = f1(args);
...
/// LAMB step
fn builtin_ml_lamb_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
...
fn builtin_ml_lars_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let eta = f1(args);
...
fn builtin_ml_yogi_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = f1(args);
...
fn builtin_ml_amsgrad_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v_hat = f1(args);
...
fn builtin_ml_adabelief_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let m_hat = f1(args);
...
fn builtin_ml_shampoo_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
...
fn builtin_ml_lion_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta = f1(args);
...
fn builtin_ml_sophia_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let prev_m = f1(args);
    let g = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_ml_gradient_clip_norm(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
    let norm = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
...
fn builtin_ml_gradient_clip_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
...
fn builtin_ml_gradient_accumulate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
...
fn builtin_ml_gradient_centralize(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let g = f1(args);
    let mean = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_ml_weight_decay_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let theta = f1(args);
...
/// He init: N(0, √(2/n))
fn builtin_ml_he_init_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_in = f1(args);
...
/// Xavier init: N(0, √(1/n))
fn builtin_ml_xavier_init_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_in = f1(args);
    if n_in <= 0.0 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_ml_glorot_init_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n_in = f1(args);
...
fn builtin_ml_orthogonal_init(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let gain = f1(args);
...
fn builtin_ml_truncnormal_init(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let sigma = f1(args);
    Ok(StrykeValue::float(sigma * 0.886_226_925_452_758))
}

/// Kaiming init (alias of He)
fn builtin_ml_kaiming_init(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_ml_lecun_init_value(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_ml_zero_init(_args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(0.0))
}

/// Constant init: every cell = c (returns c). Defining property.
fn builtin_ml_constant_init(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    Ok(StrykeValue::float(f1(args)))
}

/// Uniform init: U(-r, r)
fn builtin_ml_uniform_init(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let r = f1(args);
    Ok(StrykeValue::float(r))
}

/// One-hot index from class id
fn builtin_ml_one_hot_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let cls = i1(args);
...
fn builtin_ml_label_to_id(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let label = i1(args);
...
fn builtin_ml_id_to_label_step(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let id = i1(args);
...
fn builtin_ml_token_logit_top_k(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
/// Top-k argmax index
fn builtin_ml_topk_argmax(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b45_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_ml_nucleus_sample_p(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mut v = b45_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_ml_temperature_decay(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let t0 = f1(args);
...
fn builtin_ml_repetition_penalty(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let logit = f1(args);
...
fn builtin_ml_eos_logit_boost(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let logit = f1(args);

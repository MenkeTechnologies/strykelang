...
fn b57_to_pairs(v: &StrykeValue) -> Vec<(f64, f64)> {
...
fn builtin_vlookup(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    if col >= n_cols { return Ok(StrykeValue::UNDEF); }
    let rows = table.len() / n_cols;
    for r in 0..rows {
        if (table[r * n_cols] - key).abs() < 1e-12 {
            return Ok(StrykeValue::float(table[r * n_cols + col]));
        }
    }
...
fn builtin_hlookup(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    if row >= n_rows { return Ok(StrykeValue::UNDEF); }
    for c in 0..n_cols {
        if (table[c] - key).abs() < 1e-12 {
            return Ok(StrykeValue::float(table[row * n_cols + c]));
        }
    }
    Ok(StrykeValue::UNDEF)
}
...
fn builtin_xlookup(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    for (i, &k) in lookup.iter().enumerate() {
        if (k - key).abs() < 1e-12 && i < returns.len() {
            return Ok(StrykeValue::float(returns[i]));
        }
    }
    Ok(StrykeValue::float(default))
}
...
fn builtin_index_match(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    if pos < arr.len() { Ok(StrykeValue::float(arr[pos])) } else { Ok(StrykeValue::UNDEF) }
}
...
fn builtin_indirect(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    for (k, v) in pairs {
        if (k - id).abs() < 1e-12 { return Ok(StrykeValue::float(v)); }
    }
    Ok(StrykeValue::UNDEF)
}
...
fn builtin_offset(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::integer(start + rows * n_cols + cols))
}
...
fn builtin_averageif(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    let n = builtin_countif(&[args[0].clone(),
        ...
        args.get(3).cloned().unwrap_or(StrykeValue::integer(0))])?.to_number();
...
fn builtin_rank_eq(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::integer(above as i64 + 1))
}
...
fn builtin_rank_avg(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(above + (equal + 1.0) / 2.0))
}
...
fn builtin_percentrank(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::float((k - 1.0) / (n as f64 - 1.0)))
}
...
fn builtin_quartile_inc(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(arr[lo] + frac * (arr[hi] - arr[lo])))
}
...
fn builtin_quartile_exc(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    let p = (q as f64) / 4.0;
    let h = (n + 1.0) * p - 1.0;
    if h < 0.0 || h > n - 1.0 { return Ok(StrykeValue::float(f64::NAN)); }
    let lo = h.floor() as usize;
...
fn builtin_rate(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    for _ in 0..100 {
        ...
        let new_r = r - f / dr;
        if (new_r - r).abs() < 1e-12 { return Ok(StrykeValue::float(new_r)); }
        r = new_r;
    }
...
fn builtin_yield_to_maturity(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    for _ in 0..100 {
        ...
        let new_y = y - f / dp;
        if (new_y - y).abs() < 1e-10 { return Ok(StrykeValue::float(new_y * freq)); }
        y = new_y;
    }
    Ok(StrykeValue::float(y * freq))
}
...
fn builtin_accrued_interest(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(face * cr * days_since / basis_days))
}
...
fn builtin_skill_score(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    if bs_climo <= 0.0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(1.0 - bs / bs_climo))
}
...
fn builtin_reliability_diagram(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    if n == 0 { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(rel / n as f64))
}
...
fn builtin_taylor_diagram_score(args: &[StrykeValue]) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::float(4.0 * (1.0 + r) / denom))
}

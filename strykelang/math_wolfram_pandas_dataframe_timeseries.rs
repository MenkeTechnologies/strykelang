...
fn builtin_df_groupby(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_aggregate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_apply(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_transform(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
/// flat-values array.
fn builtin_df_pivot(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_pivot_table(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    if v.is_empty() { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(v.iter().sum::<f64>() / v.len() as f64))
}
...
fn builtin_df_melt(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_stack(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_unstack(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_explode(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_get_dummies(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let value = i1(args);
...
fn builtin_df_crosstab(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_merge(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_join(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let b = args.get(1).map(b76_to_floats).unwrap_or_default();
...
fn builtin_df_concat(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let a = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let b = args.get(1).map(b76_to_floats).unwrap_or_default();
...
fn builtin_df_resample(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_rolling(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    if v.is_empty() { return Ok(StrykeValue::float(0.0)); }
    Ok(StrykeValue::float(v.iter().sum::<f64>() / v.len() as f64))
}
...
fn builtin_df_expanding(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_df_rolling(args)
}

/// `df_ewm` — exponentially weighted moving average step: y_t = α·x + (1-α)·y_{t-1}.
fn builtin_df_ewm(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_df_shift(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_diff(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_pct_change(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_corr(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_cov(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let xs = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let ys = args.get(1).map(b76_to_floats).unwrap_or_default();
    let n = xs.len().min(ys.len());
    if n < 2 { return Ok(StrykeValue::float(0.0)); }
...
fn builtin_df_corrwith(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_df_corr(args)
}

/// `df_describe` — returns N count.
fn builtin_df_describe(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.len() as i64))
}

/// `df_kurtosis` — sample excess kurtosis.
fn builtin_df_kurtosis(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let n = v.len() as f64;
...
fn builtin_df_skew(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let n = v.len() as f64;
...
fn builtin_df_sem(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let n = v.len() as f64;
...
fn builtin_df_mad(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let n = v.len() as f64;
...
fn builtin_df_dropna(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.iter().filter(|x| !x.is_nan()).count() as i64))
}

/// `df_fillna` — count of rows that received a fill value.
fn builtin_df_fillna(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.iter().filter(|x| x.is_nan()).count() as i64))
}

/// `df_interpolate` — linearly interpolate NaNs; count of filled.
fn builtin_df_interpolate(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_df_fillna(args)
}

/// `df_replace` — count of replacements.
fn builtin_df_replace(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_isnull(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_df_fillna(args)
}

/// `df_notnull` — non-null count.
fn builtin_df_notnull(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_sort_values(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let s: std::collections::HashSet<u64> = v.iter().map(|x| x.to_bits()).collect();
...
fn builtin_df_rank(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_quantile(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_value_counts(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_df_groupby(args)
}

/// `df_sample` — sample n with replacement; deterministic given seed.
fn builtin_df_sample(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let n = i1(args).max(0);
...
fn builtin_df_nlargest(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mut v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_nsmallest(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let mut v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let n = args.get(1).map(|x| x.to_number() as usize).unwrap_or(1).max(1);
    if v.is_empty() { return Ok(StrykeValue::float(0.0)); }
    v.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
...
fn builtin_df_idxmax(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
...
fn builtin_df_idxmin(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    if v.is_empty() { return Ok(StrykeValue::integer(-1)); }
    let mut best = (0_usize, v[0]);
    for (i, &x) in v.iter().enumerate() { if x < best.1 { best = (i, x); } }
...
fn builtin_df_clip(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_df_round(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let x = f1(args);
...
fn builtin_df_to_datetime(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let y = i1(args);
...
fn builtin_df_to_timedelta(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let days = f1(args);
...
/// failure return NaN.
fn builtin_df_to_numeric(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_eval(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let xs = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let ys = args.get(1).map(b76_to_floats).unwrap_or_default();
...
fn builtin_df_query(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    Ok(StrykeValue::integer(v.iter().filter(|&&x| x != 0.0).count() as i64))
}

/// `df_filter` — count of items matching label-set.
fn builtin_df_filter(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_drop_duplicates(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    builtin_df_groupby(args)
}

/// `df_duplicated` — count of duplicated rows.
fn builtin_df_duplicated(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
    let v = b76_to_floats(args.first().unwrap_or(&StrykeValue::array(vec![])));
    let s: std::collections::HashSet<u64> = v.iter().map(|x| x.to_bits()).collect();
...
fn builtin_df_set_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {
...
fn builtin_df_reset_index(args: &[StrykeValue]) -> StrykeResult<StrykeValue> {

...
fn builtin_mollweide_project(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_robinson_project(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_sinusoidal_project(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let r = args.get(2).map(|v| v.to_number()).unwrap_or(6378137.0);
    Ok(PerlValue::array(vec![PerlValue::float(r * lon * lat.cos()), PerlValue::float(r * lat)]))
}
fn builtin_equirectangular_project(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let phi0 = args.get(2).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let r = args.get(3).map(|v| v.to_number()).unwrap_or(6378137.0);
    Ok(PerlValue::array(vec![PerlValue::float(r * lon * phi0.cos()), PerlValue::float(r * lat)]))
}
fn builtin_lambert_azimuthal_project(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
...
fn builtin_albers_conic_project(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let lat = args.first().map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    let lon = args.get(1).map(|v| v.to_number().to_radians()).unwrap_or(0.0);
    ...
    Ok(PerlValue::array(vec![PerlValue::float(rho * theta.sin()), PerlValue::float(rho0 - rho * theta.cos())]))
}
fn builtin_geohash_encode(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_geohash_decode(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![
        PerlValue::float((lat_range.0 + lat_range.1) / 2.0),
        PerlValue::float((lon_range.0 + lon_range.1) / 2.0),
    ]))
}
fn builtin_gabor_kernel(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_unsharp_mask_kernel(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_emboss_kernel(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(matrix_to_value(&[vec![-2.0, -1.0, 0.0], vec![-1.0, 1.0, 1.0], vec![0.0, 1.0, 2.0]]))
}
fn builtin_box_blur_kernel(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = args.first().map(|v| v.to_number() as usize).unwrap_or(1);
    let n = 2 * r + 1;
    let v = 1.0 / (n * n) as f64;
    Ok(matrix_to_value(&vec![vec![v; n]; n]))
}
fn builtin_motion_blur_kernel(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_sharpen_kernel(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(matrix_to_value(&[vec![0.0, -1.0, 0.0], vec![-1.0, 5.0, -1.0], vec![0.0, -1.0, 0.0]]))
}
fn builtin_edge_detect_kernel(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(matrix_to_value(&[vec![-1.0, -1.0, -1.0], vec![-1.0, 8.0, -1.0], vec![-1.0, -1.0, -1.0]]))
}
fn builtin_sobel_diagonal_kernel(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(matrix_to_value(&[vec![0.0, 1.0, 2.0], vec![-1.0, 0.0, 1.0], vec![-2.0, -1.0, 0.0]]))
}
fn builtin_haar_2d_step(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![matrix_to_value(&ll), matrix_to_value(&hh)]))
}
fn builtin_db4_coeffs(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    let h = vec![0.6830127, 1.1830127, 0.3169873, -0.1830127];
    let s = 1.0 / 2.0_f64.sqrt();
    Ok(PerlValue::array(h.into_iter().map(|x| PerlValue::float(x * s)).collect()))
}
fn builtin_db6_coeffs(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::array(vec![
        PerlValue::float(0.47046721), PerlValue::float(1.14111692),
        PerlValue::float(0.65036501), PerlValue::float(-0.19093442),
        PerlValue::float(-0.12083221), PerlValue::float(0.04981750),
    ]))
}
fn builtin_sym4_coeffs(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::array(vec![
        PerlValue::float(-0.07576571), PerlValue::float(-0.02963553),
        PerlValue::float(0.49761867), PerlValue::float(0.80373875),
        PerlValue::float(0.29785780), PerlValue::float(-0.09921954),
        PerlValue::float(-0.01260396), PerlValue::float(0.03222310),
    ]))
}
fn builtin_coif1_coeffs(_args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::array(vec![
        PerlValue::float(-0.01565572), PerlValue::float(-0.07273262),
        PerlValue::float(0.38486485), PerlValue::float(0.85257202),
        PerlValue::float(0.33789767), PerlValue::float(-0.07273262),
    ]))
}
fn builtin_aes_sbox_byte(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(SBOX[i] as i64))
}
fn builtin_aes_inv_sbox_byte(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = (i1(args) & 0xff) as usize;
    static INV: [u8; 256] = [
    ...
    Ok(PerlValue::integer(INV[i] as i64))
}
fn builtin_chacha20_qround(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![
        PerlValue::integer(a as i64), PerlValue::integer(b as i64),
        PerlValue::integer(c as i64), PerlValue::integer(d as i64),
    ]))
}
fn builtin_xtea_round(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut v0 = i1(args) as u32;
    let mut v1 = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0) as u32;
    let key = arg_to_vec(&args.get(2).cloned().unwrap_or(PerlValue::UNDEF));
    let k: [u32; 4] = [
    ...
    Ok(PerlValue::array(vec![PerlValue::integer(v0 as i64), PerlValue::integer(v1 as i64)]))
}
fn builtin_speck_round(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    y = y.rotate_left(3) ^ x;
    Ok(PerlValue::array(vec![PerlValue::integer(x as i64), PerlValue::integer(y as i64)]))
}
fn builtin_simon_round(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::array(vec![PerlValue::integer(new_x as i64), PerlValue::integer(x as i64)]))
}
fn builtin_geohash_neighbor(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
    let dir = args.get(1).map(|v| v.to_string()).unwrap_or_default();
    let coords = arg_to_vec(&builtin_geohash_decode(&[PerlValue::string(s.clone())])?);
    ...
    builtin_geohash_encode(&[
        PerlValue::float(lat + dlat), PerlValue::float(lon + dlon),
        PerlValue::integer(s.len() as i64),
    ])
}
fn builtin_geohash_bbox(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let s = args.first().map(|v| v.to_string()).unwrap_or_default();
    let base32 = "0123456789bcdefghjkmnpqrstuvwxyz";
    let mut lat_range = (-90.0_f64, 90.0_f64);
    let mut lon_range = (-180.0_f64, 180.0_f64);
    ...
    Ok(PerlValue::array(vec![
        PerlValue::float(lat_range.0), PerlValue::float(lon_range.0),
        PerlValue::float(lat_range.1), PerlValue::float(lon_range.1),
    ]))
}

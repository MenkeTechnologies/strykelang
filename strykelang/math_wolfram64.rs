...
fn b64_to_floats(v: &PerlValue) -> Vec<f64> {
...
fn builtin_cents_between_freqs(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f1_v = f1(args).max(1e-9);
    let f2 = args.get(1).map(|v| v.to_number()).unwrap_or(f1_v).max(1e-9);
    Ok(PerlValue::float(1200.0 * (f2 / f1_v).log2()))
}
...
fn builtin_note_name_from_midi(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let midi = i1(args);
    let pc = midi.rem_euclid(12);
    let octave = midi.div_euclid(12) - 1;
    Ok(PerlValue::integer(pc * 100 + octave))
}
...
fn builtin_interval_quality_size(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer((size as i64) * 100 + quality as i64))
}
...
fn builtin_scale_pitches_major(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::integer(packed))
}

/// Natural minor: 0 2 3 5 7 8 10.
fn builtin_scale_pitches_minor(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = i1(args).rem_euclid(12);
...
fn builtin_mode_pitches_dorian(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = i1(args).rem_euclid(12);
...
fn builtin_mode_pitches_phrygian(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = i1(args).rem_euclid(12);
...
fn builtin_mode_pitches_lydian(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = i1(args).rem_euclid(12);
...
fn builtin_chord_root_inversion(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pcs = b64_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if pcs.is_empty() { return Ok(PerlValue::integer(0)); }
    ...
    Ok(PerlValue::integer(bass * 10 + inversion))
}

/// Chord quality classify (major / minor / dim / aug / 7th). Returns id 0..7.
fn builtin_chord_quality_classify(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pcs = b64_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if pcs.is_empty() { return Ok(PerlValue::integer(-1)); }
    let bass = pcs[0] as i64;
    let mut intervals: Vec<i64> = pcs.iter()
        .map(|p| (*p as i64 - bass).rem_euclid(12))
        .collect();
...
fn builtin_chord_voicing_close(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut pcs = b64_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    pcs.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    if pcs.len() < 2 { return Ok(PerlValue::float(0.0)); }
...
fn builtin_key_signature_sharps(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = i1(args).rem_euclid(12);
    let order = [0_i64, 7, 2, 9, 4, 11, 6, 1, 8, 3, 10, 5];
    Ok(PerlValue::integer(order.iter().position(|&t| t == tonic).unwrap_or(0) as i64))
}

/// Number of flats: C=0, F=1, B♭=2, ... (anti-circle).
fn builtin_key_signature_flats(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = i1(args).rem_euclid(12);
...
fn builtin_tempo_to_ms(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bpm = f1(args).max(1e-9);
    Ok(PerlValue::float(60000.0 / bpm))
}

/// Beat to seconds at given BPM.
fn builtin_beat_to_seconds(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let beats = f1(args);
    let bpm = args.get(1).map(|v| v.to_number()).unwrap_or(120.0).max(1e-9);
    Ok(PerlValue::float(beats * 60.0 / bpm))
}
...
fn builtin_time_sig_subdivision(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let num = i1(args);
    ...
    Ok(PerlValue::integer(num * sub_div_factor / denom))
}

/// Equal-tempered frequency: f = A4 · 2^((n - 69) / 12) for MIDI n.
fn builtin_equal_tempered_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let n = f1(args);
    Ok(PerlValue::float(B64_A4 * 2f64.powf((n - B64_MIDI_A4 as f64) / 12.0)))
}
...
fn builtin_just_intonation_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = f1(args);
    ...
    Ok(PerlValue::float(tonic * ratios[semis as usize]))
}

/// Pythagorean tuning: ratios via stacked perfect 5ths (3/2)^n / 2^k.
fn builtin_pythagorean_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = f1(args);
    let semis = i1(&args[1..]).rem_euclid(12);
    let ratios = [
...
fn builtin_mean_tone_freq(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = f1(args);
    let semis = i1(&args[1..]).rem_euclid(12);
    ...
    Ok(PerlValue::float(tonic * 2f64.powf(cents) * fifth.powf(0.0)))
}

/// Werckmeister III "well-tempered": closest ratios from standard table.
fn builtin_werckmeister_iii(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = f1(args);
    let semis = i1(&args[1..]).rem_euclid(12);
    let cents = [0.0_f64, 90.225, 192.180, 294.135, 390.225, 498.045,
                 588.270, 696.090, 792.180, 888.270, 996.090, 1092.180];
    Ok(PerlValue::float(tonic * 2f64.powf(cents[semis as usize] / 1200.0)))
}

/// Kirnberger III well-tempered tuning.
fn builtin_kirnberger_iii(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let tonic = f1(args);
    let semis = i1(&args[1..]).rem_euclid(12);
...
fn builtin_dynamics_db_level(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let id = i1(args).clamp(0, 7) as usize;
    let levels = [-54.0_f64, -42.0, -30.0, -18.0, -12.0, -6.0, 6.0, 18.0];
    Ok(PerlValue::float(levels[id]))
}

/// Harmonic partial frequency: f_n = n · f₀.
fn builtin_harmonics_partial(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f0 = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(n * f0))
}

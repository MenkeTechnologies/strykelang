...
fn bytes_from_value(v: &StrykeValue) -> Vec<u8> {
...
pub(crate) fn hmac_sha1(key: &StrykeValue, msg: &StrykeValue) -> PerlResult<StrykeValue> {
    ...
    Mac::update(&mut mac, &bytes_from_value(msg));
    Ok(StrykeValue::string(hex::encode(mac.finalize().into_bytes())))
}
...
pub(crate) fn base58_decode(v: &StrykeValue) -> PerlResult<StrykeValue> {
    ...
    let s = s.trim();
    if s.is_empty() {
        return Ok(StrykeValue::bytes(Arc::new(vec![])));
    }
    let mut digits: Vec<u8> = vec![0];
    for c in s.chars() {
    ...
    Ok(StrykeValue::bytes(Arc::new(result)))
}
...
pub(crate) fn des3_decrypt(key: &StrykeValue, ciphertext_b64: &StrykeValue) -> PerlResult<StrykeValue> {
...
/// CAST5-CBC decrypt.
pub(crate) fn cast5_decrypt(key: &StrykeValue, ciphertext_b64: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn secretbox_seal(key: &StrykeValue, plaintext: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn secretbox_open(key: &StrykeValue, ciphertext_b64: &StrykeValue) -> PerlResult<StrykeValue> {
    use crypto_secretbox::{aead::Aead, KeyInit, XSalsa20Poly1305};
...
pub(crate) fn nacl_box_keygen() -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::array(vec![
        StrykeValue::string(hex::encode(sk.to_bytes())),
        StrykeValue::string(hex::encode(pk.to_bytes())),
    ]))
}
...
pub(crate) fn qr_svg(data: &StrykeValue) -> PerlResult<StrykeValue> {
    ...
    let svg_string = code
        ...
        .build();
    Ok(StrykeValue::string(svg_string))
}
...
pub(crate) fn barcode_svg(data: &StrykeValue, barcode_type: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn poly1305_mac(key: &StrykeValue, msg: &StrykeValue) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::string(hex::encode(tag.as_slice())))
}
...
pub(crate) fn rsa_encrypt(pub_pem: &StrykeValue, plaintext: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn ecdsa_p256_keygen() -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::array(vec![
        StrykeValue::string(priv_hex),
        StrykeValue::string(pub_hex),
    ]))
}

/// ECDSA P-256 sign. private_key_hex, message. Returns signature as hex (DER-encoded).
pub(crate) fn ecdsa_p256_sign(priv_hex: &StrykeValue, message: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn ecdsa_p384_sign(priv_hex: &StrykeValue, message: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn pbkdf2_derive(
    ...
    Ok(StrykeValue::string(hex::encode(key)))
}
...
pub(crate) fn random_bytes_hex(n: &StrykeValue) -> PerlResult<StrykeValue> {
    ...
    let mut buf = vec![0u8; len];
    rand::thread_rng().fill_bytes(&mut buf);
    Ok(StrykeValue::string(hex::encode(buf)))
}
...
pub(crate) fn aes_decrypt(key: &StrykeValue, ciphertext_b64: &StrykeValue) -> PerlResult<StrykeValue> {
    let key_bytes = bytes_from_value(key);
    if key_bytes.len() != 32 {
        return Err(PerlError::runtime(
...
/// Returns base64(nonce || ciphertext || tag).
pub(crate) fn chacha_encrypt(key: &StrykeValue, plaintext: &StrykeValue) -> PerlResult<StrykeValue> {
    let key_bytes = bytes_from_value(key);
    if key_bytes.len() != 32 {
        return Err(PerlError::runtime(
            format!(
...
pub(crate) fn chacha_decrypt(key: &StrykeValue, ciphertext_b64: &StrykeValue) -> PerlResult<StrykeValue> {
    let key_bytes = bytes_from_value(key);
    if key_bytes.len() != 32 {
        return Err(PerlError::runtime(
            format!(
...
pub(crate) fn ed25519_keygen() -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::array(vec![
        StrykeValue::string(hex::encode(secret.to_bytes())),
        StrykeValue::string(hex::encode(public.to_bytes())),
    ]))
}

/// Ed25519 sign. private_key_hex (64 chars / 32 bytes), message.
/// Returns signature as hex (128 chars / 64 bytes).
pub(crate) fn ed25519_sign(private_key: &StrykeValue, message: &StrykeValue) -> PerlResult<StrykeValue> {
    ...
    let sig = signing_key.sign(&bytes_from_value(message));
    Ok(StrykeValue::string(hex::encode(sig.to_bytes())))
}
...
pub(crate) fn x25519_dh(my_private: &StrykeValue, their_public: &StrykeValue) -> PerlResult<StrykeValue> {
    ...
    Ok(StrykeValue::string(hex::encode(shared.as_bytes())))
}
...
pub(crate) fn math_betainc(x: &StrykeValue, a: &StrykeValue, b: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn tar_extract(tar_data: &StrykeValue, dest_dir: &StrykeValue) -> PerlResult<StrykeValue> {
...
pub(crate) fn zip_extract(zip_data: &StrykeValue, dest_dir: &StrykeValue) -> PerlResult<StrykeValue> {
    use std::fs;
    use std::io::Write;
    use std::path::Path;
...
pub(crate) fn datetime_parse_local(s: &StrykeValue, tz_name: &StrykeValue) -> PerlResult<StrykeValue> {
    let tz: Tz = parse_tz(tz_name)?;
...
pub(crate) fn datetime_add_seconds(epoch: &StrykeValue, secs: &StrykeValue) -> PerlResult<StrykeValue> {
...
fn xml_element_to_perl(node: roxmltree::Node<'_, '_>) -> PerlResult<StrykeValue> {
    ...
    for a in node.attributes() {
        let an = a.name();
        map.insert(format!("@{an}"), StrykeValue::string(a.value().to_string()));
    }
    ...
    let t = text_buf.trim();
    if !t.is_empty() {
        map.insert("#text".into(), StrykeValue::string(t.to_string()));
    }
    let mut groups: IndexMap<String, Vec<StrykeValue>> = IndexMap::new();
    for c in node.children() {
        if c.is_element() {
...
fn xml_write_element(out: &mut String, tag: &str, v: &StrykeValue) -> PerlResult<()> {
    ...
    let mut children: Vec<(String, StrykeValue)> = Vec::new();
    for (k, val) in map {
...
fn toml_to_perl(v: toml::Value) -> StrykeValue {
    match v {
        toml::Value::String(s) => StrykeValue::string(s),
        toml::Value::Integer(n) => StrykeValue::integer(n),
        toml::Value::Float(x) => StrykeValue::float(x),
        toml::Value::Boolean(b) => StrykeValue::integer(i64::from(b)),
        toml::Value::Datetime(d) => StrykeValue::string(d.to_string()),
        toml::Value::Array(a) => StrykeValue::array(a.into_iter().map(toml_to_perl).collect()),
        toml::Value::Table(t) => {
            let mut map = IndexMap::new();
            for (k, v) in t {
...
fn yaml_to_perl(v: serde_yaml::Value) -> StrykeValue {
    match v {
        serde_yaml::Value::Null => StrykeValue::UNDEF,
        serde_yaml::Value::Bool(b) => StrykeValue::integer(i64::from(b)),
        ...
        serde_yaml::Value::String(s) => StrykeValue::string(s),
        serde_yaml::Value::Sequence(seq) => {
            StrykeValue::array(seq.into_iter().map(yaml_to_perl).collect())
        }
...
mod tests {
    ...
    #[test]
    fn sha256_vector() {
        let p = sha256(&StrykeValue::string("abc".into())).expect("sha256");
        assert_eq!(
            p.to_string(),
    ...
    fn md5_and_sha1_vectors() {
        assert_eq!(
            md5_digest(&StrykeValue::string("".into()))
                .unwrap()
                .to_string(),
            "d41d8cd98f00b204e9800998ecf8427e"
        );
        assert_eq!(
            md5_digest(&StrykeValue::string("abc".into()))
                .unwrap()
                .to_string(),
            "900150983cd24fb0d6963f7d28e17f72"
        );
        assert_eq!(
            sha1_digest(&StrykeValue::string("abc".into()))
                .unwrap()
                .to_string(),
    ...
    fn toml_yaml_encode_roundtrip_simple() {
        let h = StrykeValue::hash_ref(Arc::new(RwLock::new({
            let mut m = IndexMap::new();
            m.insert("x".into(), StrykeValue::integer(7));
            m.insert("k".into(), StrykeValue::string("v".into()));
            m
        })));
    ...
    fn gzip_roundtrip() {
        ...
        let g = gzip(&StrykeValue::string(s.into())).expect("gzip");
    ...
    fn zstd_roundtrip() {
        let s = "zstd payload";
        let z = zstd_compress(&StrykeValue::string(s.into())).expect("zstd");
    ...
    fn xml_encode_decode_roundtrip() {
        ...
        inner.insert("@id".into(), StrykeValue::string("9".into()));
        inner.insert("#text".into(), StrykeValue::string("hi".into()));
        let mut root = IndexMap::new();
        root.insert(
            "msg".into(),
            StrykeValue::hash_ref(Arc::new(RwLock::new(inner))),
        );
        let v = StrykeValue::hash_ref(Arc::new(RwLock::new(root)));
    ...
    fn datetime_parse_format_america_new_york() {
        let epoch = datetime_parse_local(
            &StrykeValue::string("2024-06-15 12:00:00".into()),
            &StrykeValue::string("America/New_York".into()),
        )
        .expect("parse");
        let wall = datetime_format_tz(
            &epoch,
            &StrykeValue::string("America/New_York".into()),
            &StrykeValue::string("%Y-%m-%d %H:%M:%S".into()),
        )
        .expect("fmt");
    ...
    fn datetime_add_seconds_delta() {
        let out =
            datetime_add_seconds(&StrykeValue::float(1_000.0), &StrykeValue::float(2.25)).unwrap();

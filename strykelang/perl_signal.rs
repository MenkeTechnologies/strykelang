...
//!
//! Signal hooks install lazily the first time Perl code assigns `$SIG{NAME}` (or the stryke
...
pub fn poll(interp: &mut VMHelper) -> StrykeResult<()> {
    #[cfg(unix)]
    {
        unix::poll(interp)
    }
...
mod unix {
    ...
    pub(super) fn poll(interp: &mut VMHelper) -> StrykeResult<()> {

...
fn b77_to_floats(v: &PerlValue) -> Vec<f64> {
    arg_to_vec(v).iter().map(|x| x.to_number()).collect()
}

/// `image_resize` — bilinear new pixel = weighted average of 4 source pixels.
fn builtin_image_resize(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p00 = f1(args);
    ...
    Ok(PerlValue::float(p00 * (1.0 - dx) * (1.0 - dy)
        + p01 * dx * (1.0 - dy)
        + p10 * (1.0 - dx) * dy
        + p11 * dx * dy))
}
...
fn builtin_image_grayscale(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
...
fn builtin_image_threshold(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let t = args.get(1).map(|v| v.to_number()).unwrap_or(127.0);
    Ok(PerlValue::integer(if p >= t { 1 } else { 0 }))
}

/// `image_blur_gaussian` — 1-D Gaussian kernel coef at offset k for σ.
fn builtin_image_blur_gaussian(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = f1(args);
...
fn builtin_image_blur_box(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let k = i1(args).max(0) as f64;
    Ok(PerlValue::float(1.0 / (2.0 * k + 1.0)))
}

/// `image_sharpen` — apply unsharp mask: I + α (I − blur).
fn builtin_image_sharpen(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = f1(args);
    let blur = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let alpha = args.get(2).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(i + alpha * (i - blur)))
}

/// `image_edge_canny` — Canny non-max-suppression: keep magnitude if local max.
fn builtin_image_edge_canny(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mag = f1(args);
    ...
    if suppressed >= hi { Ok(PerlValue::integer(2)) }
    else if suppressed >= lo { Ok(PerlValue::integer(1)) }
    else { Ok(PerlValue::integer(0)) }
}

/// `image_edge_sobel` — Sobel Gx kernel = [[−1,0,1],[−2,0,2],[−1,0,1]];
/// returns gradient magnitude √(Gx² + Gy²).
fn builtin_image_edge_sobel(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let gx = f1(args);
    let gy = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((gx * gx + gy * gy).sqrt()))
}

/// `image_edge_laplacian` — Laplacian = ∇²I = Σ_n (I_n − 4 I).
fn builtin_image_edge_laplacian(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    ...
    Ok(PerlValue::float(n + s + e + w - 4.0 * p))
}

/// `image_dilate` — morphological dilation: max in window.
fn builtin_image_dilate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    Ok(PerlValue::float(v.iter().cloned().fold(f64::NEG_INFINITY, f64::max)))
}

/// `image_erode` — morphological erosion: min in window.
fn builtin_image_erode(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let v = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_image_morphology_open(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let inner = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    ...
    let _ = dil;
    Ok(PerlValue::float(final_max))
}

/// `image_morphology_close` — dilation followed by erosion: max of window's
/// max-pool, then min over outer windows.
fn builtin_image_morphology_close(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let inner = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let outer = args.get(1).map(b77_to_floats).unwrap_or_default();
    let dilated = inner.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
    let final_min = outer.iter().cloned().fold(dilated, f64::min);
    Ok(PerlValue::float(final_min))
}

/// `image_histogram` — counts at given bin (0..255).
fn builtin_image_histogram(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pixels = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let bin = args.get(1).map(|v| v.to_number() as i64).unwrap_or(0);
    Ok(PerlValue::integer(pixels.iter().filter(|&&p| p as i64 == bin).count() as i64))
}

/// `image_equalize` — histogram equalization: cdf-based intensity remap.
fn builtin_image_equalize(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let cdf_p = f1(args);
    ...
    Ok(PerlValue::float(((cdf_p - cdf_min) / (n - cdf_min).max(1e-15)) * (l - 1.0)))
}

/// `image_clahe` — Contrast Limited Adaptive Histogram Equalization clip step.
fn builtin_image_clahe(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let count = f1(args);
    let clip = args.get(1).map(|v| v.to_number()).unwrap_or(40.0);
    Ok(PerlValue::float(count.min(clip)))
}

/// `image_contrast` — linear contrast: out = α·(in − 128) + 128.
fn builtin_image_contrast(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let alpha = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::float(alpha * (p - 128.0) + 128.0))
}

/// `image_brightness` — additive brightness: out = in + β.
fn builtin_image_brightness(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let beta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(p + beta))
}

/// `image_gamma` — power-law: out = 255 · (in/255)^γ.
fn builtin_image_gamma(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args).max(0.0);
    let gamma = args.get(1).map(|v| v.to_number()).unwrap_or(1.0).max(1e-15);
    Ok(PerlValue::float(255.0 * (p / 255.0).powf(gamma)))
}

/// `image_invert` — out = 255 − in.
fn builtin_image_invert(args: &[PerlValue]) -> PerlResult<PerlValue> {
    Ok(PerlValue::float(255.0 - f1(args)))
}

/// `image_sepia` — sepia tone: out_r = 0.393R + 0.769G + 0.189B.
fn builtin_image_sepia(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
    let g = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let b = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float((0.393 * r + 0.769 * g + 0.189 * b).min(255.0)))
}

/// `image_posterize` — quantize to N levels per channel.
fn builtin_image_posterize(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let levels = args.get(1).map(|v| v.to_number()).unwrap_or(8.0).max(1.0);
    let step = 255.0 / levels;
    Ok(PerlValue::float((p / step).floor() * step))
}

/// `image_solarize` — invert pixels above threshold.
fn builtin_image_solarize(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let p = f1(args);
    let t = args.get(1).map(|v| v.to_number()).unwrap_or(128.0);
    Ok(PerlValue::float(if p > t { 255.0 - p } else { p }))
}

/// `convolve_2d` — element of 2-D convolution sum at one position.
fn builtin_convolve_2d(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let kernel = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
...
fn builtin_filter_median(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mut v = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    if v.is_empty() { return Ok(PerlValue::float(0.0)); }
    v.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    let mid = v.len() / 2;
    Ok(PerlValue::float(if v.len() % 2 == 1 { v[mid] }
                        else { (v[mid - 1] + v[mid]) / 2.0 }))
}

/// `filter_bilateral` — bilateral filter weight for spatial / range distance.
fn builtin_filter_bilateral(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dx = f1(args);
...
fn builtin_filter_nlmeans(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let dist_sq = f1(args).max(0.0);
    let h = args.get(1).map(|v| v.to_number()).unwrap_or(10.0).max(1e-15);
    Ok(PerlValue::float((-dist_sq / (h * h)).exp()))
}

/// `gabor_filter` — Gabor kernel value at (x, y) with frequency f, σ, θ.
fn builtin_gabor_filter(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
...
fn builtin_hog_features(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let mag = f1(args);
    ...
    Ok(PerlValue::float(mag * bin as f64))
}

/// `harris_corners` — Harris response: det(M) − k · trace(M)².
fn builtin_harris_corners(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ixx = f1(args);
...
fn builtin_template_match(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(cov / (var_i * var_t).sqrt()))
}

/// `face_detect_haar` — Haar feature: rectangle-difference sum.
fn builtin_face_detect_haar(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let bright = f1(args);
    let dark = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(bright - dark))
}

/// `watershed_segment` — flooding fill water level threshold.
fn builtin_watershed_segment(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let elevations = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let level = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::integer(elevations.iter().filter(|&&e| e <= level).count() as i64))
}

/// `slic_superpixels` — k-means iteration step over (x, y, l*, a*, b*).
fn builtin_slic_superpixels(args: &[PerlValue]) -> PerlResult<PerlValue> {
...
fn builtin_graph_cut_segment(args: &[PerlValue]) -> PerlResult<PerlValue> {
    ...
    Ok(PerlValue::float(data_term + lambda * smooth_term))
}

/// `hough_lines` — accumulator vote at (ρ, θ).
fn builtin_hough_lines(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
...
fn builtin_hough_circles(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let a = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    let b = args.get(3).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(((x - a).powi(2) + (y - b).powi(2)).sqrt()))
}

/// `ransac_homography` — inlier count given consensus threshold.
fn builtin_ransac_homography(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let residuals = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let t = args.get(1).map(|v| v.to_number()).unwrap_or(1.0);
    Ok(PerlValue::integer(residuals.iter().filter(|&&r| r.abs() < t).count() as i64))
}

/// `optical_flow_lk` — Lucas-Kanade matrix solve: u = (A^T A)⁻¹ A^T b (det form).
fn builtin_optical_flow_lk(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let ixx = f1(args);
    let iyy = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let ixy = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    ...
    let det = ixx * iyy - ixy * ixy;
    if det.abs() < 1e-15 { return Ok(PerlValue::float(0.0)); }
    Ok(PerlValue::float((iyy * (-ixt) - ixy * (-iyt)) / det))
}

/// `optical_flow_farneback` — quadratic polynomial expansion coefficient.
fn builtin_optical_flow_farneback(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let r = f1(args);
    let theta = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(r * theta.cos()))
}

/// `corner_subpix` — subpixel refinement via paraboloid fit.
fn builtin_corner_subpix(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let f0 = f1(args);
    ...
    Ok(PerlValue::float((f0 - f2) / denom))
}

/// `image_rotate` — rotate (x, y) about origin by angle θ, return new x.
fn builtin_image_rotate(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let y = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let theta = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(x * theta.cos() - y * theta.sin()))
...
fn builtin_image_flip_h(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let x = f1(args);
    let w = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(w - 1.0 - x))
}

/// `image_flip_v` — vertical flip: y' = (H − 1) − y.
fn builtin_image_flip_v(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let y = f1(args);
    let h = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(h - 1.0 - y))
}

/// `image_emboss` — emboss kernel response: 2I − N − S − 128.
fn builtin_image_emboss(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let i = f1(args);
    let n = args.get(1).map(|v| v.to_number()).unwrap_or(0.0);
    let s = args.get(2).map(|v| v.to_number()).unwrap_or(0.0);
    Ok(PerlValue::float(2.0 * i - n - s + 128.0))
}
...
fn builtin_image_motion_blur(args: &[PerlValue]) -> PerlResult<PerlValue> {
    let pixels = b77_to_floats(args.first().unwrap_or(&PerlValue::array(vec![])));
    let l = pixels.len() as f64;
    if l == 0.0 { return Ok(PerlValue::float(0.0)); }
    let mid = (l - 1.0) / 2.0;
    ...
    Ok(PerlValue::float(acc / z.max(1e-15)))
}

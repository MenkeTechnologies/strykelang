...
//! (`Interpreter` defaults and `execute_tree`), `pchannel` (`send`/`recv`/`pselect` helpers), `parallel_trace` (fan worker / trace-noop paths), `profiler` (line/sub aggregation), `par_walk::collect_paths`, `perl_signal::poll`, lexer (`&`, `&&`/`||`/`+=`, `==`/`!=`,

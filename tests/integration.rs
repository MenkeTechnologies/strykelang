...
//! tests cover `parse()`, `run`, lexer (`==`/`!=`, `**`/`..`, `q{}`, `qr//`, octal/binary, `-e`
//! file tests, floats,
...
//! `Display`). Integration covers `our`/`local`, subs with prototypes, builtins like `require`, and
//! regex (`=~` / `!~`). Run with `cargo test`.

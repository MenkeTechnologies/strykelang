...
fn read_lines_reads_file_into_array() {
    ...
    let path = dir.join(format!("perlrs_test_rl_{}.txt", std::process::id()));
...
fn read_lines_scalar_context_count() {
    let dir = std::env::temp_dir();
    let path = dir.join(format!("perlrs_test_rlc_{}.txt", std::process::id()));
    let ps = path.to_string_lossy().replace('\\', "/");
    fs::write(&path, "one\ntwo\nthree\n").unwrap();
...
fn read_lines_no_trailing_newline() {
    let dir = std::env::temp_dir();
    let path = dir.join(format!("perlrs_test_rlnt_{}.txt", std::process::id()));
    let ps = path.to_string_lossy().replace('\\', "/");
    fs::write(&path, "first\nsecond").unwrap();
    let code = format!(r#"my @l = read_lines("{ps}"); scalar @l"#);
    let got = eval_int(&code);
...
fn append_file_creates_and_appends() {
    let dir = std::env::temp_dir();
    let path = dir.join(format!("perlrs_test_af_{}.txt", std::process::id()));
    let ps = path.to_string_lossy().replace('\\', "/");
    let code = format!(
...
fn write_json_read_json_roundtrip_hash() {
    let dir = std::env::temp_dir();
    let path = dir.join(format!("perlrs_test_json_{}.json", std::process::id()));
    let ps = path.to_string_lossy().replace('\\', "/");
    let code = format!(
...
fn read_json_nested_hash() {
    let dir = std::env::temp_dir();
    let path = dir.join(format!("perlrs_test_json3_{}.json", std::process::id()));
    let ps = path.to_string_lossy().replace('\\', "/");
...
fn which_all_nonexistent_returns_empty() {
    assert_eq!(
        eval_int(r#"my @w = which_all("__perlrs_nonexistent_cmd_xyz__"); scalar @w"#),
        0,
    );
}

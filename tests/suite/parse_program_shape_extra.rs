...
fn unless_statement_kind() {
    let p = stryke::parse("unless (1) { 0; }").expect("parse");
...
fn until_loop_statement_kind() {
    let p = stryke::parse("until (1) { }").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn do_while_statement_kind() {
    let p = stryke::parse("do { 1 } while (0)").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn c_style_for_statement_kind() {
    let p = stryke::parse("for (my $i = 0; $i < 2; $i = $i + 1) { 1; }").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn our_declaration_statement_kind() {
    let p = stryke::parse("our $g = 1").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn local_declaration_statement_kind() {
    let p = stryke::parse("local $l = 2").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn my_list_declares_two_scalars() {
    let p = stryke::parse("my ($a, $b)").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn if_with_one_elsif_branch() {
    let p = stryke::parse("if (0) { 1; } elsif (1) { 2; } else { 3; }").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn ternary_expression_shape() {
    let p = stryke::parse("1 ? 2 : 3").expect("parse");
    assert_eq!(p.statements.len(), 1);
...
fn range_expression_shape() {
    let p = stryke::parse("1..5").expect("parse");
    assert_eq!(p.statements.len(), 1);
    let StmtKind::Expression(expr) = &p.statements[0].kind else {
        panic!("expected Expression");
    };
...
fn repeat_operator_expression_shape() {
    let p = stryke::parse(r#""a" x 3"#).expect("parse");
    assert_eq!(p.statements.len(), 1);
    let StmtKind::Expression(expr) = &p.statements[0].kind else {
        panic!("expected Expression");
    };
...
fn qw_list_expression_shape() {
    let p = stryke::parse("qw(a b)").expect("parse");
    assert_eq!(p.statements.len(), 1);
    let StmtKind::Expression(expr) = &p.statements[0].kind else {
        panic!("expected Expression");
    };
...
fn undef_literal_expression_shape() {
    let p = stryke::parse("undef").expect("parse");
    assert_eq!(p.statements.len(), 1);
    let StmtKind::Expression(expr) = &p.statements[0].kind else {
        panic!("expected Expression");
    };

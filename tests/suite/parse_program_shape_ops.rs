...
use forge::ast::{BinOp, ExprKind, StmtKind, UnaryOp};

#[test]
fn binop_subtract() {
    let p = forge::parse("9 - 4").expect("parse");
...
#[test]
fn binop_multiply() {
    let p = forge::parse("3 * 4").expect("parse");
    let StmtKind::Expression(e) = &p.statements[0].kind else {
        panic!("expr stmt");
    };
    let ExprKind::BinOp { op, .. } = &e.kind else {
...
#[test]
fn binop_divide() {
    let p = forge::parse("8 / 2").expect("parse");
    let StmtKind::Expression(e) = &p.statements[0].kind else {
        panic!("expr stmt");
    };
    let ExprKind::BinOp { op, .. } = &e.kind else {
...
#[test]
fn binop_modulo() {
    let p = forge::parse("7 % 3").expect("parse");
    let StmtKind::Expression(e) = &p.statements[0].kind else {
        panic!("expr stmt");
    };
    let ExprKind::BinOp { op, .. } = &e.kind else {
...
#[test]
fn binop_power() {
    let p = forge::parse("2 ** 8").expect("parse");
    let StmtKind::Expression(e) = &p.statements[0].kind else {
        panic!("expr stmt");
    };
    let ExprKind::BinOp { op, .. } = &e.kind else {
...
fn unary_numeric_negate() {
    let p = forge::parse("-15").expect("parse");
    let StmtKind::Expression(e) = &p.statements[0].kind else {
        panic!("expr stmt");
    };
...
fn unary_logical_not_bang() {
    let p = forge::parse("!0").expect("parse");
    let StmtKind::Expression(e) = &p.statements[0].kind else {
        panic!("expr stmt");
    };
    let ExprKind::UnaryOp { op, .. } = &e.kind else {

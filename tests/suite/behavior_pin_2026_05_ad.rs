...
fn string_cases_ad() {
    ...
    assert_eq!(
        eval_string(r#"constant_case("foo_bar_baz")"#),
        "FOO_BAR_BAZ"
    );
...
fn string_search_replace_ad() {
    assert_eq!(
        eval_string(r#"join(",", indexes_of("banana", "a"))"#),
        "1,3,5"
    );
    assert_eq!(
        eval_string(r#"replace_first("abcabc", "b", "z")"#),
        "azcabc"
    );
    assert_eq!(
        eval_string(r#"replace_all_str("abcabc", "b", "z")"#),
        "azcazc"
    );
}

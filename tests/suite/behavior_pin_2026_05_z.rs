...
fn url_parts_extraction() {
    ...
    assert_eq!(
        eval_string(&format!(r#"url_host('{}')"#, url)),
        "user:pass@example.com:8080"
    );
    assert_eq!(
        eval_string(&format!(r#"url_path('{}')"#, url)),
        "/path/to/page"
    );
...
fn email_parts_extraction() {
    ...
    assert_eq!(
        eval_string(&format!(r#"email_local('{}')"#, email)),
        "gemini.cli"
    );
    assert_eq!(
        eval_string(&format!(r#"email_domain('{}')"#, email)),
        "google.com"
    );
}
...
fn list_helpers_z() {
    ...
    assert_eq!(
        eval_string(r#"longest("tiny", "huge-mungous", "small")"#),
        "huge-mungous"
    );
    assert_eq!(
        eval_string(r#"shortest("tiny", "huge-mungous", "small")"#),
        "tiny"
    );

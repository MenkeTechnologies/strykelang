...
fn slurp_valid_utf8_round_trips() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_slurp_utf8_{}", std::process::id()));
...
fn slurp_invalid_utf8_octets_map_to_u_plus_00xx() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_slurp_latin1_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn slurp_mixed_utf8_and_latin1_lines_per_line_decode() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_slurp_mixed_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn readline_decodes_high_octet_line_as_latin1() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_readline_l1_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn tell_read_handle_matches_bytes_after_sysread() {
    ...
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_tell_read_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn require_scans_inc_in_array_order_first_dir_shadows_later() {
    let base: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_inc_order_{}", std::process::id()));
...
fn slurp_crlf_file_contains_both_bytes_without_replacement_char() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_slurp_crlf_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn sysseek_seek_set_then_tell_reports_offset() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_sysseek_tell_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn open_append_mode_preserves_then_extends_file() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_append_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn filetest_e_true_for_existing_path() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_fte_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    ...
    assert_eq!(
        eval_int(r#"(-e "/nonexistent_perlrs_fte_987654321") ? 1 : 0"#),
        0
    );
    std::fs::remove_dir_all(&dir).ok();
}
...
fn filetest_f_and_s_for_regular_file() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_fs_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn mkdir_creates_directory_and_negative_d_test() {
    let base: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_mkdir_d_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&base);
...
fn unlink_removes_file_and_negative_e_test() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_unlink_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn rename_moves_file_contents_preserved() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_rename_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn readdir_lists_created_files_in_directory() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_rd_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn readdir_list_context_second_read_on_same_handle_is_empty() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_rd2_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join("alpha.txt"), b"a").unwrap();
    std::fs::write(dir.join("beta.txt"), b"b").unwrap();
...
fn readdir_rewinddir_refills_stream_same_length_as_first_list_read() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_rdrw_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join("alpha.txt"), b"a").unwrap();
    std::fs::write(dir.join("beta.txt"), b"b").unwrap();
...
fn telldir_after_list_readdir_matches_number_of_entries_read() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_telld_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn glob_expands_star_pattern_in_directory() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_glob_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn filetest_f_is_false_for_directory_path() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_ftd_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    let p = dir.to_str().expect("utf-8");
    assert_eq!(
...
fn seekdir_zero_then_scalar_readdir_matches_first_list_entry() {
    let dir: PathBuf =
        std::env::temp_dir().join(format!("perlrs_fix_skdir_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn filetest_z_true_for_empty_regular_file() {
    let dir: PathBuf = std::env::temp_dir().join(format!("perlrs_fix_z_{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();

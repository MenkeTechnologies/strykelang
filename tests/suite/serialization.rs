...
fn to_html_pipeline_to_file_returns_path() {
    let html =
        eval_string(r#"my $path = th("test") |> to_file("/tmp/stryke_test_th.html"); $path"#);
    assert_eq!(html, "/tmp/stryke_test_th.html");
}
...
fn to_file_returns_path_not_content() {
    ...
    let result = eval_string(r#""line1\nline2" |> to_file("/tmp/stryke_tf_test.txt")"#);
    assert_eq!(result, "/tmp/stryke_tf_test.txt");
}

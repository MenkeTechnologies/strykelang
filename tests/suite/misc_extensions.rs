...
fn misc_collection_helpers() {
    // riffle inserts separator (last arg) between elements.
    // riffle(1, 2, 3, 0) → [1, 0, 2, 0, 3]
    assert_eq!(eval_string("join ',', riffle(1, 2, 3, 0)"), "1,0,2,0,3");
    assert_eq!(
        eval_string("join ',', intersperse([1, 2, 3], 0)"),
        "1,0,2,0,3"
    );
    assert_eq!(
        eval_string("join ',', every_nth(2, 1, 2, 3, 4, 5, 6)"),
        "1,3,5"
    );

    assert_eq!(eval_string("join ',', drop_n(2, 1, 2, 3, 4)"), "3,4");
    assert_eq!(eval_string("join ',', take_n(2, 1, 2, 3, 4)"), "1,2");

    assert_eq!(eval_string("join ',', rotate(1, 1, 2, 3)"), "2,3,1");
